...
fn first_token_diff(expected: &[u32], actual: &[u32]) -> Option<usize> {
    expected
        .iter()
        .zip(actual)
...
fn run_prefill_abab(engine: &mut rnb_llm::Engine, tokens: &[u32], repeat: usize) {
    ...
    // --- explicit baseline warmup (측정 제외, pipeline/carrier 캐시 워밍) ---
    ...
    // 별도 warmup 뒤의 A/B 측정값을 모두 포함해 balanced median 비교.
    ...
    eprintln!("[ABAB] A ({a_label}) raw ms: {:?}", a_ms);
    eprintln!("[ABAB] B ({b_label}) raw ms: {:?}", b_ms);
    eprintln!(
        "[ABAB] A median = {:.3}ms over {} measured runs",
        a_med,
        a_ms.len()
    );
    eprintln!(
        "[ABAB] B median = {:.3}ms over {} measured runs",
        b_med,
        b_ms.len()
    );
    eprintln!(
...
fn parse_mtp_env_request(raw: Option<&str>) -> MtpEnvRequest {
    // pm118: 미설정 = auto — 채택된 MTP 정책(GLM+Metal batch verify 등)은
    // 무-env 로 켜지고, auto policy 가 모델/자원 조건으로 안전 gate 한다.
    let Some(raw) = raw.map(str::trim).filter(|value| !value.is_empty()) else {
        return MtpEnvRequest::Auto;
    };
...
fn mtp_auto_sampling_allowed(request: MtpEnvRequest, repetition_penalty: f32) -> bool {
    !matches!(request, MtpEnvRequest::Auto) || repetition_penalty == 1.0
}
...
fn parse_mtp_abab_repeat(raw: Option<&str>) -> Result<Option<usize>, String> {
    ...
    let repeat = raw
        .trim()
        .parse::<usize>()
        .map_err(|_| format!("RNB_MTP_ABAB_REPEAT must be an even integer >= 4, got {raw:?}"))?;
    if repeat < 4 || repeat % 2 != 0 {
        return Err(format!(
            "RNB_MTP_ABAB_REPEAT must be an even integer >= 4, got {repeat}"
        ));
    }
    Ok(Some(repeat))
}
...
fn run_mtp_abab(
    ...
    let canonical: Option<(String, usize)>;
    ...
    eprintln!(
        "[MTP_ABAB] start measured_runs={repeat} order=AB comparison={comparison} device_verify=0 max_tokens={} a_spec_k={} b_spec_k={} temperature={}",
        params.max_tokens,
        params.spec_k,
    ...
    // The baseline warmup is explicit and excluded; all following A/B runs are measured.
    // SAFETY: the dev-cli harness changes this env only between single-threaded generate calls.
    unsafe {
    ...
    let warmup = engine
        ...
        .unwrap_or_else(|err| panic!("MTP ABAB baseline warmup failed: {err}"));
    let warmup_wall_ms = warmup_started.elapsed().as_secs_f64() * 1000.0;
    let warmup_chars = warmup.text.chars().count();
    let warmup_sha256 = generated_text_sha256_hex(&warmup.text);
    eprintln!(
        "[MTP_ABAB] warmup variant=A/Baseline measured=false batch_verify=0 spec_k={} device_verify=0 wall_ms={warmup_wall_ms:.3} generated_tokens={} generated_chars={warmup_chars} text_sha256={warmup_sha256}",
        params.spec_k,
        warmup.tokens_generated,
    );
    canonical = Some((warmup.text, warmup.tokens_generated));
    ...
    for run_index in 0..repeat {
        ...
        eprintln!(
            "[MTP_ABAB] run={} variant={variant_label} batch_verify={batch_verify} spec_k={} device_verify=0 measured=true wall_ms={wall_ms:.3} generated_tokens={} generated_chars={generated_chars} text_sha256={generated_sha256} equality={equality_label}",
            run_index + 1,
            run_params.spec_k,
            result.tokens_generated,
        );
    ...
    let b_median_ms = b_median_ms
        ...
        .unwrap_or_else(|| "NA".to_string());
    eprintln!(
        "[MTP_ABAB] summary warmup=1 measured_runs={repeat} a_samples={} b_samples={} a_median_ms={a_median_ms} b_median_ms={b_median_ms}",
        a_ms.len(),
        b_ms.len()
    );
}
...
fn report_kv_cache_metrics(engine: &rnb_llm::Engine) {
    let metrics = engine.kv_cache_metrics();
    let key_snr = metrics
        .key_quant_snr_db
        .map(|value| format!("{value:.2}"))
        .unwrap_or_else(|| "n/a".to_string());
    let value_snr = metrics
        .value_quant_snr_db
        .map(|value| format!("{value:.2}"))
        .unwrap_or_else(|| "n/a".to_string());
    let capacity_reduction_pct = if metrics.f16_equivalent_capacity_bytes == 0 {
        0.0
    } else {
        100.0 * (metrics.f16_equivalent_capacity_bytes as f64 - metrics.capacity_bytes as f64)
            / metrics.f16_equivalent_capacity_bytes as f64
    };
    eprintln!(
        "RNB_KV_CACHE_METRICS tokens={} compressed_layers={} quantized_token_rows={} allocated_mib={:.3} capacity_mib={:.3} f16_equivalent_capacity_mib={:.3} capacity_reduction_pct={:.2} key_quant_snr_db={} value_quant_snr_db={}",
        metrics.current_tokens,
        metrics.compressed_layers,
        metrics.quantized_token_rows,
        metrics.allocated_bytes as f64 / (1024.0 * 1024.0),
        metrics.capacity_bytes as f64 / (1024.0 * 1024.0),
        metrics.f16_equivalent_capacity_bytes as f64 / (1024.0 * 1024.0),
...
fn build_gemma_chat_tokens(engine: &rnb_llm::Engine, bos: u32, prompt: &str) -> Vec<u32> {
    // Prefer the GGUF-owned Jinja template. The legacy hand-written form stays
    // available for diagnostic role/template overrides and old files without
    // tokenizer.chat_template metadata.
    let custom_format = std::env::var_os("RNB_GEMMA_MINIMAL_CHAT").is_some()
        || std::env::var_os("RNB_GEMMA_USER_ROLE").is_some()
        || std::env::var_os("RNB_GEMMA_MODEL_ROLE").is_some();
    let rendered = if custom_format || engine.tokenizer.chat_template().is_none() {
        None
    } else {
        let mut messages = Vec::with_capacity(2);
        if let Ok(system) = std::env::var("RNB_SYSTEM_PROMPT") {
            if !system.trim().is_empty() {
                messages.push(rnb_llm::ChatMessage::new("system", system));
            }
        }
        messages.push(rnb_llm::ChatMessage::new("user", prompt));
        engine
            .tokenizer
            .render_chat_prompt(
                &messages,
                rnb_llm::ChatTemplateOptions {
                    add_generation_prompt: true,
                    enable_thinking: std::env::var("RNB_GEMMA_INCLUDE_THINK")
                        .map(|value| value != "0")
                        .unwrap_or(false),
                },
            )
    ...
    let text = rendered.unwrap_or_else(|| build_gemma_chat_prompt_text(prompt));
...
fn main() {
    ...
    let load_config = rnb_llm::EngineLoadConfig::default();
    ...
    let engine_init_ms = load_start.elapsed().as_secs_f64() * 1000.0;
    ...
    if let Ok(raw) = std::env::var("RNB_PREFILL_ABAB_REPEAT") {
        ...
        if repeat < 2 {
            eprintln!(
                "[ABAB] RNB_PREFILL_ABAB_REPEAT={raw} 파싱 실패 또는 2 미만 — screen은 N=2, 최종 판정은 N=3이어야 함. 종료."
            );
            return;
        }
    ...
    if matches!(mtp_env_request, MtpEnvRequest::Auto) {
        let resource = mtp_policy
            ...
            .unwrap_or_default();
        let reason = if mtp_policy.enabled && !mtp_sampling_allowed {
            "sampling-not-greedy"
        } else {
            mtp_policy.reason
        };
    ...
    if spec_mode.as_deref() == Some("1") && !mtp_env_requested {
        ...
        if std::env::var("RNB_BENCH_WALL").is_ok() {
            eprintln!(
                "RNB_BENCH_WALL request_class=cold engine_init_ms={:.3} spec_ms={:.3} total_ms={:.3} prompt_tokens={} decode_tokens={}",
                engine_init_ms,
                spec_ms,
                engine_init_ms + spec_ms,
                result.prompt_tokens,
                result.tokens_generated,
            );
        }
    ...
    if mtp_generate_mode_requested(spec_mode.as_deref(), mtp_env_requested, generate_env_set) {
        ...
        let result = match engine.generate_stream(effective_prompt, &params, |piece| {
            if !quiet_generate {
                eprint!("{}", piece);
            }
            true
        ...
        if std::env::var("RNB_BENCH_WALL").is_ok() {
            eprintln!(
                "RNB_BENCH_WALL request_class=cold engine_init_ms={:.3} generate_ms={:.3} total_ms={:.3} prompt_tokens={} decode_tokens={}",
                engine_init_ms,
                generate_ms,
                engine_init_ms + generate_ms,
                result.prompt_tokens,
                result.tokens_generated,
            );
        }
        let warm_reruns = std::env::var("RNB_WARM_RERUNS")
            .ok()
        ...
        if warm_reruns > 0 {
            ...
            let mut steady_tokens_stable = true;
            for run_idx in 0..warm_reruns {
                ...
                let warm_result = engine
                    ...
                    .unwrap();
                dump_generated_text_hash_if_requested(&warm_result);
                let warm_generate_ms = warm_start.elapsed().as_secs_f64() * 1000.0;
                steady_generate_ms.push(warm_generate_ms);
                let first_diff_cold =
                    first_token_diff(&cold_reference_tokens, &warm_result.generated_token_ids);
                let first_diff_steady = steady_reference_tokens.as_ref().and_then(|reference| {
                    first_token_diff(reference, &warm_result.generated_token_ids)
                });
                steady_tokens_stable &= first_diff_steady.is_none();
                if steady_reference_tokens.is_none() {
                    steady_reference_tokens = Some(warm_result.generated_token_ids.clone());
                }
                eprintln!(
                    "RNB_STEADY_RUN run={} sequence_run={} generate_ms={:.3} prompt_tokens={} decode_tokens={} cached_prompt_tokens={} cold_token_match={} first_diff_cold={first_diff_cold:?} steady_token_match={} first_diff_steady={first_diff_steady:?}",
                    run_idx + 1,
                    run_idx + 2,
                    warm_generate_ms,
                    warm_result.prompt_tokens,
                    warm_result.tokens_generated,
                    warm_result.cached_prompt_tokens,
                    first_diff_cold.is_none(),
                    first_diff_steady.is_none(),
                );
            }
            steady_generate_ms.sort_by(f64::total_cmp);
            let median_generate_ms = if steady_generate_ms.len() % 2 == 0 {
                let upper = steady_generate_ms.len() / 2;
                (steady_generate_ms[upper - 1] + steady_generate_ms[upper]) / 2.0
            } else {
                steady_generate_ms[steady_generate_ms.len() / 2]
            };
            eprintln!(
                "RNB_STEADY_SUMMARY runs={} median_generate_ms={median_generate_ms:.3} token_stable={steady_tokens_stable}",
                steady_generate_ms.len()
            );
        }
        report_kv_cache_metrics(&engine);
        print_profile_reports();
        return;
    }
    ...
    if std::env::var("RNB_BENCH_WALL").is_ok() {
        ...
        eprintln!(
            "RNB_BENCH_WALL request_class=cold engine_init_ms={:.3} prefill_ms={:.3} decode_ms={:.3} total_ms={:.3} prompt_tokens={} decode_tokens={}",
            engine_init_ms,
            prefill_ms,
            decode_ms,
            total_ms,
...
<|im_start|>assistant\n<think></think>"
    ...
    fn mtp_env_request_distinguishes_auto_from_force() {
        // pm118: 미설정 = auto (채택 정책 무-env 기본 ON, policy 가 안전 gate).
        assert_eq!(parse_mtp_env_request(None), MtpEnvRequest::Auto);
    ...
    #[test]
    fn mtp_auto_requires_greedy_sampling_but_force_stays_explicit() {
        assert!(mtp_auto_sampling_allowed(MtpEnvRequest::Auto, 1.0));
        assert!(!mtp_auto_sampling_allowed(MtpEnvRequest::Auto, 1.1));
        assert!(mtp_auto_sampling_allowed(MtpEnvRequest::Force, 1.1));
    }
    ...
    #[test]
    fn mtp_abab_median_includes_all_post_warmup_measurements() {
        let (a_median, b_median) = mtp_abab_medians(&[30.0, 10.0, 20.0], &[40.0, 20.0, 30.0]);
        assert_eq!(a_median, Some(20.0));
        assert_eq!(b_median, Some(30.0));
        assert_eq!(mtp_abab_medians(&[10.0], &[40.0]), (Some(10.0), Some(40.0)));
    }
