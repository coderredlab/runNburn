...
pub fn dequantize_bytes_to_f32(bytes: &[u8], dequant_type: DequantType) -> Vec<f32> {
    ...
    match dequant_type {
        ...
        DequantType::IQ4XS => dequant_iq4_xs(bytes),
    }
}
    ...
    bytes
        ...
        .for_each(|(chunk, dst)| {
            ...
            for ib in 0..8 {
                ...
                let base = ib * 32;
                for j in 0..16 {
                    dst[base + j] = dl * KVALUES_IQ4NL[(q[j] & 0x0f) as usize];
                    dst[base + j + 16] = dl * KVALUES_IQ4NL[(q[j] >> 4) as usize];
                }
            }
        });
    out
