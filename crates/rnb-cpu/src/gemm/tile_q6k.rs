...
pub fn gemm_q6k_packed(
    ...
    let required_output_len = rows
        .checked_mul(seq_len)
        .expect("Q6_K GEMM output length overflow");
    assert!(
        output.len() >= required_output_len,
        "Q6_K GEMM output is shorter than seq_len * rows"
    );
...
#[inline(always)]
fn q6k_parallel_row_range(task: usize, rows: usize) -> std::ops::Range<usize> {
    let start = task * 8;
...
#[inline(always)]
unsafe fn q6k_parallel_output_add(
    out_addr: usize,
    output_len: usize,
...
unsafe fn gemm_q6k_packed_neon(
    ...
    (0..row_groups).into_par_iter().for_each(|task| {
        let row_range = q6k_parallel_row_range(task, rows);
...
mod tests {
    ...
    #[test]
    fn q6k_parallel_row_tasks_are_disjoint_and_cover_all_rows() {
        for rows in 1usize..18 {
            let ranges = (0..rows.div_ceil(8))
                .map(|task| q6k_parallel_row_range(task, rows))
                .collect::<Vec<_>>();
    ...
    #[test]
    #[should_panic(expected = "Q6_K GEMM output is shorter than seq_len * rows")]
    fn q6k_gemm_rejects_short_output_before_parallel_write() {
        let mut output = [];
        gemm_q6k_packed(&[], &[], &[], &mut output, 1, 0, 1);
    }

    #[test]
    #[should_panic(expected = "Q6_K GEMM output length overflow")]
    fn q6k_gemm_rejects_output_length_overflow() {
        let mut output = [];
        gemm_q6k_packed(&[], &[], &[], &mut output, 2, 0, usize::MAX / 2 + 1);
    }
