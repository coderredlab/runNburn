...
fn output_logits_impl(
    ...
    match ggml_type {
        GGMLType::Q4_K => backend::q4k_gemv(raw, rows, cols, normed_data).ok(),
        GGMLType::Q6_K => backend::q6k_gemv(raw, rows, cols, normed_data).ok(),
        GGMLType::Q8_0 => backend::q8_0_gemv(raw, rows, cols, normed_data).ok(),
        _ => None,
    }
}
