...
pub struct MtpDeviceVerifyWindowRequest<'a> {
    ...
    pub attention_kvarn_priors: &'a [Option<rnb_backend_api::KvarnDecodeRequest<'a>>],
...
#[derive(Debug)]
pub struct MtpDeviceDraftRequest<'a> {
    ...
    pub rope_neox: bool,
    pub rope_theta: f32,
    pub layer: &'a MtpDeviceVerifyAttentionMoeLayer<'a>,
...
#[derive(Debug)]
pub struct MtpDeviceDraftResult {
    pub token: u32,
    pub hidden: Vec<f32>,
    pub attention_kv: MtpDeviceVerifyAttentionKvState,
}
...
pub struct MtpDeviceVerifySsmLayerPrefixState {
    ...
    pub resident_conv_snapshot: Option<backend::DeltaStateSnapshot>,
...
fn backend_attention_moe_layer<'a>(
    layer: &'a MtpDeviceVerifyAttentionMoeLayer<'_>,
    kvarn_prior: Option<rnb_backend_api::KvarnDecodeRequest<'a>>,
) -> backend::Qwen35MtpDeviceVerifyAttentionMoeLayer<'a> {
    backend::Qwen35MtpDeviceVerifyAttentionMoeLayer {
        ...
        prior_sequence_epoch: layer.prior_sequence_epoch,
        kvarn_prior,
        ...
        q_norm: layer.q_norm,
        k_norm: layer.k_norm,
        qk_norm_unit_offset: layer.qk_norm_unit_offset,
        v_no_scale_norm: layer.v_no_scale_norm,
        rope_freq_factors: layer.rope_freq_factors,
        post_attn_norm: layer.post_attn_norm,
        post_attn_norm_unit_offset: layer.post_attn_norm_unit_offset,
        ffn_norm_unit_offset: layer.ffn_norm_unit_offset,
        post_ffw_norm_unit_offset: layer.post_ffw_norm_unit_offset,
        ffn_norm: layer.ffn_norm,
        post_ffw_norm: layer.post_ffw_norm,
        out_scale: layer.out_scale,
        ple_gate: layer.ple_gate,
        ple_gate_quant: layer.ple_gate_quant as u32,
        ple_gate_rows: layer.ple_gate_rows,
        ple_gate_cols: layer.ple_gate_cols,
        ple_proj: layer.ple_proj,
        ple_proj_quant: layer.ple_proj_quant as u32,
        ple_proj_rows: layer.ple_proj_rows,
        ple_proj_cols: layer.ple_proj_cols,
        ple_post_norm: layer.ple_post_norm,
        ple_post_norm_unit_offset: layer.ple_post_norm_unit_offset,
        ple_input: &layer.ple_input,
        ffn_uses_gelu: layer.ffn_uses_gelu,
        ...
        expert_gating_func: layer.expert_gating_func,
        shared_expert_gated: layer.shared_expert_gated,
        ...
        shared_gate_quant: layer.shared_gate_quant as u32,
        shared_up: layer.shared_up,
        shared_up_quant: layer.shared_up_quant as u32,
...
#[derive(Debug)]
pub struct QwenAttentionDeviceInputOutput {
    pub normalized_id: DeviceTensorId,
    pub normalized_desc: DeviceTensorDesc,
    pub residual_id: DeviceTensorId,
    pub residual_desc: DeviceTensorDesc,
    pub attention_kv: MtpDeviceVerifyAttentionKvState,
}
...
pub fn qwen35_prefill_attention_device_input(
    ...
    layer: &MtpDeviceVerifyAttentionMoeLayer<'_>,
    seq_len: usize,
    hidden_dim: usize,
    rope_dim: usize,
    rope_neox: bool,
    ...
    let backend_layer = backend_attention_moe_layer(layer, None);
    let output = backend::qwen35_prefill_attention_device_input(
        input_id,
        input_desc,
        &backend_layer,
        seq_len,
    ...
    Ok(QwenAttentionDeviceInputOutput {
        ...
        residual_id: output.residual_id,
        residual_desc: output.residual_desc,
        attention_kv: MtpDeviceVerifyAttentionKvState {
            layer_idx: output.attention_kv.layer_idx,
            window_tokens: output.attention_kv.window_tokens,
            kv_rows: output.attention_kv.kv_rows,
            k_bits: output.attention_kv.k_bits,
            v_bits: output.attention_kv.v_bits,
            device_resident: output.attention_kv.device_resident,
        },
    })
}

pub fn qwen35_mtp_device_draft_step(
    request: MtpDeviceDraftRequest<'_>,
) -> Result<MtpDeviceDraftResult> {
    let backend_layer = backend_attention_moe_layer(request.layer, None);
    let result = backend::qwen35_mtp_device_draft_step(backend::Qwen35MtpDeviceDraftRequest {
        projected_hidden: request.projected_hidden,
    ...
    Ok(MtpDeviceDraftResult {
        ...
        attention_kv: MtpDeviceVerifyAttentionKvState {
            layer_idx: result.attention_kv.layer_idx,
            window_tokens: result.attention_kv.window_tokens,
            kv_rows: result.attention_kv.kv_rows,
            k_bits: result.attention_kv.k_bits,
            v_bits: result.attention_kv.v_bits,
            device_resident: result.attention_kv.device_resident,
        },
    })
}
...
pub fn qwen35_mtp_device_verify_window(
    ...
    if request.attention_kvarn_priors.len() != request.attention_moe_layers.len() {
        return Err(format!(
            "MTP device verify KVarN prior count {} != attention layer count {}",
            request.attention_kvarn_priors.len(),
            request.attention_moe_layers.len()
        ));
    }
    let backend_attention_layers = request
        .attention_moe_layers
        .iter()
        .zip(request.attention_kvarn_priors.iter().copied())
        .map(|(layer, kvarn_prior)| backend_attention_moe_layer(layer, kvarn_prior))
        .collect::<Vec<_>>();
    ...
    let mut backend_layers = request
        ...
        .map(|layer| backend::Qwen35MtpDeviceVerifyGdnMoeLayer {
            ...
            gate_quant: layer.gate_quant as u32,
    ...
    backend::qwen35_mtp_device_verify_window(backend::Qwen35MtpDeviceVerifyRequest {
        ...
        pos_start: request.pos_start,
        rope_pos_start: request.rope_pos_start,
        hidden_dim: request.hidden_dim,
        rope_dim: request.rope_dim,
        rope_neox: request.rope_neox,
        rope_theta: request.rope_theta,
...
fn qwen_moe_prefill_combined_f32_shared_sparse_device_topk_selected_base_device_input_impl(
    ...
    let packed_shared = [shared_gate_quant, shared_up_quant, shared_down_quant]
        .into_iter()
        .all(|quant| matches!(quant, GGMLType::Q4_K | GGMLType::Q6_K | GGMLType::Q8_0));
    let output = if packed_shared {
        backend::qwen35_prefill_moe_quant_shared_sparse_device_topk_selected_base_device_input(
            shared_gate_bytes,
            shared_gate_quant as u32,
            shared_up_bytes,
            shared_up_quant as u32,
            shared_down_bytes,
            shared_down_quant as u32,
            shared_input_scale,
            gate_all,
...
pub fn qwen_moe_configure_layer_cache(
    model_moe_bytes: usize,
    runtime_headroom_bytes: usize,
    min_layer_bytes: usize,
) -> Result<usize> {
    backend::qwen35_configure_moe_layer_cache(
        model_moe_bytes,
        runtime_headroom_bytes,
        min_layer_bytes,
    )
    .map_err(|err| format!("CUDA MoE layer cache configuration failed: {err}"))
}
