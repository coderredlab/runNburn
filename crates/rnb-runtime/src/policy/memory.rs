...
impl HostMemoryPlan {
    ...
    /// Builds a plan from an explicit budget, or physical RAM when none was supplied.
    ///
    /// Automatic policy reserves one quarter of physical RAM for the operating
    /// system, KV cache, runtime buffers, and other processes. The remaining
    /// three quarters bound engine-owned file-backed sparse-expert residency.
    pub fn automatic(
        budget: Option<MemoryBudget>,
        mapped_weight_bytes: u64,
        cuda_available: bool,
    ...
    fn from_capacity(
        ...
        let reserved_bytes = total_bytes / HOST_RUNTIME_RESERVE_DIVISOR;
...
mod tests {
    ...
    #[test]
    fn automatic_budget_reserves_quarter_of_physical_ram() {
        let plan = HostMemoryPlan::from_capacity(None, 100 * GIB, true, Some(16 * GIB));
    ...
    fn explicit_budget_overrides_detected_physical_ram() {
        ...
        let plan = HostMemoryPlan::from_capacity(Some(explicit), 100 * GIB, true, Some(16 * GIB));
    ...
    fn unavailable_physical_ram_keeps_automatic_budget_disabled() {
        let plan = HostMemoryPlan::from_capacity(None, 100 * GIB, true, None);
