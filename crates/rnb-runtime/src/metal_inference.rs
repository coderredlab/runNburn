...
fn qwen_prefill_chain_quant(
    ggml_type: GGMLType,
) -> Option<(rnb_backend_metal::TensoropsQuant, usize, usize)> {
    match ggml_type {
        GGMLType::Q4_K => Some((rnb_backend_metal::TensoropsQuant::Q4K, 256, 144)),
        GGMLType::Q5_K => Some((rnb_backend_metal::TensoropsQuant::Q5K, 256, 176)),
        GGMLType::Q6_K => Some((rnb_backend_metal::TensoropsQuant::Q6K, 256, 210)),
        GGMLType::Q8_0 => Some((rnb_backend_metal::TensoropsQuant::Q8_0, 32, 34)),
        GGMLType::Q2_K => Some((rnb_backend_metal::TensoropsQuant::Q2K, 256, 84)),
        GGMLType::Q3_K => Some((rnb_backend_metal::TensoropsQuant::Q3K, 256, 110)),
        _ => None,
    }
}
...
fn qwen_prefill_chain_quant_weight<'a>(
    ...
    let (quant, block_elements, block_bytes) = qwen_prefill_chain_quant(weight.ggml_type)?;
    if weight.rows != expected_rows
        || weight.cols != expected_cols
        || weight.cols == 0
        || weight.cols % block_elements != 0
    {
        return None;
    }
    let expected_raw_len = weight
        .rows
        .checked_mul(weight.cols / block_elements)?
...
fn qwen_prefill_chain_expected_quant_bytes(weight: &MetalQuantWeightRef<'_>) -> Option<usize> {
    let (_, block_elements, block_bytes) = qwen_prefill_chain_quant(weight.ggml_type)?;
    if weight.cols == 0 || weight.cols % block_elements != 0 {
        return None;
    }
    weight
        .rows
...
pub fn metal_kvarn_attention_decode_into_if_supported(
    request: KvarnDecodeRequest<'_>,
    attn_out: &mut [f32],
) -> Result<bool> {
    if !crate::policy::kvarn_attention_enabled(
        crate::policy::KvarnAttentionBackend::Metal,
        false,
        request.kv_len(),
        request.num_heads(),
        request.head_dim(),
    ...
    request.validate()?;
    if attn_out.len() < request.num_heads().saturating_mul(request.head_dim()) {
        return Err("Metal KVarN attention output slice is too short".to_string());
    }
    #[cfg(target_os = "macos")]
    {
        let output = METAL.with(|backend| backend.kvarn_attention_decode(request))?;
        attn_out[..output.len()].copy_from_slice(&output);
        Ok(true)
    }
...
/// 소유 타입 `GdnChainSpecRef` 를 직접 못 만든다. 그래서 seam 친화 spec 을 두고 seam 이
/// `GdnChainSpecRef` 로 변환한다. weight 와 state 는 backend 호출 동안만 borrow 한다.
#[cfg(feature = "metal")]
...
fn gdn_quant_code(t: GGMLType) -> Option<u8> {
    match t {
        ...
        GGMLType::F32 => Some(4),
        GGMLType::Q3_K => Some(5),
        GGMLType::Q2_K => Some(6),
        _ => None,
    }
}
...
fn gdn_moe_qwen_chain_ref_if_supported<'a>(
    ...
    let shared_q8_0 = s.shared_gate_ggml == GGMLType::Q8_0
        && s.shared_up_ggml == GGMLType::Q8_0
        && s.shared_down_ggml == GGMLType::Q8_0;
    if s.gate_exps_ggml != GGMLType::Q4_K || s.up_exps_ggml != GGMLType::Q4_K {
        return Err("unsupported qwen moe gate/up quant");
    }
    if !shared_q8_0 && (s.shared_gate_ggml != GGMLType::Q4_K || s.shared_up_ggml != GGMLType::Q4_K)
    {
        return Err("unsupported qwen moe shared gate/up quant");
    }
    let Some(down_quant) = qwen_moe_decode_down_quant_code(s.down_exps_ggml) else {
        return Err("unsupported qwen moe down quant");
    };
    if !shared_q8_0 && s.shared_down_ggml != s.down_exps_ggml {
        return Err("unsupported qwen moe shared down quant");
    }
    if shared_q8_0 && (s.hidden_dim % 32 != 0 || s.n_ff % 32 != 0) {
        return Err("unsupported qwen moe shared q8_0 shape");
    }
    ...
    if s.gate_exps_raw.len() != s.n_expert * s.gate_expert_bytes
        ...
        || s.shared_gate_raw.len()
            != if shared_q8_0 {
                s.n_ff * (s.hidden_dim / 32) * 34
            } else {
                s.gate_expert_bytes
            }
        || s.shared_up_raw.len()
        ...
        || s.shared_down_raw.len()
            != if shared_q8_0 {
                s.hidden_dim * (s.n_ff / 32) * 34
            } else {
                s.down_expert_bytes
            }
    {
...
fn attn_moe_qwen_chain_ref_if_supported<'a>(
    ...
    // q/k/v/o decode GEMV: Q4_K/Q5_K/Q6_K/Q8_0 지원(unsloth UD 는 q/k/v/o=Q8_0).
    // gdn_quant_code 와 동일 code 공간(0=Q4K/1=Q5K/2=Q6K/3=Q8_0)을 encode_gemv_quant 가 dispatch.
    let (Some(q_q), Some(k_q), Some(v_q), Some(o_q)) = (
        gdn_quant_code(s.q_ggml),
        gdn_quant_code(s.k_ggml),
        gdn_quant_code(s.v_ggml),
        gdn_quant_code(s.o_ggml),
    ) else {
...
/// `MetalChainLayer` → backend `ChainLayerSpecRef` 변환. 단일(`metal_decode_chain_run`)과
/// 배치(`metal_decode_chain_run_batched_collect_attn_kv`) seam 이 공유한다. 미지원 quant/shape
/// 는 `Err(reason)` — caller 가 fallback report 로 매핑한다.
#[cfg(feature = "metal")]
fn chain_layer_to_ref<'a>(
    s: &'a MetalChainLayer<'a>,
    ...
    Ok(match s {
        MetalChainLayer::Attn(a) => {
            ...
            let ffn_down_is_q6k = match a.ffn_down_ggml {
                GGMLType::Q4_K => false,
                GGMLType::Q6_K => true,
                _ => return Err("unsupported attention ffn_down quant"),
            };
        ...
        MetalChainLayer::AttnMoeQwen(s) => rnb_backend_metal::ChainLayerSpecRef::AttnMoeQwen(
            attn_moe_qwen_chain_ref_if_supported(s)?,
        ),
        MetalChainLayer::Gdn(s) => {
            ...
            rnb_backend_metal::ChainLayerSpecRef::Gdn(rnb_backend_metal::GdnChainSpecRef {
                layer: s.layer,
                conv_state: s.conv_state,
                delta_state: s.delta_state,
                attn_norm_weight: s.attn_norm_weight,
        ...
        MetalChainLayer::GdnMoeQwen(s) => rnb_backend_metal::ChainLayerSpecRef::GdnMoeQwen(
            gdn_moe_qwen_chain_ref_if_supported(s)?,
        ),
    })
}

/// output argmax tail(vocab projection) 변환. 단일/배치 seam 공유. 미지원 quant 는 `Err`.
#[cfg(feature = "metal")]
fn chain_output_argmax_to_ref<'a>(
    output_argmax: Option<MetalDecodeOutputArgmax<'a>>,
) -> std::result::Result<Option<rnb_backend_metal::DecodeOutputArgmaxSpecRef<'a>>, &'static str> {
    let Some(output) = output_argmax else {
        return Ok(None);
    };
    ...
    Ok(Some(rnb_backend_metal::DecodeOutputArgmaxSpecRef {
...
pub fn metal_glm_mla_head_slots_gemv_into_if_supported(
    ...
    if heads == 0 || slots == 0 || input.len() != slots * cols || out.len() != slots * rows_per_head
    {
        return Ok(false);
    }
    // pm117: Q5_K(o/q_a) slots 커널 추가 — super-block 이라 cols % 256 필요.
    // `RNB_METAL_GLM_MLA_Q5K_SLOTS=0` 은 CPU fallback 대조용 진단 opt-out.
    match ggml_type {
        GGMLType::Q8_0 if cols % 32 == 0 => {}
        GGMLType::Q5_K if cols % 256 == 0 && !env_falsey("RNB_METAL_GLM_MLA_Q5K_SLOTS") => {
            let result = METAL.with(|backend| {
                backend.glm_mla_head_gemv_q5k_slots_resident(
                    raw,
                    input,
                    slots,
                    heads,
...
/// pm116: GLM MLA prefill attention (absorbed form) 을 단일 dispatch 로.
/// `RNB_METAL_GLM_PREFILL_ATTN=0` opt-out, 기본 ON.
#[allow(clippy::too_many_arguments)]
pub fn metal_glm_mla_prefill_attn_into_if_supported(
    q_absorbed: &[f32],
    q_pe: &[f32],
    cache: &[u16],
    slots: usize,
    ...
    if env_falsey("RNB_METAL_GLM_PREFILL_ATTN") {
        return Ok(false);
    }
    if slots == 0
        || head_count == 0
        ...
        || kv_rank > 512
        || rope_dim > 64
        || q_absorbed.len() != slots * kv_rank
        || q_pe.len() != slots * rope_dim
        || latent_out.len() != slots * kv_rank
        || cache.len() < (pos_start + slots / head_count) * (kv_rank + rope_dim)
    {
        return Ok(false);
    }
    let result = METAL.with(|backend| {
        backend.glm_mla_prefill_attn(
            q_absorbed, q_pe, cache, slots, head_count, kv_rank, rope_dim, pos_start, scale,
        )
    });
    latent_out.copy_from_slice(&result);
    Ok(true)
}
...
/// pm118: fused 는 verify 처럼 seq 가 작을 때만 이득 — 큰 prefill 배치에선
/// 중간 concat 의 device-private 할당(층당 수십 MB)이 왕복 절감을 역전한다
/// (537tok 2페어 ON 열세). 경계는 pm117 후속 3 stage overlap 과 같은 기전
/// 영역이라 동일 기본 32 를 쓰고 `RNB_METAL_GLM_MLA_VB_O_FUSED_MAX_SEQ` 로
/// override (seq_len >= 경계면 분리 경로 fallback).
fn glm_mla_vb_o_fused_max_seq() -> usize {
    static MAX_SEQ: std::sync::OnceLock<usize> = std::sync::OnceLock::new();
    *MAX_SEQ.get_or_init(|| {
        std::env::var("RNB_METAL_GLM_MLA_VB_O_FUSED_MAX_SEQ")
            .ok()
...
/// pm119 layer fused 반환 (glue 용 재-export).
pub use rnb_backend_metal::GlmMlaLayerFusedOut;

/// pm119: verify/작은 prefill 배치의 MLA 층 전체 (front→kv rms/rope→q_pe
/// rope→attn→v_b→o) 를 단일 command buffer 로. cache 는 base(CPU 업로드) +
/// tail(GPU f16 기록), 반환은 (projected, cache_tail).
/// `RNB_METAL_GLM_MLA_LAYER_FUSED=0` opt-out, 기본 ON.
#[allow(clippy::too_many_arguments)]
pub fn metal_glm_mla_layer_fused_if_supported(
    qa_quant: GGMLType,
    qb_quant: GGMLType,
    kva_quant: GGMLType,
    kb_quant: GGMLType,
    ...
    theta: f32,
    scale: f32,
) -> Result<Option<GlmMlaLayerFusedOut>> {
    if env_falsey("RNB_METAL_GLM_MLA_GEMV") || env_falsey("RNB_METAL_GLM_MLA_LAYER_FUSED") {
        return Ok(None);
    }
    if seq_len == 0 || seq_len >= glm_mla_vb_o_fused_max_seq() {
        return Ok(None);
    }
    let kv_width = kv_rank + rope_dim;
    let o_is_q5k = match o_quant {
        GGMLType::Q5_K if o_cols % 256 == 0 && !env_falsey("RNB_METAL_GLM_MLA_Q5K_SLOTS") => true,
        GGMLType::Q8_0 if o_cols % 32 == 0 => false,
        _ => return Ok(None),
    };
    if qa_quant != GGMLType::Q5_K
        || qb_quant != GGMLType::Q8_0
        ...
        || heads == 0
        || q_dim % heads != 0
        || o_cols != heads * value_dim
        || qa_norm_weight.len() != q_rank
        || kv_norm_weight.len() != kv_rank
        || cache_base.len() != pos_start * kv_width
        || normed.len() != seq_len * hidden
    {
        return Ok(None);
    }
    let qk_dim = q_dim / heads;
    if qk_dim != q_nope_dim + rope_dim {
        return Ok(None);
    }
    let out = METAL.with(|backend| {
        backend.glm_mla_layer_fused_resident(
            qa_raw,
            qa_norm_weight,
            qb_raw,
            kva_raw,
...
/// pm118 연장: verify/작은 prefill 배치의 front 4 dispatch (q_a→rms→q_b/kv_a
/// →pack→k_b) 를 단일 command buffer 로. slots 커널 형태 유지, 중간 버퍼가
/// seq 비례라 vb→o fused 와 같은 seq 경계 안에서만.
/// `RNB_METAL_GLM_MLA_FRONT_FUSED=0` opt-out, 기본 ON.
#[allow(clippy::too_many_arguments)]
pub fn metal_glm_mla_front_slots_fused_into_if_supported(
    qa_quant: GGMLType,
    qb_quant: GGMLType,
    kva_quant: GGMLType,
    kb_quant: GGMLType,
    ...
    if env_falsey("RNB_METAL_GLM_MLA_GEMV") || env_falsey("RNB_METAL_GLM_MLA_FRONT_FUSED") {
        return Ok(false);
    }
    if seq_len == 0 || seq_len >= glm_mla_vb_o_fused_max_seq() {
        return Ok(false);
    ...
    if qa_quant != GGMLType::Q5_K
        ...
        || qa_norm_weight.len() != q_rank
        || normed.len() != seq_len * hidden
        || q_out.len() != seq_len * q_dim
        || kv_out.len() != seq_len * kv_width
        || qabs_out.len() != seq_len * heads * kv_rank
    {
        return Ok(false);
    }
    let qk_dim = q_dim / heads;
    if qk_dim <= q_nope_dim {
        return Ok(false);
    }
    let (q, kv_raw, qabs) = METAL.with(|backend| {
        backend.glm_mla_front_slots_fused_resident(
            qa_raw,
            qa_norm_weight,
            qb_raw,
            kva_raw,
    ...
    q_out.copy_from_slice(&q);
    kv_out.copy_from_slice(&kv_raw);
    qabs_out.copy_from_slice(&qabs);
    Ok(true)
}

/// pm118: verify/prefill 배치의 v_b slots → o slots 를 단일 command buffer 로.
/// slots 커널 형태 유지 (pm117 후속 6: chain 개별-dispatch 이식 금지).
/// `RNB_METAL_GLM_MLA_VB_O_FUSED=0` opt-out, 기본 ON.
#[allow(clippy::too_many_arguments)]
pub fn metal_glm_mla_vb_o_fused_into_if_supported(
    vb_quant: GGMLType,
    o_quant: GGMLType,
    vb_raw: &[u8],
    o_raw: &[u8],
    ...
    if env_falsey("RNB_METAL_GLM_MLA_GEMV") || env_falsey("RNB_METAL_GLM_MLA_VB_O_FUSED") {
        return Ok(false);
    }
    if heads == 0 || slots == 0 || slots % heads != 0 {
        return Ok(false);
    }
    let seq_len = slots / heads;
    if seq_len >= glm_mla_vb_o_fused_max_seq() {
        return Ok(false);
    }
    let o_is_q5k = match o_quant {
        GGMLType::Q5_K if o_cols % 256 == 0 && !env_falsey("RNB_METAL_GLM_MLA_Q5K_SLOTS") => true,
    ...
    if vb_quant != GGMLType::Q8_0
        || kv_rank % 32 != 0
        || o_cols != heads * value_dim
        || latent.len() != slots * kv_rank
        || out.len() != seq_len * o_rows
    {
        return Ok(false);
    }
    let result = METAL.with(|backend| {
        backend.glm_mla_vb_o_fused_resident(
            vb_raw, o_raw, o_is_q5k, latent, slots, heads, value_dim, kv_rank, o_rows,
        )
    });
    out.copy_from_slice(&result);
    Ok(true)
...
pub fn metal_prefill_gdn_proj_into_if_supported_with_trace(
    ...
    let quant = match weight_ggml {
        ...
        GGMLType::Q8_0 => TensoropsQuant::Q8_0,
        GGMLType::Q2_K => TensoropsQuant::Q2K,
        GGMLType::Q3_K => TensoropsQuant::Q3K,
        // F32 등 tensorops 미지원 quant → host fallback (블록 크기 오독 방지)
        _ => return Ok(false),
    };
...
fn tensorops_quant_from_ggml(ggml: GGMLType) -> Option<rnb_backend_metal::TensoropsQuant> {
    match ggml {
        ...
        GGMLType::Q8_0 => Some(rnb_backend_metal::TensoropsQuant::Q8_0),
        GGMLType::Q2_K => Some(rnb_backend_metal::TensoropsQuant::Q2K),
...
mod metal_qwen_moe_prefill_accum_policy_tests {
    ...
    fn qwen_moe_prefill_mulmmid_v3_defaults_off_with_truthy_opt_in() {
        ...
        std::env::set_var(llama_id_key, "0");
        std::env::remove_var(key);
