...
mod policy_tests {
    ...
    #[test]
    fn moe_prefill_full_cores_defaults_on_android_and_supports_override() {
        let _guard = env_lock().lock().expect("policy test env lock poisoned");
        const ENV: &str = "RNB_MOE_PREFILL_FULL_CORES";

        std::env::remove_var(ENV);
        assert_eq!(
            moe_prefill_full_cores_enabled(),
            cfg!(all(target_arch = "aarch64", target_os = "android"))
        );

        std::env::set_var(ENV, "1");
        assert!(moe_prefill_full_cores_enabled());
    ...
    #[test]
    fn cuda_nemotron_gpu_only_policies_ignore_legacy_opt_outs() {
        let _guard = env_lock().lock().expect("policy test env lock poisoned");
        for key in [
            "RNB_CUDA_NEMOTRON_DEVICE_HIDDEN_CARRIER",
            "RNB_CUDA_NEMOTRON_DEVICE_ROUTE_PACK",
    ...
    #[test]
    fn kvarn_attention_policy_scales_with_work_and_honors_overrides() {
        let _guard = env_lock().lock().expect("policy test env lock poisoned");
        std::env::remove_var("RNB_CUDA_KVARN_ATTN");
        std::env::remove_var("RNB_METAL_KVARN_ATTN");

        assert!(kvarn_attention_enabled(
            KvarnAttentionBackend::Cuda,
            true,
            1,
            16,
            256,
        ...
        std::env::set_var("RNB_CUDA_KVARN_ATTN", "0");
        assert!(!kvarn_attention_enabled(
            KvarnAttentionBackend::Cuda,
            true,
            2048,
        ...
        std::env::set_var("RNB_METAL_KVARN_ATTN", "1");
        assert!(kvarn_attention_enabled(
            KvarnAttentionBackend::Metal,
            false,
            393,
