...
//!
//! The expert row is **11 columns** as of Session 68 Phase 0. The final two
//! columns resolve the two measurement-semantics issues Session 67 uncovered:
//!  - `expert_original_id`: popularity-space id (inverse of `.rnb` hot-sort).
//!    Equals `expert` when no permutation is active. Downstream analyzers
//!    use this when filtering by offline top-N popularity.
//!  - `tier`: which backing store the GEMV read from — `hot` (packed_model
//!    mmap or hot pool), `cold_mmap` (mmap'd cold companion), or
//!    `cold_pread` (O_DIRECT owned buffer). mincore on `cold_pread` is
//!    always 100% resident (heap), so analyzers must exclude that tier to
//!    avoid a bimodal-average artifact.
//!
