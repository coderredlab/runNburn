...
impl TokenStreamDecoder {
    ...
    pub fn push(&mut self, tokenizer: &Tokenizer, id: u32) -> String {
        self.pending.extend(tokenizer.decoded_token_bytes(id));
        self.take_valid_text(false)
    }

    pub fn finish(&mut self) -> String {
        self.take_valid_text(true)
    }

    fn take_valid_text(&mut self, finish: bool) -> String {
        let mut output = String::new();
        loop {
            let (valid_len, invalid_len) = match std::str::from_utf8(&self.pending) {
                Ok(text) => {
...
impl Tokenizer {
    ...
    pub fn decode(&self, tokens: &[u32]) -> String {
        let bytes: Vec<u8> = tokens
            .iter()
            .flat_map(|&id| self.decoded_token_bytes(id))
            .collect();
...
mod tests {
    ...
    #[test]
    fn gpt2_stream_decoder_preserves_utf8_split_across_tokens() {
        let (byte_to_unicode, _) = build_byte_unicode_maps();
        let emoji = "😄";
        let bytes = emoji.as_bytes();
        let first_half: String = bytes[..2]
            .iter()
            .map(|&byte| byte_to_unicode[byte as usize])
            .collect();
        let second_half: String = bytes[2..]
            .iter()
            .map(|&byte| byte_to_unicode[byte as usize])
            .collect();
        let vocab = Vocab::new(
        ...
        assert_eq!(tokenizer.decode(&[3, 4]), emoji);

        let mut decoder = TokenStreamDecoder::new();
        assert_eq!(decoder.push(&tokenizer, 3), "");
        assert_eq!(decoder.push(&tokenizer, 4), emoji);
        assert_eq!(decoder.finish(), "");

        let mut incomplete = TokenStreamDecoder::new();
        assert_eq!(incomplete.push(&tokenizer, 3), "");
