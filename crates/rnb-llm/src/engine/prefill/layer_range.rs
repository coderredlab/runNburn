...
fn qwen_gdn_moe_output_device_enabled() -> bool {
    ...
    {
        std::env::var("RNB_CUDA_GDN_PREFILL_CHAIN_MOE_OUTPUT_DEVICE")
            .ok()
            .as_deref()
            == Some("1")
    }
...
fn trace_cuda_device_hidden_hash(
    ...
    if std::env::var_os("RNB_DEBUG_HIDDEN_HASH_TRACE").is_none() {
        return Ok(());
    }
    if let Some(filter) = std::env::var("RNB_DEBUG_HIDDEN_HASH_LAYER").ok() {
...
fn nemotron_prefill_v2_workspace_decision_label(
    decision: rnb_runtime::NemotronPrefillWorkspaceDecision,
) -> &'static str {
    match decision {
        rnb_runtime::NemotronPrefillWorkspaceDecision::Fits => "fits",
        rnb_runtime::NemotronPrefillWorkspaceDecision::Chunk { .. } => "chunk",
        rnb_runtime::NemotronPrefillWorkspaceDecision::Fallback => "fallback",
    }
}

#[cfg(any(feature = "cuda", test))]
fn nemotron_prefill_v2_workspace_trace_line(
    plan: &rnb_runtime::NemotronPrefillWorkspacePlan,
) -> String {
    format!(
...
fn maybe_plan_nemotron_prefill_v2_workspace(
    ...
    weights: &ModelWeights,
    seq_len: usize,
) -> Option<rnb_runtime::NemotronPrefillWorkspacePlan> {
    ...
    let request = rnb_runtime::NemotronPrefillWorkspaceRequest {
        seq_len,
    ...
    match rnb_runtime::plan_nemotron_prefill_workspace(request) {
        Ok(plan) => {
            if super::policy::cuda_device_prefill_trace_enabled() {
...
fn try_run_nemotron_moe_mamba2_device_pair(
    ...
    if !models::nemotron::mamba::device_prefill_mamba2_requested_for_chain() {
        return Ok(NemotronMoeMamba2DevicePairAttempt::MambaUnavailable);
    }
    let Some(moe_output) = models::nemotron::moe::forward_moe_layer_device_output_for_chain(
        metadata,
        hidden,
        moe_w,
        norm_eps,
        ...
        return Ok(NemotronMoeMamba2DevicePairAttempt::MoeUnavailable);
    };
    match models::nemotron::mamba::forward_mamba2_layer_from_device(
        ...
        models::nemotron::mamba::NemotronMamba2DeviceAttempt::Fallback(moe_output) => {
            let released = moe_output.release()?;
            if !released {
                return Err(crate::error::LlmError::Forward(
                    "CUDA MoE output device tensor was already missing during Mamba2 fallback"
                        .to_string(),
                ));
            }
            Ok(NemotronMoeMamba2DevicePairAttempt::MambaUnavailable)
        }
    }
}
...
fn metal_qwen_prefill_chain_enabled() -> bool {
    std::env::var("RNB_METAL_QWEN_PREFILL_CHAIN")
        .ok()
        .map(|value| {
            !matches!(
...
fn metal_qwen_prefill_chain_diag_layers() -> Option<usize> {
    std::env::var("RNB_METAL_QWEN_PREFILL_CHAIN_DIAG_LAYERS")
        .ok()
...
fn metal_qwen_prefill_chain_trace_enabled() -> bool {
    std::env::var("RNB_METAL_QWEN_PREFILL_CHAIN_TRACE")
        .ok()
        .is_some_and(|value| {
            !matches!(
                value.to_ascii_lowercase().as_str(),
...
fn metal_qwen_route_sort_enabled() -> bool {
    std::env::var("RNB_QWEN35_MOE_ROUTE_SORT")
        .ok()
        .map(|value| {
            !matches!(
                value.to_ascii_lowercase().as_str(),
...
fn metal_qwen_prefill_host_requirement(
    ...
    if std::env::var_os("RNB_DEBUG_HIDDEN_HASH_TRACE").is_some() {
        return Some("hidden_hash_trace");
    }
    if std::env::var_os("RNB_PREFILL_NAN_TRACE").is_some() {
        return Some("nan_trace");
    }
    if std::env::var_os("RNB_DEBUG_GDN_STAGE_DUMP_DIR").is_some() {
        return Some("gdn_stage_dump");
    }
    if std::env::var("RNB_VULKAN_LAYER_TRACE").is_ok_and(|value| value != "0") {
...
fn metal_qwen_prefill_moe_weights<'a>(
    ...
    let has_shadow = moe_w.shadow_gate_bytes().is_some()
        || moe_w.shadow_up_bytes().is_some()
        || moe_w.shadow_gate_up_tile_bytes().is_some()
        || moe_w.shadow_down_bytes().is_some();
    if !norm_eps.is_finite()
        || norm_eps <= 0.0
        || !product_tuple
        || !matches!(moe_w.expert_gating_func, 0 | 1)
        || !moe_w.shared_expert_gated
        || crate::engine::models::shared_expert_moe::routing::moe_mixed_precision_enabled(
            has_shadow,
        )
    {
        return None;
...
fn run_prefill_layers_cpu_range_impl(
    ...
    while layer_idx < layer_range.end {
        ...
        if std::env::var("RNB_PREFILL_NAN_TRACE").is_ok() {
        ...
        match &weights.layers[layer_idx] {
            ...
            LayerType::NemotronMoE(w) => {
                        ...
                        || models::nemotron::mamba::device_prefill_mamba2_enabled())
                {
                    ...
                    #[cfg(feature = "cuda")]
                    let mut pair_moe_unavailable = false;
                    #[cfg(feature = "cuda")]
                    if let Some(next_idx) = next_layer_idx {
                        ...
                        match try_run_nemotron_moe_mamba2_device_pair(
                            ...
                            NemotronMoeMamba2DevicePairAttempt::MoeUnavailable => {
                                pair_moe_unavailable = true;
                            }
...
mod tests {
    ...
    #[test]
    fn device_carrier_accepts_qwen_gdn_only_when_moe_output_carry_enabled() {
        unsafe {
    ...
    fn nemotron_prefill_v2_workspace_trace_line_reports_decision_and_bytes() {
        let plan = rnb_runtime::NemotronPrefillWorkspacePlan {
            seq_len: 128,
            chunk_len: 56,
            hidden_dim: 16,
            n_expert: 8,
            ...
            decision: rnb_runtime::NemotronPrefillWorkspaceDecision::Chunk { chunk_len: 56 },
        };
