...
pub(super) fn decode_attention_output_projection(
    ...
    let gpu_oproj_ok = use_accelerator
    ...
    let gpu_oproj_ok = if use_accelerator && !gpu_oproj_ok {
    ...
    if !gpu_oproj_ok {
        if !use_accelerator {
            w.o_weight.gemv_into_host_quantized(
                &scratch.attn_out[..q_dim],
                &mut scratch.proj_buf[..hidden_dim],
            )?;
        } else if force_generic_attn_oproj_layer(layer_idx) {
