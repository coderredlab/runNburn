#[cfg(feature = "cuda")]
use super::backend_runtime::{decode_gemv_cuda as cuda_decode_gemv, prefill_gemv_cuda};
use super::cpu_runtime::kernels;
#[cfg(not(feature = "cuda"))]
use super::dense_dispatch::gemv_f32;
#[cfg(not(feature = "cuda"))]
use super::dense_dispatch::{gemv_bf16, gemv_f16};
...
use super::quantized_dispatch::{
    ...
    QuantizedQ8Block as Q8Block, QuantizedQ8KBlock as Q8KBlock,
};
use super::quantized_weight_types::QuantizedWeight;
use super::scalar_gemv::{gemv_full_dequant_f32, gemv_generic, gemv_output_f64_logit};
#[cfg(not(feature = "cuda"))]
use super::scalar_gemv::{gemv_q4_0, gemv_q8_0};
...
fn all_f32_override_enabled() -> bool {
    crate::engine::policy::env_string("RNB_ALL_F32_OVERRIDE")
        .map(|v| v != "0")
        .unwrap_or(false)
}
...
impl QuantizedWeight {
    ...
    pub(super) fn gemv_vec(&self, input: &[f32]) -> crate::error::Result<Vec<f32>> {
        ...
        #[cfg(feature = "cuda")]
        if seq_len > 1 {
            return prefill_gemv_cuda(self, bytes, input, seq_len).unwrap_or_else(|| {
                Err(crate::error::LlmError::Forward(format!(
                    "CUDA prefill GEMV unavailable for {:?}; CPU fallback is disabled",
                    self.ggml_type
                )))
            });
        }
        ...
        #[cfg(feature = "cuda")]
        if seq_len == 1 {
            return cuda_decode_gemv(self, bytes, input).unwrap_or_else(|| {
                Err(crate::error::LlmError::Forward(format!(
                    "CUDA decode GEMV unavailable for {:?}; CPU fallback is disabled",
                    self.ggml_type
                )))
            });
        }
        ...
        return Err(crate::error::LlmError::Forward(
            "CUDA GEMV received an empty input; CPU fallback is disabled".to_string(),
        ));
    ...
    #[cfg(feature = "cuda")]
    pub(super) fn embedding_gather_cuda(
        &self,
        token_ids: &[u32],
        ...
        let bytes = self
            .data
            .as_bytes()
            .ok_or_else(|| crate::error::LlmError::Forward("embedding weight: no bytes".into()))?;
        super::cuda_runtime::embedding_gather(
            self.ggml_type,
            bytes,
            self.rows,
            self.cols,
        ...
        .ok_or_else(|| {
            crate::error::LlmError::Forward(format!(
                "CUDA embedding gather unavailable for {:?}; CPU quantized fallback is disabled",
                self.ggml_type
            ))
        })?
    ...
    pub(super) fn gemv_into(&self, input: &[f32], output: &mut [f32]) -> crate::error::Result<()> {
        ...
        #[cfg(feature = "cuda")]
        if seq_len > 1 {
            let result = prefill_gemv_cuda(self, bytes, input, seq_len).ok_or_else(|| {
                crate::error::LlmError::Forward(format!(
                    "CUDA prefill GEMV unavailable for {:?}; CPU fallback is disabled",
                    self.ggml_type
                ))
        ...
        #[cfg(feature = "cuda")]
        if seq_len == 1 {
            let result = cuda_decode_gemv(self, bytes, input).ok_or_else(|| {
                crate::error::LlmError::Forward(format!(
                    "CUDA decode GEMV unavailable for {:?}; CPU fallback is disabled",
                    self.ggml_type
                ))
        ...
        #[cfg(not(feature = "cuda"))]
        {
            ...
            if seq_len == 1 && crate::engine::policy::env_string("RNB_LAYER_GEMV_F64").is_some() {
                gemv_output_f64_logit(
                    bytes,
                    input,
                    output,
