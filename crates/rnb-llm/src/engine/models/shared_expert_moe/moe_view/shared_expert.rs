...
pub(in crate::engine) fn compute_shared_expert(
    ...
    #[cfg(any(target_arch = "aarch64", not(feature = "cuda")))]
    ...
    let mut shared_out = if glm_cuda_supported {
        match qwen_moe_backend::glm_moe_decode_shared_expert_q5k_q6k(
            ...
            Ok(out) => out,
            Err(err) => {
                eprintln!("[WARN] CUDA GLM shared expert fused path failed, retrying generic CUDA GEMV: {err}");
                Vec::new()
            }
        }
    } else {
    ...
    // Q8_0 shared expert (unsloth UD): fast aarch64 int8 dotprod instead of scalar dot_k_block_row.
        ...
        && n_ff % 32 == 0
    {
        ...
        crate::engine::quantized_dispatch::gemv_q8_0_raw(
            view.shared_gate_bytes,
            h,
            &mut sh_gate,
            n_ff,
        ...
        let mut down_out = vec![0f32; n_embd];
        crate::engine::quantized_dispatch::gemv_q8_0_raw(
            view.shared_down_bytes,
            &sh_gate,
            &mut down_out,
    ...
    scale_f32_inplace(&mut shared_out, gate_scalar);
    if profile_enabled {
