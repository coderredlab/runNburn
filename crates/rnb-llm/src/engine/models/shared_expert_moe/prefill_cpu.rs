...
#[cfg(target_arch = "aarch64")]
fn qwen35_verify_expert_major_requested(seq_len: usize) -> bool {
    (2..=4).contains(&seq_len)
        && crate::engine::policy::env_string("RNB_QWEN35_VERIFY_EXPERT_MAJOR").is_some_and(
            |value| {
                matches!(
...
pub(super) fn qwen35_cpu_expert_major_enabled(
    ...
    !cfg!(feature = "cuda")
        ...
        && (has_expert_batch_density(seq_len, view.n_expert, view.n_expert_used)
            || qwen35_verify_expert_major_requested(seq_len))
