/// Token-level mixed-precision tag used by the MoE mixed-precision dispatcher.
/// Assigned per (rank-sorted) selected expert based on cumulative unimportance.
#[derive(Debug, Clone, Copy, PartialEq, Eq)]
pub(in crate::engine) enum MoePrecision {
    /// Load the base Q4_K / Q5_1 expert weights.
    High,
    /// Use the Q2_K shadow copy of gate_up_exps; down_exps still high
    /// (n_ff=704 is not a multiple of 256 so Q2_K row-wise GEMV is not
    /// applicable for down in this PoC).
    Low,
    /// Drop contribution entirely (weight folded into renormalization).
    Skip,
}

#[derive(Clone, Copy, PartialEq, Eq)]
pub(in crate::engine) enum LowGateUpPath {
    RowMajor,
    TileMajor,
}

pub(in crate::engine) fn moe_mixed_precision_enabled(has_shadow: bool) -> bool {
    crate::engine::policy::moe_mixed_precision_enabled(has_shadow)
}

pub(in crate::engine) fn assign_moe_precisions(
    idx: &mut [usize],
    exps: &mut [f32],
    mixed_precision_on: bool,
) -> Vec<MoePrecision> {
    if !mixed_precision_on {
        return vec![MoePrecision::High; idx.len()];
    }

    let (t1, t2) = crate::engine::policy::moe_mixed_precision_thresholds();

    // Sort (idx, exps) by exps desc, stable within ties for determinism.
    let mut pairs: Vec<(usize, f32)> = idx.iter().zip(exps.iter()).map(|(&e, &w)| (e, w)).collect();
    pairs.sort_by(|a, b| b.1.partial_cmp(&a.1).unwrap_or(std::cmp::Ordering::Equal));
    for (i, (e, w)) in pairs.iter().enumerate() {
        idx[i] = *e;
        exps[i] = *w;
    }

    let mut prec = Vec::with_capacity(exps.len());
    let mut cum = 0.0f32;
    for &w in exps.iter() {
        let s = cum;
        cum += w;
...
pub(in crate::engine) fn choose_low_gate_up_path(
    has_tile_shadow: bool,
    has_row_shadow: bool,
    low_selected_count: usize,
    selected_count: usize,
) -> LowGateUpPath {
    match crate::engine::policy::moe_mixed_precision_low_path().as_deref() {
        Some("row") if has_row_shadow => LowGateUpPath::RowMajor,
        Some("tile") if has_tile_shadow => LowGateUpPath::TileMajor,
        _ if has_tile_shadow && has_row_shadow => {
            if low_selected_count == selected_count && low_selected_count > 0 {
                LowGateUpPath::TileMajor
            } else {
                LowGateUpPath::RowMajor
            }
        }
        _ if has_tile_shadow => LowGateUpPath::TileMajor,
        _ => LowGateUpPath::RowMajor,
    }
}
...
fn qwen35_moe_route_sort_enabled() -> bool {
    qwen35_moe_route_sort_enabled_from_env(
        std::env::var("RNB_QWEN35_MOE_ROUTE_SORT").ok().as_deref(),
    )
}
