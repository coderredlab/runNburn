...
fn dump_task11_generic_moe_stage(layer_idx: usize, stage: &str, values: &[f32]) {
    ...
    let Some(dir) = std::env::var_os("RNB_TASK11_MOE_STAGE_DUMP_DIR") else {
        return;
    };
...
fn add_moe_output_to_hidden(
    ...
    kernels::elementwise::add_inplace(&mut moe_out, hidden_data);
...
fn qwen35_prefill_expert_major_slots_enabled() -> bool {
    std::env::var("RNB_CUDA_QWEN35_PREFILL_EXPERT_MAJOR_SLOTS")
        .map(|value| {
...
fn hy3_q2q3_expert_major_prefill_enabled() -> bool {
    std::env::var("RNB_HY3_Q2Q3_EXPERT_MAJOR_PREFILL")
        .map(|value| {
            let value = value.to_ascii_lowercase();
            !matches!(value.as_str(), "0" | "false" | "off" | "no")
        })
...
fn qwen35_prefill_device_topk_enabled() -> bool {
    std::env::var("RNB_CUDA_QWEN35_PREFILL_DEVICE_TOPK")
        .map(|value| {
            let value = value.to_ascii_lowercase();
            !matches!(value.as_str(), "0" | "false" | "off" | "no")
        })
...
fn qwen35_prefill_device_topk_combined_enabled() -> bool {
    std::env::var("RNB_CUDA_QWEN35_PREFILL_DEVICE_TOPK_COMBINED")
        .ok()
        .as_deref()
        == Some("1")
}
...
fn qwen35_prefill_selected_base_enabled() -> bool {
    std::env::var("RNB_CUDA_QWEN35_PREFILL_SELECTED_BASE")
        .map(|value| {
            let value = value.to_ascii_lowercase();
            !matches!(value.as_str(), "0" | "false" | "off" | "no")
        })
...
pub(in crate::engine) fn qwen35moe_device_input_supported(
    ...
    backend_runtime::qwen_moe_prefill_enabled_for_seq(seq_len)
        ...
        && rnb_runtime::compute::tuning::shared_f32_enabled()
}
...
fn hy3_q2q3_expert_major_prefill_supported(
    ...
    hy3_q2q3_expert_major_prefill_enabled()
        ...
        && view.shadow_gate_bytes.is_none()
        && view.shadow_up_bytes.is_none()
        && view.shadow_gate_up_tile_bytes.is_none()
        && view.shadow_down_bytes.is_none()
...
fn forward_hy3_q2q3_expert_major_moe(
    ...
    while group_start < route_slots.len() {
        ...
        for (batch_idx, slot) in slots.iter().enumerate() {
            ...
            for (dst, &value) in dst.iter_mut().zip(src.iter()) {
                *dst = value * slot.weight;
            }
        }
    ...
    for token in 0..seq_len {
        ...
        for rank in 0..selected_count {
            ...
            for (dst, &value) in token_output.iter_mut().zip(rank_output.iter()) {
                *dst += value;
            }
        }
        let shared = &shared_outputs[token * n_embd..(token + 1) * n_embd];
        let shared_scale = shared_gate_scalars[token];
        for (dst, &value) in token_output.iter_mut().zip(shared.iter()) {
            *dst += value * shared_scale;
        }
    }
}
...
pub(in crate::engine) fn forward_shared_expert_moe(
    ...
    let view = crate::engine::moe::SharedExpertMoEView {
        ...
        shadow_gate_bytes: moe_w.shadow_gate_bytes(),
        shadow_up_bytes: moe_w.shadow_up_bytes(),
        shadow_gate_up_tile_bytes: moe_w.shadow_gate_up_tile_bytes(),
        shadow_down_bytes: moe_w.shadow_down_bytes(),
        moe_section_decode: moe_w.moe_section_decode_view(),
        gate_residency: moe_w.gate_residency.as_deref(),
        up_residency: moe_w.up_residency.as_deref(),
        down_residency: moe_w.down_residency.as_deref(),
    };
        ...
        && view.down_residency.is_none()
    {
        ...
        if super::glm_prefill::enabled() {
            match super::glm_prefill::forward(
                ...
                Err(error) => {
                    ...
                    eprintln!(
                        "[WARN] CUDA GLM batched sparse prefill failed, using token dispatch: {error}"
                    );
                }
            }
        }
        ...
        for token in 0..seq_len {
            ...
            if !completed {
                for (dst, &value) in output[start..start + hidden_dim]
                    .iter_mut()
                    .zip(moe_out.iter())
                {
                    *dst += value;
    ...
    let legacy_q4_shared_tuple = moe_w.gate_quant == GGMLType::Q4_K
        && moe_w.up_quant == GGMLType::Q4_K
        && matches!(
            moe_w.down_quant,
            GGMLType::Q4_K | GGMLType::Q5_K | GGMLType::Q6_K
    ...
    if qwen_moe_prefill_outer_guard(
        ...
        norm_all
            ...
            .enumerate()
            .for_each(|(t, (norm_t, shared_route_weight))| {
                let attn_out_t = &attn_out_data[t * hidden_dim..(t + 1) * hidden_dim];
                apply_model_norm_into(attn_out_t, ffn_norm_data, norm_eps, norm_t, architecture);
        ...
        if std::env::var_os("RNB_METAL_QWEN35_MOE_PREFILL_ACCUM_TRACE").is_some() {
            eprintln!(
        ...
        #[cfg(feature = "cuda")]
        if rnb_runtime::compute::tuning::shared_f32_enabled() {
            let t_combined = std::time::Instant::now();
            let full_layer_min_permille =
                rnb_runtime::compute::tuning::prefill_moe_full_layer_min_expert_permille();
            let use_full_layer = !use_selected_base
                && (rnb_runtime::compute::tuning::moe_layer_cache_enabled()
                    || rnb_runtime::compute::tuning::prefill_moe_range_slab_enabled()
                    || (rnb_runtime::compute::tuning::prefill_moe_full_layer_enabled()
        ...
        for (dst, shared) in out_buf.iter_mut().zip(shared_out.iter()) {
            *dst += *shared;
        }
    ...
    for t in 0..seq_len {
        ...
        for (dst, moe) in out_buf[t * hidden_dim..(t + 1) * hidden_dim]
            .iter_mut()
            .zip(moe_out.iter())
        {
            *dst += *moe;
...
mod tests {
    ...
    fn qwen_moe_prefill_outer_guard_keeps_legacy_policy() {
        assert!(!qwen_moe_prefill_outer_guard(
    ...
    fn hy3_q2q3_expert_major_matches_token_major_reference() {
        ...
        let previous_batch = std::env::var(batch_key).ok();
        let n_embd = 256;
        let n_ff = 256;
        let n_expert = 4;
        let n_expert_used = 2;
