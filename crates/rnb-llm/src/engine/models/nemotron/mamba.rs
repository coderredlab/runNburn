...
use crate::engine::{
    backend_runtime, cpu_runtime::kernels, quantized_weight_types::QuantizedWeight,
};
...
pub(in crate::engine) fn forward_mamba2_layer(
    ...
    let normed = kernels::norm::rms_norm(&hidden, &w.norm, norm_eps).map_err(fwd)?;
    ...
    let scan_out = if seq_len > 1
        && std::env::var("RNB_CUDA_NEMOTRON_MAMBA_PREFILL_SCAN")
            .ok()
            .as_deref()
            != Some("0")
    {
        ...
        && std::env::var("RNB_CUDA_NEMOTRON_MAMBA_SCAN")
            .ok()
            .as_deref()
            == Some("1")
    {
    ...
    for (dst, z) in gated.iter_mut().zip(z_data.iter()) {
        *dst *= z / (1.0 + (-z).exp());
    }
    ...
    let output = kernels::elementwise::add(&hidden, &proj_tensor).map_err(fwd);
...
pub(in crate::engine) fn device_prefill_mamba2_enabled() -> bool {
    std::env::var("RNB_CUDA_DEVICE_PREFILL_MAMBA2")
        .ok()
        .as_deref()
        == Some("1")
}
...
pub(in crate::engine) fn device_prefill_mamba2_requested_for_chain() -> bool {
    match std::env::var("RNB_CUDA_DEVICE_PREFILL_MAMBA2")
        .ok()
        .as_deref()
    {
        Some("1") => true,
...
pub(in crate::engine) fn device_prefill_trace_enabled() -> bool {
    std::env::var("RNB_CUDA_DEVICE_PREFILL_TRACE")
        .ok()
        .as_deref()
        == Some("1")
}
...
mod tests {
    ...
    #[test]
    fn mamba2_device_prefill_is_opt_in() {
        let _lock = env_lock().lock().expect("env lock poisoned");
        let _env_guard = EnvVarGuard::new("RNB_CUDA_DEVICE_PREFILL_MAMBA2");
        std::env::remove_var("RNB_CUDA_DEVICE_PREFILL_MAMBA2");

        assert!(!super::device_prefill_mamba2_enabled());

        std::env::set_var("RNB_CUDA_DEVICE_PREFILL_MAMBA2", "1");
        assert!(super::device_prefill_mamba2_enabled());
        std::env::remove_var("RNB_CUDA_DEVICE_PREFILL_MAMBA2");
    }

    #[test]
    fn mamba2_chain_request_respects_explicit_opt_out() {
        let _lock = env_lock().lock().expect("env lock poisoned");
        let _mamba_guard = EnvVarGuard::new("RNB_CUDA_DEVICE_PREFILL_MAMBA2");
        let _smoke_guard = EnvVarGuard::new("RNB_CUDA_DEVICE_PREFILL_CHAIN_SMOKE");
        std::env::remove_var("RNB_CUDA_DEVICE_PREFILL_MAMBA2");
        std::env::remove_var("RNB_CUDA_DEVICE_PREFILL_CHAIN_SMOKE");

        assert!(!super::device_prefill_mamba2_requested_for_chain());

        std::env::set_var("RNB_CUDA_DEVICE_PREFILL_CHAIN_SMOKE", "1");
        assert!(super::device_prefill_mamba2_requested_for_chain());

        std::env::set_var("RNB_CUDA_DEVICE_PREFILL_MAMBA2", "0");
        assert!(!super::device_prefill_mamba2_requested_for_chain());
    ...
    #[cfg(feature = "cuda")]
    #[test]
    fn mamba2_device_disabled_path_returns_input_owner() {
        let _lock = env_lock().lock().expect("env lock poisoned");
        let _mamba_guard = EnvVarGuard::new("RNB_CUDA_DEVICE_PREFILL_MAMBA2");
        let _smoke_guard = EnvVarGuard::new("RNB_CUDA_DEVICE_PREFILL_CHAIN_SMOKE");
        std::env::remove_var("RNB_CUDA_DEVICE_PREFILL_MAMBA2");
        ...
        let metadata = mamba2_device_test_metadata();
        let weights = mamba2_device_test_weights();
        let input = dummy_device_layer_output(23);
        let output_id = input.output_id;
        let output_desc = input.output_desc;

        let attempt = super::forward_mamba2_layer_from_device(
            &mut kv_cache,
            &metadata,
            input,
            &weights,
        ...
        match attempt {
            ...
            super::NemotronMamba2DeviceAttempt::Done(_, _) => {
                panic!("disabled path returned done")
            }
        }
    }
