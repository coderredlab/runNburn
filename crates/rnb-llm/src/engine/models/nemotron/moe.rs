...
fn prepare_moe_prefill(
    ...
    let normed = kernels::norm::rms_norm(hidden, &w.norm, norm_eps)
...
pub(in crate::engine) fn forward_moe_layer_for_prefill(
    ...
    let stage_start = (seq_len > 1).then(Instant::now);
    let shared_all =
        if let Some(shared_all) = prefill_shared_q8_fused(w, hidden_dim, seq_len, normed_data)? {
        ...
        prefill_sparse_q5_batch(w, routes, n_expert, n_ff, hidden_dim, seq_len, normed_data)?
    {
        ...
        let stage_start = (seq_len > 1).then(Instant::now);
        for i in 0..out.len() {
            out[i] = input[i] + shared_all[i] + sparse_all[i];
        }
    ...
    for token_idx in 0..seq_len {
        ...
        for (&expert_idx, &weight) in route.experts.iter().zip(route.weights.iter()) {
            let mut mid = expert_project(&w.expert_up, expert_idx, n_expert, n_ff, hidden_dim, h)?;
            for value in &mut mid {
                *value = rnb_model_nemotron::relu_sqr(*value);
            }
            let down =
                expert_project(&w.expert_down, expert_idx, n_expert, hidden_dim, n_ff, &mid)?;
            for (dst, &value) in token_out.iter_mut().zip(down.iter()) {
                *dst += value * weight;
            }
        }

        for i in 0..hidden_dim {
            out[start + i] = input[start + i] + token_out[i];
        }
    }
...
fn device_prefill_probe_enabled() -> bool {
    std::env::var("RNB_CUDA_DEVICE_PREFILL").ok().as_deref() == Some("1")
}

#[cfg(any(feature = "cuda", test))]
fn device_prefill_handoff_smoke_enabled() -> bool {
    std::env::var("RNB_CUDA_DEVICE_PREFILL_HANDOFF_SMOKE")
        .ok()
        .as_deref()
        == Some("1")
}
...
pub(in crate::engine) fn device_prefill_chain_smoke_enabled() -> bool {
    std::env::var("RNB_CUDA_DEVICE_PREFILL_CHAIN_SMOKE")
        .ok()
        .as_deref()
        == Some("1")
}
...
fn emit_device_prefill_chain_trace(
    ...
    if std::env::var("RNB_CUDA_DEVICE_PREFILL_TRACE")
        .ok()
        .as_deref()
        != Some("1")
    {
...
fn prepare_q8_shared_sparse_device_route_inputs<'a>(
    ...
    if std::env::var("RNB_CUDA_NEMOTRON_PREFILL_Q8_SHARED_SPARSE_FUSED")
        .ok()
        .as_deref()
        != Some("1")
    {
    ...
    if std::env::var("RNB_CUDA_NEMOTRON_PREFILL_EXPERT_MAJOR")
        .ok()
        .as_deref()
        != Some("0")
    {
...
#[cfg(feature = "cuda")]
#[allow(clippy::too_many_arguments)]
pub(in crate::engine) fn forward_moe_layer_device_output_for_chain(
    metadata: &ModelMetadata,
    hidden: &Tensor,
    w: &NemotronMoELayerWeights,
    norm_eps: f32,
    ...
    let residual = kernels::tensor_as_f32_slice(hidden);
    emit_carrier_tensor_trace(
        Some(layer_idx),
        "host_device",
        "input",
    ...
    let prepared = prepare_moe_prefill(metadata, hidden, w, norm_eps)?;
    emit_carrier_route_trace(Some(layer_idx), "host_device", &prepared.routes);
    ...
    let Some((
        ...
        token_ids,
        shared_ff,
    )) = prepare_q8_shared_sparse_device_inputs(w, &prepared, layer_idx, trace_next_layer_idx)?
    else {
        return Ok(None);
    };
    ...
    let output = handoff.into_layer_output()?;
    ...
    {
        eprintln!(
            "[cuda:device-prefill-chain] op=nemotron_moe_output layer={} output_device=1 d2h_bytes=0",
            layer_idx
        );
    }
    Ok(Some(output))
...
fn device_router_weight_f32_for_chain(
    ...
    let Some(router_bytes) = router.data.as_bytes() else {
        return Ok(None);
    };
    ...
    let values = if router.ggml_type == GGMLType::F32 {
        let mut router_chunks = router_bytes.chunks_exact(std::mem::size_of::<f32>());
        let values = router_chunks
            .by_ref()
            .map(|chunk| f32::from_le_bytes([chunk[0], chunk[1], chunk[2], chunk[3]]))
            .collect::<Vec<_>>();
        if !router_chunks.remainder().is_empty() {
            return Err(crate::error::LlmError::Forward(
                "Nemotron router F32 byte length is not divisible by four".to_string(),
            ));
        }
        values
    } else if router.ggml_type == GGMLType::I32 {
        return Ok(None);
    } else {
        crate::engine::dequant::dequantize_bytes_to_f32(router_bytes, router.ggml_type)
    };
...
pub(in crate::engine) fn forward_moe_layer_from_device_for_chain(
    ...
    if crate::engine::policy::cuda_nemotron_device_route_pack_enabled()
        && std::env::var("RNB_CUDA_NEMOTRON_PREFILL_EXPERT_MAJOR")
            .ok()
            .as_deref()
            != Some("0")
    {
...
fn prefetch_sparse_q5_batch(
    ...
    if std::env::var("RNB_CUDA_NEMOTRON_PREFILL_COPY_PREFETCH")
        .ok()
        .as_deref()
        != Some("1")
    {
...
fn prewarm_prefill_routes<'a>(
    ...
    let last_route_enabled = std::env::var("RNB_CUDA_NEMOTRON_PREFILL_LAST_ROUTE_PREWARM")
        .ok()
        .as_deref()
        != Some("0");
    let topk = std::env::var("RNB_CUDA_NEMOTRON_PREFILL_ROUTE_PREWARM_TOPK")
        .ok()
...
mod tests {
    ...
    #[test]
    fn device_prefill_chain_smoke_is_opt_in() {
        let _lock = env_lock().lock().expect("env lock poisoned");
        let _env_guard = EnvVarGuard::new("RNB_CUDA_DEVICE_PREFILL_CHAIN_SMOKE");
        std::env::remove_var("RNB_CUDA_DEVICE_PREFILL_CHAIN_SMOKE");

        assert!(!super::device_prefill_chain_smoke_enabled());

        std::env::set_var("RNB_CUDA_DEVICE_PREFILL_CHAIN_SMOKE", "1");
        assert!(super::device_prefill_chain_smoke_enabled());
        std::env::remove_var("RNB_CUDA_DEVICE_PREFILL_CHAIN_SMOKE");
    }
