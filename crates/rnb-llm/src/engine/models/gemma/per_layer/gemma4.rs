...
fn gemma4_cuda_ple_out_scale_enabled() -> bool {
    crate::engine::policy::env_string("RNB_CUDA_GEMMA_PLE_OUT_SCALE")
        .map(|value| {
...
pub(in crate::engine) fn apply_gemma4_per_layer_branch_with_output_scale(
    ...
    if seq_len == 1 {
        ...
        apply_model_gate_mul_inplace(&mut gate, per_layer_input, ModelArchitecture::Gemma4);
        if dump_bin_dir().is_some() {
        ...
        add_f32_inplace(&mut out, &normed);
        if dump_bin_dir().is_some() {
    ...
    let normed = apply_plain_rms_norm(&projected_tensor, &layer.post_norm, norm_eps)
