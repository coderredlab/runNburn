...
pub(in crate::engine) fn forward_ffn_gemma4_moe_hybrid(
    ...
    let view = crate::engine::moe::MoeLayerView {
        ...
        rank_to_original: moe_w.rank_to_original.as_deref(),
        shadow_gate_up_bytes: moe_w.shadow_gate_up_bytes(),
        gate_up_residency: moe_w.gate_up_residency.as_deref(),
    ...
    let hidden = kernels::elementwise::add(&hidden, &down).map_err(fwd)?;
    Ok(hidden)
}
