...
#[allow(clippy::too_many_arguments)]
pub(in crate::engine) fn decode_gdn_layer_qwen_core(
    kv_cache: &mut KVCache,
    metadata: &ModelMetadata,
    scratch: &mut ScratchBuffers,
    w: &GdnLayerWeights,
...
fn decode_gdn_layer_qwen_impl(
    ...
    {
        sigmoid_f32_inplace(&mut scratch.beta_buf[..num_v_heads]);
    ...
    scale_f32_inplace(&mut scratch.gdn_q_norm[..q_dim], scale);
        ...
        match out {
            ...
            Err(err) => {
                ...
                return Err(crate::error::LlmError::Forward(format!(
                    "CUDA GDN delta-net failed; CPU fallback is disabled: {err}"
                )));
                #[cfg(not(feature = "cuda"))]
                {
    ...
    if !gpu_delta_ok {
        return Err(crate::error::LlmError::Forward(
            "CUDA GDN delta-net is unavailable; CPU fallback is disabled".into(),
        ));
    }
    #[cfg(not(feature = "cuda"))]
    if !gpu_delta_ok {
    ...
    scratch.gated_out[..d_inner].copy_from_slice(&scratch.z_buf[..d_inner]);
    if trace_gdn_stages {
        emit_gdn_decode_stage_trace(
            "cpu-dec-gated",
            layer_idx,
