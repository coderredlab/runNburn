...
pub(super) fn decode_attention_compute(
        ...
        match backend_runtime::try_decode_attention_single_head_if_supported(
            ...
            Err(e) => {
                ...
                return Err(crate::error::LlmError::Forward(format!(
                    "CUDA single-head decode attention failed; CPU fallback is disabled: {e}"
                )));
                #[cfg(not(feature = "cuda"))]
                {
    ...
    if !gpu_attn_ok {
        ...
        if !super::policy::cache_read_disabled() && kv_cache.layer_uses_kvarn(kv_cache_layer) {
            ...
            let view = kv_cache.kvarn_view(kv_cache_layer, kv_len).ok_or_else(|| {
                crate::error::LlmError::Forward(
                    "KVarN cache view is unavailable after compaction".to_string(),
                )
            })?;
            #[cfg(feature = "cuda")]
            if let Some(dev_target) = output_dev_target {
                if let Some(result) = backend_runtime::decode_attention_kvarn_to_device_if_supported(
                    layer_idx,
                    q_slice,
                    view,
                    num_heads,
            ...
            #[cfg(feature = "cuda")]
            if backend_runtime::decode_attention_kvarn_into_if_supported(
                layer_idx,
                q_slice,
                view,
                &mut attn_out[..q_dim],
            ...
            #[cfg(feature = "metal")]
            if backend_runtime::metal_decode_attention_kvarn_into_if_supported(
                layer_idx,
                q_slice,
                view,
                &mut attn_out[..q_dim],
            ...
            #[cfg(feature = "cuda")]
            return Err(crate::error::LlmError::Forward(format!(
                "CUDA KVarN decode attention is unavailable for layer {layer_idx}; CPU fallback is disabled"
            )));
            #[cfg(not(feature = "cuda"))]
            {
                super::cpu_runtime::quantize::kvarn::attention_decode(
                    q_slice,
                    view,
                    &mut attn_out[..q_dim],
                    num_heads,
        ...
        if !attention_done {
            #[cfg(feature = "cuda")]
            return Err(crate::error::LlmError::Forward(format!(
                "CUDA decode attention is unavailable for layer {layer_idx}: heads={num_heads} kv_heads={num_kv_heads} head_dim={head_dim} kv_len={kv_len}; CPU fallback is disabled"
            )));
            #[cfg(not(feature = "cuda"))]
            {
