...
fn cu71_long_kv_split_preferred(head_dim: usize, kv_len: usize) -> bool {
    head_dim == 512 && kv_len >= 256 && crate::runtime::cuda::decode_attention_hd512_split_enabled()
}
...
pub(super) fn decode_attention_layer_with_rope_pos(
    ...
    let rms_used_cuda = if chain_emits_hidden_carrier {
    ...
    if layer_idx == 0
        && std::env::var("RNB_CUDA_EAGER_ATTN_NORM_PROBE")
            .ok()
            .as_deref()
            == Some("1")
    {
    ...
    #[cfg(feature = "cuda")]
    let cu65_device_qkv_enabled = crate::runtime::cuda::cu65_device_qkv_enabled();
    #[cfg(feature = "cuda")]
    let cu68_layer_graph_enabled = crate::runtime::cuda::cu68_layer_graph_enabled();
    #[cfg(feature = "cuda")]
        ...
        && cu65_device_qkv_enabled
    {
            ...
            backend_runtime::acquire_decode_norm_buf_carrier(norm_carrier_bytes)
        {
            ...
            let use_graph = super::decode_layer_graph::cu68_qkv_graph_enabled(
                std::env::var("RNB_CU65_GRAPH").ok().as_deref() == Some("1"),
    ...
    let (last_token_k_dev, last_token_v_dev): (Option<u64>, Option<u64>) = {
        if used_device_qkv
            && chain_emits_hidden_carrier
            && rnb_runtime::policy::cuda_decode_device_kv_cache_enabled()
            && !has_gated_attn
            && !gemma4_reuse_q_only
        {
    ...
    if has_gated_attn {
        kernels::activation::sigmoid_inplace(&mut scratch.gate_split[..q_dim]);
        kernels::elementwise::mul_inplace(
            &mut scratch.attn_out[..q_dim],
            &scratch.gate_split[..q_dim],
        );
    }
    emit_mtp_finite_trace(
        "decode-attn",
    ...
    #[cfg(feature = "cuda")]
    {
        ...
        if let Some(args) = super::forward::chain_args::compute_chain_function_args(&ctx) {
            ...
            if cu71_layer_segment_graph_allowed
                && crate::runtime::cuda::cu71_layer_segment_graph_trace_enabled()
            {
                eprintln!(
