...
impl super::Engine {
    pub(crate) fn target_argmax_excluding_from_hidden(
        &self,
        hidden: &[f32],
        excluded_token: u32,
        ...
        if hidden.len() != self.metadata.hidden_dim {
            return Err(crate::error::LlmError::Forward(format!(
                "excluded-token target hidden width mismatch: got {}, expected {}",
                hidden.len(),
        ...
        let hidden = Tensor::from_slice(hidden, &[1, self.metadata.hidden_dim]);
        let normed = apply_model_norm(
            &hidden,
            &weights.output_norm,
        ...
        *logits.get_mut(excluded_token as usize).ok_or_else(|| {
            crate::error::LlmError::Forward(format!(
                "excluded token {excluded_token} exceeds logits length {logits_len}"
            ))
        })? = f32::NEG_INFINITY;
...
pub(super) fn finalize_prefill_all_logits(
    ...
    for t in 0..seq_len {
        ...
        let normed = if gemma_effective_unit_offset_output_norm_prefill(
            ...
            apply_model_norm_unit_offset(&token_hidden, &weights.output_norm, norm_eps)
                .map_err(|e| crate::error::LlmError::Forward(e.to_string()))?
        } else {
