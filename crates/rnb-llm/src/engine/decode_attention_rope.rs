...
pub(in crate::engine) fn apply_decode_rope(
    ...
    #[cfg(feature = "cuda")]
    {
        let (mode, n_rot, factors) = if let Some(mrope_dim) = qwen_mrope_dim {
            (
        ...
        backend_runtime::cuda_rope_f32_inplace(
            q_slice, q_dim, head_dim, n_rot, pos, rope_theta, mode, factors,
        )
        .unwrap_or_else(|err| panic!("CUDA decode Q RoPE failed: {err}"));
        if !gemma4_reuse_q_only {
            backend_runtime::cuda_rope_f32_inplace(
                k_slice, kv_dim, head_dim, n_rot, pos, rope_theta, mode, factors,
            )
            .unwrap_or_else(|err| panic!("CUDA decode K RoPE failed: {err}"));
        }
    }
