...
pub(super) fn try_persistent_decode_dispatch_batch(
    ...
    let _trace_eligible = std::env::var("RNB_CUDA_PERSISTENT_PREFILL_BATCH_TRACE").is_ok();
    ...
    let trace_enabled = std::env::var("RNB_CUDA_PERSISTENT_DECODE_TRACE")
        .ok()
        .as_deref()
        == Some("1");
    ...
    let probe_enabled = std::env::var("RNB_CUDA_PERSISTENT_DECODE_PROBE")
        .ok()
        .as_deref()
        == Some("1");
    ...
    let phase_probe_enabled = std::env::var("RNB_CUDA_PERSISTENT_DECODE_PHASE_PROBE")
        .ok()
        .as_deref()
        == Some("1");
    ...
    let layer_cap = std::env::var("RNB_CUDA_PERSISTENT_DECODE_LAYERS")
        .ok()
    ...
    let rope_freqs_enabled = std::env::var("RNB_CUDA_PERSISTENT_ROPE_FREQS")
        .ok()
        .as_deref()
        != Some("0");
    ...
    if probe_enabled {
        ...
        if let Ok(path) = std::env::var("RNB_CUDA_PERSISTENT_DECODE_PROBE_OUT") {
    ...
    if phase_probe_enabled {
        ...
        for (name, buf) in probes {
            ...
            if let Ok(prefix) = std::env::var("RNB_CUDA_PERSISTENT_DECODE_PHASE_OUT_PREFIX") {
    ...
    if argmax_out < 0 {
        if std::env::var("RNB_CUDA_PERSISTENT_PREFILL_BATCH_TRACE").is_ok() {
            eprintln!(
