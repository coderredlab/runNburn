...
#[cfg(feature = "cuda")]
use crate::engine::packed_runtime;
...
#[cfg(feature = "cuda")]
fn packed_gemm_slice(weight: &QuantizedWeight) -> Option<&[u8]> {
    let (ptr, len) = weight.packed_gemm_data?;
    if len == 0 {
        return None;
    }
    // SAFETY: packed_gemm_data points into the engine-owned PackedModel mmap.
    // The PackedModel is kept alive for at least as long as QuantizedWeight.
    Some(unsafe { std::slice::from_raw_parts(ptr, len) })
}

#[cfg(feature = "cuda")]
fn sidecar_packed_slice(
    weight: &QuantizedWeight,
    quant_type: packed_runtime::QuantType,
) -> Option<&[u8]> {
    (weight.packed_gemm_quant_type == Some(quant_type))
        .then(|| packed_gemm_slice(weight))
        .flatten()
}
...
#[cfg(feature = "cuda")]
type Q4SidecarPackedSinglePrewarm<'a> = (&'a [u8], &'a [u8], usize, usize);
#[cfg(feature = "cuda")]
type Q6PackedDownPrewarm<'a> = (&'a [u8], usize, usize);
#[cfg(feature = "cuda")]
type Q6SidecarPackedDownPrewarm<'a> = (&'a [u8], &'a [u8], usize, usize);
...
struct CudaProductPrewarmRequests<'a> {
    ...
    q4_sidecar: Vec<Q4SidecarPackedSinglePrewarm<'a>>,
    q6_down: Vec<Q6PackedDownPrewarm<'a>>,
    q6_sidecar: Vec<Q6SidecarPackedDownPrewarm<'a>>,
}
...
impl CudaProductPrewarmRequests<'_> {
    fn kinds(&self) -> Vec<CudaProductPrewarmRequestKind> {
        ...
        if !self.q4_sidecar.is_empty() {
            kinds.push(CudaProductPrewarmRequestKind::Q4SidecarPackedSingle);
        }
        if !self.q6_down.is_empty() {
            kinds.push(CudaProductPrewarmRequestKind::Q6PackedDown);
        }
        if !self.q6_sidecar.is_empty() {
            kinds.push(CudaProductPrewarmRequestKind::Q6SidecarPackedDown);
        }
        kinds
    }
}
...
pub(in crate::engine) fn decode_gemv_cuda(
    ...
    let view = weight.backend_view()?;
    if !decode_gemv_cuda_supports_quant(view.quant()) {
        return None;
    }
    let result = match view.quant() {
        QuantFormat::Q2K => cuda_runtime::q2k_gemv(bytes, view.rows(), view.cols(), input),
        QuantFormat::Q3K => cuda_runtime::q3k_gemv(bytes, view.rows(), view.cols(), input),
        QuantFormat::Q4K => cuda_runtime::q4k_gemv(bytes, view.rows(), view.cols(), input),
        QuantFormat::Q5K => cuda_runtime::q5k_gemv(bytes, view.rows(), view.cols(), input),
        QuantFormat::Q6K => cuda_runtime::q6k_gemv(bytes, view.rows(), view.cols(), input),
        QuantFormat::Q80 => cuda_runtime::q8_0_gemv(bytes, view.rows(), view.cols(), input),
        QuantFormat::IQ4XS => cuda_runtime::iq4_xs_gemv(bytes, view.rows(), view.cols(), input),
        _ => unreachable!("decode_gemv_cuda_supports_quant filtered unsupported quant"),
    };
    Some(result.map_err(cuda_error))
}

#[cfg(feature = "cuda")]
fn decode_gemv_cuda_supports_quant(quant: QuantFormat) -> bool {
    matches!(
        quant,
        QuantFormat::Q2K
            | QuantFormat::Q3K
...
mod tests {
    ...
    #[test]
    fn cuda_decode_gemv_supports_q2k_and_q3k_weights() {
        assert!(decode_gemv_cuda_supports_quant(QuantFormat::Q2K));
        assert!(decode_gemv_cuda_supports_quant(QuantFormat::Q3K));
    }

    #[test]
    fn cuda_decode_gemv_supports_q8_0_weights() {
        assert!(decode_gemv_cuda_supports_quant(QuantFormat::Q80));
    }

    #[test]
    fn cuda_decode_gemv_supports_iq4_xs_weights() {
        assert!(decode_gemv_cuda_supports_quant(QuantFormat::IQ4XS));
    }
...
/// chain run 1개 layer 의 입력 state(owned). decode loop 이 kv_cache(immutable borrow)에서
/// **clone 한 owned** 를 넘긴다 — 호출 후 GDN state write(mutable borrow)와 read/write 분리.
/// - Attn: 첫 token KV init 용 prior KV(host f16). filled!=0 이면 backend 가 무시(가드).
/// - Gdn: 직전 token conv/delta state.
#[cfg_attr(not(all(feature = "metal", not(feature = "cuda"))), allow(dead_code))]
pub(in crate::engine) enum ChainLayerInput {
    Attn {
        prior_k: Vec<u16>,
        prior_v: Vec<u16>,
    },
...
fn qwen_moe_chain_materials(
    ...
    if moe_w.gate_quant != GGMLType::Q4_K
        ...
        || moe_w.shared_gate.ggml_type != GGMLType::Q4_K
...
pub(in crate::engine) fn metal_decode_chain_run(
    ...
    #[cfg(all(feature = "metal", not(feature = "cuda")))]
    {
        ...
        let fallback = |reason: &'static str| MetalDecodeChainRunReport {
        ...
        if layers.is_empty() {
            return Ok(MetalDecodeChainRunReport {
                did_run: true,
                ..MetalDecodeChainRunReport::default()
            });
        }
        ...
        for (i, (layer_idx, lt)) in layers.iter().enumerate() {
            ...
            match lt {
                LayerType::Attention(w) => {
                    let (ChainLayerInput::Attn { prior_k, prior_v }, Some(shape)) =
                    ...
                    if let Some(moe_w) = w.shared_expert_moe.as_ref() {
                        if q_ggml != GGMLType::Q4_K
                            || k_ggml != GGMLType::Q4_K
                            || !matches!(v_ggml, GGMLType::Q4_K | GGMLType::Q6_K)
                            || o_ggml != GGMLType::Q4_K
                        {
        ...
        return metal_decode_chain_runtime_result(metal_runtime::metal_decode_chain_run(
            hidden,
            &specs,
            out_states,
            output_tail,
    ...
    #[cfg(not(all(feature = "metal", not(feature = "cuda"))))]
    {
        ...
        Ok(MetalDecodeChainRunReport::default())
    }
}
...
pub(in crate::engine) fn decode_gemv_into_if_supported(
    ...
    #[cfg(feature = "cuda")]
    {
        ...
        if rnb_runtime::policy::cuda_decode_device_chain_enabled() && rms_used_cuda {
...
pub(in crate::engine) fn dense_q4k_attention_qkv_rope_hd128_if_supported(
    ...
    #[cfg(feature = "cuda")]
    {
        ...
        let master = std::env::var("RNB_CUDA_HD128_FUSED_QKV_ROPE")
            .ok()
            .map(|v| v == "1" || v.eq_ignore_ascii_case("true"))
            .unwrap_or(false);
        let decode_only = std::env::var("RNB_CUDA_HD128_FUSED_QKV_ROPE_DECODE")
            .ok()
            .map(|v| v == "1" || v.eq_ignore_ascii_case("true"))
            .unwrap_or(false);
...
pub(in crate::engine) fn dense_q4k_attention_qkv_rope_hd128_prefill_if_supported(
    ...
    #[cfg(feature = "cuda")]
    {
        let master = std::env::var("RNB_CUDA_HD128_FUSED_QKV_ROPE")
            ...
            .map(|v| v == "1" || v.eq_ignore_ascii_case("true"))
            .unwrap_or(false);
        let prefill_only = std::env::var("RNB_CUDA_HD128_FUSED_QKV_ROPE_PREFILL")
            .ok()
            .map(|v| v == "1" || v.eq_ignore_ascii_case("true"))
            .unwrap_or(false);
...
pub(in crate::engine) fn try_rms_norm_into_decode_carrier_if_supported(
    ...
    #[cfg(feature = "cuda")]
    {
        if !rnb_runtime::policy::cuda_decode_device_chain_enabled() {
            return Ok(false);
        }
        ...
        if layer_idx > 0 {
            ...
            if std::env::var("RNB_CU44_DIAG_PRE_RMS").is_ok() {
...
#[cfg_attr(not(feature = "cuda"), allow(dead_code, unused_variables))]
pub(in crate::engine) fn bf16_gemv(
    weight: &QuantizedWeight,
    input: &[f32],
) -> crate::error::Result<Option<Vec<f32>>> {
    if weight.ggml_type != rnb_loader::GGMLType::BF16 || input.len() != weight.cols {
        return Ok(None);
    }
    let Some(raw) = weight.data.as_bytes() else {
        return Ok(None);
    ...
    #[cfg(feature = "cuda")]
    {
        return cuda_runtime::bf16_gemv(raw, weight.rows, weight.cols, input)
            .map(Some)
            .map_err(cuda_error);
    }
    #[cfg(not(feature = "cuda"))]
...
#[cfg_attr(not(feature = "cuda"), allow(dead_code, unused_variables))]
pub(in crate::engine) fn f16_gemv(
    weight: &QuantizedWeight,
    input: &[f32],
) -> crate::error::Result<Option<Vec<f32>>> {
    #[cfg(feature = "cuda")]
    {
        ...
        return cuda_runtime::f16_gemv(raw, weight.rows, weight.cols, input)
            .map(Some)
            .map_err(cuda_error);
    }
    #[cfg(not(feature = "cuda"))]
...
fn push_dense_product_packed_candidates<'a>(
    ...
    if down.ggml_type == GGMLType::Q6_K {
        if let Some(down_raw) = down.data.as_bytes() {
            ...
            if let Some(sidecar) = sidecar_packed_slice(down, packed_runtime::QuantType::Q6K) {
                requests
                    .q6_sidecar
                    .push((down_raw, sidecar, down.rows, down.cols));
            }
        }
    }
    ...
    if let Some(gate_sidecar) = sidecar_packed_slice(gate, packed_runtime::QuantType::Q4K) {
        requests
            .q4_sidecar
            .push((gate_raw, gate_sidecar, gate.rows, gate.cols));
    }
    if let Some(up_sidecar) = sidecar_packed_slice(up, packed_runtime::QuantType::Q4K) {
        requests
            .q4_sidecar
            .push((up_raw, up_sidecar, up.rows, up.cols));
    }
    if down.ggml_type == GGMLType::Q4_K {
        if let Some(down_raw) = down.data.as_bytes() {
            requests.q4_single.push((down_raw, down.rows, down.cols));
            if let Some(down_sidecar) = sidecar_packed_slice(down, packed_runtime::QuantType::Q4K) {
                requests
                    .q4_sidecar
                    .push((down_raw, down_sidecar, down.rows, down.cols));
            }
        }
    }
}
...
fn execute_cuda_product_prewarm_requests(
    ...
    match selection {
        CudaProductPrewarmSelection::Q4Dense => {
            let sidecar_warmed =
                cuda_runtime::prewarm_q4k_sidecar_packed_weights(&requests.q4_sidecar)
                    .map_err(cuda_error)?;
            if sidecar_warmed > 0 {
                eprintln!("[INFO] CUDA sidecar Q4_K packed weights prewarmed: {sidecar_warmed}");
            }
        ...
        CudaProductPrewarmSelection::Q6Dense => {
            let sidecar_warmed =
                cuda_runtime::prewarm_q6k_sidecar_packed_weights(&requests.q6_sidecar)
                    .map_err(cuda_error)?;
            if sidecar_warmed > 0 {
                eprintln!("[INFO] CUDA sidecar Q6_K packed weights prewarmed: {sidecar_warmed}");
            }
...
impl From<CudaProductPrewarmRequestKind> for CudaProductPrewarmRequestKindForTest {
    fn from(kind: CudaProductPrewarmRequestKind) -> Self {
        match kind {
            ...
            CudaProductPrewarmRequestKind::Q4SidecarPackedSingle => Self::Q4SidecarPackedSingle,
            CudaProductPrewarmRequestKind::Q6PackedDown => Self::Q6PackedDown,
            CudaProductPrewarmRequestKind::Q6SidecarPackedDown => Self::Q6SidecarPackedDown,
        }
    }
}
...
mod metal_decode_policy_facade_tests {
    ...
    #[test]
    fn chain_layer_input_owns_attention_and_gdn_state() {
        ...
        match attention {
            ChainLayerInput::Attn { prior_k, prior_v } => {
                assert_eq!(prior_k, [1, 2]);
                assert_eq!(prior_v, [3, 4]);
            }
            ChainLayerInput::Gdn { .. } => panic!("expected owned attention input"),
        }
        match gdn {
            ChainLayerInput::Gdn {
                conv_state,
            ...
            ChainLayerInput::Attn { .. } => panic!("expected owned GDN input"),
        }
    }
