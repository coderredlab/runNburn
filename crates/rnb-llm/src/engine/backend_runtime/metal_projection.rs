...
#[allow(clippy::too_many_arguments)]
#[cfg_attr(not(feature = "metal"), allow(dead_code, unused_variables))]
pub(in crate::engine) fn metal_gemma_attention_o_ffn_qkv_chain_into_if_supported(
    attn_out: &[f32],
    hidden: &mut [f32],
    o_weight: &QuantizedWeight,
    post_attn_norm_weight: &[f32],
    ...
    next_q_weight: &QuantizedWeight,
    next_k_weight: &QuantizedWeight,
    next_v_weight: &QuantizedWeight,
    next_q_out: &mut [f32],
    next_k_out: &mut [f32],
    next_v_out: &mut [f32],
    hidden_dim: usize,
    ...
    let (
        ...
        up_weight.backend_view(),
        down_weight.backend_view(),
        next_q_weight.backend_view(),
        next_k_weight.backend_view(),
        next_v_weight.backend_view(),
    )
    else {
        return Ok(false);
    };
    #[cfg(all(feature = "metal", not(feature = "cuda")))]
    {
        return metal_runtime::metal_gemma_attention_o_ffn_qkv_chain_into_if_supported(
            backend_ggml_type(o_v.quant()),
            backend_ggml_type(gate_v.quant()),
            backend_ggml_type(up_v.quant()),
            backend_ggml_type(down_v.quant()),
            backend_ggml_type(next_q_v.quant()),
            backend_ggml_type(next_k_v.quant()),
            backend_ggml_type(next_v_v.quant()),
            o_v.raw(),
            gate_v.raw(),
            up_v.raw(),
            down_v.raw(),
    ...
    #[cfg(not(all(feature = "metal", not(feature = "cuda"))))]
    {
        let _ = (o_v, gate_v, up_v, down_v, next_q_v, next_k_v, next_v_v);
        Ok(false)
    }
}
