...
#[cfg(not(feature = "cuda"))]
use crate::engine::quantized_dispatch::dispatch_f32_gemv;
#[cfg(not(feature = "cuda"))]
...
fn qwen_moe_cpu_prefill_batch_enabled(seq_len: usize) -> bool {
    ...
    match crate::engine::policy::env_string("RNB_QWEN35_PREFILL_CPU_BATCH") {
        Some(value) => !env_falsey(&value),
        None => true,
    }
}
    ...
    #[cfg(not(feature = "cuda"))]
    {
        #[cfg(feature = "metal")]
        {
            return all_high_precision
                && route_count > 0
                && crate::engine::policy::env_string("RNB_METAL_QWEN35_MOE_DECODE_BATCH")
                    .as_deref()
                    == Some("1");
        }
...
#[cfg(feature = "cuda")]
pub(in crate::engine) fn qwen_moe_configure_layer_cache(
    model_moe_bytes: usize,
    runtime_headroom_bytes: usize,
    min_layer_bytes: usize,
) -> crate::error::Result<usize> {
    cuda_runtime::qwen_moe_configure_layer_cache(
        model_moe_bytes,
...
#[cfg_attr(not(feature = "cuda"), allow(unused_variables))]
pub(in crate::engine) fn glm_mla_head_gemv_batch(
    ggml_type: GGMLType,
    raw: &[u8],
    head_count: usize,
    rows_per_head: usize,
    ...
    #[cfg(feature = "cuda")]
    {
        if ggml_type != GGMLType::Q8_0 || token_count <= 1 {
            return Ok(None);
        }
        let enabled = crate::engine::policy::env_string("RNB_CUDA_GLM_MLA_HEAD_BATCH")
            .map(|value| {
        ...
        return cuda_runtime::q8_0_head_gemv_batch(
            raw,
            head_count,
            rows_per_head,
            cols,
...
#[cfg_attr(not(feature = "cuda"), allow(unused_variables))]
#[allow(clippy::too_many_arguments)]
pub(in crate::engine) fn glm_mla_prefill_attention(
    q_absorbed: &[f32],
    q_pe: &[f32],
    cache: &[u16],
    pos_start: usize,
    ...
    #[cfg(feature = "cuda")]
    {
        return cuda_runtime::glm_mla_prefill_attention_f16(
...
/// pm119: layer fused 반환 (Metal glue 용 자체 타입 — backend type 비노출).
pub(in crate::engine) struct GlmMlaLayerFusedResult {
    pub(in crate::engine) projected: Vec<f32>,
    pub(in crate::engine) cache_tail: Vec<u16>,
}

/// pm119: MLA 층 전체 단일 command buffer Metal glue.
#[cfg_attr(
    not(all(feature = "metal", not(feature = "cuda"))),
    allow(unused_variables)
)]
#[allow(clippy::too_many_arguments)]
pub(in crate::engine) fn glm_mla_layer_fused(
    quants: [GGMLType; 6],
    qa_raw: &[u8],
    qa_norm_weight: &[f32],
    qb_raw: &[u8],
    ...
    theta: f32,
    scale: f32,
) -> std::result::Result<Option<GlmMlaLayerFusedResult>, String> {
    #[cfg(all(feature = "metal", not(feature = "cuda")))]
    {
        let [qa_quant, qb_quant, kva_quant, kb_quant, vb_quant, o_quant] = quants;
        return metal_runtime::metal_glm_mla_layer_fused_if_supported(
            qa_quant,
            qb_quant,
            kva_quant,
            kb_quant,
        ...
        .map(|out| {
            out.map(|out| GlmMlaLayerFusedResult {
                projected: out.projected,
                cache_tail: out.cache_tail,
            })
        })
        .map_err(|err| err.to_string());
    }
...
/// pm118 연장: front 4 dispatch 단일 command buffer Metal glue.
#[cfg_attr(
    not(all(feature = "metal", not(feature = "cuda"))),
    allow(unused_variables)
)]
#[allow(clippy::too_many_arguments)]
pub(in crate::engine) fn glm_mla_front_slots_fused_into(
    qa_quant: GGMLType,
    qb_quant: GGMLType,
    kva_quant: GGMLType,
    kb_quant: GGMLType,
    ...
    #[cfg(all(feature = "metal", not(feature = "cuda")))]
    {
        return metal_runtime::metal_glm_mla_front_slots_fused_into_if_supported(
            qa_quant,
            qb_quant,
            kva_quant,
            kb_quant,
...
/// pm118: v_b slots → o slots 단일 command buffer Metal glue.
#[cfg_attr(
    not(all(feature = "metal", not(feature = "cuda"))),
    allow(unused_variables)
)]
#[allow(clippy::too_many_arguments)]
pub(in crate::engine) fn glm_mla_vb_o_fused_into(
    vb_quant: GGMLType,
    o_quant: GGMLType,
    vb_raw: &[u8],
    o_raw: &[u8],
    ...
    #[cfg(all(feature = "metal", not(feature = "cuda")))]
    {
        return metal_runtime::metal_glm_mla_vb_o_fused_into_if_supported(
            vb_quant, o_quant, vb_raw, o_raw, slots, heads, value_dim, kv_rank, o_rows, o_cols,
            latent, out,
        )
        .map_err(|err| err.to_string());
    }
...
/// pm116: GLM MLA prefill attention Metal glue.
#[cfg_attr(
    not(all(feature = "metal", not(feature = "cuda"))),
    allow(unused_variables)
)]
#[allow(clippy::too_many_arguments)]
pub(in crate::engine) fn glm_mla_prefill_attn_into(
    q_absorbed: &[f32],
    q_pe: &[f32],
    cache: &[u16],
    slots: usize,
    ...
    #[cfg(all(feature = "metal", not(feature = "cuda")))]
    {
        return metal_runtime::metal_glm_mla_prefill_attn_into_if_supported(
