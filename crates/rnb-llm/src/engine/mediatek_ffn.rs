...
pub(super) fn cache_max_layers() -> usize {
    let raw = crate::engine::policy::env_string(CACHE_MAX_LAYERS_ENV);
...
fn selected_layer_decision_with_cap(
    ...
    if let Some(value) = crate::engine::policy::env_string(LAYERS_ENV) {
        if !compiled_reuse {
    ...
    let selected = match crate::engine::policy::env_string(LAYER_ENV) {
        Some(value) if value.trim() == "all" => {
...
pub(super) fn decide_prefill_npu(
    layer_idx: usize,
    seq_len: usize,
    cache_state: crate::engine::mediatek_runtime::MediaTekPrefillCacheState,
    request_mode: crate::engine::mediatek_runtime::MediaTekPrefillRequestMode,
    sync_compile_enabled: bool,
) -> crate::engine::mediatek_runtime::MediaTekPrefillNpuDecision {
    crate::engine::mediatek_runtime::decide_gemma_prefill_npu(
        crate::engine::mediatek_runtime::MediaTekPrefillPolicyInput {
            layer_idx,
            seq_len,
            cache_state,
            request_mode,
...
pub(super) fn decide_prefill_npu_for_cache_presence(
    ...
    let cache_state = crate::engine::mediatek_runtime::classify_gemma_prefill_cache_state(
...
pub(super) fn resolve_device_name() -> Result<String, MediaTekFfnRejectReason> {
    match crate::engine::policy::env_string(DEVICE_ENV) {
        Some(value) if value.trim().is_empty() => Err(MediaTekFfnRejectReason::InvalidDevice),
        Some(value) => Ok(value.trim().to_string()),
        None => Ok(DEFAULT_DEVICE.to_string()),
    }
}

pub(super) fn opt_in_enabled() -> bool {
    crate::engine::policy::env_string(ENABLE_ENV)
        .map(|value| value == "1")
        .unwrap_or(false)
}

fn parity_required() -> bool {
    crate::engine::policy::env_string(PARITY_ENV)
        .map(|value| value != "0")
        .unwrap_or(false)
}

pub(super) fn compiled_reuse_enabled() -> bool {
    crate::engine::policy::env_string(COMPILED_REUSE_ENV)
        .map(|value| value != "0")
        .unwrap_or(true)
}

pub(super) fn quantized_enabled() -> bool {
    crate::engine::policy::env_string(QUANTIZED_ENV)
        .map(|value| value == "1")
        .unwrap_or(false)
}

pub(super) fn quantized_stage_probe_enabled() -> bool {
    crate::engine::policy::env_string(QUANTIZED_STAGE_PROBE_ENV)
        .map(|value| value == "1")
        .unwrap_or(false)
}

pub(super) fn prefill_enabled() -> bool {
    crate::engine::policy::env_string(PREFILL_ENV)
        .map(|value| value == "1")
        .unwrap_or(false)
}

pub(super) fn trace_enabled() -> bool {
    crate::engine::policy::env_string(TRACE_ENV)
        .map(|value| value == "1")
        .unwrap_or(false)
}
...
fn trace_prefill_policy_fallback(
    layer_idx: usize,
    reason: crate::engine::mediatek_runtime::MediaTekPrefillFallbackReason,
) {
    if reason == crate::engine::mediatek_runtime::MediaTekPrefillFallbackReason::LayerNotSelected
        || !trace_enabled()
    {
        return;
    }
...
pub(super) fn stage_probe_line(
    layer_idx: usize,
    stage: crate::engine::mediatek_runtime::MediaTekQuantizedGatedGeluFfnStage,
...
pub(super) fn timing_line(
    ...
    backend: crate::engine::mediatek_runtime::RunGatedGeluFfnF32Timings,
...
pub(super) struct QuantizedGatedGeluAdmission {
    weights: crate::engine::mediatek_runtime::RunCachedQuantizedGatedGeluFfnWeights,
    reference: Vec<f32>,
...
pub(super) struct QuantizedGatedGeluStageReference {
    pub(super) stage: crate::engine::mediatek_runtime::MediaTekQuantizedGatedGeluFfnStage,
...
pub(super) fn build_quantized_gated_gelu_admission(
    ...
    let quant_params =
        crate::engine::mediatek_runtime::MediaTekQuantizedGatedGeluFfnQuantParams::new(
            input_quant_params,
            ...
            crate::engine::mediatek_runtime::MediaTekQuantParams::new(1.0 / 128.0, 128),
            crate::engine::mediatek_runtime::MediaTekQuantParams::new(1.0 / 128.0, 0),
            quant_params_for_nonnegative_quantized_gated_gelu(&[&one_plus]),
            quant_params_for_quantized_gated_gelu(&[&gelu_factor]),
            crate::engine::mediatek_runtime::MediaTekQuantParams::new(1.0 / 255.0, 0),
    ...
    let weights = crate::engine::mediatek_runtime::RunCachedQuantizedGatedGeluFfnWeights {
        quant_params,
...
pub(super) fn quant_params_for_quantized_gated_gelu(
    tensors: &[&[f32]],
) -> crate::engine::mediatek_runtime::MediaTekQuantParams {
    let mut min = 0.0f32;
    let mut max = 0.0f32;
    ...
    if !saw_finite || max <= min {
        return crate::engine::mediatek_runtime::MediaTekQuantParams::new(1.0e-6, 0);
    }
    let scale = ((max - min) / 255.0).max(1.0e-6);
    let zero_point = (-min / scale).round().clamp(0.0, 255.0) as i32;
    crate::engine::mediatek_runtime::MediaTekQuantParams::new(scale, zero_point)
}
...
fn quant_params_for_symmetric_quantized_gated_gelu(
    ...
    crate::engine::mediatek_runtime::MediaTekQuantParams::new(scale, 128)
}
...
fn quant_params_for_nonnegative_quantized_gated_gelu(
    ...
    crate::engine::mediatek_runtime::MediaTekQuantParams::new(scale, 0)
}

fn quant_params_for_nonnegative_scalar(
    value: f32,
) -> crate::engine::mediatek_runtime::MediaTekQuantParams {
...
fn quantize_f32_to_u8(
    values: &[f32],
    params: crate::engine::mediatek_runtime::MediaTekQuantParams,
) -> Vec<u8> {
    values
        .iter()
        .map(|value| {
...
fn dequantize_u8(value: u8, params: crate::engine::mediatek_runtime::MediaTekQuantParams) -> f32 {
    (i32::from(value) - params.zero_point()) as f32 * params.scale()
}

pub(super) fn quantized_fc_output_reference(
    input: &[u8],
    input_params: crate::engine::mediatek_runtime::MediaTekQuantParams,
    weight: &[u8],
    weight_params: crate::engine::mediatek_runtime::MediaTekQuantParams,
    output_params: crate::engine::mediatek_runtime::MediaTekQuantParams,
    rows: usize,
    cols: usize,
    label: &'static str,
...
fn build_quantized_gated_gelu_stage_references(
    ...
    quant_params: crate::engine::mediatek_runtime::MediaTekQuantizedGatedGeluFfnQuantParams,
    gate_projection: &[f32],
    up_projection: &[f32],
    gelu: &[f32],
    gated: &[f32],
    ...
    Ok(vec![
        QuantizedGatedGeluStageReference {
            stage: crate::engine::mediatek_runtime::MediaTekQuantizedGatedGeluFfnStage::GateFc,
            f32_reference: gate_projection.to_vec(),
            cpu_quantized_reference: gate_fc.clone(),
        },
        QuantizedGatedGeluStageReference {
            stage: crate::engine::mediatek_runtime::MediaTekQuantizedGatedGeluFfnStage::UpFc,
            f32_reference: up_projection.to_vec(),
            cpu_quantized_reference: up_fc.clone(),
        },
        QuantizedGatedGeluStageReference {
            stage: crate::engine::mediatek_runtime::MediaTekQuantizedGatedGeluFfnStage::GateDequant,
            f32_reference: gate_projection.to_vec(),
            cpu_quantized_reference: gate_fc,
        },
        QuantizedGatedGeluStageReference {
            stage: crate::engine::mediatek_runtime::MediaTekQuantizedGatedGeluFfnStage::UpDequant,
            f32_reference: up_projection.to_vec(),
            cpu_quantized_reference: up_fc,
        },
        QuantizedGatedGeluStageReference {
            stage: crate::engine::mediatek_runtime::MediaTekQuantizedGatedGeluFfnStage::Gelu,
            f32_reference: gelu.to_vec(),
            cpu_quantized_reference: gelu_quantized.clone(),
        },
        QuantizedGatedGeluStageReference {
            stage: crate::engine::mediatek_runtime::MediaTekQuantizedGatedGeluFfnStage::Gated,
            f32_reference: gated.to_vec(),
            cpu_quantized_reference: gated_quantized.clone(),
        },
        QuantizedGatedGeluStageReference {
            stage: crate::engine::mediatek_runtime::MediaTekQuantizedGatedGeluFfnStage::Output,
...
pub(super) fn build_quantized_gated_gelu_act_weight_splits(
    ...
    let build_one = |stage: crate::engine::mediatek_runtime::MediaTekQuantizedGatedGeluFfnStage,
                     weight_f32: &[f32],
                     weight_q8: &[u8],
                     weight_params: crate::engine::mediatek_runtime::MediaTekQuantParams,
                     out_params: crate::engine::mediatek_runtime::MediaTekQuantParams,
                     projection: &[f32]|
    ...
    let gate = build_one(
        crate::engine::mediatek_runtime::MediaTekQuantizedGatedGeluFfnStage::GateFc,
        gate_weight,
        gate_weight_quantized,
        quant_params.gate_weight,
        quant_params.gate,
    ...
    .ok()?;
    let up = build_one(
        crate::engine::mediatek_runtime::MediaTekQuantizedGatedGeluFfnStage::UpFc,
        up_weight,
        up_weight_quantized,
        quant_params.up_weight,
        quant_params.up,
...
fn emit_quantized_stage_probe_lines(
    ...
    let shape = crate::engine::mediatek_runtime::MediaTekGatedGeluFfnShape::new(
        hidden_dim, ffn_inner, hidden_dim,
    );
    for stage_reference in stage_references {
        let request = crate::engine::mediatek_runtime::RunQuantizedGatedGeluFfnStageProbeRequest {
        ...
        let stage_output =
            match crate::engine::mediatek_runtime::run_quantized_gated_gelu_ffn_stage_probe(request)
            {
        ...
        let nnapi_vs_f32 = if stage_reference.stage
            == crate::engine::mediatek_runtime::MediaTekQuantizedGatedGeluFfnStage::Output
        {
...
pub(super) fn try_mediatek_gemma_prefill_ffn_down(
    ...
    try_mediatek_gemma_prefill_ffn_down_with_mode(
        ...
        crate::engine::mediatek_runtime::MediaTekPrefillRequestMode::UserPath,
        false,
    )
}
...
pub(super) fn try_mediatek_gemma_prefill_ffn_down_with_mode(
    ...
    let base_cache_key = match (
        ...
        (Some(gate), Some(up), Some(down)) => {
            crate::engine::mediatek_runtime::GatedGeluFfnF32CacheKey {
    ...
    let cache_key = crate::engine::mediatek_runtime::GatedGeluFfnF32BatchedCacheKey {
        base: base_cache_key,
        batch: plan.batch_size,
    };
    let runtime_cache_hit =
        crate::engine::mediatek_runtime::is_gated_gelu_ffn_f32_batched_cached(&cache_key);
    ...
    let max_cache_entries = match prefill_decision {
        crate::engine::mediatek_runtime::MediaTekPrefillNpuDecision::UseWarmNpu {
            max_cache_entries,
        } => max_cache_entries,
        crate::engine::mediatek_runtime::MediaTekPrefillNpuDecision::AllowPrewarmCompile {
            max_cache_entries,
        } => max_cache_entries,
        crate::engine::mediatek_runtime::MediaTekPrefillNpuDecision::FallbackCpu { reason } => {
    ...
    let mut runtime_weights = if runtime_cache_hit {
        ...
        Some(
            crate::engine::mediatek_runtime::RunCachedGatedGeluFfnF32Weights {
    ...
    for chunk_idx in 0..plan.full_chunks {
        ...
        let request = crate::engine::mediatek_runtime::RunCachedGatedGeluFfnF32BatchedRequest {
        ...
        let chunk_output =
            match crate::engine::mediatek_runtime::run_cached_gated_gelu_ffn_f32_batched(request) {
                Ok(output) => output,
                Err(err) => {
                    eprintln!(
...
pub(super) fn try_mediatek_gemma_ffn_down(
    ...
    let cache_key = if use_compiled_reuse {
        match (
            ...
            (Some(gate), Some(up), Some(down)) => {
                Some(crate::engine::mediatek_runtime::GatedGeluFfnF32CacheKey {
                    device_name_substring: device_name.clone(),
                    layer_idx,
                    input_size: hidden_dim,
                    ffn_inner_size: ffn_inner,
    ...
    let runtime_cache_hit = cache_key
        ...
        .map(|key| {
            if use_quantized {
                ...
                crate::engine::mediatek_runtime::is_gated_gelu_ffn_f32_cached(key)
            }
        })
        .unwrap_or(false);
        ...
        if use_compiled_reuse {
            if use_quantized {
                ...
                let admitted_cache_hit = cache_key
                    .as_ref()
                    .map(|key| {
                        crate::engine::mediatek_runtime::is_gated_gelu_ffn_quantized_cached_with_params(
                            key,
                ...
                let request =
                    crate::engine::mediatek_runtime::RunCachedQuantizedGatedGeluFfnRequest {
                        cache_key: cache_key
                            .as_ref()
                ...
                let output =
                    match crate::engine::mediatek_runtime::run_cached_quantized_gated_gelu_ffn(
                        request,
                    ) {
                        Ok(output) => output,
                        Err(err) => {
                            ...
                            if let Some(key) = cache_key.as_ref() {
                                crate::engine::mediatek_runtime::remove_gated_gelu_ffn_quantized_cache_entry(
                                    key,
                                );
                            }
                            return Ok(None);
                ...
                let request = crate::engine::mediatek_runtime::RunCachedGatedGeluFfnF32Request {
                    cache_key: cache_key
                        .as_ref()
                        .expect("compiled reuse enabled builds a cache key")
                        .clone(),
                ...
                let runtime_start = std::time::Instant::now();
                let output =
                    match crate::engine::mediatek_runtime::run_cached_gated_gelu_ffn_f32(request) {
                        Ok(output) => output,
                        Err(err) => {
                            eprintln!(
                                "[mediatek-ffn] used=false layer={layer_idx} reason={} error={err}",
            ...
            let materialize_ns = elapsed_ns(materialize_start);
            let request = crate::engine::mediatek_runtime::RunGatedGeluFfnF32Request {
            ...
            let runtime_start = std::time::Instant::now();
            let output = match crate::engine::mediatek_runtime::run_gated_gelu_ffn_f32(request) {
                Ok(output) => output,
                Err(err) => {
                    eprintln!(
                        "[mediatek-ffn] used=false layer={layer_idx} reason={} error={err}",
    ...
    if output.output.len() != hidden_dim {
        ...
        if use_quantized {
            if let Some(key) = cache_key.as_ref() {
                crate::engine::mediatek_runtime::remove_gated_gelu_ffn_quantized_cache_entry(key);
            }
            if output.cache_hit {
