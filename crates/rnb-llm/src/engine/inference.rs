...
#[cfg(feature = "vulkan")]
use super::models::gemma::uses_neox_rope;
#[cfg(feature = "cuda")]
use super::models::gemma::{
    active_sliding_window, qwen_text_mrope_dim, resolve_attention_scale, resolve_attention_softcap,
    resolve_rope_params, shared_kv_source_layer,
};
...
#[cfg(feature = "cuda")]
use super::prefill::{
    run_prefill_layers_cpu_range_carrier, run_prefill_layers_cpu_range_mtp_resident_kv,
};
#[cfg(test)]
...
impl Engine {
    ...
    pub(super) fn prepare_vulkan_fullpath_model_for_load(&mut self) -> crate::error::Result<()> {
        if !matches!(
            self.select_prefill_path(2),
            crate::runtime::scheduler::PrefillExecutionPath::Fullpath
        ) {
            return Ok(());
        }
        ...
        let result = (|| {
            let gpu = gpu_runtime.as_mut().ok_or_else(|| {
                crate::error::LlmError::Forward(
                    "prepare_vulkan_fullpath_model_for_load: active fullpath has no GPU runtime"
                        .into(),
                )
            })?;
            ...
            let ffn_inner = Self::fullpath_ffn_inner_dim_or_error(
                weights,
                "prepare_vulkan_fullpath_model_for_load",
            )?;
            let output_table = weights.output.data.as_bytes().ok_or_else(|| {
                crate::error::LlmError::Forward(
                    "prepare_vulkan_fullpath_model_for_load: output has no contiguous host bytes"
                        .into(),
                )
            })?;
            let vocab = Self::fullpath_vocab_rows_or_error(
                weights,
                "prepare_vulkan_fullpath_model_for_load",
            )?;
            gpu.prepare_fullpath_model(
    ...
    pub(super) fn select_prefill_path(&self, token_count: usize) -> PrefillExecutionPath {
        ...
        let path = crate::runtime::scheduler::select_prefill_path_for_profile(
            profile,
            token_count,
            GpuPrefillExecutor::for_slice1(&self.metadata).is_some(),
        ...
        #[cfg(feature = "vulkan")]
        if matches!(path, PrefillExecutionPath::Fullpath) && !self.fullpath_prefill_supported() {
            return if crate::runtime::scheduler::should_attempt_slice1_gpu_prefill(
                token_count,
                GpuPrefillExecutor::for_slice1(&self.metadata).is_some(),
            ) && self.has_active_gpu_prefill_path()
            {
        ...
        #[cfg(feature = "cuda")]
        {
            ...
            if !matches!(
                ...
                return Err(crate::error::LlmError::Forward(format!(
                    "RNB_MTP_DEVICE_VERIFY=1 요청됐지만 token_embd가 Q4_K/Q6_K/Q8_0가 아님: {:?}. prefill verify로 우회하지 않음",
                    token_embd.ggml_type
                )));
            }
            ...
            let gemma_ple_base = if super::models::gemma::gemma_per_layer_enabled_for_model(
                weights,
                &self.metadata,
                self.architecture,
            ) {
                let mut base_hidden = token_embd.embedding_gather_cuda(&verify_tokens)?;
                if !gemma_ple_pre_emb_scale_base() {
                    apply_embedding_scale_inplace(
                        &mut base_hidden,
                        &self.metadata,
                        self.architecture,
                    );
                ...
                prepare_gemma_per_layer_base(
                    weights,
                    &super::Tensor::from_vec(
                        base_hidden,
                        &[verify_tokens.len(), self.metadata.hidden_dim],
                    ),
                    &verify_tokens,
                    &self.metadata,
                    self.architecture,
            ...
            let device_rope_neox = super::mtp::mtp_device_rope_neox(
                self.architecture,
            ...
            let device_request = crate::engine::cuda_runtime::MtpDeviceVerifyWindowRequest {
                ...
                attention_kvarn_priors: &layer_graph.attention_kvarn_priors,
    ...
    fn forward_prefill_argmax_tokens_collect_mtp_impl(
        ...
        if verify_profiling {
            vp_ms[0] = vp_mark.elapsed().as_secs_f64() * 1000.0;
        ...
        if verify_profiling {
            vp_ms[1] = vp_mark.elapsed().as_secs_f64() * 1000.0;
            vp_mark = std::time::Instant::now();
        }
        let mtp_hidden_rows = self
            .mtp_spec_requested()
        ...
        let mut prefix_states = prefix_collector
            .map(|collector| {
                ...
                // layers 가 정상 — KV truncate 전용 prefix state 로 반환.
                if crate::speculative::architecture_has_recurrent_state(self.architecture) {
                    collector.finish_many_required()
                } else {
                    Ok(collector.finish_many_allow_empty())
                }
            })
            .transpose()?
            .unwrap_or_default();
        ...
        if verify_profiling {
            vp_ms[3] = vp_mark.elapsed().as_secs_f64() * 1000.0;
            eprintln!(
                "[glm-verify-profile] seq={seq_len} embed={:.1} layers={:.1} argmax={:.1} tail={:.1}",
    ...
    pub(crate) fn restore_verify_window_prefix_state(
        ...
        for layer in &prefix_state.layers {
            ...
            #[cfg(feature = "cuda")]
            let conv_restored_resident =
                if let Some(snapshot) = layer.resident_conv_snapshot.as_ref() {
                    ...
                    if !restored {
                        return Err(crate::error::LlmError::Forward(format!(
                            "missing resident conv state for verify prefix layer {}",
                            layer.layer_idx
                        )));
                    }
                    true
            ...
            #[cfg(not(feature = "cuda"))]
            let conv_restored_resident = false;
            if conv_restored_resident {
                #[cfg(feature = "cuda")]
                {
            ...
            if let Some(delta_state) = layer.delta_state.as_ref() {
                if state.delta_state.len() != delta_state.len() {
                    return Err(crate::error::LlmError::Forward(format!(
                        "verify prefix delta_state mismatch for layer {}: got {}, expected {}",
                        layer.layer_idx,
                        delta_state.len(),
    ...
    #[cfg(any(feature = "cuda", test))]
    pub(crate) fn commit_device_verify_window_prefix_states(
        &mut self,
        base_kv_len: usize,
        result: &crate::engine::verify_window::VerifyWindowResult,
        committed_tokens: usize,
    ) -> crate::error::Result<()> {
        if committed_tokens == 0 || committed_tokens > result.len() {
            return Err(crate::error::LlmError::Forward(format!(
                "device verify committed prefix out of range: committed={committed_tokens}, window={}",
                result.len()
            )));
        }
        if committed_tokens == result.len() {
            return self.commit_device_verify_window_final_states(base_kv_len, result);
        }

        if let Some(prefix_state) = result.prefix_state_for(committed_tokens) {
            self.restore_verify_window_prefix_state(base_kv_len, prefix_state)?;
        } else {
            if !result.ssm_final_states.is_empty() {
                return Err(crate::error::LlmError::Forward(format!(
                    "device verify committed prefix {committed_tokens} has no recurrent state"
                )));
            }
            self.kv_cache
                .set_len((base_kv_len + committed_tokens).min(self.kv_cache.max_seq_len));
        }
        ...
        for layer in &result.attention_kv_states {
            ...
            let committed_values =
                committed_tokens.checked_mul(layer.kv_rows).ok_or_else(|| {
                    crate::error::LlmError::Forward(format!(
                        "device verify attention K/V prefix length overflow for layer {}",
                        layer.layer_idx
                    ))
                })?;
            let expected_values =
    ...
    fn fullpath_run_prefill(
        ...
        let embed_quant = Self::fullpath_embed_quant_or_error(
            weights.token_embd.ggml_type,
            "fullpath_run_prefill",
        )?;
        let output_quant =
            Self::fullpath_output_quant_or_error(weights.output.ggml_type, "fullpath_run_prefill")?;
        let token_embd_quantized = weights.token_embd.data.as_bytes().ok_or_else(|| {
            crate::error::LlmError::Forward(
    ...
    /// 256MiB. Production fullpath stages only requested rows into a compact
    /// table, so this is kept only for older smoke paths.
    ///
    ...
    pub(super) fn ensure_fullpath_token_embd_bound_inner(
        ...
        let embed_quant = Self::fullpath_embed_quant_or_error(
            weights.token_embd.ggml_type,
            "ensure_fullpath_token_embd_bound",
        )?;
        ...
        gpu_runtime
            .bind_token_embd(bytes, embed_quant)
            .map_err(crate::error::LlmError::Forward)?;
        *bound_flag = true;
        Ok(())
    }
    ...
    pub(super) fn fullpath_output_quant_or_error(
        ...
        super::gpu_runtime::ggml_to_fullpath_quant(ggml_type).ok_or_else(|| {
            crate::error::LlmError::Forward(format!(
                "{caller}: output ggml_type {ggml_type:?} is not supported by Vulkan fullpath",
            ))
        })
    }

    #[cfg(feature = "vulkan")]
    pub(super) fn fullpath_embed_quant_or_error(
        ggml_type: rnb_loader::GGMLType,
        caller: &'static str,
    ) -> crate::error::Result<super::gpu_runtime::Quant> {
        super::gpu_runtime::ggml_to_fullpath_quant(ggml_type).ok_or_else(|| {
            crate::error::LlmError::Forward(format!(
                "{caller}: token_embd ggml_type {ggml_type:?} is not supported by Vulkan fullpath",
            ))
        })
    }
    ...
    pub(super) fn fullpath_rope_config(&self) -> (usize, bool) {
        ...
        let rope_neox = uses_neox_rope(self.architecture);
        (rope_dim, rope_neox)
    }
    ...
    pub fn forward(&mut self, tokens: &[u32]) -> crate::error::Result<Vec<f32>> {
        ...
        if tokens.len() == 1 && self.scratch.is_some() && self.weights.is_some() {
            ...
            // prefill. Pure-attention models have no slice1 boundary plan, so
            // decode eligibility must use the fullpath weight contract rather
            // than `GpuPrefillExecutor::for_slice1`.
            #[cfg(feature = "vulkan")]
            if resident_fullpath_decode_ready(
                self.scratch
                    .as_ref()
                    .is_some_and(|scratch| scratch.fullpath_resident_kv_active),
                self.fullpath_prefill_supported(),
                self.has_active_gpu_prefill_path(),
            ) {
        ...
        if tokens.len() > 1
            && self.scratch.is_some()
            && self.weights.is_some()
            && crate::engine::policy::env_string("RNB_CUDA_PERSISTENT_PREFILL_BATCH").is_some()
        {
        ...
        if tokens.len() > 1
            && self.scratch.is_some()
            && self.weights.is_some()
            && crate::engine::policy::env_string("RNB_CUDA_PERSISTENT_PREFILL").is_some()
        {
            let trace =
                crate::engine::policy::env_string("RNB_CUDA_PERSISTENT_PREFILL_TRACE").is_some();
            ...
            if crate::engine::policy::env_string("RNB_KV_DUMP_AFTER_FORWARD").is_some() {
    ...
    pub(super) fn forward_persistent_prefill_batch(
        ...
        let use_raw_for_ple_base = super::models::gemma::gemma_ple_pre_emb_scale_base();
        {
            ...
            let gathered = embd.embedding_gather_cuda(tokens)?;
            for row in gathered.chunks_exact(hidden_dim) {
...
pub(super) struct MtpDeviceVerifyLayerGraph<'a> {
    ...
    pub(super) attention_kvarn_priors: Vec<Option<super::cuda_runtime::KvarnDecodeRequest<'a>>>,
...
pub(super) fn build_mtp_device_verify_layer_graph<'a>(
    ...
    gemma_ple_base: Option<&super::models::gemma::GemmaPerLayerBase>,
) -> crate::error::Result<MtpDeviceVerifyLayerGraph<'a>> {
    for layer_idx in 0..kv_cache.num_layers() {
        if kv_cache.layer_uses_kvarn(layer_idx) {
            kv_cache
                .compact_layer(layer_idx)
                .map_err(crate::error::LlmError::Forward)?;
        }
    }
    let prior_tokens = kv_cache.current_len();
    let prior_sequence_epoch = kv_cache.sequence_epoch();
    let (cache_layers, ssm_states) = kv_cache.layer_and_ssm_parts_mut();
    let gdn_moe_layers =
        build_mtp_device_verify_gdn_moe_layers_from_states(weights, metadata, ssm_states, true)?;
    let (attention_moe_layers, attention_kvarn_priors) =
        build_mtp_device_verify_attention_moe_layers_inner(
            weights,
            metadata,
            architecture,
            cache_layers,
...
#[cfg(feature = "cuda")]
fn mtp_device_prior_kv_bits(
    layer_cache: &crate::kv_cache::LayerCache,
    prior_tokens: usize,
) -> (Vec<u16>, Vec<u16>) {
    if prior_tokens == 0 {
        return (Vec::new(), Vec::new());
    }
    let Some(cached) = layer_cache.read_up_to_materialized_if_initialized(prior_tokens) else {
        return (Vec::new(), Vec::new());
    };
...
#[cfg(feature = "cuda")]
pub(in crate::engine) fn build_mtp_device_verify_attention_moe_layers<'weights>(
    weights: &'weights super::layer_weights::ModelWeights,
    metadata: &ModelMetadata,
    architecture: super::ModelArchitecture,
    kv_cache: &crate::kv_cache::KVCache,
) -> crate::error::Result<Vec<super::cuda_runtime::MtpDeviceVerifyAttentionMoeLayer<'weights>>> {
    let (layers, _) = build_mtp_device_verify_attention_moe_layers_inner(
        weights,
        metadata,
        architecture,
        kv_cache.layers(),
...
#[cfg(feature = "cuda")]
fn build_mtp_device_verify_attention_moe_layers_inner<'weights, 'cache>(
    weights: &'weights super::layer_weights::ModelWeights,
    metadata: &ModelMetadata,
    architecture: super::ModelArchitecture,
    cache_layers: &'cache [crate::kv_cache::LayerCache],
    prior_tokens: usize,
    prior_sequence_epoch: u64,
    gemma_ple_base: Option<&super::models::gemma::GemmaPerLayerBase>,
) -> crate::error::Result<(
    Vec<super::cuda_runtime::MtpDeviceVerifyAttentionMoeLayer<'weights>>,
    Vec<Option<super::cuda_runtime::KvarnDecodeRequest<'cache>>>,
)> {
    let mut layers = Vec::new();
    let mut kvarn_priors = Vec::new();
    for (layer_idx, layer) in weights.layers.iter().enumerate() {
        ...
        let kv_attn = match kv_source_layer {
            Some(source_idx) => match weights.layers.get(source_idx) {
                Some(super::layer_weights::LayerType::Attention(source)) => source,
                _ => {
                    return Err(crate::error::LlmError::Forward(format!(
                        "MTP device verify layer {layer_idx} shared KV source layer {source_idx} is not attention"
                    )))
                }
            },
            None => attn,
        ...
        let (k_q4k, k_quant) =
            k_quant_weight_bytes_for_mtp_device(&kv_attn.k_weight, layer_idx, "attn_k")?;
        let (v_q4k, v_quant) =
            k_quant_weight_bytes_for_mtp_device(&kv_attn.v_weight, layer_idx, "attn_v")?;
        let (o_q4k, o_quant) =
        ...
        let k_norm = kv_attn.k_norm.as_ref().ok_or_else(|| {
            crate::error::LlmError::Forward(format!(
                "MTP device verify layer {layer_idx} attention K/V source k_norm missing"
            ))
        })?;
        let gemma_block_semantics = super::models::gemma::use_gemma_block_semantics(architecture);
        let post_attn_norm = if gemma_block_semantics {
            attn.post_attn_norm
                .as_ref()
                .map(super::cpu_runtime::kernels::tensor_as_f32_slice)
                .ok_or_else(|| {
                    crate::error::LlmError::Forward(format!(
                        "MTP device verify Gemma layer {layer_idx} attention post_attn_norm missing"
                    ))
                })?
        } else {
            &[]
        ...
        let (
            ...
            let (shared_gate, shared_gate_quant) =
                k_quant_weight_bytes_for_mtp_device(&moe.shared_gate, layer_idx, "shared_gate")?;
            let (shared_up, shared_up_quant) =
                k_quant_weight_bytes_for_mtp_device(&moe.shared_up, layer_idx, "shared_up")?;
            let (shared_down, shared_down_quant) =
                k_quant_weight_bytes_for_mtp_device(&moe.shared_down, layer_idx, "shared_down")?;
        ...
        let cache_layer = cache_layers.get(cache_layer_idx).ok_or_else(|| {
            crate::error::LlmError::Forward(format!(
                "MTP device verify layer {layer_idx} KV cache source layer {cache_layer_idx} missing"
            ))
        })?;
        let attention_scale = resolve_attention_scale(metadata, architecture);
        let sliding_window = active_sliding_window(metadata, architecture, layer_idx);
        let attention_softcap = resolve_attention_softcap(architecture);
        let kvarn_prior = cache_layer
            .kvarn_view_if_initialized(prior_tokens)
            .map(|view| {
                super::cuda_runtime::KvarnDecodeRequest::new_device_query(
                    cache_layer_idx,
                    view.device_blocks,
                    view.sink_key,
                    view.sink_value,
                    ...
                    view.num_kv_heads,
                    view.head_dim,
                    view.config.key_bits,
                    view.config.value_bits,
                    view.config.group,
                    view.config.sink_tokens,
                    view.device_layout.block_bytes,
                    attention_scale,
                    sliding_window,
                    attention_softcap,
                )
            });
        if let Some(request) = kvarn_prior {
            request.validate_device_query(1).map_err(|err| {
                let row_width = request
                    .num_kv_heads()
                    .saturating_mul(request.head_dim())
                    .max(1);
                crate::error::LlmError::Forward(format!(
                    "MTP device verify layer {layer_idx} KVarN prior invalid: {err}; kv_len={} sink_rows={} packed_blocks={} tail_start={} tail_rows={} group={}",
                    request.kv_len(),
                    request.sink_key().len() / row_width,
                    request.packed_blocks().len() / request.block_bytes().max(1),
                    request.tail_start(),
                    request.tail_key().len() / row_width,
                    request.group(),
                ))
            })?;
        }
        let (prior_k_bits, prior_v_bits) = if kvarn_prior.is_some() {
            (Vec::new(), Vec::new())
        } else {
            mtp_device_prior_kv_bits(cache_layer, prior_tokens)
        };
        let expected_prior_values = prior_tokens
            .checked_mul(kv_attn.k_weight.rows)
            .ok_or_else(|| {
                crate::error::LlmError::Forward(format!(
                    "MTP device verify layer {layer_idx} prior KV size overflow: tokens={prior_tokens} rows={}",
                    kv_attn.k_weight.rows
                ))
            })?;
        let prior_is_device_resident = kvarn_prior.is_none()
            && prior_tokens > 0
            && prior_k_bits.is_empty()
            && prior_v_bits.is_empty();
        if kvarn_prior.is_none()
            && !prior_is_device_resident
        ...
        let post_ffw_norm = if gemma_block_semantics {
            attn.post_ffw_norm
                .as_ref()
                .map(super::cpu_runtime::kernels::tensor_as_f32_slice)
                .unwrap_or(&[])
        ...
        let (
            ...
            ple_post_norm,
            ple_input,
        ) = if let (Some(gemma), Some(base)) = (weights.gemma_per_layer.as_ref(), gemma_ple_base) {
            let ple = gemma.layers.get(layer_idx).ok_or_else(|| {
                crate::error::LlmError::Forward(format!(
                    "MTP device verify Gemma PLE layer {layer_idx} missing"
                ))
            })?;
            let gate = ple.inp_gate.data.as_bytes().ok_or_else(|| {
                crate::error::LlmError::Forward(format!(
                    "MTP device verify Gemma PLE layer {layer_idx} gate bytes missing"
                ))
            })?;
            let proj = ple.proj.data.as_bytes().ok_or_else(|| {
                crate::error::LlmError::Forward(format!(
                    "MTP device verify Gemma PLE layer {layer_idx} proj bytes missing"
                ))
            })?;
            let d = metadata.embedding_length_per_layer_input;
            let total_d = metadata.num_layers.checked_mul(d).ok_or_else(|| {
                crate::error::LlmError::Forward(
                    "MTP device verify Gemma PLE width overflow".to_string(),
                )
            })?;
            if total_d == 0 || base.mixed.len() % total_d != 0 {
                return Err(crate::error::LlmError::Forward(format!(
                    "MTP device verify Gemma PLE base len {} is not divisible by total width {total_d}",
                    base.mixed.len()
                )));
            }
            let seq_len = base.mixed.len() / total_d;
            let layer_start = layer_idx * d;
            let mut input = Vec::with_capacity(seq_len * d);
            for row in base.mixed.chunks_exact(total_d) {
                input.extend_from_slice(&row[layer_start..layer_start + d]);
            }
            (
                gate,
                ple.inp_gate.ggml_type,
                ple.inp_gate.rows,
                ple.inp_gate.cols,
                proj,
                ple.proj.ggml_type,
                ple.proj.rows,
                ple.proj.cols,
                super::cpu_runtime::kernels::tensor_as_f32_slice(&ple.post_norm),
                input,
            )
        } else {
            (
        ...
        let unit_offset_ffn_ple_norm = gemma_block_semantics
        ...
        layers.push(super::cuda_runtime::MtpDeviceVerifyAttentionMoeLayer {
            ...
            k_q4k,
            k_quant,
            k_rows: kv_attn.k_weight.rows,
            k_cols: kv_attn.k_weight.cols,
            v_q4k,
            v_quant,
            v_rows: kv_attn.v_weight.rows,
            v_cols: kv_attn.v_weight.cols,
            prior_k_bits,
            prior_v_bits,
            prior_tokens,
            prior_sequence_epoch,
            ...
            qk_norm_unit_offset: super::models::gemma::use_gemma_block_semantics(architecture)
                && super::policy::gemma_unit_offset_norm_enabled(),
            v_no_scale_norm: super::models::gemma::use_gemma_block_semantics(architecture),
            rope_freq_factors: super::models::gemma::gemma_rope_freq_factors(
            ...
            post_attn_norm_unit_offset:
                super::models::gemma::gemma_effective_unit_offset_post_attn_enabled(
                    architecture,
                    super::models::gemma::detect_gemma_runtime_flavor(metadata, weights),
                    layer_idx,
                ),
            ffn_norm_unit_offset: unit_offset_ffn_ple_norm,
            post_ffw_norm_unit_offset: unit_offset_ffn_ple_norm,
            ffn_norm: super::cpu_runtime::kernels::tensor_as_f32_slice(
            ...
            ple_proj_cols,
            ple_post_norm,
            ple_post_norm_unit_offset: unit_offset_ffn_ple_norm,
            ple_input,
            ffn_uses_gelu: gemma_block_semantics,
            ffn_gate_q4k,
            ffn_gate_rows,
            ffn_gate_cols,
            ffn_up_q4k,
    ...
    Ok((layers, kvarn_priors))
}
#[cfg(feature = "cuda")]
pub(in crate::engine) fn build_mtp_device_draft_attention_moe_layer<'a>(
    ...
    let q_norm = attn.q_norm.as_ref().ok_or_else(|| {
        crate::error::LlmError::Forward("MTP device draft attention q_norm missing".to_string())
    })?;
    let k_norm = attn.k_norm.as_ref().ok_or_else(|| {
        crate::error::LlmError::Forward("MTP device draft attention k_norm missing".to_string())
    })?;
    let post_attn_norm = attn.post_attn_norm.as_ref().ok_or_else(|| {
        crate::error::LlmError::Forward(
            "MTP device draft attention post_attn_norm missing".to_string(),
        )
    })?;
    let moe = attn.shared_expert_moe.as_ref().ok_or_else(|| {
        crate::error::LlmError::Forward(
            "MTP device draft requires a shared-expert attention block".to_string(),
        )
    })?;
    if moe.n_embd != metadata.hidden_dim {
        return Err(crate::error::LlmError::Forward(format!(
            "MTP device draft MoE n_embd {} != hidden_dim {}",
            moe.n_embd, metadata.hidden_dim
        )));
    }
    if moe.gate_quant != rnb_loader::GGMLType::Q4_K || moe.up_quant != rnb_loader::GGMLType::Q4_K {
        return Err(crate::error::LlmError::Forward(format!(
            "MTP device draft sparse gate/up must be Q4_K: gate={:?} up={:?}",
            moe.gate_quant, moe.up_quant
        )));
    }
    if !matches!(
        ...
        return Err(crate::error::LlmError::Forward(format!(
            "MTP device draft sparse down unsupported quant {:?}",
            moe.down_quant
        )));
    }
    let gate_all = moe.gate_exps_bytes().ok_or_else(|| {
        crate::error::LlmError::Forward("MTP device draft gate_exps raw bytes missing".to_string())
    })?;
    let up_all = moe.up_exps_bytes().ok_or_else(|| {
        crate::error::LlmError::Forward("MTP device draft up_exps raw bytes missing".to_string())
    })?;
    let down_all = moe.down_exps_bytes().ok_or_else(|| {
        crate::error::LlmError::Forward("MTP device draft down_exps raw bytes missing".to_string())
    })?;
    let (shared_gate, shared_gate_quant) =
        k_quant_weight_bytes_for_mtp_device(&moe.shared_gate, device_layer_idx, "shared_gate")?;
    let (shared_up, shared_up_quant) =
        k_quant_weight_bytes_for_mtp_device(&moe.shared_up, device_layer_idx, "shared_up")?;
    let (shared_down, shared_down_quant) =
        k_quant_weight_bytes_for_mtp_device(&moe.shared_down, device_layer_idx, "shared_down")?;
    let router_w = moe.router_f32().ok_or_else(|| {
        crate::error::LlmError::Forward("MTP device draft router raw f32 missing".to_string())
    })?;
    let prior_tokens = kv_cache.current_len();
    let prior_sequence_epoch = kv_cache.sequence_epoch();
    let cache_layer = kv_cache.layers().first().ok_or_else(|| {
        crate::error::LlmError::Forward("MTP device draft KV cache layer 0 missing".to_string())
    })?;
    ...
    let expected_prior_values = prior_tokens
        ...
        .ok_or_else(|| {
            crate::error::LlmError::Forward(format!(
                "MTP device draft prior KV size overflow: tokens={prior_tokens} rows={}",
                attn.k_weight.rows
            ))
        })?;
    ...
    {
        return Err(crate::error::LlmError::Forward(format!(
            "MTP device draft prior KV len mismatch: k={} v={} expected={expected_prior_values}",
            prior_k_bits.len(),
            prior_v_bits.len()
        )));
    }
    Ok(super::cuda_runtime::MtpDeviceVerifyAttentionMoeLayer {
        ...
        prior_tokens,
        prior_sequence_epoch,
        attention_scale: (metadata.head_dim as f32).sqrt().recip(),
        o_q4k,
        o_quant,
        o_rows: attn.o_weight.rows,
        o_cols: attn.o_weight.cols,
        ...
        ffn_norm: super::cpu_runtime::kernels::tensor_as_f32_slice(post_attn_norm),
        post_ffw_norm: &[],
        out_scale: &[],
        ple_gate: &[],
        ...
        ffn_up_cols: 0,
        ffn_down: &[],
        ffn_down_quant: rnb_loader::GGMLType::Q4_K,
        ffn_down_rows: 0,
        ffn_down_cols: 0,
        router_w,
        ...
        shared_input_scale: super::cpu_runtime::kernels::tensor_as_f32_slice(
...
#[cfg(feature = "cuda")]
pub(in crate::engine) fn qwen35_prefill_attention_moe_device_layers_supported(
    weights: &super::layer_weights::ModelWeights,
    seq_len: usize,
) -> bool {
    ...
    for layer in &weights.layers {
        ...
        let Some(moe) = attn.shared_expert_moe.as_ref() else {
            return false;
        };
        ...
        if !attention_quant_supported
            ...
            || !matches!(moe.expert_gating_func, 0 | 1)
            || !moe.shared_expert_gated
            || moe.shared_down.ggml_type == rnb_loader::GGMLType::F32
            || !super::models::shared_expert_moe::qwen35moe_device_input_supported(moe, seq_len)
        {
            return false;
        }
    }
...
pub(super) fn build_mtp_device_verify_gdn_moe_layers<'a>(
    ...
    build_mtp_device_verify_gdn_moe_layers_from_states(
        weights,
        metadata,
        &mut kv_cache.ssm_states,
        false,
    )
}
...
#[cfg(feature = "cuda")]
fn build_mtp_device_verify_gdn_moe_layers_from_states<'a>(
    weights: &'a super::layer_weights::ModelWeights,
    metadata: &ModelMetadata,
    ssm_states: &'a mut [Option<crate::kv_cache::SsmLayerState>],
    ...
    if ssm_states.len() < weights.layers.len() {
        return Err(crate::error::LlmError::Forward(format!(
    ...
    let mut layers = Vec::new();
    for (layer_idx, (layer, ssm_slot)) in
        weights.layers.iter().zip(ssm_states.iter_mut()).enumerate()
    {
        let gdn = match layer {
        ...
        let (gate_q4k, gate_quant) =
            k_quant_weight_bytes_for_mtp_device(&gdn.gate_weight, layer_idx, "attn_gate")?;
    ...
    for (idx, layer) in weights.layers.iter().enumerate() {
        match layer {
            super::layer_weights::LayerType::Attention(attn) => {
                ...
                if !fullpath_attention_semantics_supported(attn) {
                    return Err(crate::error::LlmError::Forward(format!(
                        "build_fullpath_layer_raw_weights: layer {idx} uses attention semantics not represented by the Vulkan fullpath contract",
                    )));
                }
...
///
/// Mirrors the Attention branch in `build_fullpath_layer_raw_weights`: fixed
...
fn quant_weight_to_raw_tuple<'a>(
    ...
    let quant = super::gpu_runtime::ggml_to_fullpath_quant(w.ggml_type).ok_or_else(|| {
        crate::error::LlmError::Forward(format!(
...
/// Extract GDN alpha/beta projection bytes while preserving their source format.
///
/// `None` means an F32 Raw32 upload; `Some(quant)` means the normal Vulkan
/// fullpath SoA upload and quantized GEMV pipeline.
#[cfg(feature = "vulkan")]
pub(super) fn gdn_alpha_beta_to_raw_tuple<'a>(
    w: &'a super::quantized_weight_types::QuantizedWeight,
    layer_idx: usize,
    label: &'static str,
) -> crate::error::Result<(&'a [u8], usize, usize, Option<super::gpu_runtime::Quant>)> {
    if w.ggml_type != rnb_loader::GGMLType::F32 {
        let (bytes, rows, cols, quant) = quant_weight_to_raw_tuple(w, layer_idx, label)?;
        return Ok((bytes, rows, cols, Some(quant)));
    }
    ...
    if bytes.len() != expected_len {
        return Err(crate::error::LlmError::Forward(format!(
            "build_fullpath_layer_raw_weights: layer {layer_idx} {label} F32 byte len {} != rows*cols*4 {}",
            bytes.len(),
            expected_len,
        )));
    }
    Ok((bytes, w.rows, w.cols, None))
}

#[cfg(any(feature = "vulkan", test))]
fn resident_fullpath_decode_ready(
    resident_kv_active: bool,
    fullpath_supported: bool,
    gpu_runtime_active: bool,
) -> bool {
    resident_kv_active && fullpath_supported && gpu_runtime_active
}

#[cfg(test)]
mod resident_fullpath_decode_tests {
    use super::resident_fullpath_decode_ready;

    #[test]
    fn resident_fullpath_decode_requires_established_gpu_state() {
        assert!(resident_fullpath_decode_ready(true, true, true));
        assert!(!resident_fullpath_decode_ready(false, true, true));
        assert!(!resident_fullpath_decode_ready(true, false, true));
        assert!(!resident_fullpath_decode_ready(true, true, false));
    }
}
