...
use dense_chain::{
    ...
    try_prefill_f16kv_muse_attention_output_ffn_chain, try_prefill_muse_attention_output_ffn_chain,
    try_prefill_muse_target_attention_output_ffn_chain,
};
...
fn forward_attention_layer_impl(
        ...
        )
    {
        ...
        for (token_idx, row) in hidden_data.chunks_exact(hidden_dim).enumerate() {
            ...
            output.extend_from_slice(&scratch.hidden[..hidden_dim]);
        }

        return Ok(PrefillAttentionLayerOutput::host(Tensor::from_vec(
            output,
            &[seq_len, hidden_dim],
        )));
    }
    ...
    F: Fn(&str, std::time::Instant),
{
    ...
    if !non_causal {
        if let Some((cached_k_f16, cached_v_f16)) = cached_kv_f16 {
            if let (Some((sequence_epoch, cache_layer, _)), Some(attention_gate)) =
                (metal_f16_kv_resident, attn_gate.as_ref())
            {
                if let Some(chained_hidden) = try_prefill_muse_target_attention_output_ffn_chain(
                    metadata,
                    architecture,
                    &hidden,
                    w,
            ...
            if let Some(attention_gate) = attn_gate.as_ref() {
                if let Some(chained_hidden) = try_prefill_f16kv_muse_attention_output_ffn_chain(
                    metadata,
                    architecture,
                    &hidden,
                    w,
