...
#[cfg(target_arch = "aarch64")]
pub(super) fn q8_direct_candidate(weight: &QuantizedWeight) -> bool {
...
pub(in crate::engine) fn prefill_gate_up_vectors(
    ...
    #[cfg(target_arch = "aarch64")]
    {
        if fast_dotprod_enabled() {
            ...
            if let Some(fused) = fused_gate_up.filter(|fused| q8_direct_candidate(fused)) {
            ...
            if q8_direct_candidate(gate_weight) && q8_direct_candidate(up_weight) {
                let q8 = gemm_runtime::quantize_input_q8(input);
