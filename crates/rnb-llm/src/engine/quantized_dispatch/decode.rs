    ...
    F: FnMut(&'static str, Instant),
{
    ...
    #[cfg(target_arch = "aarch64")]
    {
        ...
        } else if has_dotprod && super::prefill::q8_direct_candidate(up_weight) {
    ...
    F: FnMut(&'static str, Instant),
{
    ...
    #[cfg(target_arch = "aarch64")]
    {
        ...
        if has_dotprod {
            if let Some(fused) =
                fused_gate_up.filter(|fused| super::prefill::q8_direct_candidate(fused))
            {
                gemm_runtime::quantize_input_q8_into(input, &mut scratch.arch_scratch.q8_scratch);
                let gate_rows = gate_weight.rows;
            ...
            if super::prefill::q8_direct_candidate(gate_weight)
                && super::prefill::q8_direct_candidate(up_weight)
            {
                gemm_runtime::quantize_input_q8_into(input, &mut scratch.arch_scratch.q8_scratch);
                let t_gate_up = Instant::now();
                gate_weight
                    .gemv_into_q8(&scratch.arch_scratch.q8_scratch, &mut scratch.ffn_gate)?;
                up_weight.gemv_into_q8(&scratch.arch_scratch.q8_scratch, &mut scratch.ffn_up)?;
