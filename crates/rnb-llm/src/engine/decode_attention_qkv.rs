    ...
    F: FnMut(&str, std::time::Instant),
{
    ...
    let gpu_qkv_ok = use_accelerator
        && backend_runtime::try_decode_attention_qkv_if_supported(
            #[cfg(feature = "vulkan")]
    ...
    let gpu_qkv_ok = if use_accelerator && !gpu_qkv_ok && !gemma4_reuse_q_only {
    ...
    let gpu_qkv_ok = if use_accelerator && !gpu_qkv_ok {
