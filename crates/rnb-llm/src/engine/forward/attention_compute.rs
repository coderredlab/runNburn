...
pub(super) fn compute_prefill_attention(
    ...
    let f16_neon_prefill = crate::engine::policy::env_string("RNB_ATTN_F16_NEON")
        .map(|v| v != "0")
        .unwrap_or(cfg!(target_os = "macos"));
    let f16_prefill_tokenwise = crate::engine::policy::env_string("RNB_ATTN_F16_PREFILL_TOKENWISE")
        .map(|v| v != "0")
        .unwrap_or(false);
    ...
    let mut attn_out = if let Some((k_f16, v_f16)) = kv_f16_for_simd {
        ...
        #[cfg(feature = "cuda")]
        return Err(crate::error::LlmError::Forward(
            "token-wise CPU prefill attention was requested for a CUDA build; CPU fallback is disabled"
                .to_string(),
        ));
        #[cfg(not(feature = "cuda"))]
        {
        ...
        #[cfg(feature = "cuda")]
        return Err(crate::error::LlmError::Forward(format!(
            "CUDA prefill attention is unavailable for layer {layer_idx}: seq_len={seq_len} kv_len={kv_len} heads={num_heads} kv_heads={num_kv_heads} head_dim={head_dim} sliding_window={has_sliding_window} softcap={has_softcap}; CPU fallback is disabled"
        )));
        #[cfg(not(feature = "cuda"))]
        {
    ...
    if let Some(gate) = attn_gate {
        let mut gated = kernels::tensor_as_f32_slice(&attn_out).to_vec();
        sigmoid_mul_f32_inplace(&mut gated, kernels::tensor_as_f32_slice(gate));
        attn_out = Tensor::from_vec(gated, attn_out.shape());
    }
