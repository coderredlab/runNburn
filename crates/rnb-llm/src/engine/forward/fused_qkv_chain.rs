...
pub(super) fn try_prefill_q4k_muse_hd128_dense_chain(
    ...
    let fused = backend_runtime::prefill_q4k_muse_hd128_dense_chain_if_supported(
        &w.q_weight,
        &w.k_weight,
        &w.v_weight,
        attention_gate,
...
fn gemma_reuse_q_dense_chain_requested() -> bool {
    let parse = |name: &str| {
        crate::engine::policy::env_string(name).map(|value| {
...
fn gemma4_prefill_ple_dense_chain_requested() -> bool {
    crate::engine::policy::env_string("RNB_CUDA_GEMMA_PLE_DENSE_CHAIN")
        .map(|value| {
            let value = value.to_ascii_lowercase();
            !matches!(value.as_str(), "0" | "false" | "off" | "no")
        })
...
fn gemma4_f32_ple_device_fusion_requested() -> bool {
    ...
    crate::engine::policy::env_string("RNB_CUDA_GEMMA_F32_PLE_DEVICE_FUSION")
        .map(|value| {
            let value = value.to_ascii_lowercase();
            !matches!(value.as_str(), "0" | "false" | "off" | "no")
        })
...
fn gemma4_device_hidden_carrier_requested() -> bool {
    ...
    let carrier = crate::engine::policy::env_string("RNB_CUDA_GEMMA_DEVICE_HIDDEN_CARRIER")
        .map(|value| {
            let value = value.to_ascii_lowercase();
            !matches!(value.as_str(), "0" | "false" | "off" | "no")
        })
        .unwrap_or(true);
    let qkv_carrier = crate::engine::policy::env_string("RNB_CUDA_GEMMA_QKV_DEVICE_HIDDEN_CARRIER")
        .map(|value| {
            let value = value.to_ascii_lowercase();
            !matches!(value.as_str(), "0" | "false" | "off" | "no")
        })
...
fn env_bool_or(name: &str, default: bool) -> bool {
    parse_env_bool_or(crate::engine::policy::env_string(name).as_deref(), default)
}
