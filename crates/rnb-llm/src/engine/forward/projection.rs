...
pub(in crate::engine::forward) fn atn_proj_metal(
    ...
    if std::env::var("RNB_METAL_PREFILL_ATN_PROJ").as_deref() == Ok("0") {
        return Ok(None);
    }
...
pub(super) fn try_prefill_atn_full_layer_metal(
    ...
    if std::env::var("RNB_METAL_PREFILL_ATN_FULL_LAYER_TAIL").as_deref() == Ok("0") {
        return Ok(None);
    }
...
pub(super) fn try_prefill_attn_chain_metal(
    ...
    // gated attention: attn_out *= sigmoid(gate) (CPU ground truth 와 동일, attention 후 적용).
    let t_gate0 = std::time::Instant::now();
    for (o, g) in attn_out.iter_mut().zip(gate_vec.iter()) {
        *o *= 1.0f32 / (1.0f32 + (-*g).exp());
    }
    if timing_enabled {
        eprintln!(
            "[prefill-atn-stage-time] layer={layer_idx} stage=gate_sigmoid_cpu m={seq_len} ms={:.3}",
...
pub(super) fn project_prefill_attention(
    ...
    let (mut q_full, mut k, mut v) = if let Some((q_vec, k_vec, v_vec)) = fused_qkv {
        ...
        let qk_f32_override = std::env::var("RNB_QK_F32_OVERRIDE")
            .map(|v| v != "0")
            .unwrap_or(false);
    ...
    if let Some(bias) = &w.q_bias {
        q_full = kernels::elementwise::add(&q_full, bias).map_err(fwd)?;
    }
    if let (Some(k_tensor), Some(bias)) = (&mut k, &w.k_bias) {
        *k_tensor = kernels::elementwise::add(k_tensor, bias).map_err(fwd)?;
    }
    if let (Some(v_tensor), Some(bias)) = (&mut v, &w.v_bias) {
        *v_tensor = kernels::elementwise::add(v_tensor, bias).map_err(fwd)?;
    }
