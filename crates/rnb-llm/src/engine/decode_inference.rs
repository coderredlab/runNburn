...
/// milestone 5(MTP): 배치 decode-chain verify 의 accept-n 커밋 payload. `forward_batched_decode_
/// verify_window` 이 반환하고 `commit_batched_verify` 가 소비한다(host kv_cache append + GDN
/// ssm_state write + set_len). 배치 forward 는 프로덕션 state 를 건드리지 않는 pure 계산이며,
/// 커밋은 이 payload 로만 이뤄진다(checkpoint/restore 불필요).
#[cfg(all(feature = "metal", not(feature = "cuda")))]
pub(crate) struct BatchedVerifyCommit {
    base_pos: usize,
    /// 이 pass 가 처리한 lane 수(=batch). 커밋 시 base_pos..base_pos+batch 로 전진한다.
...
impl Engine {
    ...
    pub(crate) fn batched_decode_chain_ready(&self) -> bool {
        // Default-on: Qwen35 greedy MTP wins ~-19% at 500-token decode
        // (same-state ABABAB, token-identical output). `RNB_METAL_BATCH_FUSED=0` opts out.
        if !crate::engine::policy::env_flag_default_on("RNB_METAL_BATCH_FUSED") {
            return false;
        }
    ...
    /// milestone 5(MTP): 배치(B-lane) decode-chain verify FORWARD. `verify_input` 의 B 토큰을
    /// 하나의 fused command buffer 로 시퀀스 처리해(lane i = pos base_pos+i, prefix [0..base_pos+i]
    /// attend; GDN lane i 는 i-1 에서 전진) dense weight 를 B lane 에 1회만 읽는다. 프로덕션 state 를
    /// 건드리지 않는 pure 계산 — attn prior 는 host kv_cache 에서, GDN base state 는 ssm_state 에서
    /// 읽고, 결과 window K/V + 최종 GDN state 를 `BatchedVerifyCommit` 로 반환한다(커밋은 caller 가
    /// accept-n 확정 후 `commit_batched_verify`). 미지원(비-full-chain / KVarn / 미지원 quant)이면
    /// `Ok(None)` → caller 가 기존 verify 경로로 폴백.
    #[cfg(all(feature = "metal", not(feature = "cuda")))]
    ...
    > {
        ...
        let mut run: Vec<(usize, &LayerType)> = Vec::with_capacity(num_layers);
        let mut inputs: Vec<backend_runtime::ChainLayerInput<'_>> = Vec::with_capacity(num_layers);
        let mut attn_shapes: Vec<Option<backend_runtime::ChainAttnShape>> =
            Vec::with_capacity(num_layers);
        let mut kv_dims: Vec<Option<usize>> = Vec::with_capacity(num_layers);
        for li in 0..num_layers {
            match &weights.layers[li] {
                ...
                {
                    let Some(st) = self.kv_cache.get_ssm_state(li) else {
                        return Ok(None);
                    };
                    inputs.push(backend_runtime::ChainLayerInput::Gdn {
                        conv_state: &st.conv_state,
                        delta_state: &st.delta_state,
                    });
                ...
                LayerType::Attention(w) => {
                    // 배치 attn fused 는 owns_kv + f16 KV 전용.
        ...
        if run.len() != num_layers {
            return Ok(None);
        }

        // B 토큰 임베딩 → hidden [B*hidden_dim].
        let mut hidden = vec![0f32; batch * hidden_dim];
        {
            let embd = &weights.token_embd;
            let embd_bytes = embd
                .data
                .as_bytes()
                .ok_or_else(|| crate::error::LlmError::Forward("token_embd bytes".to_string()))?;
            let row_bytes = embd_bytes.len() / embd.rows;
            for (lane, &token) in verify_input.iter().enumerate() {
                ...
                let dst = &mut hidden[lane * hidden_dim..(lane + 1) * hidden_dim];
                dst.copy_from_slice(&f32_row[..hidden_dim]);
                apply_embedding_scale_inplace(dst, metadata, architecture);
            }
        }
        ...
        for r in &reports {
            match r.output_argmax_token {
                Some(t) => target_tokens.push(t),
                None => return Ok(None),
            }
        }
        ...
        let commit = BatchedVerifyCommit {
            base_pos,
            batch,
            layer_indices,
            gdn_state_handle,
    ...
    /// carrier 에 보존한 값(재실행 0). 1<=committed<=commit.batch. committed==batch 면 final 과 동일.
    #[cfg(all(feature = "metal", not(feature = "cuda")))]
    pub(crate) fn commit_batched_verify(
        &mut self,
        ...
        let decode_result: crate::error::Result<Option<Vec<f32>>> = (|| {
            ...
            if (cuda_runtime::cu65_device_qkv_enabled() || full_device_decode_requested) && pos > 0
            {
                let representative_layer = weights
                    .layers
                    .iter()
                    .position(|layer| matches!(layer, LayerType::Attention(_)));
                if let Some(representative_layer) = representative_layer {
                    let representative = kv_cache.read_up_to(representative_layer, pos);
                    let (representative_k, _) = representative.as_slices();
                    let representative_kv_dim = representative_k.len() / pos;
                    let resident_matches = cuda_runtime::device_kv_cache_f16_matches(
                        representative_layer,
                        representative_kv_dim,
                        pos,
                    )
                    .map_err(crate::error::LlmError::Forward)?;
                    if !resident_matches {
                        for (layer_idx, layer) in weights.layers.iter().enumerate() {
                            if !matches!(layer, LayerType::Attention(_)) {
                                continue;
                            }
                            let cached = kv_cache.read_up_to(layer_idx, pos);
                            let (k_bits, v_bits) = cached.as_slices();
                            let layer_kv_dim = k_bits.len() / pos;
            ...
            if !full_device_decode_done && !persistent_decode_done {
                ...
                let eager_layer_cap: Option<usize> =
                    crate::engine::policy::env_string("RNB_CUDA_EAGER_DECODE_LAYERS")
                ...
                if let Some(path) =
                    crate::engine::policy::env_string("RNB_CUDA_EAGER_DECODE_HIDDEN_OUT")
                {
...
fn try_run_decode_chain(
    ...
    let chain_timing =
        crate::engine::policy::env_string("RNB_METAL_CHAIN_GPU_TIME").as_deref() == Some("1");
    let chain_start = chain_timing.then(std::time::Instant::now);
    let num_layers = metadata.num_layers;
    let hidden_dim = metadata.hidden_dim;
    ...
    // KVarn chain attention (opt-in): host KVarn attn layer 를 chain 에 편입한다.
    // collection 중엔 kvarn_view borrow 로 kv_cache mutable 불가라, 미리 compact 해
    // view 가 유효(tail<group)하도록 한다. compact 는 tail<group 이면 no-op.
    let kvarn_chain_env = crate::runtime::metal_kvarn_chain_attention_enabled();
    if kvarn_chain_env {
        for li in start_layer..num_layers {
            if matches!(&weights.layers[li], LayerType::Attention(_)) {
                let kv_source_layer = shared_kv_source_layer(metadata, architecture, li);
                if kv_source_layer.is_none() && kv_cache.layer_uses_kvarn(li) {
                    kv_cache
                        .compact_layer(li)
                        .map_err(crate::error::LlmError::Forward)?;
                }
    ...
    let mut inputs: Vec<backend_runtime::ChainLayerInput<'_>> = Vec::new();
    ...
    while li < num_layers {
        match &weights.layers[li] {
            ...
            LayerType::Attention(w) if attn_layer_env || qwen_moe_decode_chain_env => {
                ...
                let owns_kv = kv_source_layer.is_none();
                let layer_kvarn = kv_cache.layer_uses_kvarn(kv_cache_layer);
                if layer_kvarn && (!kvarn_chain_env || w.shared_expert_moe.is_none()) {
                    break;
                }
                let gemma4_reuse_q_only =
                ...
                let (prior_k, prior_v): (&[u16], &[u16]) = if layer_kvarn {
                    (&[], &[])
                } else if decode_chain_prior_kv_required(carrier_filled, pos) {
                    kv_cache.get_up_to(kv_cache_layer, pos)
                } else {
                    (&[], &[])
                };
                let kvarn = if layer_kvarn {
                    let attn_scale = resolve_attention_scale(metadata, architecture);
                    let sliding_window = active_sliding_window(metadata, architecture, li);
                    let softcap = resolve_attention_softcap(architecture);
                    let view = kv_cache.kvarn_view(kv_cache_layer, pos).ok_or_else(|| {
                        crate::error::LlmError::Forward("KVarN chain view unavailable".to_string())
                    })?;
                    Some(crate::runtime::KvarnChainView {
                        layer_idx: li,
                        packed_blocks: view.device_blocks,
                        sink_key: view.sink_key,
                        sink_value: view.sink_value,
                        tail_key: view.tail_key,
                        tail_value: view.tail_value,
                        kv_len: view.len,
                        tail_start: view.tail_start,
                        num_heads: layout.num_heads,
                        num_kv_heads: view.num_kv_heads,
                        head_dim: view.head_dim,
                        key_bits: view.config.key_bits,
                        value_bits: view.config.value_bits,
                        group: view.config.group,
                        sink_tokens: view.config.sink_tokens,
                        block_bytes: view.device_layout.block_bytes,
                        scale: attn_scale,
                        sliding_window,
                        softcap,
                    })
    ...
    if run.is_empty() {
        ...
        if crate::engine::policy::env_string("RNB_METAL_DECODE_CHAIN_TRACE").as_deref() == Some("1")
        {
            eprintln!(
    ...
    let backend_start = chain_timing.then(std::time::Instant::now);
    ...
    if let (Some(chain_start), Some(backend_start)) = (chain_start, backend_start) {
        let total_ms = chain_start.elapsed().as_secs_f64() * 1000.0;
        let backend_ms = backend_start.elapsed().as_secs_f64() * 1000.0;
        eprintln!(
            "[chain-prepare] prepare_ms={:.2} backend_ms={:.2} total_ms={:.2}",
            total_ms - backend_ms,
            backend_ms,
            total_ms
        );
    ...
    for ((layer_idx, lt), out) in run.iter().zip(out_states.iter()) {
        match (lt, out) {
            (LayerType::GatedDeltaNet(_), Some((conv_new, delta_new))) => {
            ...
            (LayerType::Attention(_), Some((k_new, v_new))) => {
                // KVarn chain attn: 새 토큰 k/v(post-rope)를 host KVarn cache 에 append
                // (host = source of truth, MTP checkpoint-safe) + compact.
                let kv_source_layer = shared_kv_source_layer(metadata, architecture, *layer_idx);
                if kv_source_layer.is_none() {
                    kv_cache.append(*layer_idx, pos, k_new, v_new);
                    kv_cache
