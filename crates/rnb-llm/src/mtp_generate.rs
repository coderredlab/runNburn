use crate::engine::verify_window::{MtpVerifyBonus, MtpVerifyWindowRequest, VerifyWindowResult};
...
fn trace_mtp_sequence_state(engine: &Engine, label: &str) {
    ...
    let hash = |data: &[f32]| {
        data.iter().fold(0xcbf29ce484222325u64, |hash, value| {
            (hash ^ value.to_bits() as u64).wrapping_mul(0x100000001b3)
        })
    };
    let states = (0..engine.metadata.num_layers)
        .filter_map(|layer_idx| {
            engine.kv_cache.get_ssm_state(layer_idx).map(|state| {
                format!(
                    "{layer_idx}:{:016x}:{:016x}",
                    hash(&state.conv_state),
                    hash(&state.delta_state),
                )
            })
        })
        .collect::<Vec<_>>()
        .join(",");
    let kv_len = engine.kv_cache.current_len();
    let kv_states = (0..engine.metadata.num_layers)
        .filter_map(|layer_idx| {
            let cached = engine.kv_cache.read_up_to(layer_idx, kv_len);
            let (k, v) = cached.as_slices();
            (!k.is_empty()).then(|| {
                let hash_u16 = |data: &[u16]| {
                    data.iter().fold(0xcbf29ce484222325u64, |hash, value| {
                        (hash ^ *value as u64).wrapping_mul(0x100000001b3)
                    })
                };
                format!("{layer_idx}:{:016x}:{:016x}", hash_u16(k), hash_u16(v))
            })
        })
        .collect::<Vec<_>>()
        .join(",");
    eprintln!("[MTP_STATE] {label} kv_len={kv_len} states=[{states}]");
    eprintln!("[MTP_KV] {label} kv_len={kv_len} kv=[{kv_states}]");
}
...
pub(crate) fn mtp_greedy_verify_allowed(params: &GenerateParams) -> bool {
...
fn mtp_verify_execution(
    device_verify: bool,
    batch_verify: bool,
    batch_decode_chain: bool,
) -> MtpVerifyExecution {
    if batch_decode_chain {
        MtpVerifyExecution::BatchDecodeChain
    } else if device_verify {
...
fn mtp_prefix_restore_base_kv_len(
    ...
    match verify_execution {
        ...
        MtpVerifyExecution::BatchDecodeChain => Err(crate::error::LlmError::Forward(
            "MTP batched decode-chain verify manages state without prefix restore".to_string(),
        )),
    }
}
...
fn next_token_from_current_logits(
    ...
    params.suppress_eos_logit(logits, eos)?;
...
fn replace_ignored_eos_targets(
    engine: &Engine,
    params: &GenerateParams,
    window: &mut VerifyWindowResult,
    eos: u32,
) -> crate::error::Result<()> {
    if !params.ignore_eos {
        return Ok(());
    }
    for index in 0..window.target_tokens.len() {
...
pub(crate) fn generate_stream_mtp_resuming(
    engine: &mut Engine,
...
fn generate_stream_mtp_with_tokens(
    ...
    if crate::runtime::mtp_draft_only_enabled() {
        ...
        while tokens_remaining > 0 {
            ...
            stats.accepted += 1;
            if !generated_text.push(&engine.tokenizer, current_token, &mut callback) {
                break;
            }
            tokens_remaining -= 1;
        ...
        if crate::runtime::profiling_enabled() || crate::runtime::spec_profile_enabled() {
            ...
            eprintln!(
                "  [MTP] wall_split request_prepare={:.1}ms prompt_prefill={:.1}ms decode_loop={:.1}ms",
                request_prepare_ms,
                prompt_prefill_ms,
    ...
    while tokens_remaining > 0 {
        ...
        if params.should_stop(current_token, eos) {
            break;
        }
        ...
        let k = match verify_execution {
            // B = k+1 must be <= 8 (fused batch limit).
            MtpVerifyExecution::BatchDecodeChain => {
                mtp_batch_effective_k(params.spec_k, tokens_remaining).min(7)
            }
            ...
            current_token = if n_accepted == draft_k {
                *window.target_tokens.get(draft_k).ok_or_else(|| {
                ...
                next_forced_token.ok_or_else(|| {
                    crate::error::LlmError::Forward(
                        "MTP batched verify did not produce next token".to_string(),
                    )
                })?
            };
            continue;
        ...
        if matches!(
            ...
            if stopped || tokens_remaining == 0 {
                if no_bonus_verify && n_accepted == draft_k {
                    let phase_start = Instant::now();
                    let (_, final_hidden_row) = engine
                ...
                let state_plan =
                    (verify_execution == MtpVerifyExecution::BatchPrefill).then(|| {
                    ...
                    .is_some_and(|plan| plan.replay_committed_prefix)
                {
                    let checkpoint = full_checkpoint.as_ref().ok_or_else(|| {
                        crate::error::LlmError::Forward(
                            "MTP batch finalization requires full engine checkpoint".to_string(),
                        )
                    })?;
                    let phase_start = Instant::now();
                    let _ = replay_committed_verify_prefix_argmax(
                        engine,
                        checkpoint,
                        &verify_input,
                        committed_tokens,
                    ...
                    if mtp_prefix_state_restore_required(
                        ...
                        let prefix_state =
                            window.prefix_state_for(committed_tokens).ok_or_else(|| {
                                crate::error::LlmError::Forward(
                                    "MTP batch finalization missing prefix state".to_string(),
                                )
                            })?;
                        if verify_execution == MtpVerifyExecution::BatchPrefill {
                            full_checkpoint
                                ...
                                .ok_or_else(|| {
                                    crate::error::LlmError::Forward(
                                        "MTP batch finalization requires full checkpoint"
                                            .to_string(),
                                    )
                                })?
                                .restore_engine(engine)?;
                        }
                        engine.restore_verify_window_prefix_state(
                            restore_base_kv_len,
                            prefix_state,
                        )?;
                    }
                ...
                .is_some_and(|plan| plan.replay_committed_prefix)
            {
                ...
                if mtp_prefix_state_restore_required(
                    ...
                    trace_mtp_sequence_state(engine, "batch-restored");
                }
        ...
        if observed_hidden_rows.len() % hidden_dim != 0 {
            return Err(crate::error::LlmError::Forward(
                "MTP sequential verify returned a partial hidden row".to_string(),
            ));
        }
        let committed_tokens = observed_hidden_rows.len() / hidden_dim;
        let committed_hidden =
            mtp_hidden_prefix_rows(&observed_hidden_rows, hidden_dim, committed_tokens)?;
        let phase_start = Instant::now();
        if fast_retain {
            engine.mtp_retain_draft_after_spec(
                mtp_checkpoint.as_ref(),
    ...
    while tokens_remaining > 0 {
        ...
        if !stopped && tokens_remaining > 0 {
            ...
            if params.stop_tokens.contains(&target_tok) || target_tok == eos {
                ...
                if !generated_text.push(&engine.tokenizer, target_tok, &mut callback) {
                    stopped = true;
                }
                tokens_remaining -= 1;
        ...
        if stopped || tokens_remaining == 0 {
            ...
            let committed_target_tokens = engine
                .kv_cache
                .current_len()
                .saturating_sub(position_before_verify as usize);
            if let Some(runtime) = engine.mtp_external_runtime_mut() {
                runtime.shift_for_accept(committed_target_tokens);
            }
            break;
        }
...
mod tests {
    ...
    fn mtp_device_verify_takes_precedence_over_prefill_batch() {
        assert_eq!(
            mtp_verify_execution(true, false, false),
            MtpVerifyExecution::DeviceResident
        );
        assert_eq!(
            mtp_verify_execution(true, true, false),
            MtpVerifyExecution::DeviceResident
        );
        assert_eq!(
            mtp_verify_execution(false, true, false),
            MtpVerifyExecution::BatchPrefill
        );
        assert_eq!(
            mtp_verify_execution(false, false, false),
            MtpVerifyExecution::Sequential
        );
    }
