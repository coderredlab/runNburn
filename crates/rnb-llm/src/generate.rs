...
pub(crate) fn generate_stream_resuming(
    ...
    let prompt_tokens = tokenize_prompt(engine, prompt);
    if state.matches_prompt_prefix(&prompt_tokens) && state.token_len() < prompt_tokens.len() {
        engine.restore_sequence_state(state)?;
        let mut result = generate_stream_impl_with_tokens(
            engine,
            prompt_tokens,
            params,
            Some(state.token_len()),
            callback,
        )?;
        result.cached_prompt_tokens = state.token_len();
        Ok(result)
    } else {
        generate_stream(engine, prompt, params, callback)
    }
...
fn generate_stream_impl_with_tokens(
    ...
    let mut logits = if constraint.is_some() {
        engine.forward_with_logits(forward_tokens)?
    } else {
        engine.forward(forward_tokens)?
    };
    ...
    for _ in 0..params.max_tokens {
        ...
        } else if let Some(token) = backend_argmax {
            token
        } else {
            sampler.sample(&mut logits, &generated_tokens, &mut rng)
        };

        if params.stop_tokens.contains(&token) || token == engine.tokenizer.vocab.special.eos {
            break;
        }
        ...
        let piece = engine.tokenizer.decode_token(token).to_string();
