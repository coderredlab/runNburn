...
pub fn generate_stream_speculative(
    ...
    while tokens_remaining > 0 {
        ...
        let piece = engine.tokenizer.decode_token(first_token).to_string();
...
pub fn generate_stream_two_model(
    ...
    let _ = draft_engine.forward(&prompt_tokens)?;
    if profiling {
        eprintln!(
    ...
    let t_prefill = Instant::now();
    let mut logits = target_engine.forward(&prompt_tokens)?;
    if profiling {
        eprintln!(
