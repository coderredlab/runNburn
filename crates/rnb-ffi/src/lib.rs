...
pub unsafe extern "C" fn rnb_next_token(ctx: *mut RnbContext) -> *const c_char {
    ...
    // 토큰 → UTF-8 텍스트
    let tok_str = ctx.engine.tokenizer.decode_token(current_token);
    let tok_bytes = tok_str.as_bytes();

    // UTF-8 버퍼에 추가
    ctx.utf8_buf.extend_from_slice(tok_bytes);
    ...
    // UTF-8 완전성 확인 후 반환
    let valid_len = valid_utf8_len(&ctx.utf8_buf);
    if valid_len == 0 {
        // 불완전 — 빈 문자열 반환 (NULL 아님)
        ctx.token_buf = CString::new("").unwrap();
        return ctx.token_buf.as_ptr();
    }

    let valid_bytes: Vec<u8> = ctx.utf8_buf.drain(..valid_len).collect();
    match CString::new(valid_bytes) {
        Ok(cs) => {
            ctx.token_buf = cs;
