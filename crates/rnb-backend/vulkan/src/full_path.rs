...
use crate::layer_gemv::{
    ...
    RuntimeCounters, VulkanLayerGemv, MAX_BATCH_OUTPUTS,
};
...
/// `crates/rnb-llm/src/engine/layer_weights.rs:47-67`. f32 raw 텐서들
/// (`attn_norm`, `post_attn_norm`, `ssm_alpha`, `ssm_beta`, `ssm_a`,
/// `ssm_conv1d`, `ssm_dt_bias`, `ssm_norm`) 은 mv28 I2에서 도입된
/// `GpuWeightMode::Raw32` 업로드 경로를 타고, 양자화 텐서들은 기존
/// `WeightCache` 경로를 그대로 재사용한다.
///
/// All `&GpuBuffer` references must remain valid for the duration of
...
///
/// 필드 순서: norm → fused QKV → z gate → SSM α/β Raw32 → SSM raw 묶음 (a /
/// conv1d / dt_bias / norm) → SSM out → post-attn norm → FFN gate/up/down.
/// f32 raw 필드를 인접하게 묶어 두면 향후 Raw32 업로드 경로 audit 이 한
/// 블록에서 끝난다.
///
/// Out of scope (5a):
...
pub struct GdnLayerWeightHandles<'a> {
    ...
    pub gate_cols: usize,
    pub gate_quant: QuantType,
    // SSM α / β (Raw32) — [num_heads, hidden]
...
pub struct FullPathPrefillInput<'a> {
    ...
    /// Q6_K-packed `token_embd.weight`. Required for the embedding lookup.
...
fn emit_fullpath_buffer_stage_trace(
    ...
    let data =
        gemv.debug_download_buffer_f32(buf, (f32_len * std::mem::size_of::<f32>()) as u64)?;
...
pub(crate) fn validate_decode_step_input(
    ...
    if input.hidden == 0 || input.hidden % 256 != 0 {
        return Err(format!(
            "full_path::run_decode_step: hidden {} must be a positive multiple of 256",
            input.hidden
        ));
    }
    if input.vocab == 0 {
...
pub fn run_decode_step(
    ...
    let next_token_id = if let Some(layer_weights) = input.layer_weights {
        ...
        counters.submits += 1;
        let embed_row_bytes = (input.hidden / 256) * 210;
...
impl SetIdxCounters {
    ...
    fn would_overflow(&self, consumed: &AttentionLayerSetsConsumed) -> bool {
        self.norm + consumed.norm > MAX_BATCH_OUTPUTS
            || self.q + consumed.q > MAX_BATCH_OUTPUTS
            || self.k + consumed.k > MAX_BATCH_OUTPUTS
            || self.v + consumed.v > MAX_BATCH_OUTPUTS
            || self.attn + consumed.attn > MAX_BATCH_OUTPUTS
            || self.o + consumed.o > MAX_BATCH_OUTPUTS
            || self.silu + consumed.silu > MAX_BATCH_OUTPUTS
            || self.add + consumed.add > MAX_BATCH_OUTPUTS
            || self.gate + consumed.gate > MAX_BATCH_OUTPUTS
            || self.up + consumed.up > MAX_BATCH_OUTPUTS
            || self.down + consumed.down > MAX_BATCH_OUTPUTS
            || self.rope + consumed.rope_q + consumed.rope_k > MAX_BATCH_OUTPUTS
    }
...
pub fn run_prefill(
    ...
    if input.hidden == 0 || input.hidden % 256 != 0 {
        return Err(format!(
            "full_path::run_prefill: hidden {} must be a positive multiple of 256",
            input.hidden
        ));
    }
    if input.vocab == 0 {
    ...
    let last_token_id = if let Some(layer_weights) = input.layer_weights {
        ...
        let loop_params = LayerLoopParams {
            seq_len,
            pos_start: 0, // prefill always starts at position 0
            num_layers: input.num_layers,
            num_heads: input.num_heads,
            num_kv_heads: input.num_kv_heads,
            head_dim: input.head_dim,
...
struct LayerLoopParams<'a> {
    /// Number of tokens in this step (prompt_len for prefill, 1 for decode).
    seq_len: usize,
    /// Absolute position of the first token (0 for prefill, kv_cursor for decode).
    pos_start: u32,
    num_layers: usize,
    num_heads: usize,
    num_kv_heads: usize,
...
fn run_layer_loop(
    ...
    let trace_tag = if params.pos_start == 0 {
        "prefill"
    } else {
        "decode"
    };
    ...
    for layer_idx in 0..params.num_layers {
        ...
        match kind {
            ...
            ModelLayerKind::Recurrent => {
                ...
                let n = seq_len;
                let estimated = AttentionLayerSetsConsumed {
                    norm: 2 * n,
                    q: 6 * n,
                    k: n,
                    v: 2 * n,
                ...
                if cmd_in_progress && set_idx.would_overflow(&estimated) {
                    submit_layer_loop_chunk(gemv, counters, attention_chunks, profile)?;
            ...
            ModelLayerKind::Attention => {
                ...
                // Conservative upper-bound used *only* for the overflow check
                // before recording begins.  Matches what
                // `record_attention_layer_fullpath` actually consumes so that
                // the chunking trigger fires at the right time.
                // NOTE: norm is 2*n — both attn_norm and ffn_norm windows are
                //       consumed inside record_attention_layer_fullpath.
                let n = seq_len;
                let kvh_n = params.num_kv_heads;
                let total_attn = n * params.num_heads; // one attn dispatch per (token, head)
                let q_norm_dispatches = if wh.q_norm_buf.is_some() {
                    n * params.num_heads
                } else {
                    0
                };
                let k_norm_dispatches = if wh.k_norm_buf.is_some() {
                    n * kvh_n
                } else {
                    0
                };
                let gemv_dispatches = n + (n * kvh_n) + (n * kvh_n) + n + n + n + n;
                let estimated = AttentionLayerSetsConsumed {
                ...
                let fullpath_input = AttentionLayerFullpathInput {
                    ...
                    rope_dim: params.rope_dim as u32,
...
mod tests {
    ...
    fn set_idx_counters_would_overflow_detects_norm_overflow() {
        ...
        assert!(s.would_overflow(&consumed), "should detect norm overflow");
    }
    ...
    fn set_idx_counters_no_overflow_within_budget() {
        ...
        for _ in 0..(cap_layers - 1) {
            assert!(!s.would_overflow(&consumed));
            s.advance(&consumed);
        }
