...
/// Fused row-major Q5_K output projection and argmax reduction.
///
/// Bindings and push constants match the Q6_K/Q8_0 argmax kernels.
pub fn emit_logit_argmax_q5k(local_size_x: u32) -> Vec<u32> {
    ...
    // Q5_K dequant for (v=cur_v, elem_in_row=cur_h).
    ...
    let qh_group_bit = m.imul(t_u32, group, c_u32_2);
    let qh_bit = m.iadd(t_u32, qh_group_bit, high_nibble);
