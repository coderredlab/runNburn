...
pub(crate) mod op {
    ...
    pub const CONSTANT_COMPOSITE: u32 = 44;
    ...
    pub const GROUP_NON_UNIFORM_FADD: u32 = 350;
}
...
impl SpirvModule {
    ...
    pub fn constant_composite(&mut self, ty: Id, constituents: &[Id]) -> Id {
        let id = self.alloc_id();
        let mut words = Vec::with_capacity(constituents.len() + 2);
        words.push(ty.0);
        words.push(id.0);
        words.extend(constituents.iter().map(|constituent| constituent.0));
        self.type_declarations
            .push(Self::encode_inst(op::CONSTANT_COMPOSITE, &words));
        id
    }
    ...
    pub fn variable_with_initializer(
        &mut self,
        ptr_type: Id,
        storage_class: u32,
        initializer: Id,
        ...
        let inst = Self::encode_inst(
            op::VARIABLE,
            &[ptr_type.0, id.0, storage_class, initializer.0],
        );
        self.global_variables.push(inst);
        id
    }
    ...
    pub fn composite_construct(&mut self, result_type: Id, constituents: &[Id]) -> Id {
        ...
        words.extend(constituents.iter().map(|constituent| constituent.0));
        let inst = Self::encode_inst(op::COMPOSITE_CONSTRUCT, &words);
        self.functions.push(inst);
        id
    }
    ...
    pub fn group_non_uniform_fadd_clustered(
        &mut self,
        ty: Id,
        execution_scope: Id,
        value: Id,
        ...
        let id = self.alloc_id();
        let inst = Self::encode_inst(
            op::GROUP_NON_UNIFORM_FADD,
            &[
                ty.0,
                id.0,
                execution_scope.0,
                3, // GroupOperation ClusteredReduce
                value.0,
                cluster_size.0,
            ],
        );
    ...
    pub fn group_non_uniform_fadd_reduce(&mut self, ty: Id, execution_scope: Id, value: Id) -> Id {
        let id = self.alloc_id();
        let inst = Self::encode_inst(
            op::GROUP_NON_UNIFORM_FADD,
            &[
    ...
    pub fn f_convert(&mut self, result_type: Id, operand: Id) -> Id {
        let id = self.alloc_id();
        let inst = Self::encode_inst(op::F_CONVERT, &[result_type.0, id.0, operand.0]);
        self.functions.push(inst);
        id
    }
    ...
    pub fn logical_and(&mut self, bool_ty: Id, a: Id, b: Id) -> Id {
        let id = self.alloc_id();
        let inst = Self::encode_inst(op::LOGICAL_AND, &[bool_ty.0, id.0, a.0, b.0]);
        self.functions.push(inst);
        id
    }
...
pub fn emit_q4k_gemv(local_size_x: u32) -> Vec<u32> {
    emit_q4k_gemv_lanes(local_size_x, 1, false, false, false, false, false)
}

/// Emit a Q4_K matrix-vector batch shader that reuses every decoded weight
/// across four prompt tokens. Input and output windows are row-major by token.
pub fn emit_q4k_gemv_batch4(local_size_x: u32) -> Vec<u32> {
    emit_q4k_gemv_lanes(local_size_x, 4, true, false, true, true, false)
}

fn emit_q4k_gemv_lanes(
    local_size_x: u32,
    lanes: usize,
    batched: bool,
    use_f16: bool,
    ...
    debug_assert!(!use_f16 || lanes >= 4 && lanes % 4 == 0);
    debug_assert!(!f32_vec4 || !use_f16 && shared_input && lanes % 4 == 0);
    debug_assert!(!shared_input || batched && local_size_x == 64);
    debug_assert!(!split_k || batched && !use_f16 && !shared_input && local_size_x == 64);
    let mut m = SpirvModule::new();

    // Capabilities & memory model
    m.capability(1); // Shader
    if use_f16 {
        m.capability(9); // Float16
    }
    m.extension("SPV_KHR_storage_buffer_storage_class");
    m.memory_model(0, 1); // Logical, GLSL450
    ...
    let t_f32 = m.type_float(32);
    let t_v4f32 = (use_f16 || f32_vec4).then(|| m.type_vector(t_f32, 4));
    let f16_types = if use_f16 {
        let t_f16 = m.type_float(16);
        let t_v4f16 = m.type_vector(t_f16, 4);
        Some((t_f16, t_v4f16, t_v4f32.unwrap()))
    } else {
        None
    };
    ...
    let t_arr_u32 = m.type_runtime_array(t_u32);
    let t_arr_f32 = m.type_runtime_array(t_f32);
    let shared_input_element_type = if use_f16 { f16_types.unwrap().0 } else { t_f32 };
    let t_shared_input = if shared_input {
        let c_shared_len = m.constant_u32(t_u32, lanes as u32 * 256);
        Some(m.type_array(shared_input_element_type, c_shared_len))
    } else {
        None
    };
    let t_shared_partials = if split_k {
        let c_partial_len = m.constant_u32(t_u32, lanes as u32 * local_size_x);
        Some(m.type_array(t_f32, c_partial_len))
    } else {
        None
    };
    ...
    let pc_fields = if batched {
        vec![t_u32, t_u32, t_u32, t_u32, t_u32]
    } else {
        vec![t_u32, t_u32, t_u32]
    };
    let t_struct_pc = m.type_struct(&pc_fields);
    ...
    let t_ptr_fn_f32 = m.type_pointer(storage_class::FUNCTION, t_f32);
    let t_ptr_fn_v4f16 =
        f16_types.map(|(_, t_v4f16, _)| m.type_pointer(storage_class::FUNCTION, t_v4f16));
    let shared_ptr_types = t_shared_input.map(|t_shared| {
        (
            m.type_pointer(storage_class::WORKGROUP, t_shared),
            m.type_pointer(storage_class::WORKGROUP, shared_input_element_type),
        )
    });
    let shared_partial_ptr_types = t_shared_partials.map(|t_shared| {
        (
            m.type_pointer(storage_class::WORKGROUP, t_shared),
            m.type_pointer(storage_class::WORKGROUP, t_f32),
        )
    });
    ...
    let c_f32_0 = m.constant_f32(t_f32, 0.0);
    let c_f16_0 = f16_types.map(|(t_f16, _, _)| m.constant_u32(t_f16, 0));
    ...
    let gvar_glob_id = m.variable(t_ptr_input_u32, storage_class::INPUT);
    let gvar_local_id =
        (shared_input || split_k).then(|| m.variable(t_ptr_input_u32, storage_class::INPUT));
    let gvar_shared_input = shared_ptr_types
        .map(|(t_ptr_wg_shared, _)| m.variable(t_ptr_wg_shared, storage_class::WORKGROUP));
    let gvar_shared_partials = shared_partial_ptr_types
        .map(|(t_ptr_wg_shared, _)| m.variable(t_ptr_wg_shared, storage_class::WORKGROUP));
    ...
    if let Some(gvar_local_id) = gvar_local_id {
        m.decorate(
            gvar_local_id,
            decoration::BUILTIN,
    ...
    // --- Entry point ---
    let func_id = m.alloc_id();
    let mut entry_interface = vec![gvar_glob_id];
    if let Some(gvar_local_id) = gvar_local_id {
        entry_interface.push(gvar_local_id);
    }
    m.entry_point(5, func_id, "main", &entry_interface);
    m.execution_mode_local_size(func_id, local_size_x, 1, 1);
    ...
    // Function variables
    let var_sums: Vec<_> = (0..lanes)
        .map(|_| m.function_variable(t_ptr_fn_f32, storage_class::FUNCTION))
        .collect();
    let var_blk = m.function_variable(t_ptr_fn_u32, storage_class::FUNCTION);
    let var_nibble_sums_f16: Vec<_> = (0..if use_f16 { lanes / 4 } else { 0 })
        .map(|_| m.function_variable(t_ptr_fn_v4f16.unwrap(), storage_class::FUNCTION))
        .collect();
    let var_input_sums_f16: Vec<_> = (0..if use_f16 { lanes / 4 } else { 0 })
        .map(|_| m.function_variable(t_ptr_fn_v4f16.unwrap(), storage_class::FUNCTION))
        .collect();

    // Split-K tiles map one workgroup to 8 output rows. Eight local lanes
    // cooperate on disjoint quant blocks for each row.
    let glob_id_vec = m.load(t_v3u32, gvar_glob_id);
    let glob_x = m.composite_extract(t_u32, glob_id_vec, 0);
    let token_group = if batched {
        m.composite_extract(t_u32, glob_id_vec, 1)
    } else {
        c_u32_0
    };
    let local_id_x = gvar_local_id.map(|gvar_local_id| {
        let local_id_vec = m.load(t_v3u32, gvar_local_id);
        m.composite_extract(t_u32, local_id_vec, 0)
    });
    let (row, k_lane) = if split_k {
        let c_local_size = m.constant_u32(t_u32, local_size_x);
        let c_k_split = m.constant_u32(t_u32, 2);
        let local_id = local_id_x.unwrap();
        let row_group = m.udiv(t_u32, glob_x, c_local_size);
        let row_in_tile = m.udiv(t_u32, local_id, c_k_split);
        let c_row_tile = m.constant_u32(t_u32, local_size_x / 2);
        let row_base = m.imul(t_u32, row_group, c_row_tile);
        (
            m.iadd(t_u32, row_base, row_in_tile),
            m.umod(t_u32, local_id, c_k_split),
        )
    } else {
        (glob_x, c_u32_0)
    };

    // Single-vector push: rows, cols, rows_per_wg.
    // Batch push: rows, cols, seq_len, input_stride_f32, output_stride_f32.
    let pc_ptr_rows = m.access_chain(t_ptr_pc_u32, gvar_pc, &[c_u32_0]);
    let pc_rows = m.load(t_u32, pc_ptr_rows);
    let pc_ptr_cols = m.access_chain(t_ptr_pc_u32, gvar_pc, &[c_u32_1]);
    let pc_cols = m.load(t_u32, pc_ptr_cols);
    let (pc_seq_len, pc_input_stride, pc_output_stride) = if batched {
        let ptr_seq = m.access_chain(t_ptr_pc_u32, gvar_pc, &[c_u32_2]);
        let ptr_in = m.access_chain(t_ptr_pc_u32, gvar_pc, &[c_u32_3]);
        let ptr_out = m.access_chain(t_ptr_pc_u32, gvar_pc, &[c_u32_4]);
        (
            m.load(t_u32, ptr_seq),
            m.load(t_u32, ptr_in),
            m.load(t_u32, ptr_out),
        )
    } else {
        (c_u32_1, pc_cols, pc_rows)
    };
    let c_lanes = m.constant_u32(t_u32, lanes as u32);
    let token_base = m.imul(t_u32, token_group, c_lanes);
    let last_token = m.isub(t_u32, pc_seq_len, c_u32_1);
    let safe_tokens: Vec<_> = (0..lanes)
        .map(|lane| {
            let lane_id = m.constant_u32(t_u32, lane as u32);
            let token = m.iadd(t_u32, token_base, lane_id);
            let valid = m.u_less_than(t_bool, token, pc_seq_len);
    ...
    if shared_input {
        let local_id_x = local_id_x.unwrap();
        let (_, t_ptr_wg_input) = shared_ptr_types.unwrap();
        let gvar_shared_input = gvar_shared_input.unwrap();
        for lane in 0..lanes {
            let lane_base = m.constant_u32(t_u32, lane as u32 * 256);
            let token_offset = m.imul(t_u32, safe_tokens[lane], pc_input_stride);
            for chunk in 0..4u32 {
                let element = if chunk == 0 {
                    local_id_x
                } else {
                    let chunk_offset = m.constant_u32(t_u32, chunk * local_size_x);
                    m.iadd(t_u32, local_id_x, chunk_offset)
                };
                let input_element = m.iadd(t_u32, blk_x_256, element);
                let input_idx = m.iadd(t_u32, token_offset, input_element);
                let input_ptr = m.access_chain(t_ptr_sb_f32, gvar_input, &[c_u32_0, input_idx]);
                let input_value = m.load(t_f32, input_ptr);
                let shared_value = if let Some((t_f16, _, _)) = f16_types {
                    m.f_convert(t_f16, input_value)
                } else {
                    input_value
                };
                let shared_idx = m.iadd(t_u32, lane_base, element);
                let shared_ptr = m.access_chain(t_ptr_wg_input, gvar_shared_input, &[shared_idx]);
                m.store(shared_ptr, shared_value);
            }
        }
    ...
    let mut total_sums = vec![c_f32_0; lanes];
    ...
    for sb_idx in 0..8u32 {
        ...
        let inp_sb_local = c_sb_offset;

        let mut nibble_input_sums = vec![c_f32_0; lanes];
        let mut input_sums = vec![c_f32_0; lanes];
        if f32_vec4 {
            let t_v4f32 = t_v4f32.unwrap();
            let zero_v4f32 = m.constant_composite(t_v4f32, &[c_f32_0, c_f32_0, c_f32_0, c_f32_0]);
            let mut nibble_sums_v4 = vec![zero_v4f32; lanes / 4];
            let mut input_sums_v4 = vec![zero_v4f32; lanes / 4];
            ...
            for w in 0..8u32 {
                ...
                for byte_idx in 0..4u32 {
                    let shift = byte_idx * 8 + if is_high { 4 } else { 0 };
                    ...
                    let nibble_f32 = m.convert_u_to_f(t_f32, nibble);
                    let nibble_v4f32 = m.composite_construct(
                        t_v4f32,
                        &[nibble_f32, nibble_f32, nibble_f32, nibble_f32],
                    );
                    let c_inner = m.constant_u32(t_u32, w * 4 + byte_idx);
                    let inp_local_element = m.iadd(t_u32, inp_sb_local, c_inner);
                    for group in 0..(lanes / 4) {
                        let mut inp_f32 = [c_f32_0; 4];
                        for (group_lane, inp) in inp_f32.iter_mut().enumerate() {
                            let lane = group * 4 + group_lane;
                            let lane_base = m.constant_u32(t_u32, lane as u32 * 256);
                            let shared_idx = m.iadd(t_u32, lane_base, inp_local_element);
                            let shared_ptr = m.access_chain(
                                shared_ptr_types.unwrap().1,
                                gvar_shared_input.unwrap(),
                                &[shared_idx],
                            );
                            *inp = m.load(t_f32, shared_ptr);
                        }
                        let inp_v4f32 = m.composite_construct(t_v4f32, &inp_f32);
                        let ni_prod = m.fmul(t_v4f32, nibble_v4f32, inp_v4f32);
                        nibble_sums_v4[group] = m.fadd(t_v4f32, nibble_sums_v4[group], ni_prod);
                        input_sums_v4[group] = m.fadd(t_v4f32, input_sums_v4[group], inp_v4f32);
                    }
                }
            }
            ...
            for group in 0..(lanes / 4) {
                for group_lane in 0..4 {
                    let lane = group * 4 + group_lane;
                    nibble_input_sums[lane] =
                        m.composite_extract(t_f32, nibble_sums_v4[group], group_lane as u32);
                    input_sums[lane] =
                        m.composite_extract(t_f32, input_sums_v4[group], group_lane as u32);
                }
            }
        } else if let Some((t_f16, t_v4f16, t_v4f32)) = f16_types {
            let zero_f16 = c_f16_0.unwrap();
            let zero_v4f16 =
                m.constant_composite(t_v4f16, &[zero_f16, zero_f16, zero_f16, zero_f16]);
            for group in 0..(lanes / 4) {
                m.store(var_nibble_sums_f16[group], zero_v4f16);
                m.store(var_input_sums_f16[group], zero_v4f16);
            }
            ...
            for w in 0..8u32 {
                ...
                for byte_idx in 0..4u32 {
                    ...
                    let nibble_f16 = m.convert_u_to_f(t_f16, nibble);
                    let nibble_v4f16 = m.composite_construct(
                        t_v4f16,
                        &[nibble_f16, nibble_f16, nibble_f16, nibble_f16],
                    );
                    let c_inner = m.constant_u32(t_u32, w * 4 + byte_idx);
                    let inp_local_element = m.iadd(t_u32, inp_sb_local, c_inner);
                    for group in 0..(lanes / 4) {
                        let mut inp_f16 = [zero_f16; 4];
                        for (group_lane, inp) in inp_f16.iter_mut().enumerate() {
                            let lane = group * 4 + group_lane;
                            let lane_base = m.constant_u32(t_u32, lane as u32 * 256);
                            let shared_idx = m.iadd(t_u32, lane_base, inp_local_element);
                            let shared_ptr = m.access_chain(
                            ...
                            *inp = m.load(t_f16, shared_ptr);
                        }
                        let inp_v4f16 = m.composite_construct(t_v4f16, &inp_f16);
                        let ni_prod = m.fmul(t_v4f16, nibble_v4f16, inp_v4f16);
                        let old_ni = m.load(t_v4f16, var_nibble_sums_f16[group]);
                        let new_ni = m.fadd(t_v4f16, old_ni, ni_prod);
                        m.store(var_nibble_sums_f16[group], new_ni);
                        let old_input = m.load(t_v4f16, var_input_sums_f16[group]);
                        let new_input = m.fadd(t_v4f16, old_input, inp_v4f16);
                        m.store(var_input_sums_f16[group], new_input);
                    }
                }
            }

            for group in 0..(lanes / 4) {
                let nibble_sum_f16 = m.load(t_v4f16, var_nibble_sums_f16[group]);
                let input_sum_f16 = m.load(t_v4f16, var_input_sums_f16[group]);
                let nibble_sum_f32 = m.f_convert(t_v4f32, nibble_sum_f16);
                let input_sum_f32 = m.f_convert(t_v4f32, input_sum_f16);
                for group_lane in 0..4 {
                    let lane = group * 4 + group_lane;
                    nibble_input_sums[lane] =
                        m.composite_extract(t_f32, nibble_sum_f32, group_lane as u32);
                    input_sums[lane] = m.composite_extract(t_f32, input_sum_f32, group_lane as u32);
                }
            }
        } else {
            for w in 0..8u32 {
                ...
                let inp_w_local_base = if w == 0 {
                    inp_sb_local
                } else {
                    let c_w4 = m.constant_u32(t_u32, w * 4);
                    m.iadd(t_u32, inp_sb_local, c_w4)
                };
                for byte_idx in 0..4u32 {
                    let shift_amt = byte_idx * 8 + if is_high { 4 } else { 0 };
                    let c_shift = m.constant_u32(t_u32, shift_amt);
                    let shifted = m.shift_right_logical(t_u32, qs_word, c_shift);
                    let nibble = m.bitwise_and(t_u32, shifted, c_u32_0f);
                    ...
                    let inp_local_element = if byte_idx == 0 {
                        inp_w_local_base
                    } else {
                        let c_bi = m.constant_u32(t_u32, byte_idx);
                        m.iadd(t_u32, inp_w_local_base, c_bi)
                    };
                    for lane in 0..lanes {
                        let inp_val = if shared_input {
                            let lane_base = m.constant_u32(t_u32, lane as u32 * 256);
                            let shared_idx = m.iadd(t_u32, lane_base, inp_local_element);
                            let shared_ptr = m.access_chain(
                                shared_ptr_types.unwrap().1,
                            ...
                            m.load(t_f32, shared_ptr)
                        } else {
                            let token_offset = m.imul(t_u32, safe_tokens[lane], pc_input_stride);
                            let inp_idx = m.iadd(t_u32, token_offset, inp_element);
                            let inp_ptr =
                                m.access_chain(t_ptr_sb_f32, gvar_input, &[c_u32_0, inp_idx]);
                            m.load(t_f32, inp_ptr)
                        };
                        let ni_prod = m.fmul(t_f32, nibble_f, inp_val);
                        nibble_input_sums[lane] = m.fadd(t_f32, nibble_input_sums[lane], ni_prod);
                        input_sums[lane] = m.fadd(t_f32, input_sums[lane], inp_val);
                    }
                }
            }
        }
        ...
        for lane in 0..lanes {
            let term1 = m.fmul(t_f32, d_sc, nibble_input_sums[lane]);
            let term2 = m.fmul(t_f32, dmin_mn, input_sums[lane]);
            let sb_result = m.fsub(t_f32, term1, term2);
            total_sums[lane] = m.fadd(t_f32, total_sums[lane], sb_result);
        }
    }

    // Accumulate this quant block for every token lane.
    for lane in 0..lanes {
        let prev_total = m.load(t_f32, var_sums[lane]);
        let new_total = m.fadd(t_f32, prev_total, total_sums[lane]);
        m.store(var_sums[lane], new_total);
    }
    if shared_input {
        let c_scope_wg = m.constant_u32(t_u32, scope::WORKGROUP);
        let c_sem = m.constant_u32(
    ...
    m.functions
        .push(SpirvModule::encode_inst(op::LABEL, &[lbl_outer_continue.0]));
    let block_step = if split_k {
        m.constant_u32(t_u32, 2)
    } else {
        c_u32_1
    };
    let blk_next = m.iadd(t_u32, blk_cur, block_step);
    m.store(var_blk, blk_next);
    m.branch(lbl_outer_header);
    ...
    if split_k {
        let local_id = local_id_x.unwrap();
        let partial_ptr_ty = shared_partial_ptr_types.unwrap().1;
        let partials = gvar_shared_partials.unwrap();
        for lane in 0..lanes {
            let final_sum = m.load(t_f32, var_sums[lane]);
            let lane_base = m.constant_u32(t_u32, lane as u32 * local_size_x);
            let partial_idx = m.iadd(t_u32, lane_base, local_id);
            let partial_ptr = m.access_chain(partial_ptr_ty, partials, &[partial_idx]);
            m.store(partial_ptr, final_sum);
        }
        let c_scope_wg = m.constant_u32(t_u32, scope::WORKGROUP);
        let c_sem = m.constant_u32(
            t_u32,
        ...
        let lbl_reduce_body = m.alloc_id();
        let lbl_reduce_merge = m.alloc_id();
        let is_reducer = m.u_less_than(t_bool, k_lane, c_u32_1);
        m.selection_merge(lbl_reduce_merge, 0);
        m.branch_conditional(is_reducer, lbl_reduce_body, lbl_reduce_merge);
        m.functions
            .push(SpirvModule::encode_inst(op::LABEL, &[lbl_reduce_body.0]));
        let row_lane_base = m.isub(t_u32, local_id, k_lane);
        for lane in 0..lanes {
            let mut final_sum = c_f32_0;
            for k in 0..2u32 {
                let partial_offset = m.constant_u32(t_u32, lane as u32 * local_size_x + k);
                let partial_idx = m.iadd(t_u32, row_lane_base, partial_offset);
                let partial_ptr = m.access_chain(partial_ptr_ty, partials, &[partial_idx]);
                let partial = m.load(t_f32, partial_ptr);
                final_sum = m.fadd(t_f32, final_sum, partial);
            }
            let token_offset = m.imul(t_u32, safe_tokens[lane], pc_output_stride);
            let out_idx = m.iadd(t_u32, token_offset, row);
            let out_ptr = m.access_chain(t_ptr_sb_f32, gvar_output, &[c_u32_0, out_idx]);
            m.store(out_ptr, final_sum);
        }
        m.branch(lbl_reduce_merge);
        m.functions
            .push(SpirvModule::encode_inst(op::LABEL, &[lbl_reduce_merge.0]));
    } else {
        for lane in 0..lanes {
            let final_sum = m.load(t_f32, var_sums[lane]);
            let token_offset = m.imul(t_u32, safe_tokens[lane], pc_output_stride);
...
pub fn emit_q5k_gemv(local_size_x: u32) -> Vec<u32> {
    emit_q5k_gemv_lanes(local_size_x, 1, false)
}

/// Emit a Q5_K matrix-vector batch shader that reuses each decoded weight
/// across two prompt tokens.
pub fn emit_q5k_gemv_batch2(local_size_x: u32) -> Vec<u32> {
    emit_q5k_gemv_lanes(local_size_x, 2, true)
}

/// Emit a Q5_K matrix-vector batch shader that reuses each decoded weight
/// across four prompt tokens.
pub fn emit_q5k_gemv_batch4(local_size_x: u32) -> Vec<u32> {
    emit_q5k_gemv_lanes(local_size_x, 4, true)
}

fn emit_q5k_gemv_lanes(local_size_x: u32, lanes: usize, batched: bool) -> Vec<u32> {
    let mut m = SpirvModule::new();
    ...
    // GlobalInvocationID.x selects a row. In batch mode y selects four tokens.
    let glob_id_vec = m.load(t_v3u32, gvar_glob_id);
    let row = m.composite_extract(t_u32, glob_id_vec, 0);
    let token_group = if batched {
        m.composite_extract(t_u32, glob_id_vec, 1)
    ...
    let c_lanes = m.constant_u32(t_u32, lanes as u32);
    let token_base = m.imul(t_u32, token_group, c_lanes);
    let (pc_input_stride, pc_output_stride, safe_tokens): (Id, Id, Vec<_>) = if batched {
        let pc_ptr_seq_len = m.access_chain(t_ptr_pc_u32, gvar_pc, &[c_u32_2]);
        let pc_seq_len = m.load(t_u32, pc_ptr_seq_len);
        let pc_ptr_input_stride = m.access_chain(t_ptr_pc_u32, gvar_pc, &[c_u32_3]);
        let input_stride = m.load(t_u32, pc_ptr_input_stride);
        let pc_ptr_output_stride = m.access_chain(t_ptr_pc_u32, gvar_pc, &[c_u32_4]);
        let output_stride = m.load(t_u32, pc_ptr_output_stride);
        let last_token = m.isub(t_u32, pc_seq_len, c_u32_1);
        let safe = (0..lanes)
            .map(|lane| {
                let lane_id = m.constant_u32(t_u32, lane as u32);
            ...
            .collect();
        (input_stride, output_stride, safe)
    } else {
        (c_u32_0, c_u32_0, vec![c_u32_0])
    };
    ...
    for qs_group in 0..4u32 {
        ...
        let mut q_input_sums_lo = vec![c_f32_0; lanes];
        let mut q_input_sums_hi = vec![c_f32_0; lanes];
        let mut input_sums_lo = vec![c_f32_0; lanes];
        let mut input_sums_hi = vec![c_f32_0; lanes];
        ...
        for w in 0..8u32 {
            ...
            for byte_idx in 0..4u32 {
                ...
                // Low and high nibbles represent adjacent 32-element input
                // sub-blocks. Each decoded weight is reused across token lanes.
                ...
                for lane in 0..lanes {
                    let token_offset = m.imul(t_u32, safe_tokens[lane], pc_input_stride);
                    let inp_lo_idx = m.iadd(t_u32, token_offset, inp_lo_element);
                    let inp_hi_idx = m.iadd(t_u32, token_offset, inp_hi_element);
                    let inp_lo_ptr =
                        m.access_chain(t_ptr_sb_f32, gvar_input, &[c_u32_0, inp_lo_idx]);
                    let inp_hi_ptr =
                        m.access_chain(t_ptr_sb_f32, gvar_input, &[c_u32_0, inp_hi_idx]);
                    ...
                    q_input_sums_lo[lane] = m.fadd(t_f32, q_input_sums_lo[lane], qi_prod_lo);
                    q_input_sums_hi[lane] = m.fadd(t_f32, q_input_sums_hi[lane], qi_prod_hi);
                    input_sums_lo[lane] = m.fadd(t_f32, input_sums_lo[lane], inp_lo);
                    input_sums_hi[lane] = m.fadd(t_f32, input_sums_hi[lane], inp_hi);
                }
            }
        }
        ...
        for lane in 0..lanes {
            let term1_lo = m.fmul(t_f32, d_sc_lo, q_input_sums_lo[lane]);
            let term2_lo = m.fmul(t_f32, dmin_mn_lo, input_sums_lo[lane]);
            let sb_result_lo = m.fsub(t_f32, term1_lo, term2_lo);
            total_sums[lane] = m.fadd(t_f32, total_sums[lane], sb_result_lo);
            let term1_hi = m.fmul(t_f32, d_sc_hi, q_input_sums_hi[lane]);
            let term2_hi = m.fmul(t_f32, dmin_mn_hi, input_sums_hi[lane]);
            let sb_result_hi = m.fsub(t_f32, term1_hi, term2_hi);
            total_sums[lane] = m.fadd(t_f32, total_sums[lane], sb_result_hi);
        }
    }
...
///
/// The cache uploads the original bytes without repacking. Each 32-lane
/// cluster splits the K dimension across two 16-lane Q4_K block groups.
pub fn emit_q4k_gemv_rowmajor(local_size_x: u32) -> Vec<u32> {
    emit_q4k_gemv_rowmajor_impl(local_size_x, false)
}

/// Emit the row-major Q4_K subgroup kernel across a strided token batch.
///
/// Workgroup X keeps the established two-row K-parallel mapping while
/// workgroup Y selects four prompt columns. Every lane decodes its weight
/// once and accumulates all four columns before the subgroup reductions.
pub fn emit_q4k_gemv_rowmajor_batched(local_size_x: u32) -> Vec<u32> {
    emit_q4k_gemv_rowmajor_impl(local_size_x, true)
}

fn emit_q4k_gemv_rowmajor_impl(local_size_x: u32, batched: bool) -> Vec<u32> {
    assert_eq!(
        local_size_x, 64,
        "Q4_K row-major two-row subgroup kernel requires 64 lanes"
    );
    ...
    let token_lanes = if batched { 4 } else { 1 };
    let var_sums: Vec<_> = (0..token_lanes)
        .map(|_| m.function_variable(t_ptr_fn_f32, storage_class::FUNCTION))
        .collect();
    ...
    let token_group = if batched {
        m.composite_extract(t_u32, wgid_vec, 1)
    } else {
        c_u32_0
    };
    ...
    let (pc_seq_len, pc_input_stride, pc_output_stride) = if batched {
        let pc_seq_len_ptr = m.access_chain(t_ptr_pc_u32, gvar_pc, &[c_u32_2]);
        ...
        (
            m.load(t_u32, pc_seq_len_ptr),
            m.load(t_u32, pc_input_stride_ptr),
            m.load(t_u32, pc_output_stride_ptr),
        )
    } else {
        (c_u32_1, pc_cols, pc_rows)
    };
    let c_token_lanes = m.constant_u32(t_u32, token_lanes as u32);
    let token_base = m.imul(t_u32, token_group, c_token_lanes);
    let last_token = m.isub(t_u32, pc_seq_len, c_u32_1);
    let tokens: Vec<_> = (0..token_lanes)
        .map(|lane| {
            let lane_id = m.constant_u32(t_u32, lane as u32);
            m.iadd(t_u32, token_base, lane_id)
        })
        .collect();
    let token_valid: Vec<_> = tokens
        .iter()
        .map(|&token| m.u_less_than(t_bool, token, pc_seq_len))
        .collect();
    let safe_tokens: Vec<_> = tokens
        .iter()
        .zip(&token_valid)
        .map(|(&token, &valid)| m.select(t_u32, valid, token, last_token))
        .collect();
    let input_token_offsets: Vec<_> = safe_tokens
        .iter()
        .map(|&token| m.imul(t_u32, token, pc_input_stride))
        .collect();
    let output_token_offsets: Vec<_> = tokens
        .iter()
        .map(|&token| m.imul(t_u32, token, pc_output_stride))
        .collect();
    let num_blocks = m.udiv(t_u32, pc_cols, c_u32_256);
    let row_stride_words = m.imul(t_u32, num_blocks, c_u32_36);
    let row_weight_base = m.imul(t_u32, row, row_stride_words);
    ...
    let packed_ptr = m.access_chain(t_ptr_sb_u32, gvar_weight, &[c_u32_0, block_base]);
    ...
    let scale0_ptr = m.access_chain(t_ptr_sb_u32, gvar_weight, &[c_u32_0, scale0_addr]);
    let scale0_word = m.load(t_u32, scale0_ptr);
    let scale1_addr = m.iadd(t_u32, block_base, c_u32_2);
    let scale1_ptr = m.access_chain(t_ptr_sb_u32, gvar_weight, &[c_u32_0, scale1_addr]);
    let scale1_word = m.load(t_u32, scale1_ptr);
    let scale2_addr = m.iadd(t_u32, block_base, c_u32_3);
    let scale2_ptr = m.access_chain(t_ptr_sb_u32, gvar_weight, &[c_u32_0, scale2_addr]);
    ...
    let quant0_ptr = m.access_chain(t_ptr_sb_u32, gvar_weight, &[c_u32_0, quant0_addr]);
    ...
    let quant64_ptr = m.access_chain(t_ptr_sb_u32, gvar_weight, &[c_u32_0, quant64_addr]);
    let quant64 = m.load(t_u32, quant64_ptr);

    let mut quant_sums = vec![[c_f32_0; 4]; token_lanes];
    let mut input_sums = vec![[c_f32_0; 4]; token_lanes];
    for element in 0..4u32 {
        ...
        for group in 0..4 {
            ...
            for token_lane in 0..token_lanes {
                let input_index = m.iadd(
                    t_u32,
                    input_token_offsets[token_lane],
                ...
                quant_sums[token_lane][group] =
                    m.fadd(t_f32, quant_sums[token_lane][group], quant_product);
                input_sums[token_lane][group] =
                    m.fadd(t_f32, input_sums[token_lane][group], input_value);
            }
        }
    }

    let mut block_sums = vec![c_f32_0; token_lanes];
    for group in 0..4 {
        ...
        for token_lane in 0..token_lanes {
            let quant_term = m.fmul(t_f32, d_scale, quant_sums[token_lane][group]);
            let min_term = m.fmul(t_f32, dmin_scale, input_sums[token_lane][group]);
            let group_sum = m.fsub(t_f32, quant_term, min_term);
            block_sums[token_lane] = m.fadd(t_f32, block_sums[token_lane], group_sum);
        }
    }
    for token_lane in 0..token_lanes {
        let previous_sum = m.load(t_f32, var_sums[token_lane]);
        let new_sum = m.fadd(t_f32, previous_sum, block_sums[token_lane]);
        m.store(var_sums[token_lane], new_sum);
    }
    ...
    m.functions
        .push(SpirvModule::encode_inst(op::LABEL, &[lbl_block_merge.0]));
    let row_sums: Vec<_> = var_sums
        .iter()
        .map(|&var_sum| {
            let lane_sum = m.load(t_f32, var_sum);
            m.group_non_uniform_fadd_clustered(t_f32, c_scope_subgroup, lane_sum, c_u32_32)
        })
        .collect();
    ...
    m.functions
        .push(SpirvModule::encode_inst(op::LABEL, &[lbl_store_partial.0]));
    let shared_row_base = m.imul(t_u32, row_in_workgroup, c_token_lanes);
    for (token_lane, &row_sum) in row_sums.iter().enumerate() {
        let shared_index = if token_lane == 0 {
            shared_row_base
        } else {
            let lane_id = m.constant_u32(t_u32, token_lane as u32);
            m.iadd(t_u32, shared_row_base, lane_id)
        };
        let partial_ptr = m.access_chain(t_ptr_wg_f32, gvar_shared, &[shared_index]);
        m.store(partial_ptr, row_sum);
    }
    m.branch(lbl_store_partial_merge);
    m.functions.push(SpirvModule::encode_inst(
        op::LABEL,
        &[lbl_store_partial_merge.0],
    ));
    ...
    m.functions
        .push(SpirvModule::encode_inst(op::LABEL, &[lbl_write_rows.0]));
    let second_row = m.iadd(t_u32, first_row, c_u32_1);
    let has_second_row = m.u_less_than(t_bool, second_row, pc_rows);
    for token_lane in 0..token_lanes {
        let lbl_write_token = m.alloc_id();
        let lbl_write_token_merge = m.alloc_id();
        m.selection_merge(lbl_write_token_merge, 0);
        m.branch_conditional(
            token_valid[token_lane],
            lbl_write_token,
            lbl_write_token_merge,
        );
        m.functions
            .push(SpirvModule::encode_inst(op::LABEL, &[lbl_write_token.0]));

        let lane_id = m.constant_u32(t_u32, token_lane as u32);
        let first_sum_ptr = m.access_chain(t_ptr_wg_f32, gvar_shared, &[lane_id]);
        let first_sum = m.load(t_f32, first_sum_ptr);
        let first_output_index = m.iadd(t_u32, output_token_offsets[token_lane], first_row);
        let first_output_ptr =
            m.access_chain(t_ptr_sb_f32, gvar_output, &[c_u32_0, first_output_index]);
        m.store(first_output_ptr, first_sum);
        ...
        m.functions
            .push(SpirvModule::encode_inst(op::LABEL, &[lbl_write_second.0]));
        let second_shared_index = m.iadd(t_u32, c_token_lanes, lane_id);
        let second_sum_ptr = m.access_chain(t_ptr_wg_f32, gvar_shared, &[second_shared_index]);
        let second_sum = m.load(t_f32, second_sum_ptr);
        let second_output_index = m.iadd(t_u32, output_token_offsets[token_lane], second_row);
        let second_output_ptr =
            m.access_chain(t_ptr_sb_f32, gvar_output, &[c_u32_0, second_output_index]);
        m.store(second_output_ptr, second_sum);
        m.branch(lbl_write_second_merge);
        m.functions.push(SpirvModule::encode_inst(
            op::LABEL,
            &[lbl_write_second_merge.0],
        ));
        m.branch(lbl_write_token_merge);
        m.functions.push(SpirvModule::encode_inst(
            op::LABEL,
            &[lbl_write_token_merge.0],
        ));
    }
...
pub fn emit_q6k_gemv(local_size_x: u32) -> Vec<u32> {
    emit_q6k_gemv_lanes(local_size_x, 1, false, false, false)
}

/// Emit a Q6_K matrix-vector batch shader that reuses every decoded weight
/// across four prompt tokens. Input and output windows are row-major by token.
pub fn emit_q6k_gemv_batch4(local_size_x: u32) -> Vec<u32> {
    emit_q6k_gemv_lanes(local_size_x, 4, true, false, false)
}

/// Q6_K batch-4 with a shared FP16 input tile and packed FP16 sub-block dot.
pub fn emit_q6k_gemv_batch4_f16(local_size_x: u32) -> Vec<u32> {
    emit_q6k_gemv_lanes(local_size_x, 4, true, true, true)
}

fn emit_q6k_gemv_lanes(
    local_size_x: u32,
    lanes: usize,
    batched: bool,
    use_f16: bool,
    ...
    let t_arr_f32 = m.type_runtime_array(t_f32);
    let t_shared_input = if shared_input {
        let c_shared_len = m.constant_u32(t_u32, 4 * 256);
        Some(m.type_array(f16_types.unwrap().0, c_shared_len))
    } else {
        None
    };
    ...
    let shared_ptr_types = t_shared_input.map(|t_shared| {
        (
            m.type_pointer(storage_class::WORKGROUP, t_shared),
            m.type_pointer(storage_class::WORKGROUP, f16_types.unwrap().0),
        )
    });
    ...
    let gvar_pc = m.variable(t_ptr_pc_struct, storage_class::PUSH_CONSTANT);
    let gvar_glob_id = m.variable(t_ptr_input_u32, storage_class::INPUT);
    let (gvar_local_id, gvar_shared_input) = if let Some((t_ptr_wg_shared, _)) = shared_ptr_types {
        (
            Some(m.variable(t_ptr_input_u32, storage_class::INPUT)),
            Some(m.variable(t_ptr_wg_shared, storage_class::WORKGROUP)),
        )
    } else {
        (None, None)
    };
    ...
    if shared_input {
        let local_id_x = local_id_x.unwrap();
        let (t_f16, _, _) = f16_types.unwrap();
        let gvar_shared_input = gvar_shared_input.unwrap();
        for lane in 0..lanes {
            let lane_base = m.constant_u32(t_u32, lane as u32 * 256);
            let token_offset = m.imul(t_u32, safe_tokens[lane], pc_input_stride);
            for chunk in 0..4u32 {
                ...
                let input_ptr = m.access_chain(t_ptr_sb_f32, gvar_input, &[c_u32_0, input_idx]);
                let input_value = m.load(t_f32, input_ptr);
                let input_f16 = m.f_convert(t_f16, input_value);
                let shared_idx = m.iadd(t_u32, lane_base, element);
                let shared_ptr = m.access_chain(
                    shared_ptr_types.unwrap().1,
                    gvar_shared_input,
    ...
    for sb_param in &sub_blocks {
        ...
        let inp_sb_local = c_elem_off;

        let d_scale = m.fmul(t_f32, d_f32, scale_f);
        let mut subblock_sum_f16 = f16_types.map(|(_, t_v4f16, _)| {
            let zero_f16 = c_f16_0.unwrap();
            m.constant_composite(t_v4f16, &[zero_f16, zero_f16, zero_f16, zero_f16])
        });
        ...
        for w in 0..4u32 {
            ...
            // Input base for this word: inp_sb_base + w*4
            let (inp_w_base, inp_w_local) = if w == 0 {
                (inp_sb_base, inp_sb_local)
            } else {
                let c_w4 = m.constant_u32(t_u32, w * 4);
                (
                    m.iadd(t_u32, inp_sb_base, c_w4),
            ...
            for byte_idx in 0..4u32 {
                ...
                let (inp_element, inp_local_element) = if byte_idx == 0 {
                    (inp_w_base, inp_w_local)
                } else {
                    let c_bi = m.constant_u32(t_u32, byte_idx);
                    (
                        m.iadd(t_u32, inp_w_base, c_bi),
                        m.iadd(t_u32, inp_w_local, c_bi),
                    )
                };
                if let Some((t_f16, t_v4f16, _)) = f16_types {
                    let mut inp_f16 = [c_f16_0.unwrap(); 4];
                    for lane in 0..lanes {
                        let lane_base = m.constant_u32(t_u32, lane as u32 * 256);
                        let shared_idx = m.iadd(t_u32, lane_base, inp_local_element);
                        let shared_ptr = m.access_chain(
                        ...
                        inp_f16[lane] = m.load(t_f16, shared_ptr);
                    }
                    let inp_v4f16 = m.composite_construct(t_v4f16, &inp_f16);
                    let q6_f16 = m.convert_s_to_f(t_f16, q6_centered);
                    let q6_v4f16 =
                        m.composite_construct(t_v4f16, &[q6_f16, q6_f16, q6_f16, q6_f16]);
                    let product = m.fmul(t_v4f16, q6_v4f16, inp_v4f16);
                    subblock_sum_f16 = Some(m.fadd(t_v4f16, subblock_sum_f16.unwrap(), product));
                } else {
                    ...
                    for lane in 0..lanes {
                        ...
                        total_sums[lane] = m.fadd(t_f32, total_sums[lane], qi_prod);
                    }
                }
            }
        }
        if let Some((_, _, t_v4f32)) = f16_types {
            let subblock_sum_v4f32 = m.f_convert(t_v4f32, subblock_sum_f16.unwrap());
            for lane in 0..lanes {
                let lane_sum = m.composite_extract(t_f32, subblock_sum_v4f32, lane as u32);
                let scaled = m.fmul(t_f32, d_scale, lane_sum);
                total_sums[lane] = m.fadd(t_f32, total_sums[lane], scaled);
            }
        }
    }
...
/// Layout:
///   binding 0: a buffer (f32, read/write)
///   binding 1: bias buffer (f32, read)
///   push constants: count (u32), bias_len (u32)
pub fn emit_elem_add_broadcast(local_size_x: u32) -> Vec<u32> {
    let mut m = SpirvModule::new();
    ...
    let t_struct_a = m.type_struct(&[t_arr_f32]);
    let t_struct_bias = m.type_struct(&[t_arr_f32]);
    let t_struct_pc = m.type_struct(&[t_u32, t_u32]);

    let t_ptr_sb_a = m.type_pointer(storage_class::STORAGE_BUFFER, t_struct_a);
    let t_ptr_sb_bias = m.type_pointer(storage_class::STORAGE_BUFFER, t_struct_bias);
    let t_ptr_pc = m.type_pointer(storage_class::PUSH_CONSTANT, t_struct_pc);
    let t_ptr_input_v3 = m.type_pointer(storage_class::INPUT, t_v3u32);
    let t_ptr_sb_f32 = m.type_pointer(storage_class::STORAGE_BUFFER, t_f32);
    let t_ptr_pc_u32 = m.type_pointer(storage_class::PUSH_CONSTANT, t_u32);
    ...
    m.decorate(t_struct_a, decoration::BLOCK, &[]);
    m.decorate(t_struct_bias, decoration::BLOCK, &[]);
    m.member_decorate(t_struct_a, 0, decoration::OFFSET, &[0]);
    m.member_decorate(t_struct_bias, 0, decoration::OFFSET, &[0]);
    m.decorate(t_arr_f32, decoration::ARRAY_STRIDE, &[4]);
    m.decorate(t_struct_pc, decoration::BLOCK, &[]);
    m.member_decorate(t_struct_pc, 0, decoration::OFFSET, &[0]);
    m.member_decorate(t_struct_pc, 1, decoration::OFFSET, &[4]);
    ...
    let gvar_a = m.variable(t_ptr_sb_a, storage_class::STORAGE_BUFFER);
    let gvar_bias = m.variable(t_ptr_sb_bias, storage_class::STORAGE_BUFFER);
    let gvar_pc = m.variable(t_ptr_pc, storage_class::PUSH_CONSTANT);
    let gvar_gid = m.variable(t_ptr_input_v3, storage_class::INPUT);

    m.decorate(gvar_a, decoration::DESCRIPTOR_SET, &[0]);
    m.decorate(gvar_a, decoration::BINDING, &[0]);
    m.decorate(gvar_bias, decoration::DESCRIPTOR_SET, &[0]);
    m.decorate(gvar_bias, decoration::BINDING, &[1]);
    m.decorate(
        gvar_gid,
        decoration::BUILTIN,
        &[builtin::GLOBAL_INVOCATION_ID],
    ...
    let gid_vec = m.load(t_v3u32, gvar_gid);
    let gid = m.composite_extract(t_u32, gid_vec, 0);
    let count_ptr = m.access_chain(t_ptr_pc_u32, gvar_pc, &[c_u32_0]);
    let count = m.load(t_u32, count_ptr);
    let bias_len_ptr = m.access_chain(t_ptr_pc_u32, gvar_pc, &[c_u32_1]);
    let bias_len = m.load(t_u32, bias_len_ptr);
    ...
    m.functions
        .push(SpirvModule::encode_inst(op::LABEL, &[lbl_body.0]));
    let bias_idx = m.umod(t_u32, gid, bias_len);
    let a_ptr = m.access_chain(t_ptr_sb_f32, gvar_a, &[c_u32_0, gid]);
    let a_val = m.load(t_f32, a_ptr);
    let bias_ptr = m.access_chain(t_ptr_sb_f32, gvar_bias, &[c_u32_0, bias_idx]);
    let bias_val = m.load(t_f32, bias_ptr);
    let sum = m.fadd(t_f32, a_val, bias_val);
    m.store(a_ptr, sum);
...
/// Precompute token/head values shared by every GDN state row.
///
/// Binding 0 is token-major conv q/k/v, bindings 1/2 are token-major
/// alpha/beta, bindings 3/4 are static ssm_a/dt_bias, and binding 5 receives
/// `[q_inv, k_inv, beta, decay]` for each token.
pub fn emit_gdn_delta_precompute(local_size_x: u32) -> Vec<u32> {
    let mut m = SpirvModule::new();
    m.capability(1);
    m.extension("SPV_KHR_storage_buffer_storage_class");
    let glsl = m.ext_inst_import("GLSL.std.450");
    ...
    let t_v3u32 = m.type_vector(t_u32, 3);
    let t_arr_f32 = m.type_runtime_array(t_f32);
    let t_struct_buf = m.type_struct(&[t_arr_f32]);
    let t_struct_pc = m.type_struct(&[
        t_u32, t_u32, t_u32, t_u32, t_u32, t_u32, t_u32, t_u32, t_u32, t_u32, t_u32,
    ]);
    let t_ptr_buf = m.type_pointer(storage_class::STORAGE_BUFFER, t_struct_buf);
    let t_ptr_pc = m.type_pointer(storage_class::PUSH_CONSTANT, t_struct_pc);
    let t_ptr_input_v3 = m.type_pointer(storage_class::INPUT, t_v3u32);
    let t_ptr_sb_f32 = m.type_pointer(storage_class::STORAGE_BUFFER, t_f32);
    let t_ptr_pc_u32 = m.type_pointer(storage_class::PUSH_CONSTANT, t_u32);
    ...
    let c4 = m.constant_u32(t_u32, 4);
    let c5 = m.constant_u32(t_u32, 5);
    let c6 = m.constant_u32(t_u32, 6);
    let c7 = m.constant_u32(t_u32, 7);
    let c8 = m.constant_u32(t_u32, 8);
    let c9 = m.constant_u32(t_u32, 9);
    let cf0 = m.constant_f32(t_f32, 0.0);
    let cf1 = m.constant_f32(t_f32, 1.0);

    m.decorate(t_struct_buf, decoration::BLOCK, &[]);
    m.member_decorate(t_struct_buf, 0, decoration::OFFSET, &[0]);
    m.decorate(t_arr_f32, decoration::ARRAY_STRIDE, &[4]);
    m.decorate(t_struct_pc, decoration::BLOCK, &[]);
    for member in 0..11 {
    ...
    let buffers: Vec<_> = (0..6)
        .map(|_| m.variable(t_ptr_buf, storage_class::STORAGE_BUFFER))
        .collect();
    let gvar_pc = m.variable(t_ptr_pc, storage_class::PUSH_CONSTANT);
    let gvar_gid = m.variable(t_ptr_input_v3, storage_class::INPUT);
    for (binding, &var) in buffers.iter().enumerate() {
    ...
    let lbl_loop_continue = m.alloc_id();
    let lbl_loop_merge = m.alloc_id();
    let lbl_gate = m.alloc_id();
    let lbl_gate_merge = m.alloc_id();
    let lbl_end = m.alloc_id();
    ...
    let gid_vec = m.load(t_v3u32, gvar_gid);
    let gid = m.composite_extract(t_u32, gid_vec, 0);
    let load_pc = |m: &mut SpirvModule, index: Id| {
        let ptr = m.access_chain(t_ptr_pc_u32, gvar_pc, &[index]);
        m.load(t_u32, ptr)
    };
    let _conv_channels = load_pc(&mut m, c0);
    let _d_inner = load_pc(&mut m, c1);
    let num_k_heads = load_pc(&mut m, c2);
    let num_v_heads = load_pc(&mut m, c3);
    let head_k_dim = load_pc(&mut m, c4);
    let _head_v_dim = load_pc(&mut m, c5);
    let eps_bits = load_pc(&mut m, c6);
    let seq_len = load_pc(&mut m, c7);
    let conv_stride = load_pc(&mut m, c8);
    let head_stride = load_pc(&mut m, c9);
    let eps = m.bitcast(t_f32, eps_bits);
    let k_lt_v = m.u_less_than(t_bool, num_k_heads, num_v_heads);
    let max_heads = m.select(t_u32, k_lt_v, num_v_heads, num_k_heads);
    let total = m.imul(t_u32, seq_len, max_heads);
    ...
    m.functions
        .push(SpirvModule::encode_inst(op::LABEL, &[lbl_body.0]));
    let token = m.udiv(t_u32, gid, max_heads);
    let head = m.umod(t_u32, gid, max_heads);
    let conv_token_offset = m.imul(t_u32, token, conv_stride);
    let head_token_offset = m.imul(t_u32, token, head_stride);
    let two_k = m.imul(t_u32, c2, num_k_heads);
    let two_v = m.imul(t_u32, c2, num_v_heads);
    let param_stride = m.iadd(t_u32, two_k, two_v);
    let param_token_offset = m.imul(t_u32, token, param_stride);

    let is_k_head = m.u_less_than(t_bool, head, num_k_heads);
    m.selection_merge(lbl_qk_merge, 0);
    m.branch_conditional(is_k_head, lbl_qk, lbl_qk_merge);
    m.functions
        .push(SpirvModule::encode_inst(op::LABEL, &[lbl_qk.0]));
    m.store(var_j, c0);
    m.store(var_q_sum, cf0);
    m.store(var_k_sum, cf0);
    m.branch(lbl_loop_header);
    m.functions
        .push(SpirvModule::encode_inst(op::LABEL, &[lbl_loop_header.0]));
    m.loop_merge(lbl_loop_merge, lbl_loop_continue, 0);
    ...
    m.functions
        .push(SpirvModule::encode_inst(op::LABEL, &[lbl_loop_cond.0]));
    let j = m.load(t_u32, var_j);
    let loop_cond = m.u_less_than(t_bool, j, head_k_dim);
    m.branch_conditional(loop_cond, lbl_loop_body, lbl_loop_merge);
    m.functions
        .push(SpirvModule::encode_inst(op::LABEL, &[lbl_loop_body.0]));
    let head_base = m.imul(t_u32, head, head_k_dim);
    let q_element = m.iadd(t_u32, head_base, j);
    let q_dim = m.imul(t_u32, num_k_heads, head_k_dim);
    let k_element = m.iadd(t_u32, q_dim, q_element);
    let q_idx = m.iadd(t_u32, conv_token_offset, q_element);
    let k_idx = m.iadd(t_u32, conv_token_offset, k_element);
    let q_ptr = m.access_chain(t_ptr_sb_f32, buffers[0], &[c0, q_idx]);
    let k_ptr = m.access_chain(t_ptr_sb_f32, buffers[0], &[c0, k_idx]);
    let q = m.load(t_f32, q_ptr);
    let k = m.load(t_f32, k_ptr);
    let q_sq = m.fmul(t_f32, q, q);
    let k_sq = m.fmul(t_f32, k, k);
    let q_old = m.load(t_f32, var_q_sum);
    let k_old = m.load(t_f32, var_k_sum);
    let q_new = m.fadd(t_f32, q_old, q_sq);
    let k_new = m.fadd(t_f32, k_old, k_sq);
    m.store(var_q_sum, q_new);
    m.store(var_k_sum, k_new);
    m.branch(lbl_loop_continue);
    m.functions
        .push(SpirvModule::encode_inst(op::LABEL, &[lbl_loop_continue.0]));
    let j_next = m.iadd(t_u32, j, c1);
    m.store(var_j, j_next);
    m.branch(lbl_loop_header);
    m.functions
        .push(SpirvModule::encode_inst(op::LABEL, &[lbl_loop_merge.0]));
    let q_sum = m.load(t_f32, var_q_sum);
    let k_sum = m.load(t_f32, var_k_sum);
    let q_denom_sq = m.fadd(t_f32, q_sum, eps);
    let k_denom_sq = m.fadd(t_f32, k_sum, eps);
    let q_denom = m.ext_inst(t_f32, glsl, 31, &[q_denom_sq]);
    let k_denom = m.ext_inst(t_f32, glsl, 31, &[k_denom_sq]);
    let q_inv_l2 = m.fdiv(t_f32, cf1, q_denom);
    let k_inv = m.fdiv(t_f32, cf1, k_denom);
    let head_k_f = m.convert_u_to_f(t_f32, head_k_dim);
    let head_k_sqrt = m.ext_inst(t_f32, glsl, 31, &[head_k_f]);
    let q_scale = m.fdiv(t_f32, cf1, head_k_sqrt);
    let q_inv = m.fmul(t_f32, q_inv_l2, q_scale);
    let q_out_idx = m.iadd(t_u32, param_token_offset, head);
    let k_base = m.iadd(t_u32, param_token_offset, num_k_heads);
    let k_out_idx = m.iadd(t_u32, k_base, head);
    let q_out = m.access_chain(t_ptr_sb_f32, buffers[5], &[c0, q_out_idx]);
    let k_out = m.access_chain(t_ptr_sb_f32, buffers[5], &[c0, k_out_idx]);
    m.store(q_out, q_inv);
    m.store(k_out, k_inv);
    m.branch(lbl_qk_merge);

    m.functions
        .push(SpirvModule::encode_inst(op::LABEL, &[lbl_qk_merge.0]));
    let is_v_head = m.u_less_than(t_bool, head, num_v_heads);
    m.selection_merge(lbl_gate_merge, 0);
    m.branch_conditional(is_v_head, lbl_gate, lbl_gate_merge);
    m.functions
        .push(SpirvModule::encode_inst(op::LABEL, &[lbl_gate.0]));
    let head_idx = m.iadd(t_u32, head_token_offset, head);
    let alpha_ptr = m.access_chain(t_ptr_sb_f32, buffers[1], &[c0, head_idx]);
    let beta_ptr = m.access_chain(t_ptr_sb_f32, buffers[2], &[c0, head_idx]);
    let ssm_ptr = m.access_chain(t_ptr_sb_f32, buffers[3], &[c0, head]);
    let dt_ptr = m.access_chain(t_ptr_sb_f32, buffers[4], &[c0, head]);
    let alpha = m.load(t_f32, alpha_ptr);
    let beta_raw = m.load(t_f32, beta_ptr);
    let ssm_a = m.load(t_f32, ssm_ptr);
    let dt_bias = m.load(t_f32, dt_ptr);
    let neg_beta = m.fnegate(t_f32, beta_raw);
    let exp_neg_beta = m.ext_inst(t_f32, glsl, 27, &[neg_beta]);
    let beta_den = m.fadd(t_f32, cf1, exp_neg_beta);
    let beta = m.fdiv(t_f32, cf1, beta_den);
    let alpha_biased = m.fadd(t_f32, alpha, dt_bias);
    let exp_alpha = m.ext_inst(t_f32, glsl, 27, &[alpha_biased]);
    let softplus_arg = m.fadd(t_f32, cf1, exp_alpha);
    ...
    let beta_base = m.iadd(t_u32, param_token_offset, two_k);
    let beta_idx = m.iadd(t_u32, beta_base, head);
    let decay_base = m.iadd(t_u32, beta_base, num_v_heads);
    let decay_idx = m.iadd(t_u32, decay_base, head);
    let beta_out = m.access_chain(t_ptr_sb_f32, buffers[5], &[c0, beta_idx]);
    let decay_out = m.access_chain(t_ptr_sb_f32, buffers[5], &[c0, decay_idx]);
    m.store(beta_out, beta);
    m.store(decay_out, decay);
    m.branch(lbl_gate_merge);
    m.functions
        .push(SpirvModule::encode_inst(op::LABEL, &[lbl_gate_merge.0]));
    m.branch(lbl_end);
    m.functions
        .push(SpirvModule::encode_inst(op::LABEL, &[lbl_end.0]));
    m.ret();
...
pub fn emit_gdn_delta_step(local_size_x: u32) -> Vec<u32> {
    emit_gdn_delta(local_size_x, false)
}

/// Emit one dispatch that advances a GDN DeltaNet state through a full token
/// sequence. Each invocation owns one state row, so token steps are serial
/// within the invocation without cross-invocation barriers.
pub fn emit_gdn_delta_sequence(local_size_x: u32) -> Vec<u32> {
    emit_gdn_delta(local_size_x, true)
}

fn emit_gdn_delta(local_size_x: u32, sequence: bool) -> Vec<u32> {
    let mut m = SpirvModule::new();
    ...
    let t_struct_out = m.type_struct(&[t_arr_f32]);
    let pc_fields = if sequence {
        vec![
            t_u32, t_u32, t_u32, t_u32, t_u32, t_u32, t_u32, t_u32, t_u32, t_u32, t_u32,
        ]
    } else {
        vec![t_u32, t_u32, t_u32, t_u32, t_u32, t_u32, t_u32]
    };
    let t_struct_pc = m.type_struct(&pc_fields);
    ...
    m.decorate(t_arr_f32, decoration::ARRAY_STRIDE, &[4]);
    m.decorate(t_struct_pc, decoration::BLOCK, &[]);
    for member in 0..pc_fields.len() as u32 {
        m.member_decorate(t_struct_pc, member, decoration::OFFSET, &[member * 4]);
    }
    ...
    let lbl_token_header = m.alloc_id();
    let lbl_token_cond = m.alloc_id();
    let lbl_token_body = m.alloc_id();
    let lbl_token_continue = m.alloc_id();
    let lbl_token_merge = m.alloc_id();
    ...
    let var_token = m.function_variable(t_ptr_fn_u32, storage_class::FUNCTION);
    ...
    let eps_bits = m.load(t_u32, pc_eps_ptr);
    let eps = m.bitcast(t_f32, eps_bits);
    let (seq_len, conv_stride, head_stride, output_stride) = if sequence {
        let seq_ptr = m.access_chain(t_ptr_pc_u32, gvar_pc, &[c_u32_7]);
        let conv_stride_ptr = m.access_chain(t_ptr_pc_u32, gvar_pc, &[c_u32_8]);
        let head_stride_ptr = m.access_chain(t_ptr_pc_u32, gvar_pc, &[c_u32_9]);
        let output_stride_ptr = m.access_chain(t_ptr_pc_u32, gvar_pc, &[c_u32_10]);
        (
            m.load(t_u32, seq_ptr),
            m.load(t_u32, conv_stride_ptr),
            m.load(t_u32, head_stride_ptr),
            m.load(t_u32, output_stride_ptr),
        )
    } else {
        (c_u32_1, c_u32_0, c_u32_0, c_u32_0)
    };
    ...
    m.store(var_token, c_u32_0);
    m.branch(lbl_token_header);
    m.functions
        .push(SpirvModule::encode_inst(op::LABEL, &[lbl_token_header.0]));
    m.loop_merge(lbl_token_merge, lbl_token_continue, 0);
    m.branch(lbl_token_cond);
    m.functions
        .push(SpirvModule::encode_inst(op::LABEL, &[lbl_token_cond.0]));
    let token = m.load(t_u32, var_token);
    let token_cond = m.u_less_than(t_bool, token, seq_len);
    m.branch_conditional(token_cond, lbl_token_body, lbl_token_merge);
    m.functions
        .push(SpirvModule::encode_inst(op::LABEL, &[lbl_token_body.0]));
    let conv_token_offset = m.imul(t_u32, token, conv_stride);
    let head_token_offset = m.imul(t_u32, token, head_stride);
    let output_token_offset = m.imul(t_u32, token, output_stride);

    let (q_inv, k_inv, beta, decay) = if sequence {
        let two_k = m.imul(t_u32, c_u32_2, num_k_heads);
        let two_v = m.imul(t_u32, c_u32_2, _num_v_heads);
        let param_stride = m.iadd(t_u32, two_k, two_v);
        let param_token_offset = m.imul(t_u32, token, param_stride);
        let q_inv_idx = m.iadd(t_u32, param_token_offset, kh);
        let k_inv_base = m.iadd(t_u32, param_token_offset, num_k_heads);
        let k_inv_idx = m.iadd(t_u32, k_inv_base, kh);
        let beta_base = m.iadd(t_u32, param_token_offset, two_k);
        let beta_idx = m.iadd(t_u32, beta_base, h);
        let decay_base = m.iadd(t_u32, beta_base, _num_v_heads);
        let decay_idx = m.iadd(t_u32, decay_base, h);
        let q_inv_ptr = m.access_chain(t_ptr_sb_f32, gvar_alpha, &[c_u32_0, q_inv_idx]);
        let k_inv_ptr = m.access_chain(t_ptr_sb_f32, gvar_alpha, &[c_u32_0, k_inv_idx]);
        let beta_ptr = m.access_chain(t_ptr_sb_f32, gvar_alpha, &[c_u32_0, beta_idx]);
        let decay_ptr = m.access_chain(t_ptr_sb_f32, gvar_alpha, &[c_u32_0, decay_idx]);
        (
            m.load(t_f32, q_inv_ptr),
            m.load(t_f32, k_inv_ptr),
            m.load(t_f32, beta_ptr),
            m.load(t_f32, decay_ptr),
        )
    } else {
        m.store(var_j, c_u32_0);
        ...
        let q_element = m.iadd(t_u32, q_base, j_qk);
        let k_element = m.iadd(t_u32, k_base, j_qk);
        let q_idx = m.iadd(t_u32, conv_token_offset, q_element);
        let k_idx = m.iadd(t_u32, conv_token_offset, k_element);
        ...
        let beta_idx = m.iadd(t_u32, head_token_offset, h);
        let beta_ptr = m.access_chain(t_ptr_sb_f32, gvar_beta, &[c_u32_0, beta_idx]);
        let beta_raw = m.load(t_f32, beta_ptr);
        let neg_beta = m.fnegate(t_f32, beta_raw);
        let exp_neg_beta = m.ext_inst(t_f32, glsl, 27, &[neg_beta]);
        ...
        let alpha_idx = m.iadd(t_u32, head_token_offset, h);
        let alpha_ptr = m.access_chain(t_ptr_sb_f32, gvar_alpha, &[c_u32_0, alpha_idx]);
    ...
    let k_element_sk = m.iadd(t_u32, k_base, j_sk);
    let k_idx_sk = m.iadd(t_u32, conv_token_offset, k_element_sk);
    ...
    let decayed_state_sk = m.fmul(t_f32, decay, state_val_sk);
    let sk_mul = m.fmul(t_f32, decayed_state_sk, k_norm_sk);
    ...
    let v_element = m.iadd(t_u32, v_base, vi);
    let v_idx = m.iadd(t_u32, conv_token_offset, v_element);
    ...
    let k_element_up = m.iadd(t_u32, k_base, j_up);
    let q_element_up = m.iadd(t_u32, q_base, j_up);
    let k_idx_up = m.iadd(t_u32, conv_token_offset, k_element_up);
    let q_idx_up = m.iadd(t_u32, conv_token_offset, q_element_up);
    ...
    let out_idx = m.iadd(t_u32, output_token_offset, gid);
    let out_ptr = m.access_chain(t_ptr_sb_f32, gvar_out, &[c_u32_0, out_idx]);
    m.store(out_ptr, out_final);
    m.branch(lbl_token_continue);
    m.functions
        .push(SpirvModule::encode_inst(op::LABEL, &[lbl_token_continue.0]));
    let token_next = m.iadd(t_u32, token, c_u32_1);
    m.store(var_token, token_next);
    m.branch(lbl_token_header);
    m.functions
        .push(SpirvModule::encode_inst(op::LABEL, &[lbl_token_merge.0]));
    m.branch(lbl_end);
...
pub fn emit_attention_decode(local_size_x: u32) -> Vec<u32> {
    ...
    let gvar_gid = m.variable(t_ptr_input_v3, storage_class::INPUT);
    let gvar_subgroup_local_id = m.variable(t_ptr_input_u32, storage_class::INPUT);
    let gvar_subgroup_size = m.variable(t_ptr_input_u32, storage_class::INPUT);
    ...
    m.entry_point(
        ...
        func_id,
        "main",
        &[gvar_gid, gvar_subgroup_local_id, gvar_subgroup_size],
    );
    m.execution_mode_local_size(func_id, local_size_x, 1, 1);
    ...
    // Each hardware subgroup computes one complete Q·K score. Lanes walk
    // disjoint head dimensions at the native subgroup width, then subgroup
    // addition shares the score with every output lane.
    let var_acc0 = m.function_variable(t_ptr_fn_f32, storage_class::FUNCTION);

    let gid_vec = m.load(t_v3u32, gvar_gid);
    let gid = m.composite_extract(t_u32, gid_vec, 0);
    let subgroup_local_id = m.load(t_u32, gvar_subgroup_local_id);
    let subgroup_size = m.load(t_u32, gvar_subgroup_size);
    ...
    m.branch_conditional(kv_cond, lbl_kv_b, lbl_kv_m);

    m.functions
        .push(SpirvModule::encode_inst(op::LABEL, &[lbl_kv_b.0]));

    m.store(var_i, subgroup_local_id);
    m.store(var_acc0, c_f32_0);

    let lbl_dot_h = m.alloc_id();
    let lbl_dot_c = m.alloc_id();
    let lbl_dot_b = m.alloc_id();
    let lbl_dot_cont = m.alloc_id();
    let lbl_dot_m = m.alloc_id();
    m.branch(lbl_dot_h);
    m.functions
        .push(SpirvModule::encode_inst(op::LABEL, &[lbl_dot_h.0]));
    m.loop_merge(lbl_dot_m, lbl_dot_cont, 0);
    m.branch(lbl_dot_c);

    m.functions
        .push(SpirvModule::encode_inst(op::LABEL, &[lbl_dot_c.0]));
    let iv = m.load(t_u32, var_i);
    let dot_cond = m.u_less_than(t_bool, iv, head_dim);
    m.branch_conditional(dot_cond, lbl_dot_b, lbl_dot_m);

    m.functions
        .push(SpirvModule::encode_inst(op::LABEL, &[lbl_dot_b.0]));
    let k_base = m.imul(t_u32, jv, head_dim);
    let q_ptr = m.access_chain(t_ptr_sb_f32, gvar_q, &[c_u32_0, iv]);
    let q_val = m.load(t_f32, q_ptr);
    let k_idx = m.iadd(t_u32, k_base, iv);
    let k_ptr = m.access_chain(t_ptr_sb_f32, gvar_k, &[c_u32_0, k_idx]);
    let k_val = m.load(t_f32, k_ptr);
    let acc = m.load(t_f32, var_acc0);
    let acc_next = m.ext_inst(t_f32, glsl, 50, &[q_val, k_val, acc]);
    m.store(var_acc0, acc_next);
    m.branch(lbl_dot_cont);

    m.functions
        .push(SpirvModule::encode_inst(op::LABEL, &[lbl_dot_cont.0]));
    let iv_next = m.iadd(t_u32, iv, subgroup_size);
    m.store(var_i, iv_next);
    m.branch(lbl_dot_h);

    m.functions
        .push(SpirvModule::encode_inst(op::LABEL, &[lbl_dot_m.0]));
    let partial_dot = m.load(t_f32, var_acc0);
    let final_dot = m.group_non_uniform_fadd_reduce(t_f32, c_scope_subgroup, partial_dot);
    let score = m.fmul(t_f32, final_dot, scale);

    // Numerically stable one-pass softmax. When the running maximum grows,
    // rescale the old denominator and value accumulator; otherwise only the
    // new score is scaled. This needs one Exp2 and one Q·K pass per KV row.
    let old_max = m.load(t_f32, var_max);
    let max_grows = m.f_ord_greater_than(t_bool, score, old_max);
    let new_max = m.select(t_f32, max_grows, score, old_max);
    let old_shift = m.fsub(t_f32, old_max, score);
    let new_shift = m.fsub(t_f32, score, old_max);
    let exp_shift = m.select(t_f32, max_grows, old_shift, new_shift);
    let exp2_arg = m.fmul(t_f32, exp_shift, c_log2e);
    let exp_delta = m.ext_inst(t_f32, glsl, 29, &[exp2_arg]);
    let old_scale = m.select(t_f32, max_grows, exp_delta, c_f32_1);
    let new_weight = m.select(t_f32, max_grows, c_f32_1, exp_delta);

    let den = m.load(t_f32, var_den);
    let den_next = m.ext_inst(t_f32, glsl, 50, &[den, old_scale, new_weight]);
    m.store(var_den, den_next);

    let v_base = m.imul(t_u32, jv, head_dim);
    let v_idx = m.iadd(t_u32, v_base, gid);
    let v_ptr = m.access_chain(t_ptr_sb_f32, gvar_v, &[c_u32_0, v_idx]);
    let v_val = m.load(t_f32, v_ptr);
    let num = m.load(t_f32, var_num);
    let scaled_num = m.fmul(t_f32, num, old_scale);
    let num_next = m.ext_inst(t_f32, glsl, 50, &[new_weight, v_val, scaled_num]);
    m.store(var_num, num_next);
    m.store(var_max, new_max);
