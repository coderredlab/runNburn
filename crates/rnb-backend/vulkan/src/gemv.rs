...
pub(crate) fn repack_q8_0_transposed_grouped(
    ...
    let words_per_row = blocks_per_row * words_per_block;
    let mut dst = vec![0u32; rows * words_per_row];
    if rows == 0 {
        return dst;
    }
    ...
    let worker_count = std::thread::available_parallelism()
        ...
        .unwrap_or(1)
        .min(group_count);
    let groups_per_worker = group_count.div_ceil(worker_count);
    let rows_per_worker = groups_per_worker * rows_per_group;
    let words_per_worker = rows_per_worker * words_per_row;

    std::thread::scope(|scope| {
        for (worker_idx, dst_chunk) in dst.chunks_mut(words_per_worker).enumerate() {
            let row_start = worker_idx * rows_per_worker;
            let row_count = dst_chunk.len() / words_per_row;
            let src_start = row_start * blocks_per_row * src_block_size;
            let src_end = src_start + row_count * blocks_per_row * src_block_size;
            let src_chunk = &src[src_start..src_end];
            scope.spawn(move || {
                let mut group_start = 0usize;
                while group_start < row_count {
                    let group_rows = (row_count - group_start).min(rows_per_group);
                    let group_base = group_start * words_per_row;
                    for local_row in 0..group_rows {
                        let worker_row = group_start + local_row;
                        for blk in 0..blocks_per_row {
                            let src_off = (worker_row * blocks_per_row + blk) * src_block_size;
                            let plane_base = blk * words_per_block;
                            let scale_word =
                                src_chunk[src_off] as u32 | ((src_chunk[src_off + 1] as u32) << 8);
                            dst_chunk[group_base + plane_base * group_rows + local_row] =
                                scale_word;

                            for w in 0..8 {
                                let qs_off = src_off + 2 + w * 4;
                                let qs_word = src_chunk[qs_off] as u32
                                    | ((src_chunk[qs_off + 1] as u32) << 8)
                                    | ((src_chunk[qs_off + 2] as u32) << 16)
                                    | ((src_chunk[qs_off + 3] as u32) << 24);
                                dst_chunk
                                    [group_base + (plane_base + 1 + w) * group_rows + local_row] =
                                    qs_word;
                            }
                        }
                    }
