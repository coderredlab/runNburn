...
//!
//! Block-size constants are reproduced here from the GGUF / llama.cpp
//! quantization specification, and verified against
//! `rnb-cpu`'s `BlockQ4_K`/`BlockQ5_K`/`BlockQ6_K`/`BlockQ8_0` (`size_of` =
//! 144 / 176 / 210 / 34) by matching unit tests in `crates/rnb-cpu/src/quantize/blocks.rs`.

use core::ops::Range;

use crate::weight_cache::QuantType;

/// K-quant super-block element count (Q4_K / Q5_K / Q6_K).
/// Matches `rnb-cpu` `QK_K`.
const QK_K: usize = 256;

/// Basic-quant block element count (Q8_0).
/// Matches `rnb-cpu` `QK8_0`.
const QK8_0: usize = 32;

/// Block byte size — one super-block of Q4_K (`f16 d + f16 dmin + 12B scales + 128B qs`).
/// Matches `size_of::<BlockQ4_K>() == 144`.
const Q4K_BLOCK_BYTES: usize = 144;

/// Block byte size — one super-block of Q5_K (`f16 d + f16 dmin + 12B scales + 32B qh + 128B qs`).
/// Matches `size_of::<BlockQ5_K>() == 176`.
const Q5K_BLOCK_BYTES: usize = 176;

/// Block byte size — one super-block of Q6_K (`128B ql + 64B qh + 16B scales + f16 d`).
/// Matches `size_of::<BlockQ6_K>() == 210`.
const Q6K_BLOCK_BYTES: usize = 210;

/// Block byte size — one block of Q8_0 (`f16 d + 32B i8 quants`).
/// Matches `size_of::<BlockQ8_0>() == 34`.
...
pub(crate) fn row_bytes_per_row(quant: QuantType, hidden: usize) -> Result<usize, String> {
    ...
    let (block_elems, block_bytes) = match quant {
        QuantType::Q4K => (QK_K, Q4K_BLOCK_BYTES),
        QuantType::Q5K => (QK_K, Q5K_BLOCK_BYTES),
        QuantType::Q6K => (QK_K, Q6K_BLOCK_BYTES),
        QuantType::Q8_0 => (QK8_0, Q8_0_BLOCK_BYTES),
    };
...
mod tests {
    ...
    fn block_byte_constants_match_rnb_cpu() {
        // The local block-byte constants in this module are documented to match
        // `rnb-cpu`'s `BlockQ*_K`/`BlockQ8_0` `size_of` and `QK_*` element-count
        // constants. Enforce that match here so a future quant-layout change in
        // `rnb-cpu` (added/removed field, alignment shift, etc.) trips this test
        // instead of silently desyncing the shard byte-range math.
        use core::mem::size_of;
        use rnb_cpu::quantize::blocks::{
            BlockQ4_K, BlockQ5_K, BlockQ6_K, BlockQ8_0, QK8_0 as RNB_QK8_0, QK_K as RNB_QK_K,
        };

        assert_eq!(size_of::<BlockQ4_K>(), Q4K_BLOCK_BYTES);
        assert_eq!(size_of::<BlockQ5_K>(), Q5K_BLOCK_BYTES);
        assert_eq!(size_of::<BlockQ6_K>(), Q6K_BLOCK_BYTES);
        assert_eq!(size_of::<BlockQ8_0>(), Q8_0_BLOCK_BYTES);
        assert_eq!(RNB_QK_K, QK_K);
        assert_eq!(RNB_QK8_0, QK8_0);
    }
}
