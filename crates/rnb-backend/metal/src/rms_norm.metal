...
// pm132: residual add + RMSNorm fused.
//   hidden[i] += residual[i]  (in-place)
//   normed[i] = hidden[i] / sqrt(mean(hidden^2) + eps) * weight[i]
// 분리 실행(residual_add → barrier → rms_norm)과 bit-identical:
//   같은 float add 순서 + 같은 tree reduction + 같은 rsqrt·mul.
// 단일 threadgroup(grid=1, tg=256). chain 내 residual→norm 연속 호출 대체.
kernel void fused_residual_rms_norm(
    device float*       hidden   [[buffer(0)]],  // in-place: += residual, then norm input
    device const float* residual [[buffer(1)]],  // add source
    device const float* weight   [[buffer(2)]],  // norm weight
    device float*       normed   [[buffer(3)]],  // norm output
    ...
    uint tg_size [[threads_per_threadgroup]])
{
    ...
    // Step 1: residual add + partial sum of squares (single pass)
    float sum = 0.0f;
    for (uint i = tid; i < dim; i += tg_size) {
