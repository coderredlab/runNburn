...
// llama.cpp kernel_mul_mv_q4_K_f32_impl 의 nr0=2 multi-row 이식 (coalesced + activation reuse).
// 1 threadgroup(=1 simdgroup, 32 lane)이 output row 2개 처리. 32 lane 을 ix=lane/8(0..3) ×
// it=lane%8(0..7) 로 나눠 super-block 을 stride-4 순회(coalesced). input(yl/yh/sumy)은 두
// row 가 공유(activation reuse) → super-block 당 1회 load. grid=ceil(N/2), tg=32.
kernel void gemv_q4k_coalesced(
