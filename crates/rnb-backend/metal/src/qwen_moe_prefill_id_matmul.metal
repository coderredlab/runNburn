    ...
    ushort lane [[thread_index_in_simdgroup]])
{
    ...
    const uint expert = tgid.x;
    if (expert >= n_expert) {
        return;
    }

    const uint route_count = n_tokens * n_expert_used;
    uint local = 0u;
    device int *expert_ids = ids + expert * n_tokens;
    for (uint route0 = 0u; route0 < route_count; route0 += SIMD_WIDTH) {
        const uint route = route0 + (uint)lane;
        const uint matches =
            route < route_count && selected_experts[route] == (int)expert ? 1u : 0u;
        const uint local_in_wave = simd_prefix_exclusive_sum(matches);
        const uint wave_count = simd_sum(matches);
        if (matches != 0u) {
            expert_ids[local + local_in_wave] = (int)route;
        }
        local += wave_count;
    }
    if (lane == 0u) {
    ...
    threadgroup half *dst)
{
    ...
    for (uint l = 0; l < 32u; l += 4u) {
        uchar4 packed = *((device const uchar4 *)(qs + l));
        float4 ql = float4(packed & uchar4(0x0Fu));
        float4 qh = float4(packed >> 4u);
        *((threadgroup half4 *)(dst + l)) = half4(d1 * ql - mm1);
        *((threadgroup half4 *)(dst + 32u + l)) = half4(d2 * qh - mm2);
    }
}
    ...
    uint tid)
{
    ...
    for (uint c = 0; c < nchunk; c++) {
        constexpr uint INPUT_VECTOR = 32u;
        constexpr uint INPUT_VECTOR4 = INPUT_VECTOR / 4u;
        for (uint vi = tid; vi < BM * (NK / INPUT_VECTOR); vi += NUM_THREADS) {
            uint local = vi / (NK / INPUT_VECTOR);
            uint kk = (vi % (NK / INPUT_VECTOR)) * INPUT_VECTOR;
            threadgroup half4 *dst =
                (threadgroup half4 *)(input_stage + local * NK + kk);
            if (local0 + local < count) {
                uint route_slot = (uint)ids[expert * N_TOKENS + local0 + local];
                uint input_slot = INPUT_BY_TOKEN ? route_slot / TOP_K : route_slot;
                device const input_t *src =
                    input + input_slot * K + c * NK + kk;
                if (INPUT_BY_TOKEN) {
                    device const float4 *src4 = (device const float4 *)src;
                    for (uint vector = 0; vector < INPUT_VECTOR4; vector++) {
                        dst[vector] = half4(src4[vector]);
                    }
                } else {
                    device const half4 *src4 = (device const half4 *)src;
                    for (uint vector = 0; vector < INPUT_VECTOR4; vector++) {
                        dst[vector] = src4[vector];
                    }
                }
            } else {
                for (uint vector = 0; vector < INPUT_VECTOR4; vector++) {
    ...
    uint tid)
{
    ...
    for (uint c = 0; c < nchunk; c++) {
        ...
        for (uint vi = tid; vi < BM * (NK / INPUT_VECTOR); vi += NUM_THREADS) {
            ...
            if (local0 + local < count) {
                uint slot = (uint)ids[expert * N_TOKENS + local0 + local];
                device const half4 *src = (device const half4 *)(
                    input + slot * K + c * NK + kk);
                for (uint vector = 0; vector < INPUT_VECTOR4; vector++) {
        ...
        for (uint wi = tid; wi < BN * 2u; wi += NUM_THREADS) {
            uint w = wi / 2u;
            uint part = wi & 1u;
            uint row = row0 + w;
            ...
            if (row < N) {
                ...
                if (WIDE_ROWS) {
                    ...
                    float d0 = d * (float)sc[scale_base + is];
                    float d1 = d * (float)sc[scale_base + is + 2u];
                    for (uint offset = 0; offset < 16u; offset += 4u) {
                        uint l = l0 + offset;
                        uchar4 ql0 = *((device const uchar4 *)(ql + ql_base + l));
                        uchar4 ql1 = *((device const uchar4 *)(ql + ql_base + l + 32u));
                        uchar4 qh4 = *((device const uchar4 *)(qh + qh_base + l));
                        uchar4 q0 = high_nibbles
                            ? (ql0 >> 4u) | (((qh4 >> 4u) & uchar4(3u)) << 4u)
                            : (ql0 & uchar4(0x0Fu)) |
                                ((qh4 & uchar4(3u)) << 4u);
                        uchar4 q1 = high_nibbles
                            ? (ql1 >> 4u) | (((qh4 >> 6u) & uchar4(3u)) << 4u)
                            : (ql1 & uchar4(0x0Fu)) |
                                (((qh4 >> 2u) & uchar4(3u)) << 4u);
                        *((threadgroup half4 *)(dst + l)) =
                            half4(d0 * (float4(q0) - 32.0f));
                        *((threadgroup half4 *)(dst + l + 32u)) =
                            half4(d1 * (float4(q1) - 32.0f));
                    }
                } else {
                    float d0 = d * (float)sc[sc_base + is];
                    float d1 = d * (float)sc[sc_base + is + 2u];
                    float d2 = d * (float)sc[sc_base + is + 4u];
                    float d3 = d * (float)sc[sc_base + is + 6u];
                    for (uint offset = 0; offset < 16u; offset += 4u) {
                        uint l = l0 + offset;
                        uchar4 ql0 = *((device const uchar4 *)(ql + ql_base + l));
                        uchar4 ql1 = *((device const uchar4 *)(ql + ql_base + l + 32u));
                        uchar4 qh4 = *((device const uchar4 *)(qh + qh_base + l));
                        uchar4 q1 = (ql0 & uchar4(0x0Fu)) |
                            ((qh4 & uchar4(3u)) << 4u);
                        uchar4 q2 = (ql1 & uchar4(0x0Fu)) |
                            (((qh4 >> 2u) & uchar4(3u)) << 4u);
                        uchar4 q3 = (ql0 >> 4u) |
                            (((qh4 >> 4u) & uchar4(3u)) << 4u);
                        uchar4 q4 = (ql1 >> 4u) |
                            (((qh4 >> 6u) & uchar4(3u)) << 4u);
                        *((threadgroup half4 *)(dst + l)) =
                            half4(d0 * (float4(q1) - 32.0f));
                        *((threadgroup half4 *)(dst + l + 32u)) =
                            half4(d1 * (float4(q2) - 32.0f));
                        *((threadgroup half4 *)(dst + l + 64u)) =
                            half4(d2 * (float4(q3) - 32.0f));
                        *((threadgroup half4 *)(dst + l + 96u)) =
                            half4(d3 * (float4(q4) - 32.0f));
                    }
                }
            } else {
                uint outputs = WIDE_ROWS ? 2u : 4u;
                for (uint segment = 0; segment < outputs; segment++) {
                    for (uint offset = 0; offset < 16u; offset++) {
                        dst[segment * 32u + l0 + offset] = (half)0;
                    }
                }
            }
        }
...
kernel void qwen_moe_chain_cast_compact_slots_f32_f16(
    device const float *input [[buffer(0)]],
    device const uint *tpe [[buffer(1)]],
    device const int *ids [[buffer(2)]],
    device half *out [[buffer(3)]],
    device const uint *block_experts [[buffer(4)]],
    device const uint *block_local0 [[buffer(5)]],
    constant uint &N_TOKENS [[buffer(6)]],
    constant uint &DIM [[buffer(7)]],
    constant uint &INPUT_BY_TOKEN [[buffer(8)]],
    constant uint &TOP_K [[buffer(9)]],
    uint2 tgid [[threadgroup_position_in_grid]],
    uint tid [[thread_index_in_threadgroup]])
{
    const uint column = tgid.x * 256u + tid;
    if (column >= DIM) {
        return;
    }
    const uint block = tgid.y;
    const uint expert = block_experts[block];
    const uint count = tpe[expert];
    const uint min_expert_count = N_TOKENS >= 1024u ? 1u : 16u;
    if (count < min_expert_count) {
        return;
    }
    const uint local0 = block_local0[block];
    const uint local_end = min(local0 + 64u, count);
    for (uint local = local0; local < local_end; ++local) {
        const uint slot = (uint)ids[expert * N_TOKENS + local];
        const uint input_row = INPUT_BY_TOKEN != 0u ? slot / TOP_K : slot;
        out[slot * DIM + column] = (half)input[input_row * DIM + column];
    }
}
...
kernel void qwen_moe_llama_expert_order_reduce_shared_f32(
    device const float *slot_values [[buffer(0)]],
    device const float *weights [[buffer(1)]],
    device const uint *selected_experts [[buffer(2)]],
    device const float *shared_values [[buffer(3)]],
    device const float *shared_weights [[buffer(4)]],
...
#pragma clang fp reassociate(off) contract(off)
    ...
    const float shared_product = shared_values[gid] * shared_weights[token];
    out[gid] = value + shared_product;
}
