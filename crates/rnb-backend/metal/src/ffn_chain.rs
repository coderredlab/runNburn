...
pub(crate) fn empty_f16_buf_with_zeroed_tail(
    ctx: &MetalContext,
    logical_len: usize,
    padded_len: usize,
) -> Retained<ProtocolObject<dyn MTLBuffer>> {
    assert!(padded_len >= logical_len);
    let buffer = empty_f16_buf(ctx, padded_len);
    let tail_len = padded_len - logical_len;
    if tail_len > 0 {
        unsafe {
            std::ptr::write_bytes(
                (buffer.contents().as_ptr() as *mut u16).add(logical_len),
                0,
                tail_len,
            );
        }
...
/// pm132: residual add + RMSNorm fused encode.
/// `hidden += residual` (in-place) 후 `normed = rms_norm(hidden)` 을 단일 dispatch 로.
/// 분리 호출(residual_add + barrier + rms_norm) 대비 dispatch 1 + barrier 1 제거.
/// buffer layout: 0=hidden(in-place), 1=residual, 2=weight, 3=normed, 4=dim, 5=eps.
pub(crate) fn encode_fused_residual_rms_norm(
    ...
    enc.setComputePipelineState(&ctx.fused_residual_rms_norm_pipeline);
    unsafe {
        enc.setBuffer_offset_atIndex(Some(hidden_buf), 0, 0);
        enc.setBuffer_offset_atIndex(Some(residual_buf), 0, 1);
        enc.setBuffer_offset_atIndex(Some(weight_buf), 0, 2);
        enc.setBuffer_offset_atIndex(Some(normed_buf), 0, 3);
...
/// (기존 `encode_rms_norm_at` 은 weight offset 변형이라 별개.)
pub(crate) fn encode_rms_norm_io_offset(
    ctx: &MetalContext,
    enc: &ProtocolObject<dyn MTLComputeCommandEncoder>,
    in_buf: &ProtocolObject<dyn MTLBuffer>,
    in_off_bytes: usize,
...
/// `encode_silu_mul` 의 gate offset(bytes) 변형 — batched GDN 이 lane i 의 z 슬롯
/// (`[B*z_dim]` buffer 의 `i*z_dim`)을 in-place SwiGLU 하도록 한다. up(gated) 은 단일
/// scratch(offset 0). gate=silu(gate)*up 결과가 lane i 슬롯에 남아 ssm_out B-column 입력이 된다.
pub(crate) fn encode_silu_mul_at(
    ctx: &MetalContext,
    enc: &ProtocolObject<dyn MTLComputeCommandEncoder>,
    gate_buf: &ProtocolObject<dyn MTLBuffer>,
    ...
    enc.setComputePipelineState(&ctx.silu_mul_pipeline);
    unsafe {
        enc.setBuffer_offset_atIndex(Some(gate_buf), gate_off_bytes, 0);
        enc.setBuffer_offset_atIndex(Some(up_buf), 0, 1);
        enc.setBuffer_offset_atIndex(Some(dim_buf), 0, 2);
    }
    let tg_width = ctx.silu_mul_pipeline.threadExecutionWidth().max(1);
...
/// `encode_residual_add` 의 hidden/down offset(bytes) 변형 — batched 경로가 lane i 의
/// hidden 슬롯(`shared_hidden` 의 `i*hidden_dim`)에 lane i 의 projection 을 더하도록 한다.
pub(crate) fn encode_residual_add_at(
    ctx: &MetalContext,
    enc: &ProtocolObject<dyn MTLComputeCommandEncoder>,
    hidden_buf: &ProtocolObject<dyn MTLBuffer>,
    hidden_off_bytes: usize,
    down_buf: &ProtocolObject<dyn MTLBuffer>,
    down_off_bytes: usize,
    dim: usize,
) {
    enc.setComputePipelineState(&ctx.residual_add_pipeline);
    set_u32_bytes(enc, dim as u32, 2);
    unsafe {
        enc.setBuffer_offset_atIndex(Some(hidden_buf), hidden_off_bytes, 0);
        enc.setBuffer_offset_atIndex(Some(down_buf), down_off_bytes, 1);
    }
    let tg_width = ctx.residual_add_pipeline.threadExecutionWidth().max(1);
    let grid = MTLSize {
        width: dim.div_ceil(tg_width),
...
#[derive(Debug, Clone, Copy, PartialEq, Eq)]
pub(crate) enum QwenMoeLlamaIdStage {
    Map,
    Gate,
    Up,
    Activation,
...
impl QwenMoeLlamaIdStageSampler {
    ...
    pub(crate) fn end(&mut self, enc: &ProtocolObject<dyn MTLComputeCommandEncoder>) {
...
impl QwenMoeLlamaIdCarrier {
    ...
    pub(crate) fn encode_profile_hidden_copy(
        &self,
        ctx: &MetalContext,
        enc: &ProtocolObject<dyn MTLComputeCommandEncoder>,
...
pub(crate) fn qwen_moe_llama_id_map0_encode(
    ...
    if simd_width != 32 || simd_width > pipeline.maxTotalThreadsPerThreadgroup() {
        return Err(QwenMoeLlamaIdError::DispatchGridOverflow);
    }
    let n_tokens =
...
fn encode_qwen_moe_chain_cast_large_slots_f32_f16(
    ...
    if compact_blocks {
        ...
        unsafe {
            ...
            enc.setBuffer_offset_atIndex(Some(block_meta), 0, 4);
            enc.setBuffer_offset_atIndex(Some(block_meta), block_local_offset, 5);
        }
        set_u32_bytes(enc, n_tokens_u32, 6);
        set_u32_bytes(enc, dim_u32, 7);
        set_u32_bytes(enc, u32::from(input_by_token), 8);
        set_u32_bytes(enc, top_k_u32, 9);
        unsafe {
            enc.dispatchThreadgroupsWithIndirectBuffer_indirectBufferOffset_threadsPerThreadgroup(
                block_meta,
                indirect_args_offset,
...
#[allow(clippy::too_many_arguments)]
fn qwen_moe_llama_expert_order_reduce_shared_f32_encode(
    ctx: &MetalContext,
    enc: &ProtocolObject<dyn MTLComputeCommandEncoder>,
    slot_values: &ProtocolObject<dyn MTLBuffer>,
    weights: &ProtocolObject<dyn MTLBuffer>,
    selected_experts: &ProtocolObject<dyn MTLBuffer>,
    shared_values: &ProtocolObject<dyn MTLBuffer>,
    shared_weights: &ProtocolObject<dyn MTLBuffer>,
    output: &ProtocolObject<dyn MTLBuffer>,
    n_tokens: usize,
    n_rank: usize,
    n_rows: usize,
    ...
    let output_bytes =
        qwen_moe_llama_id_checked_bytes(&[n_tokens, n_rows], std::mem::size_of::<f32>())?;
    let shared_weight_bytes =
        qwen_moe_llama_id_checked_bytes(&[n_tokens], std::mem::size_of::<f32>())?;
    if slot_values.length() < slot_bytes
        || weights.length() < route_bytes
        || selected_experts.length() < selected_expert_bytes
        || shared_values.length() < output_bytes
        || shared_weights.length() < shared_weight_bytes
        || output.length() < output_bytes
    {
        return Err(QwenMoeLlamaIdError::InvalidShape);
    }
    ...
    unsafe {
        ...
        enc.setBuffer_offset_atIndex(Some(weights), 0, 1);
        enc.setBuffer_offset_atIndex(Some(selected_experts), 0, 2);
        enc.setBuffer_offset_atIndex(Some(shared_values), 0, 3);
        enc.setBuffer_offset_atIndex(Some(shared_weights), 0, 4);
        enc.setBuffer_offset_atIndex(Some(output), 0, 5);
    }
    set_u32_bytes(enc, n_tokens_u32, 6);
...
#[derive(Debug, Clone, Copy, PartialEq, Eq)]
pub(crate) enum QwenMoeProfileStop {
    Route,
    SparseGate,
    SparseUp,
    SparseActivation,
...
fn qwen_moe_llama_prefill_encode_inner(
    ...
    profile_stop: Option<QwenMoeProfileStop>,
    ...
    if chain_sparse_exact {
        encode_qwen_moe_chain_cast_large_slots_f32_f16(
            ...
            carrier.block_indirect_offset + QWEN_MOE_LLAMA_ID_HIDDEN_CAST_INDIRECT_OFFSET,
            compact_id_blocks,
            &carrier.down_rank_dev,
            carrier.n_tokens,
            carrier.n_expert,
    ...
    if profile_stop == Some(QwenMoeProfileStop::SparseGate) {
        return Ok(trace);
    }
    ...
    crate::compute::chain_barrier(ctx, enc);
    if profile_stop == Some(QwenMoeProfileStop::SparseUp) {
        return Ok(trace);
    }
    ...
    crate::compute::chain_barrier(ctx, enc);
    if profile_stop == Some(QwenMoeProfileStop::SparseActivation) {
        return Ok(trace);
    }
    if chain_sparse_exact {
        ...
        encode_qwen_moe_chain_cast_large_slots_f32_f16(
            ...
            &carrier.block_meta_dev,
            carrier.block_local_offset,
            carrier.block_indirect_offset + QWEN_MOE_LLAMA_ID_FFN_CAST_INDIRECT_OFFSET,
            compact_id_blocks,
            &carrier.up_dev,
            carrier.n_tokens,
            carrier.n_expert,
    ...
    crate::compute::chain_barrier(ctx, enc);
    if profile_stop == Some(QwenMoeProfileStop::SparseDown) {
        return Ok(trace);
    }
    ...
    trace.up_dispatches += 1;
    crate::compute::chain_barrier(ctx, enc);
    if profile_stop == Some(QwenMoeProfileStop::SharedGateUp) {
        return Ok(trace);
    }
    ...
    trace.activation_dispatches += 1;
    crate::compute::chain_barrier(ctx, enc);
    if profile_stop == Some(QwenMoeProfileStop::SharedActivation) {
        return Ok(trace);
    }
    qwen_moe_llama_id_stage_begin(&mut stage_sampler, enc, QwenMoeLlamaIdStage::Down);
    qwen_moe_shared_mul_mm_quant_f32_encode(
    ...
    trace.down_dispatches += 1;
    crate::compute::chain_barrier(ctx, enc);
    if profile_stop == Some(QwenMoeProfileStop::SharedDown) {
        return Ok(trace);
    }
    // All norm readers have completed before this point. The sparse reduction
    // may now overwrite the norm_all/sparse_moe_out alias. Diagnostic snapshots
    // keep the split reductions; production writes the combined result once.
    if let Some(snapshot) = raw_sparse_snapshot {
        ...
        qwen_moe_llama_id_stage_begin(&mut stage_sampler, enc, QwenMoeLlamaIdStage::Reduce);
        qwen_moe_llama_expert_order_reduce_shared_f32_encode(
            ctx,
            enc,
            &carrier.down_rank_dev,
            &carrier.route_weights_dev,
...
pub(crate) struct QwenMoePrefillWeightBuffers {
    pub gate_all: Retained<ProtocolObject<dyn MTLBuffer>>,
    pub gate_offset: usize,
    pub up_all: Retained<ProtocolObject<dyn MTLBuffer>>,
    pub up_offset: usize,
    pub down_all: Retained<ProtocolObject<dyn MTLBuffer>>,
    pub down_offset: usize,
    pub shared_gate: Retained<ProtocolObject<dyn MTLBuffer>>,
    pub shared_gate_offset: usize,
    pub shared_up: Retained<ProtocolObject<dyn MTLBuffer>>,
    pub shared_up_offset: usize,
    pub shared_down: Retained<ProtocolObject<dyn MTLBuffer>>,
    pub shared_down_offset: usize,
}
...
fn encode_qwen_moe_prefill_layer_ops_inner(
    ...
    carrier: &QwenMoeLlamaIdCarrier,
    moe: QwenMoePrefillBackendSpecRef<'_>,
    weights: &QwenMoePrefillWeightBuffers,
    hidden_residual: &ProtocolObject<dyn MTLBuffer>,
    hidden_out: &ProtocolObject<dyn MTLBuffer>,
    raw_sparse_snapshot: Option<&ProtocolObject<dyn MTLBuffer>>,
    ...
    let trace = qwen_moe_llama_prefill_encode_inner(
        ...
        moe.sparse_quant,
        &weights.shared_gate,
        weights.shared_gate_offset,
        &weights.shared_up,
        weights.shared_up_offset,
        &weights.shared_down,
        weights.shared_down_offset,
        moe.shared_quant,
        raw_sparse_snapshot,
        true,
        None,
...
pub(crate) fn encode_qwen_moe_prefill_profile_prefix(
    ctx: &MetalContext,
    enc: &ProtocolObject<dyn MTLComputeCommandEncoder>,
    carrier: &QwenMoeLlamaIdCarrier,
    moe: QwenMoePrefillBackendSpecRef<'_>,
    ...
    if stop == QwenMoeProfileStop::Route {
        return Ok(());
    }
    qwen_moe_llama_prefill_encode_inner(
        ctx,
...
impl QwenMoeDecodeCarrier {
    pub(crate) fn new(ctx: &MetalContext, slots: usize, n_embd: usize, n_ff: usize) -> Self {
        ...
        Self {
            ...
            n_embd,
            n_ff,
            input_dev: empty_f32_buf(ctx, slots * n_embd),
    ...
    fn upload_offsets(&self, gate_off: &[u32], up_off: &[u32], down_off: &[u32]) {
        self.upload_gate_offsets(gate_off);
        self.upload_up_offsets(up_off);
        self.upload_down_offsets(down_off);
    }

    fn store_offset(buf: &ProtocolObject<dyn MTLBuffer>, value: u32) {
        unsafe {
...
/// pm116 M2: GLM MoE prefill 스테이지-major full scratch (grow-only).
/// gate/up: `[seq*slots*n_ff]`, down: `[seq*slots*n_embd]`. 스테이지-major 재배열로
/// 텐서별 pread 를 이전 스테이지 GPU 실행과 겹치기 위한 전제.
pub(crate) struct GlmPrefillStageScratch {
    pub(crate) gate_full: Retained<ProtocolObject<dyn MTLBuffer>>,
    pub(crate) up_full: Retained<ProtocolObject<dyn MTLBuffer>>,
    pub(crate) down_full: Retained<ProtocolObject<dyn MTLBuffer>>,
    seq_cap: usize,
}

impl GlmPrefillStageScratch {
    pub(crate) fn new(
        ctx: &MetalContext,
        seq_len: usize,
        slots: usize,
        ...
        Self {
            gate_full: empty_f32_buf(ctx, seq_len * slots * n_ff),
            up_full: empty_f32_buf(ctx, seq_len * slots * n_ff),
            down_full: empty_f32_buf(ctx, seq_len * slots * n_embd),
            seq_cap: seq_len,
        }
    }

    pub(crate) fn fits(&self, seq_len: usize) -> bool {
        seq_len <= self.seq_cap
    }
}

/// pm116 M2 스테이지 1/2: gate 또는 up 전 토큰 encode 를 단일 command buffer 로
/// commit 하고 **wait 없이** 반환 — GPU 실행 동안 caller 가 다음 텐서 pread.
/// out_full 레이아웃은 토큰별 `[slots*n_ff]` 세그먼트 (기존 per-token scratch 와
/// 같은 커널·같은 slot 인덱싱, 출력 base offset 만 토큰별로 이동 → 산술 불변).
#[allow(clippy::too_many_arguments)]
pub(crate) fn glm_moe_prefill_iq_stage_gate_up_dispatch(
    ctx: &MetalContext,
    carrier: &QwenMoeDecodeCarrier,
    out_full: &ProtocolObject<dyn MTLBuffer>,
    ...
    let shared_off_buf = if is_gate {
        &carrier.gate_off_buf
    } else {
        &carrier.up_off_buf
    };
    ...
    for token in 0..seq_len {
        ...
        let out_base = token * slots * n_ff * f32_bytes;
        encode_glm_moe_decode_iq_selected_slots_at(
            pipeline,
            &enc,
            &w[base..base + sparse_slots],
            &off[base..base + sparse_slots],
            input_buf,
            in_off,
            out_full,
            out_base,
        ...
        encode_shared(
            ...
            &carrier.n_ff_buf,
            &carrier.n_embd_buf,
            &shared_off_buf[shared_slot],
            in_off,
            out_base + shared_slot * n_ff * f32_bytes,
            n_ff,
        );
    }
    enc.endEncoding();
...
/// pm116 M2 스테이지 3: silu(전체) + down + reduce — commit 후 wait, readback.
#[allow(clippy::too_many_arguments)]
pub(crate) fn glm_moe_prefill_iq_stage_tail_dispatch(
    ctx: &MetalContext,
    carrier: &QwenMoeDecodeCarrier,
    scratch: &GlmPrefillStageScratch,
    seq_len: usize,
    sparse_slots: usize,
    ...
    let total_ff_buf = u32_buf(ctx, (seq_len * slots * n_ff) as u32);
    let zero_off_buf = u32_buf(ctx, 0);
    let down_pipeline = if down_iq4xs {
    ...
    for token in 0..seq_len {
        let base = token * slots;
        let ff_base = token * slots * n_ff * f32_bytes;
        let embd_base = token * slots * n_embd * f32_bytes;
        encode_glm_moe_decode_iq_selected_slots_at(
            down_pipeline,
            &enc,
        ...
        if shared_down_q8_0 {
            crate::compute::encode_gemv_q8_0_at(
                ...
                &scratch.gate_full,
                ff_base + shared_slot * n_ff * f32_bytes,
                &scratch.down_full,
                embd_base + shared_slot * n_embd * f32_bytes,
                &carrier.n_embd_buf,
                &carrier.n_ff_buf,
                &zero_off_buf,
                n_embd,
...
/// B-fusion(milestone 5) fused-shared scratch: 한 layer carrier 가 batch 크기별로 1회
/// alloc 후 verify step 마다 재사용하는 all-lane 중간 버퍼(column-major-by-lane).
struct QwenMoeBatchScratch {
    normed_all: Retained<ProtocolObject<dyn MTLBuffer>>, // [batch*n_embd]
    gate_all: Retained<ProtocolObject<dyn MTLBuffer>>,   // [batch*n_ff]
    up_all: Retained<ProtocolObject<dyn MTLBuffer>>,     // [batch*n_ff]
    down_all: Retained<ProtocolObject<dyn MTLBuffer>>,   // [batch*n_embd]
}
...
pub(crate) struct QwenMoeDecodeChainCarrier {
    ...
    slots_buf: Retained<ProtocolObject<dyn MTLBuffer>>,
    total_ff_buf: Retained<ProtocolObject<dyn MTLBuffer>>,
    // B-fusion(milestone 5): sparse-only reduce 의 silu total(n_used*n_ff) 상수.
    sparse_total_ff_buf: Retained<ProtocolObject<dyn MTLBuffer>>,
    per_gate_buf: Retained<ProtocolObject<dyn MTLBuffer>>,
    per_up_buf: Retained<ProtocolObject<dyn MTLBuffer>>,
    per_down_buf: Retained<ProtocolObject<dyn MTLBuffer>>,
    shared_expert_id_buf: Retained<ProtocolObject<dyn MTLBuffer>>,
...
impl QwenMoeDecodeChainCarrier {
    ...
    pub(crate) fn new(
        ...
        Self {
            ...
            slots_buf: u32_buf(ctx, slots as u32),
            total_ff_buf: u32_buf(ctx, (slots * n_ff) as u32),
            sparse_total_ff_buf: u32_buf(ctx, (n_used * n_ff) as u32),
            per_gate_buf: u64_buf(ctx, per_gate as u64),
            per_up_buf: u64_buf(ctx, per_up as u64),
            per_down_buf: u64_buf(ctx, per_down as u64),
            shared_expert_id_buf: u32_buf(ctx, shared_expert_id),
            ...
            batch_scratch: RefCell::new(HashMap::new()),
        }
    }

    /// batch 크기별 fused-shared scratch 를 보장(없으면 alloc)하고 핸들 clone 을 돌려준다.
    fn ensure_batch_scratch(&self, ctx: &MetalContext, batch: usize) -> QwenMoeBatchScratch {
        let mut cache = self.batch_scratch.borrow_mut();
        let entry = cache.entry(batch).or_insert_with(|| QwenMoeBatchScratch {
            normed_all: empty_f32_buf(ctx, batch * self.n_embd),
            gate_all: empty_f32_buf(ctx, batch * self.n_ff),
            up_all: empty_f32_buf(ctx, batch * self.n_ff),
            down_all: empty_f32_buf(ctx, batch * self.n_embd),
        });
        QwenMoeBatchScratch {
            normed_all: entry.normed_all.clone(),
            gate_all: entry.gate_all.clone(),
            up_all: entry.up_all.clone(),
            down_all: entry.down_all.clone(),
        }
    }
...
fn encode_qwen_moe_decode_down_slots(
    ...
    // All decode down quants (Q4_K/Q5_K/Q6_K) now share the 2-simd-group NR=2 slot layout.
    let simdgroups = qwen_moe_decode_simdgroups();
    enc.dispatchThreadgroups_threadsPerThreadgroup(
        MTLSize {
            width: n.div_ceil(2 * simdgroups),
...
fn encode_glm_moe_decode_iq_selected_slots(
    ...
    encode_glm_moe_decode_iq_selected_slots_at(
        pipeline, enc, weights, offsets, input, 0, out, 0, n_buf, k_buf, slots_buf, n, slots,
    );
}
...
fn encode_glm_moe_decode_iq_selected_slots_at(
    ...
    unsafe {
        ...
        enc.setBuffer_offset_atIndex(Some(out), out_byte_offset, 10);
        enc.setBuffer_offset_atIndex(Some(n_buf), 0, 11);
        enc.setBuffer_offset_atIndex(Some(k_buf), 0, 12);
        enc.setBuffer_offset_atIndex(Some(slots_buf), 0, 13);
    }
...
/// reduce+residual fused (chain decode): down_slots 를 route 가중 합해 hidden 의
/// byte offset 위치에 직접 accumulate. reduce_slots + residual_add 두 dispatch 를
/// 하나로 합쳐 그 사이 chain barrier 를 제거한다. moe_out scratch 를 거치지 않는다.
fn encode_qwen_moe_decode_reduce_add_slots(
    ctx: &MetalContext,
    enc: &ProtocolObject<dyn MTLComputeCommandEncoder>,
    down: &ProtocolObject<dyn MTLBuffer>,
    route_weights: &ProtocolObject<dyn MTLBuffer>,
    hidden: &ProtocolObject<dyn MTLBuffer>,
    hidden_byte_offset: usize,
    n_buf: &ProtocolObject<dyn MTLBuffer>,
    slots_buf: &ProtocolObject<dyn MTLBuffer>,
    n: usize,
) {
    enc.setComputePipelineState(ctx.qwen_moe_decode_reduce_add_slots_pipeline());
    unsafe {
        enc.setBuffer_offset_atIndex(Some(down), 0, 0);
        enc.setBuffer_offset_atIndex(Some(route_weights), 0, 1);
        enc.setBuffer_offset_atIndex(Some(hidden), hidden_byte_offset, 2);
...
fn encode_qwen_moe_decode_reduce_slots_at(
    ...
    unsafe {
        enc.setBuffer_offset_atIndex(Some(down), down_byte_offset, 0);
...
pub(crate) fn qwen_moe_decode_chain_encode_router_logits(
    ...
    down_shared_off: u32,
    hidden_off_bytes: usize,
    // pm132: Some이면 hidden += residual + rms_norm 을 fused kernel 1개로.
    // None이면 기존 rms_norm 단독. decode 경로(hidden_off=0) 전용.
    residual_buf: Option<&ProtocolObject<dyn MTLBuffer>>,
) {
...
pub(crate) fn qwen_moe_decode_chain_encode_route(
    ...
    qwen_moe_decode_chain_encode_router_logits(
        ...
        down_shared_off,
        hidden_off_bytes,
        None, // pm132: residual fusion 미사용 (caller가 별도 처리)
    );
...
pub(crate) fn qwen_moe_decode_chain_encode_after_route(
    ...
    // shared_q8_0 경로: sparse gate/up 를 n_used slot 으로 제한해 shared slot 을 건드리지
    // 않게 하고(WAW 제거), shared Q8_0 gate/up GEMV 를 같은 그룹으로 barrier 없이 병렬
    // dispatch 한다. 표준(Q4_K shared)은 shared 가 sparse 커널의 마지막 slot 이라 slots 유지.
    let gate_up_slots = if shared_q8_0 {
        carrier.n_used
    } else {
        carrier.slots
    };
    ...
    if shared_q8_0 {
        // Q8_0 shared expert: the fused Q4_K slot kernels above wrote garbage into the
        // shared slot (Q8_0 buffer >= Q4_K read size so no OOB). Recompute the shared slot
        // gate/up with correct Q8_0 GEMVs here; the silu slots stage below then silus it.
        let shared_slot = carrier.n_used;
        let f32b = std::mem::size_of::<f32>();
        crate::compute::encode_gemv_q8_0_at(
            ctx,
            enc,
            gate_shared_w,
            gate_shared_off as usize,
            &carrier.normed_dev,
            0,
            &carrier.gate_dev,
            shared_slot * carrier.n_ff * f32b,
            &carrier.n_ff_buf,
            &carrier.n_embd_buf,
    ...
    if shared_q8_0 {
        // Q8_0 shared expert down: silu'd gate_dev[shared_slot] -> down_dev[shared_slot],
        // overwriting the garbage the fused down kernel wrote. Reduce below applies the
        // shared route weight to it correctly.
        let shared_slot = carrier.n_used;
        let f32b = std::mem::size_of::<f32>();
        crate::compute::encode_gemv_q8_0_at(
            ctx,
            enc,
            down_shared_w,
            down_shared_off as usize,
            &carrier.gate_dev,
            shared_slot * carrier.n_ff * f32b,
            &carrier.down_dev,
            shared_slot * carrier.n_embd * f32b,
            &carrier.n_embd_buf,
            &carrier.n_ff_buf,
            &carrier.zero_off_buf,
            carrier.n_embd,
    ...
    encode_qwen_moe_decode_reduce_add_slots(
        ctx,
        enc,
        &carrier.down_dev,
        &carrier.route_weights_dev,
...
/// B-fusion(milestone 5) sparse-only after-route: `qwen_moe_decode_chain_encode_after_route`
/// 와 동일하되 **shared slot 을 제외**(slots = n_used)하고 Q8_0 shared 재계산도 생략한다.
/// shared expert 는 batched fused-shared 경로가 B-column 으로 1회 읽어 별도 처리한다.
/// gate/up = Q4_K sparse slot 커널, down = down_quant, reduce/residual 는 sparse slot 만.
#[allow(clippy::too_many_arguments)]
fn qwen_moe_decode_chain_encode_after_route_sparse(
    ctx: &MetalContext,
    enc: &ProtocolObject<dyn MTLComputeCommandEncoder>,
    carrier: &QwenMoeDecodeChainCarrier,
    normed_dev: &ProtocolObject<dyn MTLBuffer>,
...
/// B-fusion(milestone 5) shared-expert scatter-add wrapper: lane 별로 route weight
/// (sigmoid(dot(normed, shared_input_scale)))를 route_shared 와 동일하게 재현하고
/// hidden[lane] += rw * down_shared[lane]. grid = batch threadgroup, tg = 256.
fn encode_qwen_moe_decode_shared_add(
    ctx: &MetalContext,
    enc: &ProtocolObject<dyn MTLComputeCommandEncoder>,
    hidden: &ProtocolObject<dyn MTLBuffer>,
    normed_all: &ProtocolObject<dyn MTLBuffer>,
    down_shared_all: &ProtocolObject<dyn MTLBuffer>,
    shared_input_scale_buf: &ProtocolObject<dyn MTLBuffer>,
    n_embd_buf: &ProtocolObject<dyn MTLBuffer>,
    batch: usize,
) {
    enc.setComputePipelineState(ctx.qwen_moe_decode_shared_add_pipeline());
    unsafe {
        enc.setBuffer_offset_atIndex(Some(hidden), 0, 0);
        enc.setBuffer_offset_atIndex(Some(normed_all), 0, 1);
        enc.setBuffer_offset_atIndex(Some(down_shared_all), 0, 2);
        enc.setBuffer_offset_atIndex(Some(shared_input_scale_buf), 0, 3);
        enc.setBuffer_offset_atIndex(Some(n_embd_buf), 0, 4);
    }
    enc.dispatchThreadgroups_threadsPerThreadgroup(
        MTLSize {
            width: batch,
...
/// B-fusion(milestone 5) batched MoE FFN: router+sparse expert 는 lane 별(routing 발산),
/// shared expert(dense, always-active)는 B-column GEMV 로 weight 1회 읽기로 amortize.
///   Phase 0: lane 별 rms_norm(hidden) → normed_all[lane]
///   Phase 1: lane 별 router + sparse expert(slots=n_used) → reduce → hidden[lane] 잔차
///   Phase 2: shared gate/up(bcol) → silu → down(bcol) → hidden[lane] += rw * shared
/// batch==1 이어도(collect verify) 정확. 결과는 lane 별 단일-토큰 MoE 와 rel<3e-3.
#[allow(clippy::too_many_arguments)]
pub(crate) fn qwen_moe_decode_chain_encode_batched_shared_fused(
    ctx: &MetalContext,
    enc: &ProtocolObject<dyn MTLComputeCommandEncoder>,
    carrier: &QwenMoeDecodeChainCarrier,
    shared_hidden: &ProtocolObject<dyn MTLBuffer>,
    ...
    gate_shared_off_buf: &ProtocolObject<dyn MTLBuffer>,
    up_shared_off_buf: &ProtocolObject<dyn MTLBuffer>,
    down_shared_off_buf: &ProtocolObject<dyn MTLBuffer>,
    silu_total_buf: &ProtocolObject<dyn MTLBuffer>,
) {
    let n_embd = carrier.n_embd;
    let n_ff = carrier.n_ff;
    let f32b = std::mem::size_of::<f32>();

    // 진단/안전 A/B: RNB_METAL_MOE_SHARED_FUSE=0 → shared 도 lane 별(단일-토큰과 동일).
    // 기본(unset|!=0)은 shared B-column fused. shared fusion 순이익 정량화용.
    if std::env::var_os("RNB_METAL_MOE_SHARED_FUSE").is_some_and(|v| v == "0") {
        for lane in 0..batch {
    ...
    let scratch = carrier.ensure_batch_scratch(ctx, batch);

    // Phase 0: lane 별 rms_norm → normed_all[lane] (shared expert 입력, pre-MoE hidden 기준).
    for lane in 0..batch {
        encode_rms_norm_io_offset(
            ctx,
            enc,
    ...
    // down = Q8_0|down_quant. weight 를 B lane 에 1회 읽기.
    let gate_code = if shared_q8_0 { 3u8 } else { 0 };
    let down_code = if shared_q8_0 { 3u8 } else { down_quant };
    crate::compute::encode_gemv_quant_bcol(
        ctx,
        enc,
        gate_code,
    ...
    crate::compute::chain_barrier(ctx, enc);
    encode_qwen_moe_decode_shared_add(
        ctx,
        enc,
        shared_hidden,
        &scratch.normed_all,
...
pub(crate) fn glm_moe_prefill_iq_batch_dispatch(
    ...
    let cmd = ctx.queue.commandBuffer().expect("command buffer");
    let enc = cmd.computeCommandEncoder().expect("compute encoder");
    let gate_up_pipeline = if select.gate_up_iq3xxs {
        ctx.glm_moe_decode_iq3xxs_selected_slots_pipeline()
    } else if select.gate_up_iq2s {
        ctx.glm_moe_decode_iq2s_selected_slots_pipeline()
    } else {
        ctx.glm_moe_decode_iq2xxs_selected_slots_pipeline()
    };
    ...
    let encode_shared_gate_up = if select.shared_gate_up_q8_0 {
        crate::compute::encode_gemv_q8_0_auto_offset
    } else if select.shared_gate_up_q6k {
        crate::compute::encode_gemv_q6k_auto_offset
    } else {
        crate::compute::encode_gemv_q5k_auto_offset
    };
    ...
    enc.endEncoding();
    let encode_ms = mark.elapsed().as_secs_f64() * 1000.0;
    mark = std::time::Instant::now();
    cmd.commit();
    cmd.waitUntilCompleted();
    if profiling {
        let gpu_ms = mark.elapsed().as_secs_f64() * 1000.0;
        mark = std::time::Instant::now();
        let out = readback(&out_buf, seq_len * n_embd);
        eprintln!(
            "[glm-moe-prefill-profile] encode={encode_ms:.1} gpu={gpu_ms:.1} readback={:.1}",
...
fn encode_gemma_o_ffn_chain(
    ...
    output_scale_buf: Option<&ProtocolObject<dyn MTLBuffer>>,
    gate_w_buf: &ProtocolObject<dyn MTLBuffer>,
    gate_off_buf: &ProtocolObject<dyn MTLBuffer>,
    up_w_buf: &ProtocolObject<dyn MTLBuffer>,
    up_off_buf: &ProtocolObject<dyn MTLBuffer>,
    ...
    if let Some(output_scale_buf) = output_scale_buf {
        encode_residual_add_scaled(
            ctx,
            enc,
            &o_carrier.hidden_dev,
            ffn_residual,
...
/// Gemma layer tail followed by the next layer's attention RMSNorm and raw QKV
/// projections. Host KVarN attention stays between these carriers; this only
/// removes the tail/front command-buffer boundary.
#[allow(clippy::too_many_arguments)]
pub(crate) fn gemma_o_ffn_qkv_chain_dispatch(
    ctx: &MetalContext,
    o_carrier: &OChainCarrier,
    ffn_carrier: &FfnCarrier,
    qkv_carrier: &QkvCarrier,
    attn_out: &[f32],
    hidden: &mut [f32],
    o_w_buf: &ProtocolObject<dyn MTLBuffer>,
    o_off_buf: &ProtocolObject<dyn MTLBuffer>,
    ...
    down_is_q6k: bool,
    output_scale_buf: Option<&ProtocolObject<dyn MTLBuffer>>,
    next_attn_norm_w_buf: &ProtocolObject<dyn MTLBuffer>,
    next_q_w_buf: &ProtocolObject<dyn MTLBuffer>,
    next_q_off_buf: &ProtocolObject<dyn MTLBuffer>,
    next_k_w_buf: &ProtocolObject<dyn MTLBuffer>,
    next_k_off_buf: &ProtocolObject<dyn MTLBuffer>,
    next_v_w_buf: &ProtocolObject<dyn MTLBuffer>,
    next_v_off_buf: &ProtocolObject<dyn MTLBuffer>,
    next_v_from_k: bool,
    next_v_is_q6k: bool,
) -> (Vec<f32>, Vec<f32>, Vec<f32>) {
    o_carrier.upload(attn_out, hidden);
    ...
    encode_rms_norm(
        ...
        &enc,
        &o_carrier.hidden_dev,
        next_attn_norm_w_buf,
        &qkv_carrier.norm_dev,
        &ffn_carrier.hdim_buf,
        &ffn_carrier.eps_buf,
    );
    crate::compute::encode_gemv_q4k_auto(
        ...
        &qkv_carrier.q_dev,
        &qkv_carrier.q_n_buf,
        &qkv_carrier.k_hidden_buf,
        next_q_off_buf,
        qkv_carrier.q_out_dim,
    );
    crate::compute::encode_gemv_q4k_auto(
        ctx,
        &enc,
    ...
    readback_into(&o_carrier.hidden_dev, hidden);
    let q = readback(&qkv_carrier.q_dev, qkv_carrier.q_out_dim);
    let k = readback(&qkv_carrier.k_dev, qkv_carrier.kv_dim);
    let v = if next_v_from_k {
        k.clone()
    } else {
        readback(&qkv_carrier.v_dev, qkv_carrier.kv_dim)
    };
    (q, k, v)
}
...
mod qwen_moe_id_plan_tests {
    ...
    fn qwen_moe_llama_id_plan_uses_checked_aligned_component_sizes() {
        ...
        let (block_meta_bytes, _, _) =
            qwen_moe_llama_id_block_meta_layout(plan.n_tokens, plan.n_expert_used).unwrap();
        assert_eq!(plan.block_meta_bytes, block_meta_bytes);
...
mod tests {
    ...
    #[test]
    #[ignore = "requires a TensorOps-capable Metal device"]
    fn qwen_moe_chain_hybrid_mixed_15_129_matches_small_id_and_large_generic_bits() {
        let ctx =
            crate::compute::build_metal_context_with_opts(false, true).expect("no metal device");
        if !ctx.tensorops_capable {
            eprintln!("skipping: Metal tensor operations unavailable");
        ...
        const N_TOKENS: usize = 144;
        const N_EXPERT: usize = 2;
        const N_EXPERT_USED: usize = 1;
        const DIM: usize = 256;
        const SMALL_COUNT: usize = 15;
        const LARGE_COUNT: usize = 129;
        ...
        for quant in [QwenMoeLlamaIdQuant::Q4K, QwenMoeLlamaIdQuant::Q6K] {
            ...
            let compact_cast_out = empty_f16_buf(&ctx, N_TOKENS * DIM);
            let dense_cast_out = empty_f16_buf(&ctx, N_TOKENS * DIM);
            ...
            command.waitUntilCompleted();
            assert_eq!(command.status(), MTLCommandBufferStatus::Completed);
            let compact_cast = unsafe {
                std::slice::from_raw_parts(
                    compact_cast_out.contents().as_ptr() as *const u16,
                    N_TOKENS * DIM,
                )
            };
            let dense_cast = unsafe {
                std::slice::from_raw_parts(
                    dense_cast_out.contents().as_ptr() as *const u16,
                    N_TOKENS * DIM,
                )
            };
            assert_eq!(compact_cast, dense_cast, "compact slot cast");
    ...
    fn qwen_moe_llama_prefill_layer_chain_route_rank_boundaries_match_cpu_bits() {
        ...
        qwen_route_parity_assert_case(&ctx, 2, 256, 256, 8);
    }
    ...
    /// Per-kernel GPU-time profile of the single-token Qwen MoE decode chain at real
    /// Qwen3.6-35B-A3B dims (n_embd=2048, n_ff=512, n_expert=256, n_used=8, slots=9).
    /// Reports route/gate/up/silu/down/reduce/residual[/shared Q8_0] isolated GPU-time
    /// (own command buffer, cold experts rotated across the full 256-expert arena,
    /// median of samples), the fully-fused single-command-buffer after_route chain
    /// GPU-time, and a memory roofline (peak 307 GB/s). Diagnostic only, no assertions.
    /// Gate: RNB_METAL_MOE_DECODE_PROF unset skips; value "std"/"unsloth" runs one config,
    /// anything else runs both.
    #[test]
    #[ignore = "requires a Metal device; MoE decode per-kernel GPU-time profile"]
    fn qwen_moe_decode_prof() {
        use objc2_metal::MTLCommandBuffer;
        let select = std::env::var("RNB_METAL_MOE_DECODE_PROF").unwrap_or_else(|_| "both".into());
        let ctx = build_metal_context().expect("no metal device");
        ...
        let n_used = 8usize;
        let slots = n_used + 1;
        let shared_id = n_expert as u32;
        let eps = 1e-6f32;
        let peak_gbps = 307.0f64;
        let samples = 25usize;
        let warmup = 5usize;
        let f32b = std::mem::size_of::<f32>();

        // per-expert byte sizes (block layout per quant)
        let per_gate = n_ff * (n_embd / 256) * 144; // Q4_K gate/up [n_ff, n_embd]
        let per_up = per_gate;
        let per_down_q6k = n_embd * (n_ff / 256) * 210; // Q6_K down [n_embd, n_ff]
        let per_down_q5k = n_embd * (n_ff / 256) * 176; // Q5_K down
        let shared_gate_q8 = n_ff * (n_embd / 32) * 34; // Q8_0 shared gate/up
        let shared_down_q8 = n_embd * (n_ff / 32) * 34; // Q8_0 shared down

        // arenas (byte content irrelevant for straight-line GEMV timing)
        let fill = |len: usize| vec![0x3Cu8; len];
        let gate_sparse = fill(n_expert * per_gate);
        let up_sparse = fill(n_expert * per_up);
        let down_sparse_q6k = fill(n_expert * per_down_q6k);
        let down_sparse_q5k = fill(n_expert * per_down_q5k);
        let gate_shared_q4k = fill(per_gate);
        let up_shared_q4k = fill(per_up);
        let down_shared_q6k = fill(per_down_q6k);
        let gate_shared_q8v = fill(shared_gate_q8);
        let up_shared_q8v = fill(shared_gate_q8);
        let down_shared_q8v = fill(shared_down_q8);
        let router_w = fill(n_expert * n_embd * f32b);

        let (gate_sparse_buf, gate_sparse_off) = crate::compute::wrap_nocopy(&ctx, &gate_sparse);
        let (up_sparse_buf, up_sparse_off) = crate::compute::wrap_nocopy(&ctx, &up_sparse);
        let (down_q6k_buf, down_q6k_off) = crate::compute::wrap_nocopy(&ctx, &down_sparse_q6k);
        let (down_q5k_buf, down_q5k_off) = crate::compute::wrap_nocopy(&ctx, &down_sparse_q5k);
        let (gate_shared_q4k_buf, gate_shared_q4k_off) =
            crate::compute::wrap_nocopy(&ctx, &gate_shared_q4k);
        let (up_shared_q4k_buf, up_shared_q4k_off) =
            crate::compute::wrap_nocopy(&ctx, &up_shared_q4k);
        let (down_shared_q6k_buf, down_shared_q6k_off) =
            crate::compute::wrap_nocopy(&ctx, &down_shared_q6k);
        let (gate_shared_q8_buf, gate_shared_q8_off) =
            crate::compute::wrap_nocopy(&ctx, &gate_shared_q8v);
        let (up_shared_q8_buf, up_shared_q8_off) =
            crate::compute::wrap_nocopy(&ctx, &up_shared_q8v);
        let (down_shared_q8_buf, down_shared_q8_off) =
            crate::compute::wrap_nocopy(&ctx, &down_shared_q8v);
        let (router_buf, router_off) = crate::compute::wrap_nocopy(&ctx, &router_w);
        let router_off_buf = u64_buf(&ctx, router_off as u64);
        let ffn_norm_w = shared_f32_buf(&ctx, &vec![1.0f32; n_embd]);
        let shared_scale = shared_f32_buf(&ctx, &vec![0.01f32; n_embd]);
        let hidden = empty_f32_buf(&ctx, n_embd);

        let expert_sets: Vec<Vec<u32>> = (0..(warmup + samples))
            .map(|r| {
                let mut ids: Vec<u32> = (0..n_used)
                    .map(|j| ((r * 8 + j) as u32) % n_expert as u32)
                    .collect();
                ids.push(shared_id);
                ids
            })
            .collect();
        let route_weights_host = vec![0.1f32; slots];

        let mib = |b: usize| b as f64 / (1024.0 * 1024.0);
        let bw = |b: usize, ms: f64| (b as f64 / (ms / 1000.0)) / 1e9;
        ...
        let mut run_config = |name: &str,
                              ...
                              down_buf: &ProtocolObject<dyn MTLBuffer>,
                              down_off: u32,
                              down_shared_buf: &ProtocolObject<dyn MTLBuffer>,
                              down_shared_off: u32,
                              gate_shared_buf: &ProtocolObject<dyn MTLBuffer>,
                              gate_shared_off: u32,
                              up_shared_buf: &ProtocolObject<dyn MTLBuffer>,
                              up_shared_off: u32,
                              per_down: usize| {
            let carrier = QwenMoeDecodeChainCarrier::new(
                &ctx, n_used, n_embd, n_ff, n_expert, per_gate, per_up, per_down, shared_id, eps,
            );
            carrier.upload_offsets(
                gate_sparse_off,
                up_sparse_off,
            ...
            let time = |f: &dyn Fn(&ProtocolObject<dyn MTLComputeCommandEncoder>)| -> f64 {
                let mut samps: Vec<f64> = Vec::with_capacity(samples);
                for (i, ids) in expert_sets.iter().enumerate() {
                    carrier.upload_routes(ids, &route_weights_host);
            ...
            let shared_slot = n_used;

            let t_route = time(&|enc| {
                qwen_moe_decode_chain_encode_route(
                    &ctx,
                    enc,
                    &carrier,
            ...
            let t_resid = time(&|enc| {
                encode_residual_add_at(&ctx, enc, &hidden, 0, &carrier.moe_out_dev, 0, n_embd);
            });
            let t_shared = if shared_q8_0 {
                time(&|enc| {
                    crate::compute::encode_gemv_q8_0_at(
                        &ctx,
                        enc,
            ...
            let route_bytes = n_embd * f32b + n_expert * n_embd * f32b;
            let gate_bytes = slots * per_gate + if shared_q8_0 { shared_gate_q8 } else { 0 };
            let up_bytes = slots * per_up + if shared_q8_0 { shared_gate_q8 } else { 0 };
            let down_bytes = slots * per_down + if shared_q8_0 { shared_down_q8 } else { 0 };
            let total_bytes = route_bytes + gate_bytes + up_bytes + down_bytes;
            let sum_iso = t_route + t_gate + t_up + t_silu + t_down + t_reduce + t_resid + t_shared;
            let dispatches = 3 + 6 + if shared_q8_0 { 3 } else { 0 };

            eprintln!(
                "[moe-decode-prof] config={name} down=q{} shared_q8={shared_q8_0} dispatches/layer={dispatches}",
                down_quant + 4
            );
            eprintln!(
                "[moe-decode-prof]   route  {t_route:.4}ms  {:.2}MiB  {:.0}GB/s",
                mib(route_bytes),
                bw(route_bytes, t_route)
            );
            eprintln!(
                "[moe-decode-prof]     router_logits {t_router_logits:.4}ms  route_shared {t_route_shared:.4}ms"
            );
            eprintln!(
                "[moe-decode-prof]   gate   {t_gate:.4}ms  {:.2}MiB  {:.0}GB/s ({:.0}%)",
                mib(gate_bytes),
                bw(gate_bytes, t_gate),
                bw(gate_bytes, t_gate) / peak_gbps * 100.0
            );
            eprintln!(
                "[moe-decode-prof]   up     {t_up:.4}ms  {:.2}MiB  {:.0}GB/s ({:.0}%)",
                mib(up_bytes),
                bw(up_bytes, t_up),
                bw(up_bytes, t_up) / peak_gbps * 100.0
            );
            eprintln!("[moe-decode-prof]   silu   {t_silu:.4}ms");
            eprintln!(
                "[moe-decode-prof]   down   {t_down:.4}ms  {:.2}MiB  {:.0}GB/s ({:.0}%)",
                mib(down_bytes),
                bw(down_bytes, t_down),
                bw(down_bytes, t_down) / peak_gbps * 100.0
            );
            eprintln!("[moe-decode-prof]   reduce {t_reduce:.4}ms");
            eprintln!("[moe-decode-prof]   resid  {t_resid:.4}ms");
            if shared_q8_0 {
                eprintln!("[moe-decode-prof]   sharedQ8(g+u+d) {t_shared:.4}ms");
            }
            eprintln!(
                "[moe-decode-prof]   SUM(isolated)={sum_iso:.4}ms  CHAIN(1cmdbuf)={t_chain:.4}ms  barrier_bubble={:.4}ms",
                sum_iso - t_chain
            );
            eprintln!(
                "[moe-decode-prof]   per-layer read={:.2}MiB roofline={:.4}ms chain_eff_bw={:.0}GB/s ({:.0}% peak)  x40layers chain={:.2}ms",
                mib(total_bytes),
                total_bytes as f64 / (peak_gbps * 1e9) * 1000.0,
                bw(total_bytes, t_chain),
                bw(total_bytes, t_chain) / peak_gbps * 100.0,
                t_chain * 40.0
            );
        };

        if select == "std" || select == "both" {
            run_config(
                "std",
                2,
                false,
    ...
    /// Correctness oracle for the rewritten 2-simd-group / 2-way Q5_K decode-down slot
    /// kernel: multi-slot `down_slots(quant=1)` output must match the trusted dense Q5_K
    /// GEMV reference (`gemv_q5k_with_ctx`) per slot, for K=512 (nb=2 — the case the old
    /// 4-way layout under-utilized) and K=1024 (nb=4). Odd N exercises has_row1=false tail.
    #[test]
    #[ignore = "requires a Metal device"]
    fn qwen_moe_decode_q5k_down_slots_matches_dense_reference() {
        use rnb_cpu::quantize::quantize_q5_k_vec;
        let ctx = build_metal_context().expect("no metal device");
        for &k in &[512usize, 1024usize] {
            let n = 67usize; // odd → last simdgroup has_row1 == false
            let n_experts = 4usize;
            let slots = 3usize;
            let shared_id = n_experts as u32;
            let per_expert = n * (k / 256) * 176;
            let mk_mat = |seed: usize| -> Vec<u8> {
                let m: Vec<f32> = (0..n * k)
                    .map(|i| {
                        let r = i / k;
                        let c = i % k;
                        0.1 + (((r * 31 + c * 17 + seed * 7) % 89) as f32 - 44.0) * 0.01
                    })
                    .collect();
                quantize_q5_k_vec(&m)
            };
            let mut sparse = vec![0u8; n_experts * per_expert];
            let mut expert_mats: Vec<Vec<u8>> = Vec::new();
            for e in 0..n_experts {
                let q = mk_mat(e + 1);
                assert_eq!(q.len(), per_expert);
                sparse[e * per_expert..(e + 1) * per_expert].copy_from_slice(&q);
                expert_mats.push(q);
            }
            let shared_mat = mk_mat(99);
            let expert_ids = [1u32, 3u32, shared_id];
            let input: Vec<f32> = (0..slots * k)
                .map(|i| 0.05 + ((i % 37) as f32 - 18.0) * 0.02)
                .collect();

            let (sparse_buf, sparse_off) = crate::compute::wrap_nocopy(&ctx, &sparse);
            let (shared_buf, shared_off) = crate::compute::wrap_nocopy(&ctx, &shared_mat);
            let input_buf = shared_f32_buf(&ctx, &input);
            let out_buf = empty_f32_buf(&ctx, slots * n);
            let expert_ids_buf = shared_u32_buf(&ctx, &expert_ids);
            let n_buf = u32_buf(&ctx, n as u32);
            let k_buf = u32_buf(&ctx, k as u32);
            let per_buf = u64_buf(&ctx, per_expert as u64);
            let shared_id_buf = u32_buf(&ctx, shared_id);
            let sparse_off_buf = u64_buf(&ctx, sparse_off as u64);
            let shared_off_buf = u64_buf(&ctx, shared_off as u64);
            ...
            for (s, &eid) in expert_ids.iter().enumerate() {
                let mat = if eid == shared_id {
                    &shared_mat
                } else {
                    &expert_mats[eid as usize]
                };
                let in_slice = &input[s * k..(s + 1) * k];
                let want = crate::compute::gemv_q5k_with_ctx(&ctx, mat, in_slice, n, k);
                for r in 0..n {
                    let a = got[s * n + r];
                    let b = want[r];
                    let rel = (a - b).abs() / b.abs().max(1e-4);
                    assert!(
                        rel < 3e-3 || (a - b).abs() < 1e-3,
                        "q5k down slot mismatch k={k} slot={s} row={r} got={a} want={b} rel={rel}"
                    );
                }
            }
        }
    ...
    fn gemma_o_ffn_chain_matches_serial_device_handoffs() {
        ...
        let boundary_scale = 0.75f32;
        let scaled_fused = fused
            .iter()
            .map(|value| value * boundary_scale)
            .collect::<Vec<_>>();
        let boundary_scale_buf = f32_buf(&ctx, boundary_scale);

        let next_attn_norm: Vec<f32> = (0..hidden_dim)
            .map(|i| 0.85 + ((i % 11) as f32) * 0.005)
            .collect();
        let next_attn_norm_w_buf = shared_f32_buf(&ctx, &next_attn_norm);
        let next_norm = {
            let input = shared_f32_buf(&ctx, &scaled_fused);
            let output = empty_f32_buf(&ctx, hidden_dim);
            let dim_buf = u32_buf(&ctx, hidden_dim as u32);
            let eps_buf = f32_buf(&ctx, eps);
            let cmd = ctx.queue.commandBuffer().expect("command buffer");
        ...
        let next_q_out_dim = 512usize;
        let next_kv_dim = 256usize;
        let next_q_w: Vec<u8> = block
            .iter()
            .cycle()
            .take(next_q_out_dim * (hidden_dim / 256) * 144)
            .copied()
            .collect();
        let next_k_w: Vec<u8> = block
            .iter()
            .cycle()
            .take(next_kv_dim * (hidden_dim / 256) * 144)
            .copied()
            .collect();
        let next_v_w = next_k_w.clone();
        let next_q_w_buf = shared_u8_buf(&ctx, &next_q_w);
        let next_k_w_buf = shared_u8_buf(&ctx, &next_k_w);
        let next_v_w_buf = shared_u8_buf(&ctx, &next_v_w);
        let serial_qkv_carrier = QkvCarrier::new(&ctx, hidden_dim, next_q_out_dim, next_kv_dim);
        let serial_qkv = qkv_chain_dispatch(
            &ctx,
            &serial_qkv_carrier,
            &next_norm,
            &next_q_w_buf,
        ...
        let boundary_o = OChainCarrier::new(&ctx, hidden_dim, q_dim);
        let boundary_ffn = FfnCarrier::new(&ctx, hidden_dim, ffn_dim, eps);
        let boundary_qkv = QkvCarrier::new(&ctx, hidden_dim, next_q_out_dim, next_kv_dim);
        let mut boundary_hidden = hidden.clone();
        let boundary_qkv_out = gemma_o_ffn_qkv_chain_dispatch(
            &ctx,
            &boundary_o,
            &boundary_ffn,
            &boundary_qkv,
        ...
        let boundary_hidden_max_abs = boundary_hidden
            .iter()
            .zip(&scaled_fused)
            .map(|(got, want)| (got - want).abs())
            .fold(0.0f32, f32::max);
        assert!(
            boundary_hidden_max_abs <= 1e-5,
            "boundary hidden max_abs={boundary_hidden_max_abs}"
        );
        for (name, got, want) in [
            ("q", &boundary_qkv_out.0, &serial_qkv.0),
            ("k", &boundary_qkv_out.1, &serial_qkv.1),
            ("v", &boundary_qkv_out.2, &serial_qkv.2),
        ] {
            let max_abs = got
                .iter()
                .zip(want)
                .map(|(got, want)| (got - want).abs())
                .fold(0.0f32, f32::max);
            assert!(max_abs <= 1e-5, "boundary {name} max_abs={max_abs}");
        }
    }
        ...
        for (sparse_down_quant, shared_quant) in [
            ...
            for iteration in 0..20 {
                for map_reuse in [false, true] {
                    ...
                    assert_eq!(trace.reduce_dispatches, 1);
