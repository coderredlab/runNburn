...
// GDN decode alpha/beta 전처리 (head 별, in-place):
//   beta[h]  = sigmoid(beta[h])                       = 1/(1+exp(-beta))
//   alpha[h] = softplus(alpha[h] + dt_bias[h]) * ssm_a[h]   softplus(x)=ln(1+exp(x))
// rnb-cpu gdn_decode.rs decode_gdn_layer_qwen 의 beta sigmoid + alpha-gate 단계 1:1.
// delta_net 의 gate/beta 입력을 device 에 유지(host readback 제거). thread 1=head 1.
kernel void gdn_alpha_beta(
    device float*       alpha     [[buffer(0)]],  // in/out [num_heads]
    device float*       beta      [[buffer(1)]],  // in/out [num_heads]
    device const float* dt_bias   [[buffer(2)]],  // [num_heads]
    device const float* ssm_a     [[buffer(3)]],  // [num_heads]
    constant uint&      num_heads [[buffer(4)]],
    uint h [[thread_position_in_grid]])
{
    if (h >= num_heads) return;
    beta[h] = 1.0f / (1.0f + qwen_moe_exp_f32(-beta[h])); // sigmoid
    float a_biased = alpha[h] + dt_bias[h];
    float sp = qwen_gdn_log_f32(1.0f + qwen_moe_exp_f32(a_biased)); // softplus
    alpha[h] = sp * ssm_a[h];
}
