...
const GEMV_Q3K_SIMD_SRC: &str = include_str!("gemv_q3k_simd.metal");
const GEMV_Q2K_SIMD_SRC: &str = include_str!("gemv_q2k_simd.metal");
...
const GEMV_BCOL_SRC: &str = include_str!("gemv_bcol.metal");
...
const IQ_COMMON_SRC: &str = include_str!("iq_common.metal");
#[cfg(all(test, target_os = "macos"))]
pub(crate) fn iq_glm_concat_compiles(ctx: &MetalContext) {
    let src = format!("{IQ_COMMON_SRC}\n{GLM_MOE_DECODE_IQ_SRC}");
    let _ = build_pipeline(&ctx.device, &src, "glm_moe_decode_iq3xxs_selected_slots");
}
...
// pm127: GDN decode tail fused (delta_net_step + qk_norm(ssm_norm) + silu_mul) 커널.
const GDN_DELTA_NORM_SILU_SRC: &str = include_str!("gdn_delta_norm_silu.metal");
...
pub(crate) fn env_falsey(var: &str) -> bool {
    std::env::var(var)
        .ok()
        .map(|v| {
            matches!(
...
/// pm126 진단: Q8_0 dense GEMV 를 nsg2(2 SIMD-group, row 4/tg) 로 dispatch(default off).
/// unsloth 는 dense 만 Q8_0(비중 77%)라 이득이 안 보였을 수 있어, Q8_0 비중 큰 모델용
/// A/B 격리 스위치로 둔다. `RNB_METAL_Q8_0_NSG2=1` 로 opt-in.
fn q8_0_nsg2_enabled() -> bool {
    ...
    match CACHE.load(Ordering::Relaxed) {
        ...
        1 => false,
        _ => {
            let v = matches!(std::env::var("RNB_METAL_Q8_0_NSG2").as_deref(), Ok("1"));
...
mod env_tests {
    ...
    fn qwen_moe_llama_id_suppresses_legacy_mulmmid_requests() {
        ...
        for llama_on in ["1", "true", "on", "yes", "typo"] {
        ...
        for llama_off in ["0", "false", "off", "no"] {
...
pub struct MetalContext {
    ...
    pub q3k_simd_pipeline: Retained<ProtocolObject<dyn MTLComputePipelineState>>,
    pub q2k_simd_pipeline: Retained<ProtocolObject<dyn MTLComputePipelineState>>,
    ...
    /// Q8_0 N_SG=2/N_R0=2 shape(row 4/tg). 진단 opt-in(RNB_METAL_Q8_0_NSG2, pm126).
    pub q8_0_coalesced_nsg2_pipeline: Retained<ProtocolObject<dyn MTLComputePipelineState>>,
    /// milestone 2 (MTP verify): B-column weight-amortized GEMV pipelines. 한 dispatch 가
    /// weight tile 을 1회 읽어 B 개 입력 컬럼 → B 개 출력 컬럼을 계산(memory-bound decode
    /// 의 weight 읽기를 batch-B 로 amortize). input=[B*K] out=[B*N] column-major-by-lane.
    pub q4k_bcol_pipeline: Retained<ProtocolObject<dyn MTLComputePipelineState>>,
    pub q5k_bcol_pipeline: Retained<ProtocolObject<dyn MTLComputePipelineState>>,
    pub q6k_bcol_pipeline: Retained<ProtocolObject<dyn MTLComputePipelineState>>,
    pub q8_0_bcol_pipeline: Retained<ProtocolObject<dyn MTLComputePipelineState>>,
    /// F32 dense B-column GEMV (GDN F32 ssm_alpha/beta/ssm_out, arbitrary K).
    pub f32_bcol_pipeline: Retained<ProtocolObject<dyn MTLComputePipelineState>>,
    pub q8_0_mla_slots_pipeline: Retained<ProtocolObject<dyn MTLComputePipelineState>>,
    /// pm117: MLA prefill slot-batch Q5_K GEMV (o/q_a dense CPU fallback 제거).
    pub q5k_mla_slots_pipeline: Retained<ProtocolObject<dyn MTLComputePipelineState>>,
    /// pm116: GLM MLA prefill attention (absorbed form, online softmax) — slot 당 1 simdgroup.
    pub glm_mla_prefill_attn_pipeline: Retained<ProtocolObject<dyn MTLComputePipelineState>>,
    /// pm118 연장: front fused chain 의 q_nope stride-pack (q_b 출력 → kb slots 입력).
    pub glm_mla_qnope_pack_pipeline: Retained<ProtocolObject<dyn MTLComputePipelineState>>,
    /// pm119: kv rms+rope → cache f16 직접 기록 (CPU rope 스테이지의 GPU 이식).
    pub glm_mla_kv_rms_rope_pipeline: Retained<ProtocolObject<dyn MTLComputePipelineState>>,
    /// pm119: q_pe pack+rope (slot 별 [q_nope, qk) 구간 rope).
    pub glm_mla_qpe_rope_pipeline: Retained<ProtocolObject<dyn MTLComputePipelineState>>,
    ...
    pub qwen_moe_decode_reduce_slots_pipeline:
        OnceCell<Retained<ProtocolObject<dyn MTLComputePipelineState>>>,
    pub qwen_moe_decode_reduce_add_slots_pipeline:
        OnceCell<Retained<ProtocolObject<dyn MTLComputePipelineState>>>,
    ...
    pub attn_decode_gqa_splitk_reduce_pipeline:
        Retained<ProtocolObject<dyn MTLComputePipelineState>>,
    pub kvarn_attention_splitk_part_pipeline: Retained<ProtocolObject<dyn MTLComputePipelineState>>,
    pub kvarn_attention_splitk_reduce_pipeline:
        Retained<ProtocolObject<dyn MTLComputePipelineState>>,
    ...
    /// pm127: GDN decode tail fused — delta_net_step + qk_norm(ssm_norm) + silu_mul 을
    /// v-head threadgroup 하나로 묶는다(원본 3커널과 bit-identical). decode small op
    /// dispatch 3→1 감소.
    pub gdn_delta_norm_silu_pipeline: Retained<ProtocolObject<dyn MTLComputePipelineState>>,
    /// milestone 4 MTP: ping-pong(state_in→state_out) delta step. batched GDN 이 prefix state 를
    /// 보존해 partial-accept 커밋 재실행을 제거한다. `delta_net_step` 과 수치 동일.
    pub delta_net_step_io_pipeline: Retained<ProtocolObject<dyn MTLComputePipelineState>>,
    ...
    pub(crate) qwen_moe_llama_expert_order_reduce_f32_pipeline:
        OnceLock<Retained<ProtocolObject<dyn MTLComputePipelineState>>>,
    pub(crate) qwen_moe_llama_expert_order_reduce_shared_f32_pipeline:
        OnceLock<Retained<ProtocolObject<dyn MTLComputePipelineState>>>,
...
impl MetalContext {
    ...
    pub(crate) fn qwen_moe_decode_reduce_add_slots_pipeline(
        &self,
    ) -> &Retained<ProtocolObject<dyn MTLComputePipelineState>> {
        lazy_qwen_moe_decode_pipeline(
            &self.device,
            &self.qwen_moe_decode_reduce_add_slots_pipeline,
            "qwen_moe_decode_reduce_add_slots",
        )
    }
    ...
    pub(crate) fn qwen_moe_decode_shared_add_pipeline(
        &self,
    ) -> &Retained<ProtocolObject<dyn MTLComputePipelineState>> {
        lazy_qwen_moe_decode_pipeline(
            &self.device,
            &self.qwen_moe_decode_shared_add_pipeline,
            "qwen_moe_decode_shared_add",
        )
    }
    ...
    pub(crate) fn qwen_moe_chain_cast_compact_slots_pipeline(
        &self,
    ) -> Option<&Retained<ProtocolObject<dyn MTLComputePipelineState>>> {
        self.tensorops_capable.then(|| {
            lazy_qwen_moe_llama_pipeline(
                self,
                &self.qwen_moe_chain_cast_compact_slots_pipeline,
                "qwen_moe_chain_cast_compact_slots_f32_f16",
            )
        })
    }
    ...
    pub(crate) fn qwen_moe_llama_expert_order_reduce_shared_f32_pipeline(
        &self,
    ) -> Option<&Retained<ProtocolObject<dyn MTLComputePipelineState>>> {
        self.tensorops_capable.then(|| {
            lazy_qwen_moe_llama_pipeline(
                self,
                &self.qwen_moe_llama_expert_order_reduce_shared_f32_pipeline,
                "qwen_moe_llama_expert_order_reduce_shared_f32",
            )
        })
    }
}
...
mod qwen_moe_llama_lazy_pipeline_tests {
    ...
    const GETTERS: [Getter; 16] = [
        ...
        MetalContext::qwen_moe_llama_expert_order_reduce_shared_f32_pipeline,
    ];
...
pub fn build_metal_context_with_opts(
    ...
    let qwen_moe_llama_expert_order_reduce_shared_f32_pipeline = OnceLock::new();
    ...
    let qwen_moe_chain_cast_compact_slots_pipeline = OnceLock::new();
    ...
    let gemm_q3k_tensorops_v2_pipeline: Option<
        Retained<ProtocolObject<dyn MTLComputePipelineState>>,
    > = if tensorops_capable {
        Some(build_pipeline_v4(
            &device,
            GEMM_TENSOROPS_POC_SRC,
            "gemm_q3k_tensorops_v2_64x128",
        ))
    } else {
        None
    };
    let gemm_q2k_tensorops_v2_pipeline: Option<
        Retained<ProtocolObject<dyn MTLComputePipelineState>>,
    > = if tensorops_capable {
        Some(build_pipeline_v4(
            &device,
            GEMM_TENSOROPS_POC_SRC,
            "gemm_q2k_tensorops_v2_64x128",
        ))
    } else {
        None
    };
    ...
    let flash_attn_prefill_tg_pipeline: Option<
        Retained<ProtocolObject<dyn MTLComputePipelineState>>,
    > = if tensorops_capable {
        Some(build_pipeline_v4(
            &device,
            PREFILL_FLASH_ATTN_TG_SRC,
            "attn_prefill_flash_tg",
        ))
    } else {
        None
    };
    ...
    let q3k_simd_pipeline = build_pipeline(&device, GEMV_Q3K_SIMD_SRC, "gemv_q3k_simd");
    let q2k_simd_pipeline = build_pipeline(&device, GEMV_Q2K_SIMD_SRC, "gemv_q2k_simd");
    ...
    let q8_0_coalesced_nsg2_pipeline =
        build_pipeline(&device, GEMV_Q8_0_COALESCED_SRC, "gemv_q8_0_coalesced_nsg2");
    let q4k_bcol_pipeline = build_pipeline(&device, GEMV_BCOL_SRC, "gemv_q4k_bcol");
    let q5k_bcol_pipeline = build_pipeline(&device, GEMV_BCOL_SRC, "gemv_q5k_bcol");
    let q6k_bcol_pipeline = build_pipeline(&device, GEMV_BCOL_SRC, "gemv_q6k_bcol");
    let q8_0_bcol_pipeline = build_pipeline(&device, GEMV_BCOL_SRC, "gemv_q8_0_bcol");
    let f32_bcol_pipeline = build_pipeline(&device, GEMV_BCOL_SRC, "gemv_f32_bcol");
    let q8_0_mla_slots_pipeline =
        build_pipeline(&device, GEMV_Q8_0_MLA_SLOTS_SRC, "gemv_q8_0_mla_slots");
    let q5k_mla_slots_pipeline =
        build_pipeline(&device, GEMV_Q5K_MLA_SLOTS_SRC, "gemv_q5k_mla_slots");
    let glm_mla_prefill_attn_pipeline =
        build_pipeline(&device, GLM_MLA_PREFILL_ATTN_SRC, "glm_mla_prefill_attn");
    let glm_mla_qnope_pack_pipeline =
        build_pipeline(&device, GEMV_Q8_0_MLA_SLOTS_SRC, "glm_mla_qnope_pack");
    let glm_mla_kv_rms_rope_pipeline =
        build_pipeline(&device, GEMV_Q8_0_MLA_SLOTS_SRC, "glm_mla_kv_rms_rope_f16");
    ...
    let attn_decode_gqa_splitk_reduce_pipeline = build_pipeline(
        &device,
        ATTN_DECODE_GQA_SRC,
        "attn_decode_gqa_splitk_reduce",
    );
    let kvarn_attention_splitk_part_pipeline = build_pipeline(
        &device,
        KVARN_ATTENTION_SRC,
        "kvarn_attention_decode_splitk_part",
    );
    let kvarn_attention_splitk_reduce_pipeline = build_pipeline(
        &device,
        KVARN_ATTENTION_SRC,
        "kvarn_attention_decode_splitk_reduce",
    );
    ...
    let gdn_delta_norm_silu_pipeline =
        build_pipeline(&device, GDN_DELTA_NORM_SILU_SRC, "gdn_delta_norm_silu");
    let delta_net_step_io_pipeline =
        build_pipeline(&device, DELTA_NET_STEP_SRC, "delta_net_step_io");
    ...
    Some(MetalContext {
        ...
        q5k_mla_slots_pipeline,
        glm_mla_prefill_attn_pipeline,
        glm_mla_qnope_pack_pipeline,
        ...
        qwen_moe_decode_reduce_add_slots_pipeline,
        ...
        gdn_delta_norm_silu_pipeline,
        delta_net_step_io_pipeline,
        ...
        qwen_moe_chain_cast_compact_slots_pipeline,
...
/// pm127: GDN decode tail fused(delta+qk_norm+silu) 커널 사용 여부. 기본 ON,
/// `RNB_METAL_GDN_FUSED_TAIL=0` 으로 원본 개별 3-op 경로로 opt-out(진단/회귀 격리).
pub(crate) fn gdn_fused_tail_enabled() -> bool {
    use std::sync::atomic::{AtomicU8, Ordering};
    static CACHE: AtomicU8 = AtomicU8::new(0);
    match CACHE.load(Ordering::Relaxed) {
        2 => true,
        ...
        _ => {
            let on = !matches!(
                std::env::var("RNB_METAL_GDN_FUSED_TAIL").as_deref(),
                Ok("0")
            );
...
/// pm117: Q5_K slot-batch MLA GEMV — `glm_mla_head_gemv_q8_0_slots_dispatch` 와
/// 동일 구조, super-block(256 elem/176 byte) 만 다름.
#[allow(clippy::too_many_arguments)]
pub(crate) fn glm_mla_head_gemv_q5k_slots_dispatch(
    ctx: &MetalContext,
    w_buf: &ProtocolObject<dyn MTLBuffer>,
    page_off: u32,
    input: &[f32],
    ...
    assert_eq!(k % 256, 0, "Q5_K K must be a multiple of 256");
    assert_eq!(input.len(), slots * k);
    let shared = MTLResourceOptions::StorageModeShared;
    ...
    let cmd = ctx.queue.commandBuffer().expect("command buffer");
    let enc = cmd.computeCommandEncoder().expect("compute encoder");
    enc.setComputePipelineState(&ctx.q5k_mla_slots_pipeline);
    unsafe {
        enc.setBuffer_offset_atIndex(Some(w_buf), 0, 0);
        enc.setBuffer_offset_atIndex(Some(&i_buf), 0, 1);
        enc.setBuffer_offset_atIndex(Some(&o_buf), 0, 2);
...
/// pm118: verify/prefill 의 v_b slots → o slots 두 dispatch 를 단일 command
/// buffer 에 encode. slots 커널 형태는 유지 (pm117 후속 6: chain 개별-dispatch
/// 이식 금지) — 중간 attention_concat 을 device-resident 로 두어 commit+wait
/// 왕복 1회와 concat readback/재업로드를 제거한다.
#[allow(clippy::too_many_arguments)]
pub(crate) fn glm_mla_vb_o_fused_dispatch(
    ctx: &MetalContext,
    vb_buf: &ProtocolObject<dyn MTLBuffer>,
    vb_off: u32,
    ...
    assert_eq!(kv_rank % 32, 0, "Q8_0 K must be a multiple of 32");
    assert_eq!(latent.len(), slots * kv_rank);
    assert_eq!(slots % heads, 0, "slots must be token-major head batches");
    let seq_len = slots / heads;
    let o_cols = heads * value_dim;
    let shared = MTLResourceOptions::StorageModeShared;

    let i_buf: Retained<ProtocolObject<dyn MTLBuffer>> = unsafe {
        let ptr =
            NonNull::new(latent.as_ptr() as *mut std::ffi::c_void).expect("latent ptr is null");
        ctx.device
            .newBufferWithBytes_length_options(ptr, std::mem::size_of_val(latent), shared)
            .expect("Metal: failed to create latent buffer")
    };
    // 중간 concat: GPU 내부 전용 (readback 없음).
    let mid_buf: Retained<ProtocolObject<dyn MTLBuffer>> = ctx
        .device
        .newBufferWithLength_options(
            slots * value_dim * std::mem::size_of::<f32>(),
            MTLResourceOptions::StorageModePrivate,
        )
        .expect("Metal: failed to create concat buffer");
    let out_buf: Retained<ProtocolObject<dyn MTLBuffer>> = ctx
        .device
        .newBufferWithLength_options(seq_len * o_rows * std::mem::size_of::<f32>(), shared)
        .expect("Metal: failed to create output buffer");
    ...
    let vb_n_buf = mk_u32(value_dim as u32);
    let vb_k_buf = mk_u32(kv_rank as u32);
    let vb_off_buf = mk_u32(vb_off);
    let vb_heads_buf = mk_u32(heads as u32);
    let o_n_buf = mk_u32(o_rows as u32);
    let o_k_buf = mk_u32(o_cols as u32);
    let o_off_buf = mk_u32(o_off);
    let o_heads_buf = mk_u32(1u32);

    let cmd = ctx.queue.commandBuffer().expect("command buffer");
    let enc = cmd.computeCommandEncoder().expect("compute encoder");
    // v_b: slots × [value_dim ← kv_rank] (Q8_0, head별 weight page).
    enc.setComputePipelineState(&ctx.q8_0_mla_slots_pipeline);
    unsafe {
        enc.setBuffer_offset_atIndex(Some(vb_buf), 0, 0);
        enc.setBuffer_offset_atIndex(Some(&i_buf), 0, 1);
        enc.setBuffer_offset_atIndex(Some(&mid_buf), 0, 2);
        enc.setBuffer_offset_atIndex(Some(&vb_n_buf), 0, 3);
        enc.setBuffer_offset_atIndex(Some(&vb_k_buf), 0, 4);
        enc.setBuffer_offset_atIndex(Some(&vb_off_buf), 0, 5);
        enc.setBuffer_offset_atIndex(Some(&vb_heads_buf), 0, 6);
    }
    enc.dispatchThreadgroups_threadsPerThreadgroup(
        MTLSize {
            width: value_dim.div_ceil(2),
            height: slots,
            depth: 1,
        },
        MTLSize {
    ...
    // o: seq_len × [o_rows ← heads*value_dim] (heads=1 dense slots).
    // 같은 serial encoder 라 mid_buf 의 write→read hazard 는 자동 추적된다.
    if o_is_q5k {
        enc.setComputePipelineState(&ctx.q5k_mla_slots_pipeline);
    } else {
        enc.setComputePipelineState(&ctx.q8_0_mla_slots_pipeline);
    ...
    unsafe {
        enc.setBuffer_offset_atIndex(Some(o_w_buf), 0, 0);
        enc.setBuffer_offset_atIndex(Some(&mid_buf), 0, 1);
        enc.setBuffer_offset_atIndex(Some(&out_buf), 0, 2);
        enc.setBuffer_offset_atIndex(Some(&o_n_buf), 0, 3);
        enc.setBuffer_offset_atIndex(Some(&o_k_buf), 0, 4);
        enc.setBuffer_offset_atIndex(Some(&o_off_buf), 0, 5);
        enc.setBuffer_offset_atIndex(Some(&o_heads_buf), 0, 6);
    }
    enc.dispatchThreadgroups_threadsPerThreadgroup(
        MTLSize {
            width: o_rows.div_ceil(2),
            height: seq_len,
            depth: 1,
        },
        MTLSize {
    ...
    let out: &[f32] =
        unsafe { std::slice::from_raw_parts(contents.as_ptr() as *const f32, seq_len * o_rows) };
    out.to_vec()
}

/// pm119 layer fused 반환: o projection 결과 + GPU 가 생성한 cache tail rows
/// (f16 bits — CPU kv_cache append 용).
pub struct GlmMlaLayerFusedOut {
    pub projected: Vec<f32>,
    pub cache_tail: Vec<u16>,
}

/// pm119: verify/작은 prefill 배치의 MLA 층 전체를 단일 command buffer 로 —
/// front 6단계 (pm118 연장) 에 이어 kv rms+rope→cache f16 직접 기록, q_pe
/// pack+rope, prefill attention, v_b slots, o slots 까지 encode. cache 는
/// base(pos_start rows, CPU 업로드) + tail(GPU 기록) 한 버퍼. readback 은
/// (projected, cache_tail) 2개 — 층당 commit+wait 왕복 3→1 (front/attn/vb·o).
#[allow(clippy::too_many_arguments)]
pub(crate) fn glm_mla_layer_fused_dispatch(
    ctx: &MetalContext,
    qa_buf: &ProtocolObject<dyn MTLBuffer>,
    qa_off: u32,
    qa_norm_weight: &[f32],
    qb_buf: &ProtocolObject<dyn MTLBuffer>,
    qb_off: u32,
    kva_buf: &ProtocolObject<dyn MTLBuffer>,
    kva_off: u32,
    kb_buf: &ProtocolObject<dyn MTLBuffer>,
    kb_off: u32,
    kv_norm_weight: &[f32],
    cache_base: &[u16],
    vb_buf: &ProtocolObject<dyn MTLBuffer>,
    ...
    assert_eq!(hidden % 256, 0, "Q5_K K must be a multiple of 256");
    assert_eq!(q_rank % 32, 0, "Q8_0 K must be a multiple of 32");
    assert_eq!(q_nope_dim % 32, 0, "Q8_0 K must be a multiple of 32");
    assert_eq!(kv_rank % 32, 0, "Q8_0 K must be a multiple of 32");
    assert_eq!(normed.len(), seq_len * hidden);
    assert_eq!(q_dim, heads * qk_dim);
    assert_eq!(kv_width, kv_rank + rope_dim);
    assert_eq!(cache_base.len(), pos_start * kv_width);
    assert_eq!(rope_dim % 2, 0, "rope pairs");
    // pm119 후속: fused cb 의 wall 분해 진단 (`RNB_METAL_GLM_MLA_FUSED_PROFILE=1`).
    static FUSED_PROFILE: std::sync::OnceLock<bool> = std::sync::OnceLock::new();
    let profile = *FUSED_PROFILE
        .get_or_init(|| std::env::var("RNB_METAL_GLM_MLA_FUSED_PROFILE").as_deref() == Ok("1"));
    let wall_start = profile.then(std::time::Instant::now);
    let slots = seq_len * heads;
    let cache_len = pos_start + seq_len;
    // CPU `rope_inplace` 와 동일한 host precompute (누적 곱은 커널이 재현).
    ...
    let mk_shared_f32 = |data: &[f32]| -> Retained<ProtocolObject<dyn MTLBuffer>> {
        unsafe {
            let ptr =
                NonNull::new(data.as_ptr() as *mut std::ffi::c_void).expect("f32 ptr is null");
            ctx.device
                .newBufferWithBytes_length_options(ptr, std::mem::size_of_val(data), shared)
                .expect("Metal: failed to create f32 buffer")
        }
    };
    let normed_buf = mk_shared_f32(normed);
    let qa_norm_w_buf = mk_shared_f32(qa_norm_weight);
    let kv_norm_w_buf = mk_shared_f32(kv_norm_weight);
    // cache: base 는 CPU 복사, tail 은 GPU 가 기록.
    let cache_buf: Retained<ProtocolObject<dyn MTLBuffer>> = ctx
        .device
        .newBufferWithLength_options(cache_len * kv_width * std::mem::size_of::<u16>(), shared)
        .expect("Metal: failed to create cache buffer");
    if !cache_base.is_empty() {
        unsafe {
            std::ptr::copy_nonoverlapping(
                cache_base.as_ptr(),
                cache_buf.contents().as_ptr() as *mut u16,
                cache_base.len(),
            );
        }
    }
    let mk_private = |elems: usize| -> Retained<ProtocolObject<dyn MTLBuffer>> {
        ctx.device
            .newBufferWithLength_options(elems * f32_len, private)
            .expect("Metal: failed to create private buffer")
    };
    let qa_dev = mk_private(seq_len * q_rank);
    let qa_norm_dev = mk_private(seq_len * q_rank);
    let q_dev = mk_private(seq_len * q_dim);
    let kv_dev = mk_private(seq_len * kv_width);
    let qnope_dev = mk_private(slots * q_nope_dim);
    let qabs_dev = mk_private(slots * kv_rank);
    let qpe_dev = mk_private(slots * rope_dim);
    let latent_dev = mk_private(slots * kv_rank);
    let concat_dev = mk_private(slots * value_dim);
    let out_buf: Retained<ProtocolObject<dyn MTLBuffer>> = ctx
        .device
        .newBufferWithLength_options(seq_len * o_rows * f32_len, shared)
        .expect("Metal: failed to create output buffer");
    ...
    let mk_f32 = |v: f32| -> Retained<ProtocolObject<dyn MTLBuffer>> {
        unsafe {
            let ptr =
                NonNull::new(&v as *const f32 as *mut std::ffi::c_void).expect("f32 ptr is null");
            ctx.device
                .newBufferWithBytes_length_options(ptr, std::mem::size_of::<f32>(), shared)
                .expect("Metal: failed to create f32 scalar buffer")
        }
    };

    let cmd = ctx.queue.commandBuffer().expect("command buffer");
    let enc = cmd.computeCommandEncoder().expect("compute encoder");
    // 같은 serial encoder 라 단계 간 write→read hazard 는 자동 추적된다.
    let encode_slots = |pipeline: &Retained<ProtocolObject<dyn MTLComputePipelineState>>,
                        w_buf: &ProtocolObject<dyn MTLBuffer>,
                        i_buf: &ProtocolObject<dyn MTLBuffer>,
                        ...
                        slot_count: usize| {
        enc.setComputePipelineState(pipeline);
        let n_buf = mk_u32(n_per_head as u32);
        let k_buf = mk_u32(k as u32);
        let off_buf = mk_u32(page_off);
        let heads_buf = mk_u32(head_count as u32);
        unsafe {
            enc.setBuffer_offset_atIndex(Some(w_buf), 0, 0);
            enc.setBuffer_offset_atIndex(Some(i_buf), 0, 1);
            enc.setBuffer_offset_atIndex(Some(o_buf), 0, 2);
    ...
    // 1~6) front (pm118 연장과 동일 — q/kv/qabs 는 device 상주).
    encode_slots(
        &ctx.q5k_mla_slots_pipeline,
        qa_buf,
        &normed_buf,
        &qa_dev,
        q_rank,
    ...
    {
        let cols_buf = mk_u32(q_rank as u32);
        let eps_buf = mk_f32(eps);
    ...
    {
        enc.setComputePipelineState(&ctx.glm_mla_qnope_pack_pipeline);
        let heads_buf = mk_u32(heads as u32);
        let qk_buf = mk_u32(qk_dim as u32);
        let qnope_buf = mk_u32(q_nope_dim as u32);
        unsafe {
            enc.setBuffer_offset_atIndex(Some(&q_dev), 0, 0);
            enc.setBuffer_offset_atIndex(Some(&qnope_dev), 0, 1);
            enc.setBuffer_offset_atIndex(Some(&heads_buf), 0, 2);
            enc.setBuffer_offset_atIndex(Some(&qk_buf), 0, 3);
            enc.setBuffer_offset_atIndex(Some(&qnope_buf), 0, 4);
        }
        enc.dispatchThreadgroups_threadsPerThreadgroup(
            MTLSize {
                width: q_nope_dim.div_ceil(SIMD_WIDTH),
                height: slots,
                depth: 1,
            },
            MTLSize {
    ...
    // 7) kv rms+rope → cache tail (f16 직접 기록).
    {
        enc.setComputePipelineState(&ctx.glm_mla_kv_rms_rope_pipeline);
        let kv_rank_buf = mk_u32(kv_rank as u32);
        let kv_width_buf = mk_u32(kv_width as u32);
        let pos_buf = mk_u32(pos_start as u32);
        let eps_buf = mk_f32(eps);
        let ts_buf = mk_f32(theta_scale);
        unsafe {
            enc.setBuffer_offset_atIndex(Some(&kv_dev), 0, 0);
            enc.setBuffer_offset_atIndex(Some(&kv_norm_w_buf), 0, 1);
            enc.setBuffer_offset_atIndex(Some(&cache_buf), 0, 2);
            enc.setBuffer_offset_atIndex(Some(&kv_rank_buf), 0, 3);
            enc.setBuffer_offset_atIndex(Some(&kv_width_buf), 0, 4);
            enc.setBuffer_offset_atIndex(Some(&pos_buf), 0, 5);
            enc.setBuffer_offset_atIndex(Some(&eps_buf), 0, 6);
            enc.setBuffer_offset_atIndex(Some(&ts_buf), 0, 7);
        }
        enc.dispatchThreadgroups_threadsPerThreadgroup(
            MTLSize {
                width: seq_len,
    ...
    // 8) q_pe pack+rope.
    {
        enc.setComputePipelineState(&ctx.glm_mla_qpe_rope_pipeline);
        let heads_buf = mk_u32(heads as u32);
        let qk_buf = mk_u32(qk_dim as u32);
        let qnope_buf = mk_u32(q_nope_dim as u32);
        let rope_buf = mk_u32(rope_dim as u32);
        let pos_buf = mk_u32(pos_start as u32);
        let ts_buf = mk_f32(theta_scale);
        let slots_buf = mk_u32(slots as u32);
        unsafe {
            enc.setBuffer_offset_atIndex(Some(&q_dev), 0, 0);
            enc.setBuffer_offset_atIndex(Some(&qpe_dev), 0, 1);
            enc.setBuffer_offset_atIndex(Some(&heads_buf), 0, 2);
            enc.setBuffer_offset_atIndex(Some(&qk_buf), 0, 3);
            enc.setBuffer_offset_atIndex(Some(&qnope_buf), 0, 4);
            enc.setBuffer_offset_atIndex(Some(&rope_buf), 0, 5);
            enc.setBuffer_offset_atIndex(Some(&pos_buf), 0, 6);
            enc.setBuffer_offset_atIndex(Some(&ts_buf), 0, 7);
            enc.setBuffer_offset_atIndex(Some(&slots_buf), 0, 8);
        }
        enc.dispatchThreadgroups_threadsPerThreadgroup(
            MTLSize {
                width: slots.div_ceil(SIMD_WIDTH),
                height: 1,
                depth: 1,
            },
            MTLSize {
    ...
    // 9) attention (pm116 plain 커널 — cache 는 base+tail 완성 상태).
    {
        enc.setComputePipelineState(&ctx.glm_mla_prefill_attn_pipeline);
        let heads_buf = mk_u32(heads as u32);
        let kv_rank_buf = mk_u32(kv_rank as u32);
        let rope_buf = mk_u32(rope_dim as u32);
        let pos_buf = mk_u32(pos_start as u32);
        let scale_buf = mk_f32(scale);
        unsafe {
            enc.setBuffer_offset_atIndex(Some(&qabs_dev), 0, 0);
            enc.setBuffer_offset_atIndex(Some(&qpe_dev), 0, 1);
            enc.setBuffer_offset_atIndex(Some(&cache_buf), 0, 2);
            enc.setBuffer_offset_atIndex(Some(&latent_dev), 0, 3);
            enc.setBuffer_offset_atIndex(Some(&heads_buf), 0, 4);
            enc.setBuffer_offset_atIndex(Some(&kv_rank_buf), 0, 5);
            enc.setBuffer_offset_atIndex(Some(&rope_buf), 0, 6);
            enc.setBuffer_offset_atIndex(Some(&pos_buf), 0, 7);
    ...
    // 10~11) v_b slots → o slots (pm118 vb·o fused 와 동일).
    encode_slots(
        &ctx.q8_0_mla_slots_pipeline,
        vb_buf,
        &latent_dev,
    ...
    enc.endEncoding();
    let encode_ms = wall_start.map(|t| t.elapsed().as_secs_f64() * 1000.0);
    cmd.commit();
    cmd.waitUntilCompleted();
    if let (Some(wall_start), Some(encode_ms)) = (wall_start, encode_ms) {
        let wall_ms = wall_start.elapsed().as_secs_f64() * 1000.0;
        let gpu_ms = (cmd.GPUEndTime() - cmd.GPUStartTime()) * 1000.0;
        let drv_ms = (cmd.kernelEndTime() - cmd.kernelStartTime()) * 1000.0;
        eprintln!(
            "[glm-mla-fused-profile] seq={seq_len} pos={pos_start} wall={wall_ms:.3} alloc_encode={encode_ms:.3} gpu={gpu_ms:.3} drv={drv_ms:.3}"
        );
    }
    ...
    let cache_tail = {
        let contents: NonNull<std::ffi::c_void> = cache_buf.contents();
        let all = unsafe {
            std::slice::from_raw_parts(contents.as_ptr() as *const u16, cache_len * kv_width)
        };
        all[pos_start * kv_width..].to_vec()
    };
    GlmMlaLayerFusedOut {
        projected,
        cache_tail,
...
/// pm118 연장: verify/작은 prefill 배치의 front 4 dispatch 를 단일 command
/// buffer 로 — q_a(Q5_K slots) → q_a_norm rms(batch) → q_b(Q8_0 slots) /
/// kv_a(Q8_0 slots) → q_nope pack → k_b(Q8_0 64-head slots). slots 커널 형태
/// 유지, 중간 결과 (q_a/q_a_norm/q_nope) 는 device-resident — commit+wait
/// 왕복 4→1, readback 은 (q, kv_raw, q_absorbed) 3개만.
#[allow(clippy::too_many_arguments)]
pub(crate) fn glm_mla_front_slots_fused_dispatch(
    ctx: &MetalContext,
    qa_buf: &ProtocolObject<dyn MTLBuffer>,
    qa_off: u32,
    qa_norm_weight: &[f32],
    ...
    let normed_buf: Retained<ProtocolObject<dyn MTLBuffer>> = unsafe {
        let ptr =
            NonNull::new(normed.as_ptr() as *mut std::ffi::c_void).expect("normed ptr is null");
        ctx.device
            .newBufferWithBytes_length_options(ptr, std::mem::size_of_val(normed), shared)
            .expect("Metal: failed to create normed buffer")
    };
    let qa_norm_w_buf: Retained<ProtocolObject<dyn MTLBuffer>> = unsafe {
        let ptr = NonNull::new(qa_norm_weight.as_ptr() as *mut std::ffi::c_void)
            .expect("qa_norm ptr is null");
        ctx.device
            .newBufferWithBytes_length_options(ptr, std::mem::size_of_val(qa_norm_weight), shared)
            .expect("Metal: failed to create qa_norm weight buffer")
    };
    let mk_private = |elems: usize| -> Retained<ProtocolObject<dyn MTLBuffer>> {
        ctx.device
            .newBufferWithLength_options(elems * f32_len, private)
    ...
    let mk_shared_out = |elems: usize| -> Retained<ProtocolObject<dyn MTLBuffer>> {
        ctx.device
            .newBufferWithLength_options(elems * f32_len, shared)
            .expect("Metal: failed to create output buffer")
    };
    let qa_dev = mk_private(seq_len * q_rank);
    let qa_norm_dev = mk_private(seq_len * q_rank);
    let qnope_dev = mk_private(slots * q_nope_dim);
    let q_out_buf = mk_shared_out(seq_len * q_dim);
    let kv_out_buf = mk_shared_out(seq_len * kv_width);
    let qabs_out_buf = mk_shared_out(slots * kv_rank);
    ...
    let eps_buf: Retained<ProtocolObject<dyn MTLBuffer>> = unsafe {
        let ptr =
            NonNull::new(&eps as *const f32 as *mut std::ffi::c_void).expect("eps ptr is null");
        ctx.device
            .newBufferWithBytes_length_options(ptr, std::mem::size_of::<f32>(), shared)
            .expect("Metal: failed to create eps buffer")
    };
    ...
    // 1) q_a: normed → qa_dev (Q5_K dense slots).
    encode_slots(
        &ctx.q5k_mla_slots_pipeline,
        qa_buf,
        &normed_buf,
    ...
    // 3) q_b: qa_norm_dev → q_out (Q8_0 dense slots).
    encode_slots(
        &ctx.q8_0_mla_slots_pipeline,
        qb_buf,
        &qa_norm_dev,
    ...
    // 4) kv_a: normed → kv_out (Q8_0 dense slots).
    encode_slots(
        &ctx.q8_0_mla_slots_pipeline,
        kva_buf,
        &normed_buf,
    ...
    // 5) q_nope pack: q_out → qnope_dev.
    {
        ...
        let qnope_buf = mk_u32(q_nope_dim as u32);
        unsafe {
            enc.setBuffer_offset_atIndex(Some(&q_out_buf), 0, 0);
            enc.setBuffer_offset_atIndex(Some(&qnope_dev), 0, 1);
            enc.setBuffer_offset_atIndex(Some(&heads_buf), 0, 2);
            enc.setBuffer_offset_atIndex(Some(&qk_buf), 0, 3);
            enc.setBuffer_offset_atIndex(Some(&qnope_buf), 0, 4);
    ...
    // 6) k_b: qnope_dev → qabs_out (Q8_0 64-head slots).
    encode_slots(
        &ctx.q8_0_mla_slots_pipeline,
        kb_buf,
        &qnope_dev,
    ...
    let read_out = |buf: &Retained<ProtocolObject<dyn MTLBuffer>>, elems: usize| -> Vec<f32> {
        let contents: NonNull<std::ffi::c_void> = buf.contents();
        unsafe { std::slice::from_raw_parts(contents.as_ptr() as *const f32, elems) }.to_vec()
    };
    (
        read_out(&q_out_buf, seq_len * q_dim),
        read_out(&kv_out_buf, seq_len * kv_width),
        read_out(&qabs_out_buf, slots * kv_rank),
    )
}

/// pm116: GLM MLA prefill attention (absorbed form) 단일 dispatch.
/// slot = token*head_count+head, causal 범위는 커널이 slot 에서 유도.
/// cache 는 f16 bit (u16) row-major `[cache_len × (kv_rank+rope_dim)]`.
#[allow(clippy::too_many_arguments)]
pub(crate) fn glm_mla_prefill_attn_dispatch(
    ctx: &MetalContext,
    q_absorbed: &[f32],
    q_pe: &[f32],
    cache: &[u16],
    ...
    assert!(kv_rank <= 512 && rope_dim <= 64);
    assert_eq!(q_absorbed.len(), slots * kv_rank);
    assert_eq!(q_pe.len(), slots * rope_dim);
    let shared = MTLResourceOptions::StorageModeShared;

    let mk_bytes = |bytes: &[u8]| -> Retained<ProtocolObject<dyn MTLBuffer>> {
        unsafe {
            let ptr = NonNull::new(bytes.as_ptr() as *mut std::ffi::c_void).expect("ptr is null");
            ctx.device
                .newBufferWithBytes_length_options(ptr, bytes.len(), shared)
                .expect("Metal: failed to create buffer")
        }
    };
    let qa_buf = mk_bytes(unsafe {
        std::slice::from_raw_parts(
            q_absorbed.as_ptr() as *const u8,
            std::mem::size_of_val(q_absorbed),
        )
    });
    let qpe_buf = mk_bytes(unsafe {
        std::slice::from_raw_parts(q_pe.as_ptr() as *const u8, std::mem::size_of_val(q_pe))
    });
    let cache_buf = mk_bytes(unsafe {
        std::slice::from_raw_parts(cache.as_ptr() as *const u8, std::mem::size_of_val(cache))
    });
    let o_buf: Retained<ProtocolObject<dyn MTLBuffer>> = ctx
        .device
        .newBufferWithLength_options(slots * kv_rank * std::mem::size_of::<f32>(), shared)
        .expect("Metal: failed to create output buffer");

    let heads_u32 = head_count as u32;
    let kv_rank_u32 = kv_rank as u32;
    let rope_dim_u32 = rope_dim as u32;
    ...
    let heads_buf = mk_u32(&heads_u32);
    let kv_rank_buf = mk_u32(&kv_rank_u32);
    let rope_dim_buf = mk_u32(&rope_dim_u32);
    let pos_start_buf = mk_u32(&pos_start_u32);
    let scale_buf = unsafe {
        let ptr =
            NonNull::new(&scale as *const f32 as *mut std::ffi::c_void).expect("scale ptr is null");
        ctx.device
            .newBufferWithBytes_length_options(ptr, std::mem::size_of::<f32>(), shared)
            .expect("Metal: failed to create scale buffer")
    };
    ...
    enc.setComputePipelineState(&ctx.glm_mla_prefill_attn_pipeline);
    unsafe {
        enc.setBuffer_offset_atIndex(Some(&qa_buf), 0, 0);
        enc.setBuffer_offset_atIndex(Some(&qpe_buf), 0, 1);
        enc.setBuffer_offset_atIndex(Some(&cache_buf), 0, 2);
        enc.setBuffer_offset_atIndex(Some(&o_buf), 0, 3);
        enc.setBuffer_offset_atIndex(Some(&heads_buf), 0, 4);
        enc.setBuffer_offset_atIndex(Some(&kv_rank_buf), 0, 5);
        enc.setBuffer_offset_atIndex(Some(&rope_dim_buf), 0, 6);
        enc.setBuffer_offset_atIndex(Some(&pos_start_buf), 0, 7);
        enc.setBuffer_offset_atIndex(Some(&scale_buf), 0, 8);
    }
    enc.dispatchThreadgroups_threadsPerThreadgroup(
        MTLSize {
    ...
    let contents: NonNull<std::ffi::c_void> = o_buf.contents();
    let out: &[f32] =
        unsafe { std::slice::from_raw_parts(contents.as_ptr() as *const f32, slots * kv_rank) };
    out.to_vec()
}
...
/// Qwen 3.6 dense causal GQA prefill attention을 threadgroup-resident kernel로 encode한다.
/// q[seq*nh*256] f32, k/v[kv*nkv*256] f16, out[seq*nh*256] f32. 전역 workspace는 없다.
/// grid=(ceil(seq/8), num_heads), tg=128이다. K/V는 caller가 64-row 경계까지 0-padding한다.
#[allow(clippy::too_many_arguments)]
pub(crate) fn encode_flash_attn_prefill(
    ...
    const SCRATCH_BYTES: usize = 8 * 256 * std::mem::size_of::<u16>()
        + 8 * 256 * std::mem::size_of::<f32>()
        + 8 * 64 * std::mem::size_of::<f32>();
    let pipeline = ctx
        .flash_attn_prefill_tg_pipeline
        .as_ref()
        .expect("flash_attn_prefill_tg_pipeline must be built (tensorops_capable)");
    enc.setComputePipelineState(pipeline);
    unsafe {
...
/// Threadgroup-resident flash attention의 host 입출력 테스트/진단 seam.
/// q[seq*nh*256] f32, k/v[kv*nkv*256] f16 bits → out[seq*nh*256] f32.
/// 반환값은 `(output, GPU-time ms)`다.
#[allow(clippy::too_many_arguments)]
pub fn prefill_flash_attention_with_ctx(
    ...
    assert_eq!(head_dim, 256, "threadgroup flash kernel requires HD=256");
    assert_eq!(
        num_heads % num_kv_heads,
        0,
        "threadgroup flash kernel requires grouped-query heads"
    );
    let shared = MTLResourceOptions::StorageModeShared;
    let mk =
        |data: *const std::ffi::c_void, len: usize| -> Retained<ProtocolObject<dyn MTLBuffer>> {
...
pub fn run_q4k_tensorops_v2_variant(
    ...
    run_tensorops_v2_inner(
        ctx, wb, input, n_out, k, m_tok, fn_name, nra, nrb, 256, 144, 64,
    )
}
...
/// Q3_K v2 GEMM(64×128, NK=128=superblock 절반). block 256 elem/110B. standalone runner.
pub fn run_q3k_tensorops_v2(
    ctx: &MetalContext,
    wb: &[u8],
    input: &[f32],
    n_out: usize,
...
/// Q2_K v2 GEMM(64×128, NK=128=superblock 절반). block 256 elem/84B. standalone runner.
pub fn run_q2k_tensorops_v2(
    ctx: &MetalContext,
    wb: &[u8],
    input: &[f32],
    n_out: usize,
...
/// Q8_0 v2 GEMM(64×128, NK=32). unsloth UD attn/GDN/shared projection. block 32 elem/34B.
pub fn run_q8_0_tensorops_v2(
    ctx: &MetalContext,
    wb: &[u8],
    input: &[f32],
    n_out: usize,
...
/// Q8_0 tensorops GEMM encode(v1, unsloth UD attn/GDN/shared proj). 64×32 tile, KC=32.
#[allow(clippy::too_many_arguments)]
pub(crate) fn encode_gemm_q8_0_tensorops(
    ctx: &MetalContext,
    enc: &ProtocolObject<dyn MTLComputeCommandEncoder>,
    w_buf: &ProtocolObject<dyn MTLBuffer>,
    weight_byte_offset: u32,
...
/// Q8_0 v2 tensorops GEMM encode(llama 패턴 64×128, NK=32). GDN in_proj/ATN proj/ssm_out(Q8_0).
#[allow(clippy::too_many_arguments)]
pub(crate) fn encode_gemm_q8_0_tensorops_v2(
    ctx: &MetalContext,
    enc: &ProtocolObject<dyn MTLComputeCommandEncoder>,
    w_buf: &ProtocolObject<dyn MTLBuffer>,
    weight_byte_offset: u32,
    ...
    let pipeline = ctx
        .gemm_q8_0_tensorops_v2_pipeline
        .as_ref()
        .expect("gemm_q8_0_tensorops_v2_pipeline missing (capability=false?)");
...
/// pm123: Q3_K v2 GEMM encode. NRA=64,NRB=128,NK=128,4sg. Q3_K prefill projection.
#[allow(clippy::too_many_arguments)]
pub(crate) fn encode_gemm_q3k_tensorops_v2(
    ctx: &MetalContext,
    enc: &ProtocolObject<dyn MTLComputeCommandEncoder>,
    w_buf: &ProtocolObject<dyn MTLBuffer>,
    weight_byte_offset: u32,
    ...
    let pipeline = ctx
        .gemm_q3k_tensorops_v2_pipeline
        .as_ref()
        .expect("gemm_q3k_tensorops_v2_pipeline missing (capability=false?)");
    encode_tensorops_v2_dispatch(
        pipeline,
        enc,
        w_buf,
...
/// pm123: Q2_K v2 GEMM encode. NRA=64,NRB=128,NK=128,4sg. Q2_K prefill projection.
#[allow(clippy::too_many_arguments)]
pub(crate) fn encode_gemm_q2k_tensorops_v2(
    ctx: &MetalContext,
    enc: &ProtocolObject<dyn MTLComputeCommandEncoder>,
    w_buf: &ProtocolObject<dyn MTLBuffer>,
    weight_byte_offset: u32,
    ...
    let pipeline = ctx
        .gemm_q2k_tensorops_v2_pipeline
        .as_ref()
        .expect("gemm_q2k_tensorops_v2_pipeline missing (capability=false?)");
    encode_tensorops_v2_dispatch(
        pipeline,
        enc,
        w_buf,
...
pub(crate) fn encode_gemv_q8_0(
    ...
    if ctx.gemv_coalesced {
        let nsg2 = q8_0_nsg2_enabled();
        enc.setComputePipelineState(if nsg2 {
            &ctx.q8_0_coalesced_nsg2_pipeline
        } else {
            &ctx.q8_0_coalesced_pipeline
        });
        unsafe {
        ...
        // coalesced NR0=2: 1 SIMD-group(32 lane) → row 2, grid=ceil(N/2).
        // nsg2: 2 SIMD-group(64 lane) → row 4, grid=ceil(N/4).
        let (grid_w, tg_w) = if nsg2 {
            (n.div_ceil(4), SIMD_WIDTH * 2)
        } else {
            (n.div_ceil(2), SIMD_WIDTH)
        };
        let grid = MTLSize {
...
pub(crate) fn encode_gemv_quant(
    ...
    match quant {
        ...
        5 => encode_gemv_q3k_simd(ctx, enc, w_buf, in_buf, out_buf, n_buf, k_buf, off_buf, n),
        6 => encode_gemv_q2k_simd(ctx, enc, w_buf, in_buf, out_buf, n_buf, k_buf, off_buf, n),
        _ => panic!("unsupported gemv quant code {quant}"),
    }
}

/// quant code → B-column weight-amortized GEMV pipeline (milestone 2).
pub(crate) fn gemv_bcol_pipeline_for(
    ctx: &MetalContext,
    quant: u8,
) -> &ProtocolObject<dyn MTLComputePipelineState> {
    match quant {
        0 => &ctx.q4k_bcol_pipeline,
        1 => &ctx.q5k_bcol_pipeline,
        2 => &ctx.q6k_bcol_pipeline,
        3 => &ctx.q8_0_bcol_pipeline,
        4 => &ctx.f32_bcol_pipeline,
        _ => panic!("unsupported bcol gemv quant code {quant}"),
    }
}

/// B-column weight-amortized GEMV (milestone 2, MTP verify). 한 dispatch 가 weight tile 을
/// device memory 에서 **1회** 읽어 `b` 개 입력 컬럼(`in_buf`=[B*K], column-major-by-lane)에
/// 대해 `b` 개 출력 컬럼(`out_buf`=[B*N])을 계산한다. memory-bound decode 의 weight 읽기를
/// batch-B 로 amortize — batch-B forward 가 ~B decode 가 아니라 ~1 decode 비용에 근접.
///
/// 결과는 encode_gemv_quant(같은 quant) 를 컬럼마다 1회씩 `b` 번 호출한 것과 수치적으로
/// 동일하다(simd_sum reduction 순서만 다름, rel<1e-3). block_elements/block_bytes 는 커널
/// 내부에서 quant 별 분기(Q8_0 32/34 vs K-quant 256/superblock). Q4_K/Q5_K/Q6_K/Q8_0 지원.
#[allow(clippy::too_many_arguments)]
pub(crate) fn encode_gemv_quant_bcol(
    ctx: &MetalContext,
    enc: &ProtocolObject<dyn MTLComputeCommandEncoder>,
    quant: u8,
    w_buf: &ProtocolObject<dyn MTLBuffer>,
    ...
    let pipeline = gemv_bcol_pipeline_for(ctx, quant);
    enc.setComputePipelineState(pipeline);
    unsafe {
        enc.setBuffer_offset_atIndex(Some(w_buf), 0, 0);
        enc.setBuffer_offset_atIndex(Some(in_buf), 0, 1);
        ...
        enc.setBuffer_offset_atIndex(Some(k_buf), 0, 4);
        enc.setBuffer_offset_atIndex(Some(off_buf), 0, 5);
        enc.setBuffer_offset_atIndex(Some(b_buf), 0, 6);
    }
    // threadgroup = 1 SIMD-group(32 lane)/row, grid = N row (gemv_*_simd 와 동일 shape).
    let grid = MTLSize {
        width: n,
        height: 1,
        depth: 1,
...
/// `encode_gemv_quant_bcol` 의 out offset(bytes) 변형 — batched GDN 의 rolling conv buffer
/// (`[state | qkv_0 | ...]`)에 qkv B-column 결과를 `conv_state_len` 슬롯부터 쓰도록 out_buf 를
/// byte offset 으로 바인딩한다. 결과 레이아웃은 out[c*N + row] (column-major-by-lane) 그대로.
#[allow(clippy::too_many_arguments)]
pub(crate) fn encode_gemv_quant_bcol_out_at(
    ctx: &MetalContext,
    enc: &ProtocolObject<dyn MTLComputeCommandEncoder>,
    quant: u8,
    w_buf: &ProtocolObject<dyn MTLBuffer>,
    ...
    unsafe {
        enc.setBuffer_offset_atIndex(Some(w_buf), 0, 0);
        enc.setBuffer_offset_atIndex(Some(in_buf), 0, 1);
        enc.setBuffer_offset_atIndex(Some(out_buf), out_off_bytes, 2);
        enc.setBuffer_offset_atIndex(Some(n_buf), 0, 3);
        enc.setBuffer_offset_atIndex(Some(k_buf), 0, 4);
        enc.setBuffer_offset_atIndex(Some(off_buf), 0, 5);
        enc.setBuffer_offset_atIndex(Some(b_buf), 0, 6);
...
/// milestone 2: B-column weight-amortized GEMV 정확성 oracle + amortization 측정.
/// oracle = "1 dispatch 로 weight 를 1회 읽어 B 컬럼 계산" == "검증된 1-컬럼 커널
/// (encode_gemv_quant) 을 컬럼마다 B 번 호출". Q4_K/Q5_K/Q6_K/Q8_0 전부.
#[cfg(all(test, target_os = "macos"))]
mod gemv_bcol_tests {
    use super::*;

    fn mk_shared(ctx: &MetalContext, bytes: &[u8]) -> Retained<ProtocolObject<dyn MTLBuffer>> {
        let shared = MTLResourceOptions::StorageModeShared;
        let len = bytes.len().max(1);
        let ptr = NonNull::new(bytes.as_ptr() as *mut std::ffi::c_void).expect("ptr");
        unsafe {
            ctx.device
                .newBufferWithBytes_length_options(ptr, len, shared)
                .expect("buf")
    ...
    fn empty_shared(
        ...
        let shared = MTLResourceOptions::StorageModeShared;
        ctx.device
            .newBufferWithLength_options(len_bytes.max(1), shared)
            .expect("out")
    }
    ...
    fn build_weight(quant: u8, n: usize, k: usize) -> Vec<u8> {
        match quant {
            0 => {
                let vals: Vec<f32> = (0..n * k)
                    .map(|i| ((i * 37 % 251) as f32 - 125.0) / 97.0)
                    .collect();
                rnb_cpu::quantize::quantize_q4_k_vec(&vals)
            }
            1 => {
                let vals: Vec<f32> = (0..n * k)
                    .map(|i| ((i * 41 % 253) as f32 - 126.0) / 89.0)
                    .collect();
                rnb_cpu::quantize::quantize_q5_k_vec(&vals)
            }
            2 => crate::tests_fixture::q6k_rows_pattern(n, k),
            3 => build_q8_0_rows(n, k),
            4 => {
                // F32 row-major dense weight: N*K*4 little-endian.
                let mut out = Vec::with_capacity(n * k * 4);
                for i in 0..n * k {
                    let v = ((i * 43 % 257) as f32 - 128.0) / 83.0;
                    out.extend_from_slice(&v.to_le_bytes());
                }
                out
            }
            _ => unreachable!("unsupported quant {quant}"),
        }
    }

    /// 1-컬럼 reference: encode_gemv_quant 을 컬럼 c 입력으로 1회. 반환 out[N].
    fn run_1col(
        ctx: &MetalContext,
        quant: u8,
        w: &[u8],
        ...
        let w_buf = mk_shared(ctx, w);
        let in_buf = mk_shared(ctx, unsafe {
            std::slice::from_raw_parts(col.as_ptr() as *const u8, std::mem::size_of_val(col))
        });
        let out_buf = empty_shared(ctx, n * 4);
        let n_buf = mk_shared(ctx, &(n as u32).to_le_bytes());
        let k_buf = mk_shared(ctx, &(k as u32).to_le_bytes());
        let off_buf = mk_shared(ctx, &0u32.to_le_bytes());
        let cmd = ctx.queue.commandBuffer().expect("cmd");
        let enc = cmd.computeCommandEncoder().expect("enc");
        encode_gemv_quant(
            ctx, &enc, quant, &w_buf, &in_buf, &out_buf, &n_buf, &k_buf, &off_buf, n,
        );
        enc.endEncoding();
        cmd.commit();
        cmd.waitUntilCompleted();
    ...
    /// B-컬럼: encode_gemv_quant_bcol 1 dispatch. 입력 [B*K], 반환 out[B*N].
    fn run_bcol(
        ctx: &MetalContext,
        quant: u8,
        w: &[u8],
        ...
        let out_buf = empty_shared(ctx, b * n * 4);
        let n_buf = mk_shared(ctx, &(n as u32).to_le_bytes());
        let k_buf = mk_shared(ctx, &(k as u32).to_le_bytes());
        let off_buf = mk_shared(ctx, &0u32.to_le_bytes());
        let b_buf = mk_shared(ctx, &(b as u32).to_le_bytes());
        let cmd = ctx.queue.commandBuffer().expect("cmd");
        let enc = cmd.computeCommandEncoder().expect("enc");
        encode_gemv_quant_bcol(
            ctx, &enc, quant, &w_buf, &in_buf, &out_buf, &n_buf, &k_buf, &off_buf, &b_buf, n,
        );
        enc.endEncoding();
        cmd.commit();
        cmd.waitUntilCompleted();
        let c: NonNull<std::ffi::c_void> = out_buf.contents();
        unsafe { std::slice::from_raw_parts(c.as_ptr() as *const f32, b * n).to_vec() }
    }

    fn assert_quant(ctx: &MetalContext, quant: u8, n: usize, k: usize, b: usize) {
        let w = build_weight(quant, n, k);
        let input: Vec<f32> = (0..b * k)
            .map(|i| {
                let c = i / k;
                ((i % 197) as f32 - 98.0) / 71.0 + (c as f32) * 0.13
            })
            .collect();
        let bcol = run_bcol(ctx, quant, &w, &input, n, k, b);
        for c in 0..b {
            let col = &input[c * k..(c + 1) * k];
            let refc = run_1col(ctx, quant, &w, col, n, k);
            for row in 0..n {
                let got = bcol[c * n + row];
                let want = refc[row];
                let abs = (got - want).abs();
                let rel = abs / want.abs().max(1e-6);
                assert!(
                    rel < 1e-3 || abs < 2e-3,
                    "quant={quant} n={n} k={k} b={b} col={c} row={row} bcol={got} ref={want} abs={abs} rel={rel}"
                );
            }
        }
    }
    ...
    #[test]
    #[ignore = "requires a Metal device"]
    fn gemv_bcol_matches_single_column_all_quants() {
        let Some(ctx) = build_metal_context() else {
            return;
        };
        // K-quant: K multiple of 256; Q8_0: multiple of 32. N/K/B 다양.
        for &(n, k) in &[(8usize, 512usize), (37, 1024), (256, 2048), (129, 4096)] {
            for &b in &[1usize, 2, 3, 4] {
                assert_quant(&ctx, 0, n, k, b); // Q4_K
                assert_quant(&ctx, 1, n, k, b); // Q5_K
                assert_quant(&ctx, 2, n, k, b); // Q6_K
                assert_quant(&ctx, 3, n, k, b); // Q8_0
                assert_quant(&ctx, 4, n, k, b); // F32 (dense, arbitrary K)
            }
        }
        // F32 은 블록 제약이 없으므로 임의 K(K=1 rank-1 ssm_out 포함) 도 검증.
        for &(n, k) in &[(256usize, 1usize), (16, 7), (1, 3), (64, 33)] {
            for &b in &[1usize, 2, 4] {
                assert_quant(&ctx, 4, n, k, b);
            }
        }
    }

    /// amortization 측정: **같은 bcol 커널**을 B=1/2/4 로 재서 weight-read amortize 를
    /// apples-to-apples 로 확인한다. weight-amortized 라면 B=2/4 GPU 시간이 B=1 에 근접
    /// (≈1x, ≪ Bx). 추가로 production 1-컬럼 GEMV(encode_gemv_quant) 대비 bcol-B1 이
    /// 느리지 않음을 확인 → bcol-B2 가 per-lane baseline(2회 1-컬럼)을 진짜로 이김을 보장.
    /// weight 가 지배하도록 large tile(N=K=8192, Q4_K 37.7MB)에서 GPUStart/End 로 측정.
    #[test]
    #[ignore = "requires a Metal device"]
    fn gemv_bcol_amortizes_weight_reads() {
        let Some(ctx) = build_metal_context() else {
            return;
        };
        ...
        let reps = 50usize;
        let bmax = 4usize;
        let input: Vec<f32> = (0..bmax * k)
            .map(|i| ((i % 197) as f32 - 98.0) / 71.0)
            .collect();
        let n_buf = mk_shared(&ctx, &(n as u32).to_le_bytes());
        let k_buf = mk_shared(&ctx, &(k as u32).to_le_bytes());
        let off_buf = mk_shared(&ctx, &0u32.to_le_bytes());
        // 한 dispatch(자체 command buffer)의 GPU 시간(GPUEnd-GPUStart) 최소값. 큰 weight 라
        // launch overhead ≪ kernel time. reps 반복 → 노이즈 하한.
        let gpu_ms = |enc_fn: &dyn Fn(&ProtocolObject<dyn MTLComputeCommandEncoder>)| -> f64 {
        ...
        eprintln!("[bcol-amortize] N={n} K={k} reps={reps}");
        let mut fails: Vec<String> = Vec::new();
        for &quant in &[0u8, 1, 2, 3] {
            let w = build_weight(quant, n, k);
            let w_buf = mk_shared(&ctx, &w);
            let in_buf = mk_shared(&ctx, unsafe {
                std::slice::from_raw_parts(
            ...
            let out = empty_shared(&ctx, bmax * n * 4);
            let b_bufs: Vec<_> = (1..=bmax)
                .map(|b| mk_shared(&ctx, &(b as u32).to_le_bytes()))
                .collect();
            let bcol = |b: usize| -> f64 {
                let b_buf = &b_bufs[b - 1];
                let f = |enc: &ProtocolObject<dyn MTLComputeCommandEncoder>| {
                    encode_gemv_quant_bcol(
                        &ctx, enc, quant, &w_buf, &in_buf, &out, &n_buf, &k_buf, &off_buf, b_buf, n,
                    );
                };
                gpu_ms(&f)
            };
            let single = || -> f64 {
                let f = |enc: &ProtocolObject<dyn MTLComputeCommandEncoder>| {
                    encode_gemv_quant(
                        &ctx, enc, quant, &w_buf, &in_buf, &out, &n_buf, &k_buf, &off_buf, n,
                    );
                };
                gpu_ms(&f)
            };
            ...
            let t_b4 = bcol(4);
            eprintln!(
                "[bcol-amortize] quant={quant}  1col={t_single:.4}ms  bcol1={t_b1:.4}ms  bcol2={t_b2:.4}ms  bcol4={t_b4:.4}ms  | B2/B1={:.2}x  B4/B1={:.2}x  B2/1col={:.2}x  speedup(2x1col/B2)={:.2}x",
                t_b2 / t_b1, t_b4 / t_b1, t_b2 / t_single, 2.0 * t_single / t_b2
            );
            if t_b2 >= t_single * 1.6 {
                fails.push(format!(
                    "quant={quant}: bcol-B2 {t_b2:.4}ms not < 1.6x single {t_single:.4}ms"
                ));
            }
        }
        assert!(fails.is_empty(), "amortization failed: {fails:?}");
    }
}
...
pub(crate) fn gemv_simd_pipeline_for(
    ...
    match quant {
        ...
        5 => Some(&ctx.q3k_simd_pipeline),
        6 => Some(&ctx.q2k_simd_pipeline),
        _ => None,
    }
}
...
pub(crate) fn gemv_coalesced_dispatch_for(
    ...
    match quant {
        ...
        3 if q8_0_nsg2_enabled() => Some(GemvCoalescedDispatch {
            pipeline: &ctx.q8_0_coalesced_nsg2_pipeline,
            rows_per_threadgroup: 4,
            threads_per_threadgroup: SIMD_WIDTH * 2,
        }),
...
/// Q8_0 GEMV auto(coalesced/baseline) + input/output byte offset. shared expert(Q8_0) 용.
/// `encode_gemv_q6k_auto_offset` 와 동일 fn-pointer 시그니처.
#[allow(clippy::too_many_arguments)]
pub(crate) fn encode_gemv_q8_0_auto_offset(
    ctx: &MetalContext,
    enc: &ProtocolObject<dyn MTLComputeCommandEncoder>,
    w_buf: &ProtocolObject<dyn MTLBuffer>,
    in_buf: &ProtocolObject<dyn MTLBuffer>,
    ...
    let (pipeline, grid_w, tg_w) = if ctx.gemv_coalesced {
        (&ctx.q8_0_coalesced_pipeline, n.div_ceil(2), SIMD_WIDTH)
    } else {
        let tw = ctx.q8_0_pipeline.threadExecutionWidth().max(1);
        (&ctx.q8_0_pipeline, n.div_ceil(tw), tw)
    };
    enc.setComputePipelineState(pipeline);
    unsafe {
        enc.setBuffer_offset_atIndex(Some(w_buf), 0, 0);
...
/// pm123: Q3_K decode GEMV (1 SIMD-group/row). chain/per-op 공용 encode.
#[allow(clippy::too_many_arguments)]
pub(crate) fn encode_gemv_q3k_simd(
    ctx: &MetalContext,
    enc: &ProtocolObject<dyn MTLComputeCommandEncoder>,
    w_buf: &ProtocolObject<dyn MTLBuffer>,
    in_buf: &ProtocolObject<dyn MTLBuffer>,
    ...
    enc.setComputePipelineState(&ctx.q3k_simd_pipeline);
    unsafe {
        enc.setBuffer_offset_atIndex(Some(w_buf), 0, 0);
        enc.setBuffer_offset_atIndex(Some(in_buf), 0, 1);
        enc.setBuffer_offset_atIndex(Some(out_buf), 0, 2);
...
/// pm123: Q2_K decode GEMV (1 SIMD-group/row). chain/per-op 공용 encode.
#[allow(clippy::too_many_arguments)]
pub(crate) fn encode_gemv_q2k_simd(
    ctx: &MetalContext,
    enc: &ProtocolObject<dyn MTLComputeCommandEncoder>,
    w_buf: &ProtocolObject<dyn MTLBuffer>,
    in_buf: &ProtocolObject<dyn MTLBuffer>,
    ...
    enc.setComputePipelineState(&ctx.q2k_simd_pipeline);
    unsafe {
        enc.setBuffer_offset_atIndex(Some(w_buf), 0, 0);
        enc.setBuffer_offset_atIndex(Some(in_buf), 0, 1);
        enc.setBuffer_offset_atIndex(Some(out_buf), 0, 2);
...
/// pm123: standalone SIMD GEMV runner (오라클/측정용). pipeline grid=N tg=SIMD_WIDTH.
pub fn run_gemv_simd(
    ctx: &MetalContext,
    pipeline: &ProtocolObject<dyn MTLComputePipelineState>,
    wb: &[u8],
    input: &[f32],
    ...
    cmd.commit();
    cmd.waitUntilCompleted();
    let c: NonNull<std::ffi::c_void> = o_buf.contents();
    unsafe { std::slice::from_raw_parts(c.as_ptr() as *const f32, n).to_vec() }
}
...
/// `encode_argmax_f32` 의 offset 변형 — logits buffer 의 byte offset 을 받아 batched
/// 경로([B*rows] logits 에서 컬럼별 argmax)에서 컬럼마다 호출한다.
pub(crate) fn encode_argmax_f32_at(
    ctx: &MetalContext,
    enc: &ProtocolObject<dyn MTLComputeCommandEncoder>,
    logits_buf: &ProtocolObject<dyn MTLBuffer>,
...
/// `encode_qk_norm` 의 in/out byte-offset 변형 — batched attn 의 `q_all`/`k_all`
/// lane 슬롯(`i*q_dim`/`i*kv_dim`)을 in-place per-head RMSNorm 하도록 바인딩.
#[allow(clippy::too_many_arguments)]
pub(crate) fn encode_qk_norm_at(
    ctx: &MetalContext,
    enc: &ProtocolObject<dyn MTLComputeCommandEncoder>,
    in_buf: &ProtocolObject<dyn MTLBuffer>,
    in_off: usize,
    ...
    enc.setComputePipelineState(&ctx.qk_norm_pipeline);
    unsafe {
        enc.setBuffer_offset_atIndex(Some(in_buf), in_off, 0);
        enc.setBuffer_offset_atIndex(Some(weight_buf), 0, 1);
        enc.setBuffer_offset_atIndex(Some(out_buf), out_off, 2);
        enc.setBuffer_offset_atIndex(Some(hd_buf), 0, 3);
        enc.setBuffer_offset_atIndex(Some(eps_buf), 0, 4);
    }
    let grid = MTLSize {
...
/// → `q_all[i*q_dim]`/`gate_all[i*q_dim]`)으로 split.
#[allow(clippy::too_many_arguments)]
pub(crate) fn encode_split_qgate_at(
    ctx: &MetalContext,
    enc: &ProtocolObject<dyn MTLComputeCommandEncoder>,
    q_full_buf: &ProtocolObject<dyn MTLBuffer>,
    q_full_off: usize,
    ...
    enc.setComputePipelineState(&ctx.split_qgate_pipeline);
    unsafe {
        enc.setBuffer_offset_atIndex(Some(q_full_buf), q_full_off, 0);
        enc.setBuffer_offset_atIndex(Some(query_buf), query_off, 1);
        enc.setBuffer_offset_atIndex(Some(gate_buf), gate_off, 2);
        enc.setBuffer_offset_atIndex(Some(hd_buf), 0, 3);
    }
    let grid = MTLSize {
        width: num_heads,
...
/// `encode_rope_partial` 의 data byte-offset + per-lane pos 변형.
#[allow(clippy::too_many_arguments)]
pub(crate) fn encode_rope_partial_at(
    ctx: &MetalContext,
    enc: &ProtocolObject<dyn MTLComputeCommandEncoder>,
    data_buf: &ProtocolObject<dyn MTLBuffer>,
    data_off: usize,
    ...
    enc.setComputePipelineState(&ctx.rope_partial_pipeline);
    unsafe {
        enc.setBuffer_offset_atIndex(Some(data_buf), data_off, 0);
        enc.setBuffer_offset_atIndex(Some(hd_buf), 0, 1);
        enc.setBuffer_offset_atIndex(Some(dim_buf), 0, 2);
        enc.setBuffer_offset_atIndex(Some(nrot_buf), 0, 3);
        enc.setBuffer_offset_atIndex(Some(theta_scale_buf), 0, 4);
...
/// `encode_gate_apply` 의 byte-offset 변형 — lane 슬롯(`attn_out_all[i*q_dim]`,
/// `gate_all[i*q_dim]`)에 대해 `attn_out *= sigmoid(gate)`.
#[allow(clippy::too_many_arguments)]
pub(crate) fn encode_gate_apply_at(
    ctx: &MetalContext,
    enc: &ProtocolObject<dyn MTLComputeCommandEncoder>,
    attn_out_buf: &ProtocolObject<dyn MTLBuffer>,
    attn_out_off: usize,
    ...
    enc.setComputePipelineState(&ctx.gate_apply_pipeline);
    unsafe {
        enc.setBuffer_offset_atIndex(Some(attn_out_buf), attn_out_off, 0);
        enc.setBuffer_offset_atIndex(Some(gate_buf), gate_off, 1);
        enc.setBuffer_offset_atIndex(Some(n_buf), 0, 2);
    }
    let tg_width = ctx.gate_apply_pipeline.threadExecutionWidth().max(1);
    let grid = MTLSize {
...
/// `encode_kv_append` 의 k/v 입력 byte-offset 변형(f16 KV) — lane 슬롯
/// (`k_all[i*kv_dim]`/`v_all[i*kv_dim]`)을 KV cache 의 `pos` 슬롯에 append.
#[allow(clippy::too_many_arguments)]
pub(crate) fn encode_kv_append_at(
    ctx: &MetalContext,
    enc: &ProtocolObject<dyn MTLComputeCommandEncoder>,
    k_in_buf: &ProtocolObject<dyn MTLBuffer>,
    k_in_off: usize,
    ...
    enc.setComputePipelineState(&ctx.kv_append_pipeline);
    unsafe {
        enc.setBuffer_offset_atIndex(Some(k_in_buf), k_in_off, 0);
        enc.setBuffer_offset_atIndex(Some(v_in_buf), v_in_off, 1);
        enc.setBuffer_offset_atIndex(Some(k_cache_buf), 0, 2);
        enc.setBuffer_offset_atIndex(Some(v_cache_buf), 0, 3);
        enc.setBuffer_offset_atIndex(Some(kvd_buf), 0, 4);
        enc.setBuffer_offset_atIndex(Some(pos_buf), 0, 5);
...
/// `encode_attn_decode` 의 q 입력 + o 출력 byte-offset 변형(f16 KV) — lane 슬롯
/// (`q_all[i*q_dim]` → `attn_out_all[i*q_dim]`), KV cache/kv_len 은 per-lane.
#[allow(clippy::too_many_arguments)]
pub(crate) fn encode_attn_decode_at(
    ctx: &MetalContext,
    enc: &ProtocolObject<dyn MTLComputeCommandEncoder>,
    q_buf: &ProtocolObject<dyn MTLBuffer>,
    q_off: usize,
...
pub(crate) fn encode_attn_decode_splitk(
    ctx: &MetalContext,
    enc: &ProtocolObject<dyn MTLComputeCommandEncoder>,
    q_buf: &ProtocolObject<dyn MTLBuffer>,
...
/// grid = (num_kv_heads, num_splits). KV read 1x, query head 재사용.
pub(crate) fn encode_attn_decode_gqa_splitk(
    ctx: &MetalContext,
    enc: &ProtocolObject<dyn MTLComputeCommandEncoder>,
    q_buf: &ProtocolObject<dyn MTLBuffer>,
    k_f16: &ProtocolObject<dyn MTLBuffer>,
    ...
    assert!(
        head_dim <= 8 * SIMD_WIDTH,
        "attn_decode_gqa_splitk head_dim {head_dim} > 256"
    );
    assert!(
        num_splits >= 2,
        "attn_decode_gqa_splitk requires >=2 splits"
    );
    assert!(
        num_heads / num_kv_heads <= 16,
        "attn_decode_gqa_splitk heads_per_group {} > 16",
        num_heads / num_kv_heads
    );

    // Part kernel: grid = (num_kv_heads, num_splits)
    enc.setComputePipelineState(&ctx.attn_decode_gqa_splitk_part_pipeline);
    unsafe {
        enc.setBuffer_offset_atIndex(Some(q_buf), 0, 0);
        enc.setBuffer_offset_atIndex(Some(k_f16), 0, 1);
        enc.setBuffer_offset_atIndex(Some(v_f16), 0, 2);
    ...
    // Reduce kernel: grid = num_heads (query head별)
    enc.setComputePipelineState(&ctx.attn_decode_gqa_splitk_reduce_pipeline);
    unsafe {
        enc.setBuffer_offset_atIndex(Some(partial_acc), 0, 0);
        enc.setBuffer_offset_atIndex(Some(partial_m), 0, 1);
        enc.setBuffer_offset_atIndex(Some(partial_s), 0, 2);
...
pub struct KvarnResident {
    pub records_buf: Retained<ProtocolObject<dyn MTLBuffer>>,
    pub sink_key_buf: Retained<ProtocolObject<dyn MTLBuffer>>,
    pub sink_value_buf: Retained<ProtocolObject<dyn MTLBuffer>>,
    pub tail_key_buf: Retained<ProtocolObject<dyn MTLBuffer>>,
    pub tail_value_buf: Retained<ProtocolObject<dyn MTLBuffer>>,
    kv_rows: usize,
    key_bits: u8,
    value_bits: u8,
    group: usize,
    ...
    uploaded_record_bytes: usize,
    host_records_base: usize,
    uploaded_sink_keys: usize,
    uploaded_sink_values: usize,
    host_sink_k_base: usize,
    host_sink_v_base: usize,
}
...
impl KvarnResident {
    pub fn new(
        ...
        let kv_rows = request
            ...
            .ok_or_else(|| "Metal KVarN KV row width overflow".to_string())?;
        let record_bytes = request.packed_blocks().len().max(request.block_bytes());
        let records_capacity = record_bytes
            .checked_next_power_of_two()
            .ok_or_else(|| "Metal KVarN record capacity overflow".to_string())?;
        let sink_bytes = request
            .sink_tokens()
            .checked_mul(kv_rows)
            .and_then(|elements| elements.checked_mul(2))
            .ok_or_else(|| "Metal KVarN sink capacity overflow".to_string())?;
        let tail_bytes = request
            .group()
            .checked_mul(kv_rows)
            .and_then(|elements| elements.checked_mul(2))
            .ok_or_else(|| "Metal KVarN tail capacity overflow".to_string())?;
        let shared = MTLResourceOptions::StorageModeShared;
        let make_buffer = |bytes: usize, label: &str| {
            ctx.device
                .newBufferWithLength_options(bytes.max(1), shared)
                .ok_or_else(|| format!("Metal: failed to create {label} buffer"))
        };
        Ok(Self {
            records_buf: make_buffer(records_capacity, "KVarN records")?,
            sink_key_buf: make_buffer(sink_bytes, "KVarN sink key")?,
            sink_value_buf: make_buffer(sink_bytes, "KVarN sink value")?,
            tail_key_buf: make_buffer(tail_bytes, "KVarN tail key")?,
            tail_value_buf: make_buffer(tail_bytes, "KVarN tail value")?,
            kv_rows,
            ...
            records_capacity,
            uploaded_record_bytes: 0,
            host_records_base: 0,
            uploaded_sink_keys: 0,
            uploaded_sink_values: 0,
            host_sink_k_base: 0,
            host_sink_v_base: 0,
        })
    }

    pub fn matches(&self, request: rnb_backend_api::KvarnDecodeRequest<'_>) -> bool {
        self.kv_rows == request.num_kv_heads().saturating_mul(request.head_dim())
            && self.key_bits == request.key_bits()
            && self.value_bits == request.value_bits()
            && self.group == request.group()
            && self.sink_tokens == request.sink_tokens()
            && self.block_bytes == request.block_bytes()
    }
    ...
    pub fn update(
        ...
        if !self.matches(request) {
            return Err("Metal KVarN resident shape mismatch".to_string());
        }
        let records_bytes = request.packed_blocks().len();
        if records_bytes > self.records_capacity {
            self.records_capacity = records_bytes
                .checked_next_power_of_two()
                .ok_or_else(|| "Metal KVarN record capacity overflow".to_string())?;
            self.records_buf = ctx
                .device
                .newBufferWithLength_options(
                    self.records_capacity,
                    MTLResourceOptions::StorageModeShared,
                )
                .ok_or_else(|| "Metal: failed to grow KVarN record buffer".to_string())?;
            self.uploaded_record_bytes = 0;
            self.host_records_base = 0;
        }
        if self.host_records_base != request.packed_blocks().as_ptr() as usize
            || records_bytes < self.uploaded_record_bytes
        {
            self.uploaded_record_bytes = 0;
        }
        if records_bytes > self.uploaded_record_bytes {
            let offset = self.uploaded_record_bytes;
            unsafe {
                std::ptr::copy_nonoverlapping(
                    request.packed_blocks().as_ptr().add(offset),
                    (self.records_buf.contents().as_ptr() as *mut u8).add(offset),
                    records_bytes - offset,
                );
            }
            self.uploaded_record_bytes = records_bytes;
            self.host_records_base = request.packed_blocks().as_ptr() as usize;
        }

        let sink_key_elements = request.sink_key().len();
        if self.host_sink_k_base != request.sink_key().as_ptr() as usize
            || sink_key_elements < self.uploaded_sink_keys
        {
            self.uploaded_sink_keys = 0;
        }
        if sink_key_elements > self.uploaded_sink_keys {
            let offset = self.uploaded_sink_keys;
            unsafe {
                std::ptr::copy_nonoverlapping(
                    request.sink_key().as_ptr().add(offset),
                    (self.sink_key_buf.contents().as_ptr() as *mut u16).add(offset),
                    sink_key_elements - offset,
                );
            }
            self.uploaded_sink_keys = sink_key_elements;
            self.host_sink_k_base = request.sink_key().as_ptr() as usize;
        }

        let sink_value_elements = request.sink_value().len();
        if self.host_sink_v_base != request.sink_value().as_ptr() as usize
            || sink_value_elements < self.uploaded_sink_values
        {
            self.uploaded_sink_values = 0;
        }
        if sink_value_elements > self.uploaded_sink_values {
            let offset = self.uploaded_sink_values;
            unsafe {
                std::ptr::copy_nonoverlapping(
                    request.sink_value().as_ptr().add(offset),
                    (self.sink_value_buf.contents().as_ptr() as *mut u16).add(offset),
                    sink_value_elements - offset,
                );
            }
            self.uploaded_sink_values = sink_value_elements;
            self.host_sink_v_base = request.sink_value().as_ptr() as usize;
        }

        if !request.tail_key().is_empty() {
            unsafe {
                std::ptr::copy_nonoverlapping(
                    request.tail_key().as_ptr(),
                    self.tail_key_buf.contents().as_ptr() as *mut u16,
                    request.tail_key().len(),
                );
                std::ptr::copy_nonoverlapping(
                    request.tail_value().as_ptr(),
                    self.tail_value_buf.contents().as_ptr() as *mut u16,
                    request.tail_value().len(),
                );
            }
        }
        Ok(())
    ...
    /// decode chain in-chain: 새 토큰 k/v(f32, post-rope, row_width) 을 tail 버퍼의
    /// `slot` 위치에 f16 으로 append (encode_kv_append 재사용 — tail 을 KV cache 로 취급).
    pub(crate) fn encode_tail_append(
        &self,
        ctx: &MetalContext,
        enc: &ProtocolObject<dyn MTLComputeCommandEncoder>,
        k_dev: &ProtocolObject<dyn MTLBuffer>,
        v_dev: &ProtocolObject<dyn MTLBuffer>,
        row_width: usize,
        slot: u32,
    ) {
        let kvd_buf = crate::ffn_chain::u32_buf(ctx, row_width as u32);
        let slot_buf = crate::ffn_chain::u32_buf(ctx, slot);
        encode_kv_append(
            ctx,
            enc,
            k_dev,
...
/// decode chain in-chain: prior(kv_len-1) request 로부터 params(kv_len/tail_len +=1)
/// 버퍼 + append slot(=prior tail_len) 을 만든다. request 의 query 는 dummy 여도 되며
/// (params 는 shape/kv_len/tail_start 만 사용) validate 통과용 길이만 맞으면 된다.
pub(crate) fn kvarn_chain_params_buf(
    ctx: &MetalContext,
    request: rnb_backend_api::KvarnDecodeRequest<'_>,
) -> Result<(Retained<ProtocolObject<dyn MTLBuffer>>, u32, usize), String> {
    let mut params = kvarn_attention_params(request)?;
    let append_slot = params.tail_len;
    params.kv_len += 1;
    params.tail_len += 1;
    let ptr = NonNull::new((&params as *const KvarnAttentionParams) as *mut std::ffi::c_void)
        .ok_or_else(|| "Metal KVarN chain params pointer is null".to_string())?;
    let buf = unsafe {
        ctx.device
            .newBufferWithBytes_length_options(
                ptr,
                std::mem::size_of::<KvarnAttentionParams>(),
                MTLResourceOptions::StorageModeShared,
            )
            .ok_or_else(|| "Metal: failed to create KVarN chain params buffer".to_string())?
    };
    Ok((buf, append_slot, params.num_blocks as usize))
}

/// decode chain in-chain KVarn attention 실행 컨텍스트(borrow). encode 시 device
/// tail-append + split-K attention 에 필요한 buffer/스칼라를 묶는다.
#[derive(Clone, Copy)]
pub(crate) struct KvarnChainEncode<'a> {
    pub resident: &'a KvarnResident,
    pub params_buf: &'a ProtocolObject<dyn MTLBuffer>,
    pub num_splits_buf: &'a ProtocolObject<dyn MTLBuffer>,
    pub partial_acc: &'a ProtocolObject<dyn MTLBuffer>,
    pub partial_m: &'a ProtocolObject<dyn MTLBuffer>,
    pub partial_s: &'a ProtocolObject<dyn MTLBuffer>,
    pub append_slot: u32,
...
struct KvarnAttentionParams {
    ...
    value_bits: u32,
    block_bytes: u32,
    key_packed_offset: u32,
    key_scale_offset: u32,
    key_zero_offset: u32,
    key_token_scale_offset: u32,
    value_packed_offset: u32,
    value_channel_scale_offset: u32,
    value_token_scale_offset: u32,
    value_zero_offset: u32,
    window_start: u32,
    head_dim: u32,
    scale: f32,
    softcap: f32,
...
fn kvarn_attention_params(
    request: rnb_backend_api::KvarnDecodeRequest<'_>,
) -> Result<KvarnAttentionParams, String> {
    request.validate()?;
    let channels = request
        .num_kv_heads()
        .checked_mul(request.head_dim())
        .ok_or_else(|| "Metal KVarN channel count overflow".to_string())?;
    let token_rows = request
        .num_kv_heads()
        .checked_mul(request.group())
        .ok_or_else(|| "Metal KVarN token-row count overflow".to_string())?;
    let key_packed_bytes = channels
        .checked_mul(request.group())
        .map(|bytes| bytes / 2)
        .ok_or_else(|| "Metal KVarN packed-key size overflow".to_string())?;
    let value_packed_bytes = token_rows
        .checked_mul(request.head_dim())
        .map(|bytes| bytes / (8 / request.value_bits() as usize))
        .ok_or_else(|| "Metal KVarN packed-value size overflow".to_string())?;
    let channel_scale_bytes = channels
        .checked_mul(2)
        .ok_or_else(|| "Metal KVarN channel-scale size overflow".to_string())?;
    let token_scale_bytes = token_rows
        .checked_mul(2)
        .ok_or_else(|| "Metal KVarN token-scale size overflow".to_string())?;
    let key_packed_offset = 0usize;
    let key_scale_offset = key_packed_offset
        .checked_add(key_packed_bytes)
        .ok_or_else(|| "Metal KVarN key-scale offset overflow".to_string())?;
    let key_zero_offset = key_scale_offset
        .checked_add(channel_scale_bytes)
        .ok_or_else(|| "Metal KVarN key-zero offset overflow".to_string())?;
    let key_token_scale_offset = key_zero_offset
        .checked_add(channel_scale_bytes)
        .ok_or_else(|| "Metal KVarN key-token-scale offset overflow".to_string())?;
    let value_packed_offset = key_token_scale_offset
        .checked_add(token_scale_bytes)
        .ok_or_else(|| "Metal KVarN packed-value offset overflow".to_string())?;
    let value_channel_scale_offset = value_packed_offset
        .checked_add(value_packed_bytes)
        .ok_or_else(|| "Metal KVarN value-channel-scale offset overflow".to_string())?;
    let value_token_scale_offset = value_channel_scale_offset
        .checked_add(channel_scale_bytes)
        .ok_or_else(|| "Metal KVarN value-token-scale offset overflow".to_string())?;
    let value_zero_offset = value_token_scale_offset
        .checked_add(token_scale_bytes)
        .ok_or_else(|| "Metal KVarN value-zero offset overflow".to_string())?;
    let expected_block_bytes = value_zero_offset
        .checked_add(token_scale_bytes)
        .ok_or_else(|| "Metal KVarN block size overflow".to_string())?;
    if expected_block_bytes != request.block_bytes() {
        return Err(format!(
            "Metal KVarN block size mismatch: got {} expected {expected_block_bytes}",
            request.block_bytes()
        ));
    }
    let row_width = channels;
    ...
    let window_start = request
        .sliding_window()
        .map(|window| request.kv_len().saturating_sub(window))
        .unwrap_or(0);
    let u32_value = |value: usize, label: &str| {
        u32::try_from(value).map_err(|_| format!("Metal KVarN {label} exceeds u32"))
    };
    Ok(KvarnAttentionParams {
        kv_len: u32_value(request.kv_len(), "KV length")?,
        tail_start: u32_value(request.tail_start(), "tail start")?,
        sink_len: u32_value(sink_len, "sink length")?,
        tail_len: u32_value(tail_len, "tail length")?,
        num_blocks: u32_value(num_blocks, "block count")?,
        num_heads: u32_value(request.num_heads(), "head count")?,
        num_kv_heads: u32_value(request.num_kv_heads(), "KV head count")?,
        group: u32_value(request.group(), "group")?,
        value_bits: request.value_bits() as u32,
        block_bytes: u32_value(request.block_bytes(), "block size")?,
        key_packed_offset: u32_value(key_packed_offset, "packed-key offset")?,
        key_scale_offset: u32_value(key_scale_offset, "key-scale offset")?,
        key_zero_offset: u32_value(key_zero_offset, "key-zero offset")?,
        key_token_scale_offset: u32_value(key_token_scale_offset, "key-token-scale offset")?,
        value_packed_offset: u32_value(value_packed_offset, "packed-value offset")?,
        value_channel_scale_offset: u32_value(
            value_channel_scale_offset,
            "value-channel-scale offset",
        )?,
        value_token_scale_offset: u32_value(value_token_scale_offset, "value-token-scale offset")?,
        value_zero_offset: u32_value(value_zero_offset, "value-zero offset")?,
        window_start: u32_value(window_start, "window start")?,
        head_dim: u32_value(request.head_dim(), "head dimension")?,
        scale: request.scale(),
        softcap: request.softcap().unwrap_or(0.0),
    })
}

/// KVarN attention decode 커널을 주어진 encoder 에 encode(commit/wait 없음).
/// per-op(`kvarn_attention_decode_with_ctx`)와 decode chain in-chain 경로가 공유.
#[allow(clippy::too_many_arguments)]
pub(crate) fn encode_kvarn_attention(
    ctx: &MetalContext,
    enc: &ProtocolObject<dyn MTLComputeCommandEncoder>,
    resident: &KvarnResident,
    query_buf: &ProtocolObject<dyn MTLBuffer>,
    ...
    enc.setComputePipelineState(&ctx.kvarn_attention_pipeline);
    unsafe {
        enc.setBuffer_offset_atIndex(Some(output_buf), 0, 0);
        enc.setBuffer_offset_atIndex(Some(query_buf), 0, 1);
        enc.setBuffer_offset_atIndex(Some(&resident.records_buf), 0, 2);
        enc.setBuffer_offset_atIndex(Some(&resident.sink_key_buf), 0, 3);
        enc.setBuffer_offset_atIndex(Some(&resident.sink_value_buf), 0, 4);
        enc.setBuffer_offset_atIndex(Some(&resident.tail_key_buf), 0, 5);
        enc.setBuffer_offset_atIndex(Some(&resident.tail_value_buf), 0, 6);
        enc.setBuffer_offset_atIndex(Some(params_buf), 0, 7);
    }
    enc.dispatchThreadgroups_threadsPerThreadgroup(
        MTLSize {
            width: num_heads,
...
pub fn kvarn_attention_decode_with_ctx(
    ctx: &MetalContext,
    resident: &mut KvarnResident,
    request: rnb_backend_api::KvarnDecodeRequest<'_>,
) -> Result<Vec<f32>, String> {
    let params = kvarn_attention_params(request)?;
    resident.update(ctx, request)?;
    let shared = MTLResourceOptions::StorageModeShared;
    let make_bytes = |ptr: *const std::ffi::c_void, bytes: usize, label: &str| {
        let ptr = NonNull::new(ptr as *mut std::ffi::c_void)
            .ok_or_else(|| format!("Metal KVarN {label} pointer is null"))?;
        unsafe {
            ctx.device
                .newBufferWithBytes_length_options(ptr, bytes, shared)
                .ok_or_else(|| format!("Metal: failed to create KVarN {label} buffer"))
        }
    };
    let query_buf = make_bytes(
        request.query().as_ptr().cast(),
        std::mem::size_of_val(request.query()),
        "query",
    )?;
    let params_buf = make_bytes(
        (&params as *const KvarnAttentionParams).cast(),
        std::mem::size_of::<KvarnAttentionParams>(),
        "params",
    )?;
    let output_len = request.num_heads() * request.head_dim();
    let output_buf = ctx
        .device
        .newBufferWithLength_options(output_len * std::mem::size_of::<f32>(), shared)
        .ok_or_else(|| "Metal: failed to create KVarN output buffer".to_string())?;
    let command = ctx
        .queue
        .commandBuffer()
        .ok_or_else(|| "Metal: failed to create KVarN command buffer".to_string())?;
    let encoder = command
        .computeCommandEncoder()
        .ok_or_else(|| "Metal: failed to create KVarN compute encoder".to_string())?;
    encode_kvarn_attention(
        ctx,
        &encoder,
        resident,
        &query_buf,
    ...
    if command.status() != MTLCommandBufferStatus::Completed {
        let error = command
            ...
            .unwrap_or_else(|| "no NSError attached".to_string());
        return Err(format!(
            "Metal KVarN command failed status={:?} error={error}",
            command.status()
        ));
    }
    let output = unsafe {
        std::slice::from_raw_parts(output_buf.contents().as_ptr() as *const f32, output_len)
    };
    Ok(output.to_vec())
}

/// KVarN attention decode + device tail-append(새 토큰 k/v f32→f16)을 단일 command
/// buffer 로 실행. decode chain 이 attn layer 를 in-chain 편입할 때 쓰는 append+attention
/// 커널 조합의 standalone 버전(oracle/검증 및 chain 재사용용). `prior_request` 는 새
/// 토큰 append 이전(kv_len-1) 상태의 host KVarN view, `new_k`/`new_v` 는 rope·qk-norm
/// 적용된 새 토큰 k/v(f32, row_width = num_kv_heads*head_dim).
pub fn kvarn_attention_decode_appended_with_ctx(
    ctx: &MetalContext,
    resident: &mut KvarnResident,
    prior_request: rnb_backend_api::KvarnDecodeRequest<'_>,
    new_k: &[f32],
    new_v: &[f32],
) -> Result<Vec<f32>, String> {
    let num_heads = prior_request.num_heads();
    let head_dim = prior_request.head_dim();
    let row_width = prior_request
        .num_kv_heads()
        .checked_mul(head_dim)
        .ok_or_else(|| "Metal KVarN row width overflow".to_string())?;
    if new_k.len() != row_width || new_v.len() != row_width {
        return Err(format!(
            "Metal KVarN appended token width mismatch: k={} v={} expected={row_width}",
            new_k.len(),
            new_v.len()
        ));
    }
    let mut params = kvarn_attention_params(prior_request)?;
    let append_slot = params.tail_len; // prior tail_len = 새 토큰 append slot
    params.kv_len += 1;
    params.tail_len += 1;
    resident.update(ctx, prior_request)?;
    let shared = MTLResourceOptions::StorageModeShared;
    let make_bytes = |ptr: *const std::ffi::c_void, bytes: usize, label: &str| {
        let ptr = NonNull::new(ptr as *mut std::ffi::c_void)
            .ok_or_else(|| format!("Metal KVarN {label} pointer is null"))?;
        unsafe {
            ctx.device
                .newBufferWithBytes_length_options(ptr, bytes.max(1), shared)
                .ok_or_else(|| format!("Metal: failed to create KVarN {label} buffer"))
        }
    };
    let query_buf = make_bytes(
        prior_request.query().as_ptr().cast(),
        std::mem::size_of_val(prior_request.query()),
        "query",
    )?;
    let params_buf = make_bytes(
        (&params as *const KvarnAttentionParams).cast(),
    ...
    let new_k_buf = make_bytes(
        new_k.as_ptr().cast(),
        std::mem::size_of_val(new_k),
        "new key",
    )?;
    let new_v_buf = make_bytes(
        new_v.as_ptr().cast(),
        std::mem::size_of_val(new_v),
        "new value",
    )?;
    let row_width_u32 =
        u32::try_from(row_width).map_err(|_| "Metal KVarN row width exceeds u32".to_string())?;
    let kvd_buf = make_bytes(
        (&row_width_u32 as *const u32) as *const std::ffi::c_void,
        std::mem::size_of::<u32>(),
        "row width",
    )?;
    let slot_buf = make_bytes(
        (&append_slot as *const u32) as *const std::ffi::c_void,
        std::mem::size_of::<u32>(),
        "append slot",
    )?;
    ...
    encode_kv_append(
        ...
        &new_k_buf,
        &new_v_buf,
        &resident.tail_key_buf,
        &resident.tail_value_buf,
        &kvd_buf,
        &slot_buf,
        row_width,
    );
    ...
    if command.status() != MTLCommandBufferStatus::Completed {
        let error = command
            ...
            .unwrap_or_else(|| "no NSError attached".to_string());
        return Err(format!(
            "Metal KVarN appended command failed status={:?} error={error}",
            command.status()
        ));
    }
    let output = unsafe {
...
/// split-K KVarN attention 을 주어진 encoder 에 encode(part + reduce, commit/wait 없음).
/// partial_acc([num_splits*num_heads*head_dim]) / partial_m,partial_s([num_splits*num_heads])
/// 는 caller 소유 scratch. per-op + decode chain in-chain 공유.
#[allow(clippy::too_many_arguments)]
pub(crate) fn encode_kvarn_attention_splitk(
    ctx: &MetalContext,
    enc: &ProtocolObject<dyn MTLComputeCommandEncoder>,
    resident: &KvarnResident,
    query_buf: &ProtocolObject<dyn MTLBuffer>,
    output_buf: &ProtocolObject<dyn MTLBuffer>,
    params_buf: &ProtocolObject<dyn MTLBuffer>,
    num_splits_buf: &ProtocolObject<dyn MTLBuffer>,
    partial_acc: &ProtocolObject<dyn MTLBuffer>,
    partial_m: &ProtocolObject<dyn MTLBuffer>,
    partial_s: &ProtocolObject<dyn MTLBuffer>,
    num_heads: usize,
    ...
    enc.setComputePipelineState(&ctx.kvarn_attention_splitk_part_pipeline);
    unsafe {
        enc.setBuffer_offset_atIndex(Some(partial_acc), 0, 0);
        enc.setBuffer_offset_atIndex(Some(partial_m), 0, 1);
        enc.setBuffer_offset_atIndex(Some(partial_s), 0, 2);
        enc.setBuffer_offset_atIndex(Some(query_buf), 0, 3);
        enc.setBuffer_offset_atIndex(Some(&resident.records_buf), 0, 4);
        enc.setBuffer_offset_atIndex(Some(params_buf), 0, 5);
        enc.setBuffer_offset_atIndex(Some(num_splits_buf), 0, 6);
    }
    enc.dispatchThreadgroups_threadsPerThreadgroup(
        MTLSize {
            width: num_heads,
    ...
    enc.memoryBarrierWithScope(MTLBarrierScope::Buffers);
    enc.setComputePipelineState(&ctx.kvarn_attention_splitk_reduce_pipeline);
    unsafe {
        enc.setBuffer_offset_atIndex(Some(output_buf), 0, 0);
        enc.setBuffer_offset_atIndex(Some(partial_acc), 0, 1);
        enc.setBuffer_offset_atIndex(Some(partial_m), 0, 2);
        enc.setBuffer_offset_atIndex(Some(partial_s), 0, 3);
        enc.setBuffer_offset_atIndex(Some(query_buf), 0, 4);
        enc.setBuffer_offset_atIndex(Some(&resident.sink_key_buf), 0, 5);
        enc.setBuffer_offset_atIndex(Some(&resident.sink_value_buf), 0, 6);
        enc.setBuffer_offset_atIndex(Some(&resident.tail_key_buf), 0, 7);
        enc.setBuffer_offset_atIndex(Some(&resident.tail_value_buf), 0, 8);
        enc.setBuffer_offset_atIndex(Some(params_buf), 0, 9);
        enc.setBuffer_offset_atIndex(Some(num_splits_buf), 0, 10);
    }
    enc.dispatchThreadgroups_threadsPerThreadgroup(
        MTLSize {
            width: num_heads,
...
/// split-K KVarN attention decode(part+reduce) standalone(commit/wait 포함). oracle/검증용.
/// num_splits 는 num_blocks 에 비례(1..=8). scratch 는 매 호출 alloc(측정용 chain 은 캐시 사용).
pub fn kvarn_attention_decode_splitk_with_ctx(
    ctx: &MetalContext,
    resident: &mut KvarnResident,
    request: rnb_backend_api::KvarnDecodeRequest<'_>,
) -> Result<Vec<f32>, String> {
    let params = kvarn_attention_params(request)?;
    let num_heads = request.num_heads();
    let head_dim = request.head_dim();
    let num_splits = (params.num_blocks as usize).clamp(1, 8);
    resident.update(ctx, request)?;
    let shared = MTLResourceOptions::StorageModeShared;
    let make_bytes = |ptr: *const std::ffi::c_void, bytes: usize, label: &str| {
        let ptr = NonNull::new(ptr as *mut std::ffi::c_void)
    ...
    let make_zeroed = |bytes: usize, label: &str| {
        ctx.device
            .newBufferWithLength_options(bytes.max(1), shared)
            .ok_or_else(|| format!("Metal: failed to create KVarN {label} buffer"))
    };
    ...
    let num_splits_u32 =
        u32::try_from(num_splits).map_err(|_| "Metal KVarN num_splits exceeds u32".to_string())?;
    let num_splits_buf = make_bytes(
        (&num_splits_u32 as *const u32) as *const std::ffi::c_void,
        std::mem::size_of::<u32>(),
        "num splits",
    )?;
    let f32_size = std::mem::size_of::<f32>();
    let partial_acc = make_zeroed(num_splits * num_heads * head_dim * f32_size, "splitk acc")?;
    let partial_m = make_zeroed(num_splits * num_heads * f32_size, "splitk m")?;
    let partial_s = make_zeroed(num_splits * num_heads * f32_size, "splitk s")?;
    let output_len = num_heads * head_dim;
    let output_buf = make_zeroed(output_len * f32_size, "output")?;
    let command = ctx
        .queue
        .commandBuffer()
        .ok_or_else(|| "Metal: failed to create KVarN command buffer".to_string())?;
    ...
    if command.status() != MTLCommandBufferStatus::Completed {
        let error = command
            ...
            .unwrap_or_else(|| "no NSError attached".to_string());
        return Err(format!(
            "Metal KVarN splitk command failed status={:?} error={error}",
            command.status()
        ));
    }
    let output = unsafe {
...
#[allow(clippy::too_many_arguments)]
pub(crate) fn encode_prefill_neox_qk_norm(
    ctx: &MetalContext,
    enc: &ProtocolObject<dyn MTLComputeCommandEncoder>,
    in_buf: &ProtocolObject<dyn MTLBuffer>,
    weight_buf: &ProtocolObject<dyn MTLBuffer>,
    ...
    enc.setComputePipelineState(&ctx.prefill_neox_qk_norm_pipeline);
    unsafe {
        enc.setBuffer_offset_atIndex(Some(in_buf), 0, 0);
        enc.setBuffer_offset_atIndex(Some(weight_buf), 0, 1);
        enc.setBuffer_offset_atIndex(Some(out_buf), 0, 2);
...
pub fn delta_net_scan_ar_with_ctx(
    ...
    encode_delta_net_scan_ar(
        ctx, &enc, &q_buf, &k_buf, &v_buf, &gate_buf, &beta_buf, &state_buf, &out_buf, &sl_buf,
        &hk_buf, &hv_buf, &nh_buf, &nh_buf, num_heads, head_k_dim, head_v_dim,
    );
    enc.endEncoding();
    cmd.commit();
    cmd.waitUntilCompleted();
...
pub(crate) fn encode_delta_net_scan_ar(
    ...
    hk_buf: &ProtocolObject<dyn MTLBuffer>,
    hv_buf: &ProtocolObject<dyn MTLBuffer>,
    nkh_buf: &ProtocolObject<dyn MTLBuffer>,
    nh_buf: &ProtocolObject<dyn MTLBuffer>,
    num_heads: usize,
    head_k_dim: usize,
    head_v_dim: usize,
    ...
    unsafe {
        ...
        enc.setBuffer_offset_atIndex(Some(nkh_buf), 0, 11);
    }
    let grid = MTLSize {
...
/// `encode_ssm_conv1d_silu` 의 in_offset(bytes) 변형 — batched GDN 의 rolling conv buffer
/// (`[state | qkv_0 | qkv_1 | ...]`)에서 lane i 의 window(columns i..i+kernel)를 읽도록
/// in_buf 를 byte offset `i*channels*4` 로 바인딩한다. out 은 단일 scratch(offset 0).
#[allow(clippy::too_many_arguments)]
pub(crate) fn encode_ssm_conv1d_silu_at(
    ctx: &MetalContext,
    enc: &ProtocolObject<dyn MTLComputeCommandEncoder>,
    in_buf: &ProtocolObject<dyn MTLBuffer>,
    in_off_bytes: usize,
    ...
    enc.setComputePipelineState(&ctx.ssm_conv_silu_pipeline);
    unsafe {
        enc.setBuffer_offset_atIndex(Some(in_buf), in_off_bytes, 0);
        enc.setBuffer_offset_atIndex(Some(weight_buf), 0, 1);
        enc.setBuffer_offset_atIndex(Some(out_buf), 0, 2);
        enc.setBuffer_offset_atIndex(Some(ch_buf), 0, 3);
        enc.setBuffer_offset_atIndex(Some(ks_buf), 0, 4);
...
/// pm127: GDN decode tail fused — `delta_net_step` + `qk_norm`(ssm_norm RMSNorm) +
/// `silu_mul`(z in-place) 를 v-head threadgroup 하나로 encode 한다. 원본 3 dispatch +
/// 2 chain_barrier 를 1 dispatch 로 대체(연산·순서는 원본과 bit-identical).
/// grid = num_v_heads, tg = 256(head_v_dim RMSNorm tree reduction 을 원본 qk_norm 과
/// 같은 덧셈 순서로 유지). head_k_dim/head_v_dim <= 256 전제(caller gate).
/// `v_offset` 은 conv_out 의 v 슬라이스 byte offset((q_dim+k_dim)*4).
#[allow(clippy::too_many_arguments)]
pub(crate) fn encode_gdn_delta_norm_silu(
    ctx: &MetalContext,
    enc: &ProtocolObject<dyn MTLComputeCommandEncoder>,
    q_buf: &ProtocolObject<dyn MTLBuffer>,
    k_buf: &ProtocolObject<dyn MTLBuffer>,
    ...
    enc.setComputePipelineState(&ctx.gdn_delta_norm_silu_pipeline);
    unsafe {
        enc.setBuffer_offset_atIndex(Some(q_buf), 0, 0);
        enc.setBuffer_offset_atIndex(Some(k_buf), 0, 1);
        enc.setBuffer_offset_atIndex(Some(v_buf), v_offset, 2);
        ...
        enc.setBuffer_offset_atIndex(Some(beta_buf), 0, 4);
        enc.setBuffer_offset_atIndex(Some(state_buf), 0, 5);
        enc.setBuffer_offset_atIndex(Some(ssm_norm_w_buf), 0, 6);
        enc.setBuffer_offset_atIndex(Some(z_buf), 0, 7);
        enc.setBuffer_offset_atIndex(Some(hk_buf), 0, 8);
        enc.setBuffer_offset_atIndex(Some(hv_buf), 0, 9);
        enc.setBuffer_offset_atIndex(Some(nk_buf), 0, 10);
        enc.setBuffer_offset_atIndex(Some(eps_buf), 0, 11);
    }
    let grid = MTLSize {
        width: num_v_heads,
        height: 1,
...
/// `encode_delta_net_step` 의 gate/beta offset 변형 — batched GDN 이 lane i 의
/// alpha/beta(각각 `[B*num_v_heads]` buffer 의 `i*num_v_heads` 슬롯)를 읽도록 한다.
/// q/k/out 은 단일 scratch(offset 0), v 는 v_offset, state 는 device-resident in-place.
#[allow(clippy::too_many_arguments)]
pub(crate) fn encode_delta_net_step_at(
    ctx: &MetalContext,
    enc: &ProtocolObject<dyn MTLComputeCommandEncoder>,
    q_buf: &ProtocolObject<dyn MTLBuffer>,
    k_buf: &ProtocolObject<dyn MTLBuffer>,
    ...
    unsafe {
        ...
        enc.setBuffer_offset_atIndex(Some(k_buf), 0, 1);
        enc.setBuffer_offset_atIndex(Some(v_buf), v_offset, 2);
        enc.setBuffer_offset_atIndex(Some(gate_buf), gate_offset, 3);
        enc.setBuffer_offset_atIndex(Some(beta_buf), beta_offset, 4);
        enc.setBuffer_offset_atIndex(Some(state_buf), 0, 5);
        enc.setBuffer_offset_atIndex(Some(out_buf), 0, 6);
        enc.setBuffer_offset_atIndex(Some(hk_buf), 0, 7);
        enc.setBuffer_offset_atIndex(Some(hv_buf), 0, 8);
...
/// `encode_delta_net_step_at` 의 ping-pong 변형: state_in(slot i, read-only) → state_out
/// (slot i+1, write-only). batched GDN 이 prefix state 를 모두 보존하도록 lane 마다 in/out
/// slot 을 전진시킨다. `delta_net_step_io` 커널(수치는 in-place 와 동일).
#[allow(clippy::too_many_arguments)]
pub(crate) fn encode_delta_net_step_io_at(
    ctx: &MetalContext,
    enc: &ProtocolObject<dyn MTLComputeCommandEncoder>,
    q_buf: &ProtocolObject<dyn MTLBuffer>,
    k_buf: &ProtocolObject<dyn MTLBuffer>,
    ...
    beta_buf: &ProtocolObject<dyn MTLBuffer>,
    beta_offset: usize,
    state_in_buf: &ProtocolObject<dyn MTLBuffer>,
    state_in_offset: usize,
    state_out_buf: &ProtocolObject<dyn MTLBuffer>,
    state_out_offset: usize,
    out_buf: &ProtocolObject<dyn MTLBuffer>,
    hk_buf: &ProtocolObject<dyn MTLBuffer>,
    hv_buf: &ProtocolObject<dyn MTLBuffer>,
    nk_buf: &ProtocolObject<dyn MTLBuffer>,
    ...
    enc.setComputePipelineState(&ctx.delta_net_step_io_pipeline);
    unsafe {
        enc.setBuffer_offset_atIndex(Some(q_buf), 0, 0);
        enc.setBuffer_offset_atIndex(Some(k_buf), 0, 1);
        enc.setBuffer_offset_atIndex(Some(v_buf), v_offset, 2);
        enc.setBuffer_offset_atIndex(Some(gate_buf), gate_offset, 3);
        enc.setBuffer_offset_atIndex(Some(beta_buf), beta_offset, 4);
        enc.setBuffer_offset_atIndex(Some(state_in_buf), state_in_offset, 5);
        enc.setBuffer_offset_atIndex(Some(out_buf), 0, 6);
        enc.setBuffer_offset_atIndex(Some(hk_buf), 0, 7);
        enc.setBuffer_offset_atIndex(Some(hv_buf), 0, 8);
        enc.setBuffer_offset_atIndex(Some(nk_buf), 0, 9);
        enc.setBuffer_offset_atIndex(Some(state_out_buf), state_out_offset, 10);
    }
    let grid = MTLSize {
        width: num_heads,
        height: 1,
...
/// `encode_gdn_alpha_beta` 의 alpha/beta offset 변형 — batched GDN 이 lane i 의
/// alpha/beta 슬롯(`[B*num_heads]` buffer 의 `i*num_heads`)을 in-place 전처리하도록 한다.
/// dt_bias/ssm_a 는 per-head weight(offset 0), num_heads 는 그대로.
#[allow(clippy::too_many_arguments)]
pub(crate) fn encode_gdn_alpha_beta_at(
    ctx: &MetalContext,
    enc: &ProtocolObject<dyn MTLComputeCommandEncoder>,
    alpha_buf: &ProtocolObject<dyn MTLBuffer>,
    alpha_offset: usize,
    ...
    enc.setComputePipelineState(&ctx.gdn_alpha_beta_pipeline);
    unsafe {
        enc.setBuffer_offset_atIndex(Some(alpha_buf), alpha_offset, 0);
        enc.setBuffer_offset_atIndex(Some(beta_buf), beta_offset, 1);
        enc.setBuffer_offset_atIndex(Some(dt_buf), 0, 2);
        enc.setBuffer_offset_atIndex(Some(ssm_a_buf), 0, 3);
        enc.setBuffer_offset_atIndex(Some(nh_buf), 0, 4);
    }
...
mod batch_probe {
    ...
    /// Roofline: Q8_0 GEMV(baseline 1-thread/row vs coalesced NR0=2 production) 가
    /// 메모리 대역폭을 몇 % 활용하는지. Q4_K roofline 과 동일 방법(단일 cmdbuf M dispatch,
    /// RUNS median). Q8_0 block=32elem/34byte. 실행: cargo test -p rnb-backend-metal
    ///   --features metal --release -- --ignored --nocapture roofline_q8_0_bw
    #[test]
    #[ignore = "requires Metal device; Q8_0 GEMV 대역폭 roofline"]
    fn roofline_q8_0_bw() {
        let ctx = build_metal_context().expect("no metal device");
        const RUNS: usize = 30;
        const M: usize = 16;
        const PEAK_GBS: f64 = 307.0;
        let shapes: [(&str, usize, usize); 5] = [
            ("q/o     4096x4096 ", 4096, 4096),
            ("kv-gqa  1024x4096 ", 1024, 4096),
            ("gate/up 12288x4096", 12288, 4096),
        ...
        eprintln!(
            "[roofline-q8] M5 Pro peak={PEAK_GBS}GB/s, M={M} dispatch/cmdbuf, RUNS={RUNS} median"
        );
        for &(label, n, k) in &shapes {
            let num_blocks = k / 32;
            let wbytes = n * num_blocks * 34;
            let input = zero_buf(&ctx, k * 4);
            let n_buf = u32_buf(&ctx, n as u32);
            let k_buf = u32_buf(&ctx, k as u32);
            let off_buf = u32_buf(&ctx, 0);
            ...
            let gbs = |per_ms: f64| (wbytes as f64 / 1e9) / (per_ms / 1000.0);
            let exec_w = ctx.q8_0_pipeline.threadExecutionWidth().max(1);
            let base_per = measure(
                &ctx.q8_0_pipeline,
                MTLSize {
                    width: n.div_ceil(exec_w),
            ...
            let (bg, cg) = (gbs(base_per), gbs(coal_per));
            eprintln!(
                "[roofline-q8] {label} ({:>5.1}MB): baseline={bg:>6.1}GB/s({:>4.1}%)  coalesced={cg:>6.1}GB/s({:>4.1}%)  coal/base={:.2}x",
                wbytes as f64 / 1_048_576.0,
                bg / PEAK_GBS * 100.0,
                cg / PEAK_GBS * 100.0,
                cg / bg,
