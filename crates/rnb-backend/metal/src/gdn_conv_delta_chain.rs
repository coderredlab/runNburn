...
pub(crate) struct PrefillGdnConvDeltaCarrier {
    ...
    pub q_final_dev: Retained<ProtocolObject<dyn MTLBuffer>>, // expanded Q fallback or 1-float placeholder
    pub k_final_dev: Retained<ProtocolObject<dyn MTLBuffer>>, // expanded K fallback or 1-float placeholder
...
impl PrefillGdnConvDeltaCarrier {
    ...
    pub(crate) fn new(
        ...
        let use_ar = prefill_gdn_ar_enabled()
            ...
            && head_k_dim % compute::SIMD_WIDTH == 0
            && matches!(head_k_dim / compute::SIMD_WIDTH, 1 | 2 | 4 | 8);
        let final_len = if use_ar {
            1
        } else {
