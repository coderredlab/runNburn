...
/// Milestone 4 batched attn/mixed-chain oracle fixture: 결정적 AttnMoeQwen layer(gated
/// attention core + Qwen MoE FFN). `QwenMoeGdnDecodeChainFixture` 와 같은 HIDDEN_DIM/MoE
/// shape 를 써서 mixed(attn+GDN) chain 을 같은 hidden 차원으로 이어 붙일 수 있다.
/// q/k/v/o = Q4_K, prior KV = 결정적 f16 bits([BASE_POS*KV_DIM]).
#[cfg(target_os = "macos")]
pub struct QwenMoeAttnDecodeChainFixture {
    pub hidden: Vec<f32>,
    norm: Vec<f32>,
    q_norm: Vec<f32>,
    k_norm: Vec<f32>,
...
#[cfg(target_os = "macos")]
impl QwenMoeAttnDecodeChainFixture {
    pub const HIDDEN_DIM: usize = 256;
    pub const NUM_HEADS: usize = 2;
    pub const NUM_KV_HEADS: usize = 1;
    pub const HEAD_DIM: usize = 128;
    pub const Q_DIM: usize = Self::NUM_HEADS * Self::HEAD_DIM; // 256
    pub const Q_OUT_DIM: usize = Self::Q_DIM * 2; // gated([query|gate] 인터리브) 512
    pub const KV_DIM: usize = Self::NUM_KV_HEADS * Self::HEAD_DIM; // 128
    pub const N_ROT: usize = 128;
    ...
    /// layer `layer` 를 base position `pos` 에서 실행하는 spec(kvarn=None, f16 KV).
    pub fn spec(&self, layer: usize, pos: usize) -> crate::AttnMoeQwenChainSpecRef<'_> {
        crate::AttnMoeQwenChainSpecRef {
            layer,
    ...
    /// `spec` 과 동일하되 prior KV 를 caller 가 지정(MTP commit 검증: 배치가 반환한 window K/V 를
    /// 이어붙인 prior 로 다음 토큰을 계산해 순차 참조와 일치하는지 확인).
    #[allow(clippy::too_many_arguments)]
    pub fn spec_with_prior<'a>(
        &'a self,
        layer: usize,
        pos: usize,
        prior_k: &'a [u16],
        prior_v: &'a [u16],
    ) -> crate::AttnMoeQwenChainSpecRef<'a> {
        let mut s = self.spec(layer, pos);
        s.prior_k = prior_k;
        s.prior_v = prior_v;
        s
    }
...
#[cfg(target_os = "macos")]
pub fn qwen_moe_attn_decode_chain_fixture() -> QwenMoeAttnDecodeChainFixture {
    use QwenMoeAttnDecodeChainFixture as F;
    let q4 = q4k_block_fixed();
    let q6 = q6k_block_fixed();
    let hidden = (0..F::HIDDEN_DIM)
        .map(|i| ((i % 17) as f32 - 8.0) * 0.001)
        .collect::<Vec<_>>();
    let prior_len = F::BASE_POS * F::KV_DIM;
    let prior_k = (0..prior_len)
        .map(|i| half::f16::from_f32(((i % 13) as f32 - 6.0) * 0.01).to_bits())
        .collect::<Vec<_>>();
    let prior_v = (0..prior_len)
        .map(|i| half::f16::from_f32(((i % 11) as f32 - 5.0) * 0.013).to_bits())
        .collect::<Vec<_>>();
    QwenMoeAttnDecodeChainFixture {
        hidden,
        norm: vec![1.0; F::HIDDEN_DIM],
...
#[cfg(target_os = "macos")]
pub fn scaled_q5k_matrix(rows: usize, cols: usize, seed: usize) -> Vec<u8> {
    assert_eq!(cols % 256, 0);
    let blocks = rows * (cols / 256);
