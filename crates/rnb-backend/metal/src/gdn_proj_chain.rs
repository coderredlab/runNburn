...
fn encode_prefill_gdn_quant_proj(
    ...
    if input_f16 {
        ...
        match quant {
            ...
            TensoropsQuant::Q8_0 => compute::encode_gemm_q8_0_tensorops(
                ctx,
                enc,
                w_buf,
                off,
            ...
            TensoropsQuant::Q2K => {
                unreachable!("Q2_K prefill requires v2 tensorops path (f16 input); no v1 kernel")
            }
            TensoropsQuant::Q3K => {
                unreachable!("Q3_K prefill requires v2 tensorops path (f16 input); no v1 kernel")
            }
        }
    }
}
    ...
    match weight {
        ...
        crate::GdnBackendWeightRef::Quant(view) => {
            ...
            let pipeline = match view.quant {
                ...
                TensoropsQuant::Q8_0 => ctx.gemm_q8_0_tensorops_v2_pipeline.as_ref(),
                TensoropsQuant::Q2K => ctx.gemm_q2k_tensorops_v2_pipeline.as_ref(),
                TensoropsQuant::Q3K => ctx.gemm_q3k_tensorops_v2_pipeline.as_ref(),
            }
            ...
            unsafe {
                ...
                enc.setThreadgroupMemoryLength_atIndex(
                    match view.quant {
                        ...
                        TensoropsQuant::Q8_0 => 64 * 32 * 2,
                        TensoropsQuant::Q2K => 64 * 128 * 2,
                        TensoropsQuant::Q3K => 64 * 128 * 2,
                    },
                    0,
                );
            }
