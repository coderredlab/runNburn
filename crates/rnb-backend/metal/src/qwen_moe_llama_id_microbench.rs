...
fn print_samples(name: &str, samples: &Samples) {
    ...
    // 병렬/경쟁 실행에서는 타이밍 분산(MAD)이 폭증해 perf gate 가 무의미하다.
    // 측정이 불신뢰(margin 이 값의 절반 이상)면 assert 를 건너뛴다 — 벤치는
    // verdict 를 출력하므로 단독(`--test-threads=1`) 실행 때만 gate 로 작동한다.
    let unreliable = gate_margin >= existing_gate * 0.5 || total_margin >= existing_total * 0.5;
    if unreliable {
        eprintln!(
            "[qwen-llama-id-microbench:{name}] SKIP perf gate: timing variance too high (contended run)"
        );
        return;
    }
    assert!(
...
fn qwen_moe_llama_gate_microbench() {
    let _env_lock = crate::METAL_TEST_ENV_LOCK
        .lock()
