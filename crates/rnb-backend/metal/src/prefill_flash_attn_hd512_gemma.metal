    ...
    for (uint key_col0 = 0u; key_col0 <= max_global_position; key_col0 += C) {
        ...
        for (uint local = 0u; local < Q / NSG; ++local) {
            ...
            const uint global_position = global_row < seq_len
                ...
                : 0u;
            const uint window_start = sliding_window > 0u && global_position + 1u > sliding_window
                ? global_position + 1u - sliding_window
                : 0u;
            const uint score_col = lane * 2u;
            const uint key0 = key_col0 + score_col;
            const uint key1 = key0 + 1u;
            float2 value = float2(
                scores[row * C + score_col],
                scores[row * C + score_col + 1u]) * scale;
            if (softcap > 0.0f) {
                value = softcap * tanh(value / softcap);
            }
            if (global_row >= seq_len || key0 > global_position || key0 < window_start) {
                value[0] = -INFINITY;
            }
            if (global_row >= seq_len || key1 > global_position || key1 < window_start) {
                value[1] = -INFINITY;
            }

            const float old_max = row_max[local];
            const float new_max = simd_max(max(old_max, max(value[0], value[1])));
            const float rescale = isfinite(old_max) ? exp(old_max - new_max) : 0.0f;
            const float2 probability = isfinite(new_max)
                ? exp(value - new_max)
                : float2(0.0f);
