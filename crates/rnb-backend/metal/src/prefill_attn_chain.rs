...
use crate::ffn_chain::{empty_f16_buf, empty_f32_buf, shared_f32_buf, u32_buf};

const BR: usize = 32; // flash 커널 query block rows (FLASH_ATTN_PREFILL_SIMD_SRC 와 일치)
const BC: usize = 64; // flash kv block cols
const HD: usize = 256; // flash head_dim (컴파일타임 고정)
...
pub(crate) struct PrefillAttnChainCarrier {
    ...
    // flash device workspace (n_tg = num_heads * ceil(seq/BR) 기준).
    q_ws: Retained<ProtocolObject<dyn MTLBuffer>>,
    k_ws: Retained<ProtocolObject<dyn MTLBuffer>>,
    v_ws: Retained<ProtocolObject<dyn MTLBuffer>>,
    s_ws: Retained<ProtocolObject<dyn MTLBuffer>>,
    p_ws: Retained<ProtocolObject<dyn MTLBuffer>>,
    pv_ws: Retained<ProtocolObject<dyn MTLBuffer>>,
    acc_ws: Retained<ProtocolObject<dyn MTLBuffer>>,
    // scalar buffer.
    ...
    nqb_buf: Retained<ProtocolObject<dyn MTLBuffer>>,
...
impl PrefillAttnChainCarrier {
    ...
    pub(crate) fn new(
        ...
        Self {
            ...
            k_f16_dev: empty_f16_buf(ctx, seq_len * kv_dim),
            v_f16_dev: empty_f16_buf(ctx, seq_len * kv_dim),
            q_norm_w_dev: empty_f32_buf(ctx, head_dim),
            k_norm_w_dev: empty_f32_buf(ctx, head_dim),
            out_dev: empty_f32_buf(ctx, seq_len * q_dim),
            // flash workspace (half=2byte, f32=4byte). prefill_flash_attention_with_ctx 와 동일 크기.
            q_ws: empty_f16_buf(ctx, n_tg * BR * HD),
            k_ws: empty_f16_buf(ctx, n_tg * BC * HD),
            v_ws: empty_f16_buf(ctx, n_tg * BC * HD),
            s_ws: empty_f32_buf(ctx, n_tg * BR * BC),
            p_ws: empty_f16_buf(ctx, n_tg * BR * BC),
            pv_ws: empty_f32_buf(ctx, n_tg * BR * HD),
            acc_ws: empty_f32_buf(ctx, n_tg * BR * HD),
            ...
            nqb_buf: u32_buf(ctx, num_qblk as u32),
