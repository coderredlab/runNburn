...
pub struct MetalBackend {
    ...
    glm_direct_staging: RefCell<Option<(Retained<ProtocolObject<dyn MTLBuffer>>, usize)>>,
    ...
    pub fn new() -> Self {
        #[cfg(target_os = "macos")]
        {
            ...
            Self {
                ...
                glm_direct_staging: RefCell::new(None),
...
impl MetalBackend {
    ...
    /// `flash_attn_prefill_pipeline` 이 None(non-M5)이면 None 반환(caller CPU fallback).
    #[allow(clippy::too_many_arguments)]
    pub fn prefill_flash_attention(
        ...
        if ctx.flash_attn_prefill_pipeline.is_none() {
            return None;
        }
    ...
    /// `flash_attn_prefill_pipeline`/`cast_f32_f16_pipeline` 없으면(non-M5) None.
    #[allow(clippy::too_many_arguments)]
    pub fn prefill_attn_chain(
        ...
        let ctx = self.ctx.as_ref()?;
        if ctx.flash_attn_prefill_pipeline.is_none() || ctx.cast_f32_f16_pipeline.is_none() {
            return None;
        }
    ...
    fn atn_core_expected_raw_len(view: PrefillAtnCoreWeightView<'_>) -> Option<usize> {
        if view.cols == 0 || view.cols % 256 != 0 {
            return None;
        }
        let block_bytes = match view.quant {
            TensoropsQuant::Q4K => 144,
            TensoropsQuant::Q5K => 176,
            TensoropsQuant::Q6K => 210,
        };
        view.rows
            .checked_mul(view.cols / 256)?
    ...
    fn qwen_prefill_chain_attention_preflight(
        ...
        if core.seq_len == 0
            ...
            || ctx.flash_attn_prefill_pipeline.is_none()
            ...
            match *spec {
                QwenPrefillChainSpecRef::Attention {
                    ...
                    layer_outputs.push(LayerDeviceOut::Attention {
                        layer_idx,
                        k: ffn_chain::empty_f16_buf(ctx, len),
                        v: ffn_chain::empty_f16_buf(ctx, len),
                        len,
                    });
                }
            ...
            for (position, spec) in specs.iter().enumerate() {
                ...
                if let Some(snapshot) = stage_snapshot {
                    ...
                    ffn_chain::encode_qwen_moe_prefill_layer_ops(
                        ctx, &enc, carrier, moe, &hidden_b, &hidden_a,
                    )
                    .map_err(qwen_moe_llama_id_error)?;
                }
                compute::chain_barrier(ctx, &enc);
    ...
    ) -> std::collections::HashMap<(usize, usize), (Retained<ProtocolObject<dyn MTLBuffer>>, u32)>
    {
        ...
        // 2) 파일 핸들 (F_NOCACHE — page cache 경합 회피가 목적).
        ...
        // 3) 스테이징 buffer (grow-only 재사용).
        let staging = {
            let mut slot = self.glm_direct_staging.borrow_mut();
            ...
            slot.as_ref().expect("staged").0.clone()
        };

        // 4) 병렬 pread → 스테이징 contents (구간이 서로 겹치지 않아 안전).
            ...
            for part in plan.chunks(chunk) {
                ...
                scope.spawn(move || {
                    for &((_, len), pool_id, file_off, stage_off) in part {
                        ...
                        // 배정되며 staging 은 이 함수 밖에서 아직 GPU 미사용.
                        let dst = unsafe {
        ...
        // 5) residency 등록 (스테이징 1개 buffer — per-submit wiring 반복 방지).
        if self.weight_residency_enabled() {
            if let Some(lru) = self.weight_residency.borrow_mut().as_mut() {
        ...
        plan.into_iter()
            .map(|(dedup_key, _, _, stage_off)| (dedup_key, (staging.clone(), stage_off as u32)))
            .collect()
    }
    ...
    pub fn decode_chain_run(
        ...
        let shared_hidden = ffn_chain::empty_f32_buf(ctx, hidden_dim);
        ...
        for spec in specs {
            match spec {
                ChainLayerSpecRef::Attn(s) => {
                    ...
                    let norm_w_buf = ffn_chain::shared_f32_buf(ctx, s.norm_weight);
                    let q_norm_w_buf = ffn_chain::shared_f32_buf(ctx, s.q_norm_weight);
                    let k_norm_w_buf = ffn_chain::shared_f32_buf(ctx, s.k_norm_weight);
                    let ffn_norm_w_buf = ffn_chain::shared_f32_buf(ctx, s.ffn_norm_weight);
                ...
                ChainLayerSpecRef::AttnMoeQwen(s) => {
                    ...
                    let shared_input_scale_buf =
                        ffn_chain::shared_f32_buf(ctx, s.shared_input_scale);
                ...
                ChainLayerSpecRef::Gdn(s) => {
                    ...
                    let attn_norm_w_buf = ffn_chain::shared_f32_buf(ctx, s.attn_norm_weight);
                    let dt_bias_w_buf = ffn_chain::shared_f32_buf(ctx, s.dt_bias_weight);
                    let ssm_a_w_buf = ffn_chain::shared_f32_buf(ctx, s.ssm_a_weight);
                    let conv1d_w_buf = ffn_chain::shared_f32_buf(ctx, s.conv1d_weight);
                    let ssm_norm_w_buf = ffn_chain::shared_f32_buf(ctx, s.ssm_norm_weight);
                    let ffn_norm_w_buf = ffn_chain::shared_f32_buf(ctx, s.ffn_norm_weight);
...
"#;
    ...
    fn pm48_prefill_flash_microbench() {
        ...
        //     simd 커널이 f32 acc/m/l 이라 eager(f32 누적, q f16 round + k/v f16 dequant)와 가까워야 함.
        {
            ...
            let (simd_small, _) = crate::compute::run_prefill_flash_microbench_simd(
            ...
            let rel = simd_small
                ...
                .fold(0f32, f32::max);
            let max_abs = simd_small
                .iter()
                .zip(&oracle)
                .map(|(s, o)| (s - o).abs())
                .fold(0f32, f32::max);
            let mean_abs = simd_small
                .iter()
                .zip(&oracle)
                .map(|(s, o)| (s - o).abs())
                .sum::<f32>()
                / (simd_small.len() as f32);
            // matmul2d 는 half×half 곱(QK^T 는 256-term, PV 는 BC-term 누적)이라 f32-product eager
            // oracle 대비 f16-input GEMM 정밀도 한계(~수%)가 정상. 레이아웃/전치 버그(rel≫1, 위 첫 시도
            // 3.7e6)와는 자릿수가 다름. max_abs/mean_abs 로 broad small-scale precision 임을 확인.
            eprintln!(
                "[pm48-simd] correctness vs f32 eager oracle (seq=64): max_rel={rel:.3e} max_abs={max_abs:.3e} mean_abs={mean_abs:.3e}"
            );
            assert!(
                rel < 0.1,
                "simd flash correctness max_rel={rel} (≥0.1 = f16 precision 범위 초과, 레이아웃/규약 의심)"
            );
        }

        // (b) 성능: 큰 케이스(1139) simd 3-way median. warmup 3 + median 10.
        let mut simd_v = Vec::new();
        for i in 0..13 {
            let (_, sms) = crate::compute::run_prefill_flash_microbench_simd(
                &ctx, &q, &k_f16, &v_f16, seq, kv, nh, nkv, hd, scale,
            );
            if i >= 3 {
                simd_v.push(sms);
