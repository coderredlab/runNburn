...
// Q8_0 coalesced GEMV, two SIMD-groups/threadgroup (N_SG=2, NR0=2 → row 4/tg).
// dequant/layout는 gemv_q8_0_coalesced 와 동일, row 매핑(sg)만 다르다. grid=ceil(N/4),
// tg=64. pm126: Q8_0 dense GEMV 비중이 큰 모델용 진단 경로(RNB_METAL_Q8_0_NSG2).
kernel void gemv_q8_0_coalesced_nsg2(
    device const uchar* weight_bytes [[buffer(0)]],
    device const float* input        [[buffer(1)]],
    device float*       out          [[buffer(2)]],
    constant uint&      N            [[buffer(3)]],
