...
use crate::compute::{
    ...
    encode_gdn_alpha_beta_at, encode_gemv_quant, encode_gemv_quant_bcol,
    encode_gemv_quant_bcol_out_at, encode_l2_norm, encode_qk_norm, encode_ssm_conv1d_silu,
    encode_ssm_conv1d_silu_at, gemv_coalesced_dispatch_for, gemv_pipeline_for,
    gemv_simd_pipeline_for, MetalContext, SIMD_WIDTH,
};
use crate::ffn_chain::{
    empty_f16_buf, empty_f32_buf, encode_fused_residual_rms_norm, encode_residual_add,
    encode_residual_add_at, encode_rms_norm, encode_rms_norm_io_offset, encode_silu_mul,
    encode_silu_mul_at, f32_buf, readback, u32_buf,
};
...
/// Batched(B-lane) GDN core carrier — milestone 3 (MTP verify body fusion). 단일-토큰
/// `GdnCarrier` 와 완전히 분리(프로덕션 single-token 경로 불변). 한 command buffer 안에서
/// B lane 의 GDN core 를 처리하며 **weight 를 layer 당 1회만** 읽는다:
///   - qkv/gate/alpha/beta/ssm_out 은 B-column GEMV(`encode_gemv_quant_bcol*`)로 amortize.
///   - conv 는 rolling buffer `[state | qkv_0 | qkv_1 | ...]` + per-lane window offset 로
///     host readback 없이 lane→lane 전진(device conv-state advance).
///   - delta_net 은 device-resident `delta_state` 를 lane 순차 in-place 갱신.
/// (layer, B) 별 1회 alloc 후 토큰 간 재사용. `!Send+!Sync` 라 thread_local.
#[allow(dead_code)]
pub(crate) struct GdnBatchCarrier {
    pub b: usize,
    pub hidden_dim: usize,
    pub conv_channels: usize,
    pub conv_kernel: usize,
    ...
    conv_roll: Retained<ProtocolObject<dyn MTLBuffer>>,  // [(b+conv_kernel-1)*conv_channels]
    z_all: Retained<ProtocolObject<dyn MTLBuffer>>, // [b*z_dim] (gate out; silu in-place; ssm_out in)
    alpha_all: Retained<ProtocolObject<dyn MTLBuffer>>, // [b*num_v_heads]
    beta_all: Retained<ProtocolObject<dyn MTLBuffer>>, // [b*num_v_heads]
    ssm_proj_all: Retained<ProtocolObject<dyn MTLBuffer>>, // [b*hidden_dim]
    conv_out: Retained<ProtocolObject<dyn MTLBuffer>>, // [conv_channels] single scratch
    q_norm: Retained<ProtocolObject<dyn MTLBuffer>>, // [q_dim]
    k_norm: Retained<ProtocolObject<dyn MTLBuffer>>, // [k_dim]
    delta_out: Retained<ProtocolObject<dyn MTLBuffer>>, // [z_dim]
    gated: Retained<ProtocolObject<dyn MTLBuffer>>, // [z_dim]
    ...
    hv_buf: Retained<ProtocolObject<dyn MTLBuffer>>,
    eps_buf: Retained<ProtocolObject<dyn MTLBuffer>>,
    q_scale_buf: Retained<ProtocolObject<dyn MTLBuffer>>,
    one_buf: Retained<ProtocolObject<dyn MTLBuffer>>,
    k_hidden_buf: Retained<ProtocolObject<dyn MTLBuffer>>,
    b_buf: Retained<ProtocolObject<dyn MTLBuffer>>,
}

impl GdnBatchCarrier {
    #[allow(clippy::too_many_arguments)]
    pub(crate) fn new(
        ctx: &MetalContext,
        b: usize,
        ...
        Self {
            ...
            conv_roll: empty_f32_buf(ctx, (b + conv_kernel - 1) * conv_channels),
            z_all: empty_f32_buf(ctx, b * z_dim),
            alpha_all: empty_f32_buf(ctx, b * num_v_heads),
            beta_all: empty_f32_buf(ctx, b * num_v_heads),
            ssm_proj_all: empty_f32_buf(ctx, b * hidden_dim),
            conv_out: empty_f32_buf(ctx, conv_channels),
            q_norm: empty_f32_buf(ctx, q_dim),
            k_norm: empty_f32_buf(ctx, k_dim),
            delta_out: empty_f32_buf(ctx, z_dim),
    ...
    /// `[b*conv_channels .. +conv_state_len]`, delta_new = delta_all slot b(마지막 lane 결과).
    pub(crate) fn readback_states(&self) -> (Vec<f32>, Vec<f32>) {
        self.readback_prefix_states(self.b)
    }

    /// MTP verify accept-n 커밋: n lane 처리 후(prefix-n) GDN state 를 재실행 없이 readback.
...
/// Batched GDN core encode (milestone 3). `shared_hidden` = `[b*hidden_dim]`(lane i =
/// row i). B lane 의 GDN core(norm → in_proj → conv → delta → ssm → ssm_out → residual)를
/// 한 encoder 에 encode 하되 weight(qkv/gate/alpha/beta/ssm_out)는 B-column GEMV 로 **1회만**
/// 읽는다. conv 는 rolling buffer window offset 으로 lane→lane 전진, delta 는 device 순차
/// in-place. 결과는 lane 별 검증된 single-token `gdn_core_chain_encode` 와 동일(reduction
/// 순서 차이 rel<1e-3). MoE 는 caller 가 lane 별로 이어 붙인다.
#[allow(clippy::too_many_arguments)]
pub(crate) fn gdn_core_chain_encode_bcol(
    ctx: &MetalContext,
    enc: &ProtocolObject<dyn MTLComputeCommandEncoder>,
    carrier: &GdnBatchCarrier,
    ...
    // 1. per-lane rms_norm: shared_hidden[i] -> normed_all[i] (norm weight 는 lane 공통).
    for i in 0..b {
    ...
    // 2. B-column in_proj(weight 1회): qkv→rolling conv buffer[conv_state_len..],
    //    gate→z_all, alpha→alpha_all, beta→beta_all.
    encode_gemv_quant_bcol_out_at(
        ctx,
        enc,
        qkv_q,
        qkv_w_buf,
    ...
    for i in 0..b {
        ...
        chain_barrier(ctx, enc);
        // lane i window = conv_roll[i*conv_channels .. +conv_kernel*conv_channels].
        encode_ssm_conv1d_silu_at(
            ctx,
            enc,
            &carrier.conv_roll,
            i * conv_channels * f32b,
            conv1d_w_buf,
            &carrier.conv_out,
            &carrier.conv_ch_buf,
            &carrier.conv_k_buf,
        ...
        chain_barrier(ctx, enc);
        // ping-pong: lane i 는 delta_all slot i(read) → slot i+1(write) — 모든 prefix state 보존.
        let dsl = carrier.delta_state_len;
        compute::encode_delta_net_step_io_at(
            ctx,
            enc,
            &carrier.q_norm,
            &carrier.k_norm,
        ...
        chain_barrier(ctx, enc);
        // z_all[i] = silu(z_all[i]) * gated  (in-place at lane offset → ssm_out B-column 입력).
        encode_silu_mul_at(
            ctx,
            enc,
            &carrier.z_all,
    ...
    // 4. B-column ssm_out(weight 1회): z_all → ssm_proj_all.
    encode_gemv_quant_bcol(
        ctx,
        enc,
        ssm_out_q,
    ...
    // 5. per-lane residual: shared_hidden[i] += ssm_proj_all[i].
    for i in 0..b {
...
#[allow(clippy::too_many_arguments)]
fn encode_gdn_alpha_beta_batch(
    ctx: &MetalContext,
    enc: &ProtocolObject<dyn MTLComputeCommandEncoder>,
    alpha: &ProtocolObject<dyn MTLBuffer>,
    beta: &ProtocolObject<dyn MTLBuffer>,
    dt_bias: &ProtocolObject<dyn MTLBuffer>,
    ssm_a: &ProtocolObject<dyn MTLBuffer>,
    num_heads_buf: &ProtocolObject<dyn MTLBuffer>,
    num_heads: usize,
    num_tokens: usize,
) {
    enc.setComputePipelineState(&ctx.gdn_alpha_beta_pipeline);
    unsafe {
        enc.setBuffer_offset_atIndex(Some(alpha), 0, 0);
        enc.setBuffer_offset_atIndex(Some(beta), 0, 1);
        enc.setBuffer_offset_atIndex(Some(dt_bias), 0, 2);
        enc.setBuffer_offset_atIndex(Some(ssm_a), 0, 3);
        enc.setBuffer_offset_atIndex(Some(num_heads_buf), 0, 4);
    }
    let width = ctx.gdn_alpha_beta_pipeline.threadExecutionWidth().max(1);
    enc.dispatchThreadgroups_threadsPerThreadgroup(
        MTLSize {
            width: num_heads.div_ceil(width),
            height: num_tokens,
            depth: 1,
        },
        MTLSize {
    ...
    match weight {
        ...
        crate::GdnBackendWeightRef::Quant(view) => {
            let (block_elements, block_bytes) = match view.quant {
                crate::TensoropsQuant::Q4K => (256usize, 144usize),
                crate::TensoropsQuant::Q5K => (256, 176),
                crate::TensoropsQuant::Q6K => (256, 210),
                crate::TensoropsQuant::Q8_0 => (32, 34),
                crate::TensoropsQuant::Q2K => (256, 84),
                crate::TensoropsQuant::Q3K => (256, 110),
            };
            let blocks_per_row = view
                .cols
                .checked_div(block_elements)
                .filter(|_| view.cols % block_elements == 0)
                .ok_or_else(|| {
                    format!("GDN quant weight cols must be divisible by {block_elements}")
                })?;
            let expected = view
                .rows
            ...
            Ok(ctx.tensorops_capable
                ...
                && match view.quant {
                    ...
                    crate::TensoropsQuant::Q8_0 => ctx.gemm_q8_0_tensorops_v2_pipeline.is_some(),
                    crate::TensoropsQuant::Q2K => ctx.gemm_q2k_tensorops_v2_pipeline.is_some(),
                    crate::TensoropsQuant::Q3K => ctx.gemm_q3k_tensorops_v2_pipeline.is_some(),
                })
        }
    }
}
...
pub(crate) fn gdn_core_chain_dispatch(
    ...
    gdn_core_chain_encode(
        ...
        beta_q,
        ssm_out_q,
        true, // emit_trailing_residual: standalone call, 기존 동작 유지
    );
...
/// `emit_trailing_residual`: true면 끝에서 `hidden += ssm_proj` residual_add + barrier를
/// emit한다. false면 caller가 이어서 fused residual+norm 등을 직접 encode한다(pm132 MoE fusion).
#[allow(clippy::too_many_arguments)]
pub(crate) fn gdn_core_chain_encode(
    ...
    // alpha_beta(alpha/beta 변환)와 conv1d_silu(conv_input→conv_out)는 서로 독립 버퍼라
    // barrier 없이 병렬 dispatch. 다음 l2/delta 전 barrier(2245)가 둘 다 완료를 보장한다.
    ...
    if p.emit_small() {
        if compute::gdn_fused_tail_enabled()
            && carrier.head_k_dim <= 256
            && carrier.head_v_dim <= 256
        {
            // pm127: delta_net_step + qk_norm(ssm_norm) + silu_mul 을 v-head threadgroup
            // 하나로(원본 3 dispatch + 2 barrier → 1 dispatch, bit-identical).
            compute::encode_gdn_delta_norm_silu(
                ctx,
                enc,
                &carrier.q_norm_dev,
                &carrier.k_norm_dev,
...
pub(crate) fn gdn_chain_encode(
    ...
    if p.emit_small() {
        if compute::gdn_fused_tail_enabled()
            ...
            && carrier.head_v_dim <= 256
        {
            // pm127: delta_net_step + ssm_norm(qk_norm) + z silu·mul 을 v-head threadgroup
            // 하나로(원본 3 dispatch + 2 barrier → 1 dispatch, bit-identical).
            compute::encode_gdn_delta_norm_silu(
                ctx,
                enc,
            ...
            // ⑩ delta_net(GQA) → ⑪ ssm_norm(RMSNorm) → ⑫ z silu·mul (개별 3-op)
            encode_delta_net_step(
                ctx,
                enc,
                &carrier.q_norm_dev,
    ...
    chain_barrier(ctx, enc);
    // ⑭ residual + FFN norm fused (pm132): hidden += ssm_proj → ffn_normed = rms_norm(hidden)
    // RNB_METAL_FUSED_RESIDUAL_NORM=0 이면 기존 분리 경로(residual_add + rms_norm).
    let fused_res_norm = !crate::compute::env_falsey("RNB_METAL_FUSED_RESIDUAL_NORM");
    if fused_res_norm {
        if p.emit_small() {
    ...
    chain_barrier(ctx, enc); // residual+norm 완료 → 그룹 B(ffn_gate·up) 독립 GEMV 진입
                             // ffn gate GEMV [gemv]
    if p.emit_gemv() {
        encode_gemv_quant(
            ctx,
...
mod tests {
    ...
    #[test]
    fn gdn_alpha_beta_batch_matches_tokenwise_dispatch_bits() {
        let Some(ctx) = crate::compute::build_metal_context() else {
            return;
        };
        const TOKENS: usize = 3;
        const HEADS: usize = 5;
        let alpha = (0..TOKENS * HEADS)
            .map(|index| (index as f32 - 7.0) * 0.125)
            .collect::<Vec<_>>();
        let beta = (0..TOKENS * HEADS)
            .map(|index| (5.0 - index as f32) * 0.09375)
            .collect::<Vec<_>>();
        let dt_bias = vec![-0.25, -0.125, 0.0, 0.125, 0.25];
        let ssm_a = vec![-1.0, -0.75, -0.5, -0.25, -0.125];
        let mut expected_alpha = Vec::with_capacity(alpha.len());
        let mut expected_beta = Vec::with_capacity(beta.len());
        for token in 0..TOKENS {
            let start = token * HEADS;
            let end = start + HEADS;
            let (token_alpha, token_beta) = crate::compute::gdn_alpha_beta_with_ctx(
                &ctx,
                &alpha[start..end],
                &beta[start..end],
                &dt_bias,
            ...
            expected_alpha.extend(token_alpha);
            expected_beta.extend(token_beta);
        }

        let alpha_dev = empty_f32_buf(&ctx, alpha.len());
        let beta_dev = empty_f32_buf(&ctx, beta.len());
        let dt_bias_dev = empty_f32_buf(&ctx, HEADS);
        let ssm_a_dev = empty_f32_buf(&ctx, HEADS);
        let heads_dev = u32_buf(&ctx, HEADS as u32);
        copy_host_f32(&alpha, &alpha_dev);
        copy_host_f32(&beta, &beta_dev);
        copy_host_f32(&dt_bias, &dt_bias_dev);
        copy_host_f32(&ssm_a, &ssm_a_dev);
        let command = ctx.queue.commandBuffer().expect("command buffer");
        let encoder = chain_compute_encoder(&ctx, &command);
        encode_gdn_alpha_beta_batch(
            &ctx,
            &encoder,
            &alpha_dev,
        ...
        let got_alpha = unsafe {
            std::slice::from_raw_parts(alpha_dev.contents().as_ptr().cast::<f32>(), alpha.len())
        };
        let got_beta = unsafe {
            std::slice::from_raw_parts(beta_dev.contents().as_ptr().cast::<f32>(), beta.len())
        };
        assert_eq!(
            got_alpha
                .iter()
    ...
    /// pm127: fused `gdn_delta_norm_silu` 가 원본 3-op(delta_net_step + qk_norm(ssm_norm)
    /// + silu_mul)와 동일 결과(z, delta_state 둘 다)를 내는지 검증. 같은 연산·순서라
    /// bit-identical 기대(안전하게 rel<1e-4 판정). GQA(num_v>num_k) 케이스 포함.
    #[test]
    fn gdn_delta_norm_silu_fused_matches_split_ops() {
        let Some(ctx) = crate::compute::build_metal_context() else {
            return; // GPU 없는 CI 환경 skip
        };
        const NUM_K: usize = 2;
        const NUM_V: usize = 4; // GQA: v-head h → k-head h % NUM_K
        const HK: usize = 8;
        const HV: usize = 8;
        let z_dim = NUM_V * HV;
        let q_dim = NUM_K * HK;
        let k_dim = NUM_K * HK;
        let delta_state_len = NUM_V * HV * HK;
        let v_off_elems = q_dim + k_dim;
        let conv_len = q_dim + k_dim + z_dim;
        let eps = 1e-6f32;

        let q: Vec<f32> = (0..q_dim).map(|i| i as f32 * 0.013 - 0.1).collect();
        let k: Vec<f32> = (0..k_dim).map(|i| i as f32 * -0.017 + 0.05).collect();
        let mut conv: Vec<f32> = vec![0.0; conv_len];
        for (i, slot) in conv[v_off_elems..v_off_elems + z_dim]
            .iter_mut()
            .enumerate()
        {
            *slot = i as f32 * 0.021 - 0.2;
        }
        let alpha: Vec<f32> = (0..NUM_V).map(|i| i as f32 * 0.1 - 0.15).collect();
        let beta: Vec<f32> = (0..NUM_V).map(|i| 0.3 - i as f32 * 0.05).collect();
        let state0: Vec<f32> = (0..delta_state_len)
            .map(|i| (i % 7) as f32 * 0.03 - 0.05)
            .collect();
        let ssm_norm_w: Vec<f32> = (0..HV).map(|i| 0.9 + i as f32 * 0.01).collect();
        let z0: Vec<f32> = (0..z_dim).map(|i| i as f32 * 0.007 - 0.03).collect();

        let q_buf = empty_f32_buf(&ctx, q_dim);
        let k_buf = empty_f32_buf(&ctx, k_dim);
        let conv_buf = empty_f32_buf(&ctx, conv_len);
        let alpha_buf = empty_f32_buf(&ctx, NUM_V);
        let beta_buf = empty_f32_buf(&ctx, NUM_V);
        let ssm_norm_buf = empty_f32_buf(&ctx, HV);
        copy_host_f32(&q, &q_buf);
        copy_host_f32(&k, &k_buf);
        copy_host_f32(&conv, &conv_buf);
        copy_host_f32(&alpha, &alpha_buf);
        copy_host_f32(&beta, &beta_buf);
        copy_host_f32(&ssm_norm_w, &ssm_norm_buf);
        let hk_buf = u32_buf(&ctx, HK as u32);
        let hv_buf = u32_buf(&ctx, HV as u32);
        let nk_buf = u32_buf(&ctx, NUM_K as u32);
        let zdim_buf = u32_buf(&ctx, z_dim as u32);
        let eps_buf = f32_buf(&ctx, eps);
        let v_off_bytes = v_off_elems * 4;

        // Path A: 원본 3-op (delta → qk_norm → silu_mul).
        let state_a = empty_f32_buf(&ctx, delta_state_len);
        let z_a = empty_f32_buf(&ctx, z_dim);
        let delta_out_a = empty_f32_buf(&ctx, z_dim);
        let gated_a = empty_f32_buf(&ctx, z_dim);
        copy_host_f32(&state0, &state_a);
        copy_host_f32(&z0, &z_a);
        let cmd_a = ctx.queue.commandBuffer().expect("command buffer");
        let enc_a = chain_compute_encoder(&ctx, &cmd_a);
        encode_delta_net_step(
            &ctx,
            &enc_a,
            &q_buf,
        ...
        chain_barrier(&ctx, &enc_a);
        encode_silu_mul(&ctx, &enc_a, &z_a, &gated_a, &zdim_buf, z_dim);
        enc_a.endEncoding();
        cmd_a.commit();
        cmd_a.waitUntilCompleted();

        // Path B: fused.
        let state_b = empty_f32_buf(&ctx, delta_state_len);
        let z_b = empty_f32_buf(&ctx, z_dim);
        copy_host_f32(&state0, &state_b);
        copy_host_f32(&z0, &z_b);
        let cmd_b = ctx.queue.commandBuffer().expect("command buffer");
        let enc_b = chain_compute_encoder(&ctx, &cmd_b);
        compute::encode_gdn_delta_norm_silu(
            &ctx,
            &enc_b,
            &q_buf,
        ...
        let za =
            unsafe { std::slice::from_raw_parts(z_a.contents().as_ptr().cast::<f32>(), z_dim) };
        let zb =
            unsafe { std::slice::from_raw_parts(z_b.contents().as_ptr().cast::<f32>(), z_dim) };
        let sa = unsafe {
            std::slice::from_raw_parts(state_a.contents().as_ptr().cast::<f32>(), delta_state_len)
        };
        let sb = unsafe {
            std::slice::from_raw_parts(state_b.contents().as_ptr().cast::<f32>(), delta_state_len)
        };
        ...
        let z_rel = max_rel(za, zb);
        let s_rel = max_rel(sa, sb);
        assert!(z_rel < 1e-4, "fused z diverges: max_rel={z_rel}");
        assert!(s_rel < 1e-4, "fused delta_state diverges: max_rel={s_rel}");
    }
}
