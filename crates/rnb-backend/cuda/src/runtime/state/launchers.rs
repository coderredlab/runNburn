...
impl CudaState {
    ...
    // cu209: MTP verify 등 짧은 window 의 Q5_K batch projection 을 raw F32
    // 커널 대신 Q8_1 activation q8dot 으로 실행한다. 커널은 단일 토큰
    // rnb_q5k_gemv_q8dot_warp8 과 토큰별 bitwise 동일 산술이다.
    #[allow(clippy::too_many_arguments)]
    pub(in crate::runtime) fn launch_q5k_gemv_batch_q8dot_to_dev(
        &mut self,
        weights: &[u8],
        rows: usize,
        blocks_per_row: usize,
    ...
    #[allow(clippy::too_many_arguments)]
    pub(in crate::runtime) fn launch_q6k_gemv_batch_q8dot_paired_to_dev(
        &mut self,
        weights: &[u8],
        rows: usize,
        blocks_per_row: usize,
    ...
    // cu226: 32x64 tile — grid.y 를 반감해 a-tile 로드/unpack 을 출력당
    // 절반으로 상각한다. per-element 산술은 tile32 와 bitwise 동일.
    pub(in crate::runtime) fn launch_q4k_q8_1_matmul_mmq_tile32_seq64(
        &mut self,
        weights: &[u8],
        rows: usize,
        blocks_per_row: usize,
        ...
        let mut seq_len_arg = seq_len as u32;
        self.launch_cached_gemv(
            "rnb_q4k_q8_1_matmul_mmq_tile32_seq64",
            &[
                (&mut output_arg as *mut u64).cast::<libc::c_void>(),
                (&mut weights_arg as *mut u64).cast::<libc::c_void>(),
                (&mut input_qs_arg as *mut u64).cast::<libc::c_void>(),
            ...
            (rows.div_ceil(32) as u32, seq_len.div_ceil(64) as u32, 1),
            (256, 1, 1),
        )
    }

    // cu228: 64x64 tile (512-thread CTA) — grid.x 반감으로 b-side 로드/
    // chunk-sum 을 출력당 절반으로 상각. per-element bitwise 동일.
    pub(in crate::runtime) fn launch_q4k_q8_1_matmul_mmq_tile64_seq64(
        &mut self,
        weights: &[u8],
        rows: usize,
        blocks_per_row: usize,
        ...
        let mut seq_len_arg = seq_len as u32;
        self.launch_cached_gemv(
            "rnb_q4k_q8_1_matmul_mmq_tile64_seq64",
            &[
                (&mut output_arg as *mut u64).cast::<libc::c_void>(),
                (&mut weights_arg as *mut u64).cast::<libc::c_void>(),
                (&mut input_qs_arg as *mut u64).cast::<libc::c_void>(),
    ...
    pub(in crate::runtime) fn launch_q3k_q8_1_matmul_mmq_tile32(
        &mut self,
        weights: &[u8],
        rows: usize,
        blocks_per_row: usize,
        ...
        let mut seq_len_arg = seq_len as u32;
        self.launch_cached_gemv(
            "rnb_q3k_q8_1_matmul_mmq_tile32",
            &[
                (&mut output_arg as *mut u64).cast::<libc::c_void>(),
                (&mut weights_arg as *mut u64).cast::<libc::c_void>(),
                (&mut input_qs_arg as *mut u64).cast::<libc::c_void>(),
    ...
    pub(in crate::runtime) fn launch_q2k_q8_1_matmul_mmq_tile32(
        &mut self,
        weights: &[u8],
        rows: usize,
        blocks_per_row: usize,
        ...
        let mut seq_len_arg = seq_len as u32;
        self.launch_cached_gemv(
            "rnb_q2k_q8_1_matmul_mmq_tile32",
            &[
                (&mut output_arg as *mut u64).cast::<libc::c_void>(),
                (&mut weights_arg as *mut u64).cast::<libc::c_void>(),
                (&mut input_qs_arg as *mut u64).cast::<libc::c_void>(),
    ...
    // cu226: Q6_K 32x64 tile — Q4 seq64 와 같은 a-side 상각 (grid.y 반감,
    // per-element bitwise 동일).
    pub(in crate::runtime) fn launch_q6k_q8_1_matmul_mmq_tile32_seq64(
        &mut self,
        weights: &[u8],
        rows: usize,
        blocks_per_row: usize,
        ...
        let mut seq_len_arg = seq_len as u32;
        self.launch_cached_gemv(
            "rnb_q6k_q8_1_matmul_mmq_tile32_seq64",
            &[
                (&mut output_arg as *mut u64).cast::<libc::c_void>(),
                (&mut weights_arg as *mut u64).cast::<libc::c_void>(),
                (&mut input_qs_arg as *mut u64).cast::<libc::c_void>(),
    ...
    // cu222: Q5_K MMQ tile32 — Q4_K MMQ 와 같은 affine(min) 처리에 qh 5th-bit
    // 주입만 추가된 커널. prefill 대역(seq>=min_seq)의 Q5 배치가 q8dot wide
    // GEMV 대신 tile 재사용 matmul 을 탄다.
    pub(in crate::runtime) fn launch_q5k_q8_1_matmul_mmq_tile32(
        &mut self,
        weights: &[u8],
        rows: usize,
        blocks_per_row: usize,
        ...
        let mut seq_len_arg = seq_len as u32;
        self.launch_cached_gemv(
            "rnb_q5k_q8_1_matmul_mmq_tile32",
            &[
                (&mut output_arg as *mut u64).cast::<libc::c_void>(),
                (&mut weights_arg as *mut u64).cast::<libc::c_void>(),
                (&mut input_qs_arg as *mut u64).cast::<libc::c_void>(),
    ...
    // cu226: Q5_K 32x64 tile — qh 5th-bit 주입 유지, a-side 상각 동일.
    pub(in crate::runtime) fn launch_q5k_q8_1_matmul_mmq_tile32_seq64(
        &mut self,
        weights: &[u8],
        rows: usize,
        blocks_per_row: usize,
        ...
        let mut seq_len_arg = seq_len as u32;
        self.launch_cached_gemv(
            "rnb_q5k_q8_1_matmul_mmq_tile32_seq64",
            &[
                (&mut output_arg as *mut u64).cast::<libc::c_void>(),
                (&mut weights_arg as *mut u64).cast::<libc::c_void>(),
                (&mut input_qs_arg as *mut u64).cast::<libc::c_void>(),
    ...
    // cu228: Q5_K/Q6_K 64x64 tile (512-thread CTA) — b-side 상각, bitwise 동일.
    pub(in crate::runtime) fn launch_q5k_q8_1_matmul_mmq_tile64_seq64(
        &mut self,
        weights: &[u8],
        rows: usize,
        blocks_per_row: usize,
        ...
        let mut seq_len_arg = seq_len as u32;
        self.launch_cached_gemv(
            "rnb_q5k_q8_1_matmul_mmq_tile64_seq64",
            &[
                (&mut output_arg as *mut u64).cast::<libc::c_void>(),
                (&mut weights_arg as *mut u64).cast::<libc::c_void>(),
                (&mut input_qs_arg as *mut u64).cast::<libc::c_void>(),
    ...
    pub(in crate::runtime) fn launch_q6k_q8_1_matmul_mmq_tile64_seq64(
        &mut self,
        weights: &[u8],
        rows: usize,
        blocks_per_row: usize,
        ...
        let mut seq_len_arg = seq_len as u32;
        self.launch_cached_gemv(
            "rnb_q6k_q8_1_matmul_mmq_tile64_seq64",
            &[
                (&mut output_arg as *mut u64).cast::<libc::c_void>(),
                (&mut weights_arg as *mut u64).cast::<libc::c_void>(),
                (&mut input_qs_arg as *mut u64).cast::<libc::c_void>(),
