...
impl CudaState {
    ...
    pub(in crate::runtime) fn open_ordinal(ordinal: i32) -> Result<Self, String> {
        ...
        let resident_moe_layer_limit = moe_layer_cache_limit(&api)?;
        let resident_q4k_limit = q4k_resident_cache_limit(&api)?;
        let resident_q8_f32_limit = q8_f32_cache_limit()?;
        let resident_q4_packed_limit = q4_packed_cache_limit(&api)?;
        let resident_q4_f32_limit = q4_f32_cache_limit(&api)?;
        let resident_q6_packed_limit = q6_packed_cache_limit(&api)?;
        let resident_q6_f32_limit = q6_f32_cache_limit()?;
        let resident_q6_f16_limit = q6_f16_cache_limit(&api)?;
