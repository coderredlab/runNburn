...
impl CudaState {
    ...
    pub(in crate::runtime) fn resident_q4k_f32_ptr(
        ...
        if bytes > self.resident_q4_f32_limit
            || self.resident_q4_f32_bytes.saturating_add(bytes) > self.resident_q4_f32_limit
        {
            return Ok(None);
        }
    ...
    pub(in crate::runtime) fn resident_q4k_f32_pair_ptrs(
        ...
        if bytes > self.resident_q4_f32_limit
            || self.resident_q4_f32_bytes.saturating_add(needed) > self.resident_q4_f32_limit
        {
            return Ok(None);
        }
