...
#[derive(Clone, Copy)]
enum DeviceInputSharedWeights<'a> {
    F32 {
        gate: &'a [f32],
        up: &'a [f32],
        down: &'a [f32],
...
fn device_input_use_mixed_resident_sparse(
    ...
    exact_resident_moe_layer: bool,
) -> bool {
    explicit_mixed_resident_sparse
        || (exact_resident_moe_layer
...
mod tests {
    ...
    #[test]
    fn device_input_auto_mixed_requires_exact_resident_layer() {
        unsafe {
        ...
        assert!(!device_input_use_mixed_resident_sparse(false, false));
        assert!(device_input_use_mixed_resident_sparse(false, true));
        assert!(device_input_use_mixed_resident_sparse(true, false));
...
pub fn qwen35_prefill_moe_f32_shared_sparse_device_topk_selected_base_device_input(
    ...
    qwen35_prefill_moe_shared_sparse_device_topk_selected_base_device_input_impl(
        DeviceInputSharedWeights::F32 {
            gate: shared_gate,
            up: shared_up,
            down: shared_down,
        },
...
#[allow(clippy::too_many_arguments)]
pub fn qwen35_prefill_moe_quant_shared_sparse_device_topk_selected_base_device_input(
    shared_gate: &[u8],
    ...
    qwen35_prefill_moe_shared_sparse_device_topk_selected_base_device_input_impl(
        DeviceInputSharedWeights::Quant {
            gate: shared_gate,
...
#[allow(clippy::too_many_arguments)]
fn qwen35_prefill_moe_shared_sparse_device_topk_selected_base_device_input_impl(
    shared: DeviceInputSharedWeights<'_>,
    shared_input_scale: &[f32],
    gate_all: &[u8],
    up_all: &[u8],
    down_all: &[u8],
    ...
    match shared {
        DeviceInputSharedWeights::F32 { gate, up, down } => {
            if gate.len() != n_ff * n_embd
                || up.len() != n_ff * n_embd
                || down.len() != n_embd * n_ff
            {
                return Err(
                    "Qwen35 device-input selected-base shared f32 weight shape mismatch"
                        .to_string(),
                );
            }
        }
        DeviceInputSharedWeights::Quant {
            ...
            super::super::mtp_verify::validate_mtp_verify_k_quant_matrix(
                "Qwen35 device-input selected-base shared gate",
                gate_quant,
                gate,
                n_ff,
                n_embd,
            ...
            super::super::mtp_verify::validate_mtp_verify_k_quant_matrix(
                "Qwen35 device-input selected-base shared up",
                up_quant,
                up,
                n_ff,
                n_embd,
            ...
            super::super::mtp_verify::validate_mtp_verify_k_quant_matrix(
                "Qwen35 device-input selected-base shared down",
                down_quant,
                down,
                n_embd,
                n_ff,
    ...
    let auto_mixed_resident_sparse = selected_base_device_slot_ptrs
        && !explicit_mixed_resident_sparse
        && device_input_use_mixed_resident_sparse(false, exact_resident_moe_layer);
    ...
    let stream_selected_base =
        direct_sparse && tuning::qwen35_selected_base_stream_enabled(token_count);
    ...
    let (prepared_sparse, deferred_selected_base) = if selected_base_device_slot_ptrs {
        if mixed_resident_sparse {
            ...
            (Some(prepared), None)
        } else if fused_selected_sparse_boundary
            || overlap_selected_base_staging
            || stream_selected_base
        {
            (
                None,
    ...
    let phase_start = phase_profile.then(std::time::Instant::now);
    let output_id = match shared {
        DeviceInputSharedWeights::F32 { gate, up, down } if reuse_residual_output => state
        ...
        DeviceInputSharedWeights::F32 { gate, up, down } => state
            .qwen35_prefill_moe_f32_shared_sparse_by_token_device_input(
                gate,
                up,
                down,
                shared_input_scale,
        ...
        DeviceInputSharedWeights::Quant {
            ...
            down,
            down_quant: shared_down_quant,
        } => state.qwen35_prefill_moe_quant_shared_sparse_by_token_device_input(
            gate,
            gate_quant,
            up,
            up_quant,
...
pub fn qwen35_configure_moe_layer_cache(
    ...
    guard
        .as_mut()
        .expect("cuda compute state initialized")
        .configure_qwen35_moe_layer_cache(model_moe_bytes, runtime_headroom_bytes, min_layer_bytes)
}
