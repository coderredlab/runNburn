...
#[allow(clippy::too_many_arguments)]
pub fn glm_sparse_experts_iq_by_token(
    gate_weights: &[&[u8]],
    up_weights: &[&[u8]],
    down_weights: &[&[u8]],
    gate_quant: u32,
    ...
    if !tuning::glm_expert_parallel_enabled() || slots_per_token < 2 {
        return primary_state.glm_sparse_experts_iq_by_token(
            gate_weights,
            up_weights,
            down_weights,
            gate_quant,
    ...
    let (mut primary_output, secondary_output) =
        std::thread::scope(|scope| -> Result<(Vec<f32>, Vec<f32>), String> {
            let secondary_handle = scope.spawn(|| {
                secondary_state.glm_sparse_experts_iq_by_token(
                    &secondary_gate,
                    &secondary_up,
                    &secondary_down,
                    gate_quant,
            ...
            let primary_result = primary_state.glm_sparse_experts_iq_by_token(
                &primary_gate,
                &primary_up,
                &primary_down,
                gate_quant,
...
pub fn glm_shared_expert_iq(
    ...
    let gate_bytes = match gate_quant {
        13 => n_ff * (n_embd / 256) * 176,
        14 => n_ff * (n_embd / 256) * 210,
        other => {
            return Err(format!("unsupported GLM shared gate/up quant code {other}"));
        }
    };
    let down_bytes = match down_quant {
        14 => n_embd * (n_ff / 256) * 210,
        8 => n_embd * (n_ff / 32) * 34,
        other => return Err(format!("unsupported GLM shared down quant code {other}")),
    };
    if gate_weights.len() != gate_bytes || up_weights.len() != gate_bytes {
        return Err(format!(
            "GLM shared gate/up byte mismatch: gate={} up={} expected={gate_bytes}",
            gate_weights.len(),
            up_weights.len()
        ));
    }
    if down_weights.len() != down_bytes {
        return Err(format!(
            "GLM shared down byte mismatch: got {}, expected {down_bytes}",
            down_weights.len()
        ));
    }
    let compute = DEFAULT_CUDA_COMPUTE.get_or_init(|| Mutex::new(None));
