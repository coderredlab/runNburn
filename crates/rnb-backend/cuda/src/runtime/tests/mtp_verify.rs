...
fn cuda_qwen35_mtp_verify_attention_qk_norm_rope_matches_cpu_reference() {
    let _guard = runtime_test_lock();
    let mut state = CudaState::open().expect("open CUDA state");
    for (head_dim, rope_dim) in [(256usize, 64usize), (512usize, 256usize)] {
        let hidden_dim = 512usize;
        let window_tokens = 3usize;
        ...
        let rope_freq_factors = (0..rope_dim / 2)
            .map(|i| 1.0 + (i % 7) as f32 * 0.03125)
            .collect::<Vec<_>>();
        let hidden_rows = (0..window_tokens * hidden_dim)
...
#[test]
fn cuda_mtp_verify_attention_output_window_matches_cpu_reference() {
    let _guard = runtime_test_lock();
    let mut state = CudaState::open().expect("open CUDA state");
    for head_dim in [256usize, 512usize] {
        ...
        for t in 0..window_tokens {
            ...
            for h in 0..num_heads {
                ...
                for (j, score) in scores {
                    ...
                    for d in 0..head_dim {
                        expected[out_off + d] +=
                            p * half::f16::from_bits(v_f16[v_off + d]).to_f32();
                    }
                }
            }
        }
        ...
        assert_close_rows_abs_rel(
            "MTP verify attention output window",
            &actual,
            &expected,
            0.05,
            0.01,
...
#[test]
fn cuda_mtp_verify_attention_output_includes_prior_device_kv() {
    let _guard = runtime_test_lock();
    let mut state = CudaState::open().expect("open CUDA state");
    for head_dim in [256usize, 512usize] {
        let hidden_dim = 512usize;
        ...
        assert_close_rows_abs_rel(
            "MTP verify attention output prior window",
            &actual,
            &expected,
            0.05,
            0.01,
...
#[test]
fn cuda_qwen35_mtp_verify_attention_output_hd256_mma_stream_k_matches_f32_split() {
    let _guard = runtime_test_lock();
    let mut state = CudaState::open().expect("open CUDA state");
    let window_tokens = 257usize;
    let num_heads = 2usize;
    let num_kv_heads = 1usize;
    let head_dim = 256usize;
    let q_rows = num_heads * head_dim;
    ...
    let k_bits = (0..window_tokens * kv_rows)
        .map(|i| half::f16::from_f32(((i as f32 % 37.0) - 18.0) * 0.0107421875).to_bits())
        .collect::<Vec<_>>();
    let v_bits = (0..window_tokens * kv_rows)
        .map(|i| half::f16::from_f32(((i as f32 % 41.0) - 20.0) * 0.0087890625).to_bits())
        .collect::<Vec<_>>();
    ...
    let q_dev = state
        ...
        .expect("allocate q");
    let k_dev = state
        .mem_alloc(std::mem::size_of_val(k_bits.as_slice()))
        .expect("allocate k");
    let v_dev = state
        .mem_alloc(std::mem::size_of_val(v_bits.as_slice()))
        .expect("allocate v");
    unsafe {
        state
            .api
        ...
        state
            ...
            .memcpy_htod_async(
                ...
                std::mem::size_of_val(k_bits.as_slice()),
                state.stream,
            )
            .unwrap();
        state
            ...
            .memcpy_htod_async(
                ...
                std::mem::size_of_val(v_bits.as_slice()),
                state.stream,
            )
            .unwrap();
    }
    ...
    let baseline_buffers = state
        ...
        .expect("stage F32 split attention output");
    let mut baseline = vec![0.0f32; window_tokens * q_rows];
    unsafe {
        state
            .api
            .memcpy_dtoh_async(
                baseline.as_mut_ptr().cast::<libc::c_void>(),
                baseline_buffers.attn_out_dev,
                std::mem::size_of_val(baseline.as_slice()),
                state.stream,
            )
            .unwrap();
    }
    ...
    let candidate_buffers = state
        ...
        .expect("stage MMA stream-K attention output");
    let mut candidate = vec![0.0f32; window_tokens * q_rows];
    unsafe {
        state
            .api
            .memcpy_dtoh_async(
                candidate.as_mut_ptr().cast::<libc::c_void>(),
                candidate_buffers.attn_out_dev,
                std::mem::size_of_val(candidate.as_slice()),
                state.stream,
            )
            .unwrap();
    }
    ...
    for (&expected, &actual) in baseline.iter().zip(&candidate) {
        assert!(
            actual.is_finite(),
            "MMA stream-K produced non-finite output"
        );
...
fn cuda_qwen35_mtp_verify_attention_prior_kv_cache_reuses_layer_device_buffers() {
    ...
    let kv_rows = 8usize;
    let first_tokens = 3usize;
    let extended_tokens = 4usize;
    ...
    let (rolled_back_k_dev, rolled_back_v_dev) = state
        .stage_mtp_verify_attention_prior_kv_bits_for_layer(
            layer_index,
            7,
            &first_k,
        ...
        .expect("stage rolled-back prior kv")
        .expect("rolled-back prior kv device buffers");
    assert_eq!(rolled_back_k_dev, first_k_dev);
    assert_eq!(rolled_back_v_dev, first_v_dev);
    let cache = state
        .mtp_verify_attention_prior_kv
        .iter()
        .find(|cache| cache.layer_index == layer_index)
        .expect("rolled-back layer prior kv cache");
    assert_eq!(cache.cached_tokens, first_tokens);
    assert_eq!(cache.sequence_epoch, 7);
    assert_eq!(cache.host_k_bits, extended_k);
    assert_eq!(cache.host_v_bits, extended_v);

    let (reset_k_dev, reset_v_dev) = state
        .stage_mtp_verify_attention_prior_kv_bits_for_layer(
            layer_index,
            8,
            &first_k,
        ...
        .expect("stage new-sequence prior kv")
        .expect("new-sequence prior kv device buffers");
    assert_eq!(reset_k_dev, first_k_dev);
    assert_eq!(reset_v_dev, first_v_dev);
    let cache = state
        .mtp_verify_attention_prior_kv
        .iter()
        .find(|cache| cache.layer_index == layer_index)
        .expect("new-sequence layer prior kv cache");
    assert_eq!(cache.cached_tokens, first_tokens);
    assert_eq!(cache.sequence_epoch, 8);
    assert_eq!(cache.host_k_bits, first_k);
    assert_eq!(cache.host_v_bits, first_v);
}
...
fn cuda_qwen35_mtp_verify_attention_moe_layer_chains_resident_stages() {
    let _guard = runtime_test_lock();
    let _expanded_opt_out = EnvVarGuard::set("RNB_CUDA_ALLOW_EXPANDED_WEIGHT_CACHE", "0");
    let _resident_kv = EnvVarGuard::set("RNB_CUDA_MTP_VERIFY_RESIDENT_ATTN_KV", "0");
    let _window2_graphs = EnvVarGuard::set("RNB_CUDA_MTP_VERIFY_WINDOW2_GRAPHS", "1");
    let mut state = CudaState::open().expect("open CUDA state");
    ...
    let attention_kv = attention_kv.expect("attention graph replay state");
    ...
    let mut expected_residual = Vec::with_capacity(hidden_rows.len());
    let mut expected_normalized = Vec::with_capacity(hidden_rows.len());
    for (hidden, attn) in hidden_rows
        .chunks_exact(hidden_dim)
        .zip(attn_out.chunks_exact(hidden_dim))
    {
        ...
        expected_residual.extend_from_slice(&residual);
        expected_normalized.extend_from_slice(&moe_input);
    ...
    let input_id = state
        .upload_device_tensor_f32(input_desc, &hidden_rows)
        .expect("upload product attention input");
    let product_output = state
        ...
        .expect("run product attention device-input chain");
    assert_ne!(product_output.normalized_id, input_id);
    assert_ne!(product_output.residual_id, input_id);
    assert_ne!(product_output.normalized_id, product_output.residual_id);
    assert_eq!(
        product_output.normalized_desc,
    ...
    assert_eq!(
        product_output.attention_kv.layer_idx,
        attention_kv.layer_idx
    );
    assert_eq!(
        product_output.attention_kv.window_tokens,
        attention_kv.window_tokens
    );
    assert_eq!(product_output.attention_kv.kv_rows, attention_kv.kv_rows);
    assert_eq!(product_output.attention_kv.k_bits, attention_kv.k_bits);
    assert_eq!(product_output.attention_kv.v_bits, attention_kv.v_bits);
    let product_normalized = state
        .download_device_tensor_f32(product_output.normalized_id)
        .expect("download product attention normalized output");
    let product_residual = state
        .download_device_tensor_f32(product_output.residual_id)
        .expect("download product attention residual output");
    assert_close_rows_abs_rel(
        "product attention normalized device output",
        &product_normalized,
        &expected_normalized,
        0.05,
        0.01,
    ...
    assert_close_rows_abs_rel(
        "product attention residual device output",
        &product_residual,
        &expected_residual,
        0.25,
        0.02,
    );
    assert!(state.release_device_tensor(input_id).unwrap());
    assert!(state
        .release_device_tensor(product_output.normalized_id)
        .unwrap());
    assert!(state
        .release_device_tensor(product_output.residual_id)
        .unwrap());
}
...
fn cuda_qwen35_mtp_verify_sparse_moe_consumes_device_router_slots() {
    ...
    assert!(
        ...
        "missing MTP verify layers must stream selected experts without full-layer admission"
    );
...
fn cuda_qwen35_mtp_verify_moe_residual_chains_router_sparse_and_shared() {
    let _guard = runtime_test_lock();
    let _allow = EnvVarGuard::set("RNB_CUDA_ALLOW_EXPANDED_WEIGHT_CACHE", "1");
    let _resident_moe = EnvVarGuard::set("RNB_CUDA_MTP_VERIFY_RESIDENT_MOE_LAYER", "0");
    let mut state = CudaState::open().expect("open CUDA state");
    state.resident_q4_f32_limit = usize::MAX;
    let window_tokens = 2usize;
    let hidden_dim = 256usize;
...
fn cuda_qwen35_mtp_verify_resident_moe_layer_opt_in_registers_first_layer() {
    ...
    let _promotion_guard = EnvVarGuard::set("RNB_CUDA_MTP_VERIFY_MISSING_MOE_LAYER_PROMOTION", "1");
    let mut state = CudaState::open().expect("open CUDA state");
...
fn cuda_qwen35_mtp_device_verify_api_executes_attention_projection_before_unimplemented() {
    ...
    let attention_layer = Qwen35MtpDeviceVerifyAttentionMoeLayer {
        ...
        prior_sequence_epoch: 1,
        kvarn_prior: None,
        attention_scale: (hidden_dim as f32).sqrt().recip(),
        o_q4k: &[],
        o_quant: GGML_Q4_K,
        o_rows: 0,
        o_cols: 0,
...
fn cuda_qwen35_mtp_device_verify_api_returns_gdn_prefix_states() {
    let _guard = runtime_test_lock();
    let _allow = EnvVarGuard::set("RNB_CUDA_ALLOW_EXPANDED_WEIGHT_CACHE", "1");
    let _resident_conv = EnvVarGuard::set("RNB_CUDA_MTP_VERIFY_RESIDENT_CONV", "0");
...
#[test]
fn cuda_mtp_verify_snapshot_pool_reuses_released_device_allocation() {
    let _guard = runtime_test_lock();
    let _pool = EnvVarGuard::set("RNB_CUDA_MTP_VERIFY_SNAPSHOT_POOL", "1");
    let mut state = CudaState::open().expect("open CUDA state");

    let first = state
        .allocate_delta_state_snapshot(4096)
        .expect("allocate first snapshot");
    let first_ptr = first.ptr;
    state
        .free_delta_state_snapshot(first)
        .expect("release first snapshot");

    let second = state
        .allocate_delta_state_snapshot(4096)
        .expect("allocate reused snapshot");
    assert_eq!(second.ptr, first_ptr);
    assert_eq!(state.mtp_verify_snapshot_pool.len(), 1);
    state
        .free_delta_state_snapshot(second)
        .expect("release reused snapshot");
}
