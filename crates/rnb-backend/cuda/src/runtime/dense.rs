...
impl CudaState {
    ...
    pub(in crate::runtime) fn qwen35_prefill_moe_f32_shared_sparse_by_token_device_input(
        ...
        self.qwen35_prefill_moe_f32_shared_sparse_by_token_device_input_impl(
            shared_gate,
            shared_up,
            shared_down,
            shared_input_scale,
    ...
    #[allow(clippy::too_many_arguments)]
    fn qwen35_prefill_moe_f32_shared_sparse_by_token_device_input_impl(
        &mut self,
        shared_gate: &[f32],
        shared_up: &[f32],
        shared_down: &[f32],
        ...
        {
            return Err("Qwen35 device-input MoE shared f32 weight shape mismatch".to_string());
        }
        ...
        let run = (|| -> Result<(), String> {
            ...
            let delay_selected_sparse_prepare_until_sparse_phase =
            ...
            profile_phase!("sparse_prepare", phase_start);

            let phase_start = phase_start!();
            if qwen35_device_sparse_route_enabled() {
                if let Some(prepared) = prepared_sparse.as_mut() {
                    prepared.device_route = Some(
                        self.qwen35_prepare_device_sparse_route_by_token(route_weights, token_ids)?,
                    );
                }
            }
            profile_phase!("sparse_route_stage", phase_start);
