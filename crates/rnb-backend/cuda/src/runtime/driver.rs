...
pub(super) struct CudaApi {
    ...
    cu_device_get_name: Option<unsafe extern "C" fn(*mut libc::c_char, i32, i32) -> i32>,
    cu_device_get_pci_bus_id: Option<unsafe extern "C" fn(*mut libc::c_char, i32, i32) -> i32>,
    ...
    cu_stream_wait_event: unsafe extern "C" fn(*mut libc::c_void, *mut libc::c_void, u32) -> i32,
    cu_event_create: unsafe extern "C" fn(*mut *mut libc::c_void, u32) -> i32,
    cu_event_destroy: unsafe extern "C" fn(*mut libc::c_void) -> i32,
    cu_event_record: unsafe extern "C" fn(*mut libc::c_void, *mut libc::c_void) -> i32,
    cu_event_synchronize: unsafe extern "C" fn(*mut libc::c_void) -> i32,
    ...
    cu_module_get_function:
        unsafe extern "C" fn(*mut *mut libc::c_void, *mut libc::c_void, *const libc::c_char) -> i32,
...
impl CudaApi {
    pub(super) unsafe fn load(lib_handle: usize) -> Result<Self, String> {
        Ok(Self {
            ...
            cu_stream_wait_event: load_symbol(lib_handle, "cuStreamWaitEvent")?,
            cu_event_create: load_symbol(lib_handle, "cuEventCreate")?,
            cu_event_destroy: load_symbol(lib_handle, "cuEventDestroy_v2")?,
            cu_event_record: load_symbol(lib_handle, "cuEventRecord")?,
            cu_event_synchronize: load_symbol(lib_handle, "cuEventSynchronize")?,
    ...
    pub(super) unsafe fn stream_wait_event(
        &self,
        stream: usize,
        event: usize,
    ) -> Result<(), String> {
        check_cuda(
            (self.cu_stream_wait_event)(stream as *mut libc::c_void, event as *mut libc::c_void, 0),
            "cuStreamWaitEvent",
        )
    }

    pub(super) unsafe fn event_create(&self, flags: u32) -> Result<usize, String> {
        let mut event = std::ptr::null_mut();
        check_cuda((self.cu_event_create)(&mut event, flags), "cuEventCreate")?;
        Ok(event as usize)
    }

    pub(super) unsafe fn event_destroy(&self, event: usize) -> Result<(), String> {
        check_cuda(
            (self.cu_event_destroy)(event as *mut libc::c_void),
            "cuEventDestroy_v2",
        )
    }

    pub(super) unsafe fn event_record(&self, event: usize, stream: usize) -> Result<(), String> {
        check_cuda(
            (self.cu_event_record)(event as *mut libc::c_void, stream as *mut libc::c_void),
            "cuEventRecord",
        )
    }

    pub(super) unsafe fn event_synchronize(&self, event: usize) -> Result<(), String> {
        check_cuda(
            (self.cu_event_synchronize)(event as *mut libc::c_void),
            "cuEventSynchronize",
        )
    }
    ...
    pub(super) unsafe fn module_load_cubin(
        &self,
        cubin: &[u8],
    ) -> Result<*mut libc::c_void, String> {
        let mut module = std::ptr::null_mut();
        check_cuda(
            (self.cu_module_load_data)(&mut module, cubin.as_ptr().cast::<libc::c_void>()),
            "cuModuleLoadData(cubin)",
        )?;
        Ok(module)
    }

    pub(super) unsafe fn module_load_cubin_or_ptx(
        &self,
        cubin: &[u8],
        ptx: &str,
    ) -> Result<*mut libc::c_void, String> {
        match self.module_load_cubin(cubin) {
            Ok(module) => Ok(module),
            Err(cubin_err) => self.module_load_data(ptx).map_err(|ptx_err| {
                format!("CUDA module load failed for both cubin ({cubin_err}) and PTX ({ptx_err})")
            }),
        }
    }
...
pub(super) const Q4K_GEMV_CUBIN: &[u8] =
    include_bytes!(concat!(env!("OUT_DIR"), "/q4k_gemv.cubin"));
#[cfg(not(all(
    target_arch = "x86_64",
    any(target_os = "linux", target_os = "windows")
)))]
...
pub(super) const NEMOTRON_SELECTED_CUBIN: &[u8] =
    include_bytes!(concat!(env!("OUT_DIR"), "/nemotron_selected.cubin"));
#[cfg(not(all(
    target_arch = "x86_64",
    any(target_os = "linux", target_os = "windows")
)))]
...
pub(super) const PERSISTENT_DECODE_CUBIN: &[u8] =
    include_bytes!(concat!(env!("OUT_DIR"), "/persistent_decode.cubin"));
#[cfg(not(all(
    target_arch = "x86_64",
    any(target_os = "linux", target_os = "windows")
)))]
