...
impl CudaState {
    ...
    pub(in crate::runtime) fn allocate_device_tensor(
        &mut self,
        desc: DeviceTensorDesc,
    ) -> Result<(DeviceTensorId, u64), String> {
        let bytes = desc
            .byte_len()
            .ok_or_else(|| "CUDA device tensor allocation byte overflow".to_string())?;
        if bytes == 0 {
            return Err("CUDA device tensor allocation requires non-zero bytes".to_string());
        }
        self.set_current()?;
        let ptr = unsafe { self.api.mem_alloc(bytes)? };
        let id = self.insert_device_tensor_slot(ptr, bytes, desc)?;
        Ok((id, ptr))
    }
