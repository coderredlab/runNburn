...
pub(super) fn qwen35_selected_sparse_execution_descriptor(
    ...
    if input.q4_gate_up_silu_pack4_group8 && input.gate_up_group != Some(8) {
        return Err(
            "Qwen35 selected sparse group8 pack4 activation requires gate/up group8".to_string(),
        );
    }
    ...
    let activation_layout = if input.q4_gate_up_silu_pack4_group8 {
    ...
    let silu_launches = if input.q4_gate_up_silu_fused || input.q4_gate_up_silu_pack4_f32 {
        0
    } else {
        1
    };
    let mut group_meta_calls = usize::from(input.gate_up_group.is_some());
    if input.q4_gate_up_silu_pack4_group8 {
        group_meta_calls += 1;
    }
    if input.q6_down_pack4_f32 && input.q4_gate_up_silu_pack4_group8 {
        group_meta_calls += 1;
...
pub(super) fn q4k_resident_default_reserve_mib(total_mib: usize, mtp_device_verify: bool) -> usize {
    let base_mib = align_up(total_mib / 6, 256).clamp(1024, 4096);
...
pub(super) fn q4k_resident_configured_reserve_mib(
    total_mib: usize,
    mtp_device_verify: bool,
) -> Result<usize, String> {
    let mut reserve_mib = std::env::var("RNB_CUDA_Q4K_CACHE_RESERVE_MB")
        ...
        .transpose()?
        .unwrap_or_else(|| q4k_resident_default_reserve_mib(total_mib, mtp_device_verify));
...
pub(super) fn q4k_resident_cache_limit(api: &CudaApi) -> Result<usize, String> {
    ...
    let reserve_mib =
        q4k_resident_configured_reserve_mib(total_mib, mtp_device_verify_env_enabled())?;
...
fn device_buffer_allocation_capacity(api: &CudaApi, bytes: usize) -> Result<usize, String> {
