...
fn qwen35_selected_base_offset_group_meta_matches_slice_group_meta() {
    ...
    for max_group in [2usize, 4, 8, 16] {
...
#[test]
fn cuda_qwen35_existing_resident_role_count_detects_selected_hits() {
    let _guard = runtime_test_lock();
    let n_ff = 256usize;
    let n_embd = 256usize;
    let n_expert = 2usize;
    ...
    let mut state = match CudaState::open() {
        Ok(state) => state,
        Err(err) if cuda_driver_unavailable_for_test(&err) => {
            eprintln!("skipping existing resident hit CUDA test: {err}");
            return;
        }
        Err(err) => panic!("open CUDA state failed: {err}"),
    };
    ...
    assert_eq!(
        state
            .qwen35_selected_base_existing_resident_role_count_by_token(
                &sparse_gate,
                &sparse_up,
                &sparse_down,
                &expert_ids,
            ...
            .expect("count before resident prewarm"),
        0
    );

    let unrelated = make_test_q4k_weights(1, n_ff, n_embd / 256, 991).concat();
    assert!(state
        .preload_resident_q4k_weight_slice(&unrelated)
        .expect("preload unrelated resident weight"));
    assert_eq!(
        state
            .qwen35_selected_base_existing_resident_role_count_by_token(
                &sparse_gate,
            ...
            .expect("count with unrelated resident prewarm"),
        0
    );

    let expert_bytes = n_ff * (n_embd / 256) * 144;
    let gate0 = &sparse_gate[..expert_bytes];
    let up1 = &sparse_up[expert_bytes..expert_bytes * 2];
    assert!(state
        .preload_resident_q4k_weight_slice(gate0)
        .expect("preload resident gate expert 0"));
    assert!(state
        .preload_resident_q4k_weight_slice(up1)
        .expect("preload resident up expert 1"));
    ...
    assert_eq!(
        state
            ...
            .expect("count after resident prewarm"),
        2
    );
}
...
#[test]
fn cuda_qwen35_prefill_device_topk_route_weights_match_host_softmax_bits() {
    let _guard = runtime_test_lock();
    let mut state = CudaState::open().expect("open CUDA state");
    let seq_len = 1usize;
...
fn q4k_resident_default_reserve_adds_mtp_workspace_budget() {
    assert_eq!(q4k_resident_default_reserve_mib(4096, false), 1024);
    assert_eq!(q4k_resident_default_reserve_mib(4096, true), 2048);
    assert_eq!(q4k_resident_default_reserve_mib(8192, false), 1536);
    assert_eq!(q4k_resident_default_reserve_mib(8192, true), 3584);
    assert_eq!(q4k_resident_default_reserve_mib(11917, false), 2048);
    assert_eq!(q4k_resident_default_reserve_mib(11917, true), 5120);
    assert_eq!(q4k_resident_default_reserve_mib(16384, true), 6912);
}
...
fn q4k_resident_configured_reserve_honors_env_override() {
    ...
    assert_eq!(
        q4k_resident_configured_reserve_mib(10 * 1024, false).unwrap(),
        1536
    );
}
...
#[test]
fn gdn_prefill_chain_env_policy_defaults_off_and_enables_q4k_device_chain() {
    let _guard = runtime_test_lock();
    let shape = qwen35_gdn_prefill_chain_shape(32);
    unsafe {
...
fn cuda_glm_sparse_iq2xxs_iq3xxs_by_token_matches_individual_dispatches() {
    ...
    let scalar = glm_sparse_experts_iq2xxs_iq3xxs_by_token(
        &gate_slots,
        &up_slots,
        &down_slots,
        None,
    ...
    let grouped = glm_sparse_experts_iq2xxs_iq3xxs_by_token(
        &gate_slots,
        &up_slots,
        &down_slots,
        None,
...
#[test]
fn cuda_q4k_sidecar_row_pair_cache_matches_packed_q8_path() {
    let _guard = runtime_test_lock();
    let rows = 9usize;
    let cols = 6144usize;
    let blocks_per_row = cols / 256;
    let weights = make_test_q4k_weights(1, rows, blocks_per_row, 137)
        .pop()
        .unwrap();
    let sidecar_packed = rnb_cpu::gemm::pack_q4k::pack_q4k(&weights, rows, blocks_per_row);
    let mut input = vec![0.0f32; cols];
    for (i, x) in input.iter_mut().enumerate() {
        *x = ((i as f32 % 61.0) - 30.0) * 0.0078125;
    }
    let expected = cpu_q4k_gemv_rows(&weights, rows, blocks_per_row, &input);
    let packed =
        q4k_packed_q8_for_test(&weights, rows, cols, &input).expect("CUDA packed Q4_K Q8 GEMV");
    let sidecar = q4k_sidecar_packed_q8_for_test(&weights, &sidecar_packed, rows, cols, &input)
        .expect("CUDA sidecar-row-pair Q4_K Q8 GEMV");

    assert_close_rows("sidecar-row-pair Q4_K Q8 GEMV", &sidecar, &expected, 0.12);
    assert_close_rows("sidecar-row-pair Q4_K vs packed", &sidecar, &packed, 1.0e-5);
}
...
#[test]
fn cuda_q6k_sidecar_row_pair_cache_matches_packed_q8_path() {
    let _guard = runtime_test_lock();
    let rows = 9usize;
    let cols = 12288usize;
    let blocks_per_row = cols / 256;
    let weights = make_test_q6k_weights(1, rows, blocks_per_row, 139)
        .pop()
        .unwrap();
    let sidecar_packed = rnb_cpu::gemm::pack_q6k::pack_q6k(&weights, rows, blocks_per_row);
    let mut input = vec![0.0f32; cols];
    for (i, x) in input.iter_mut().enumerate() {
        *x = ((i as f32 % 67.0) - 33.0) * 0.0048828125;
    }
    let expected = cpu_q6k_gemv_rows(&weights, rows, blocks_per_row, &input);
    let packed =
        q6k_packed_q8_for_test(&weights, rows, cols, &input).expect("CUDA packed Q6_K Q8 GEMV");
    let sidecar = q6k_sidecar_packed_q8_for_test(&weights, &sidecar_packed, rows, cols, &input)
        .expect("CUDA sidecar-row-pair Q6_K Q8 GEMV");

    assert_close_rows("sidecar-row-pair Q6_K Q8 GEMV", &sidecar, &expected, 0.3);
    assert_close_rows("sidecar-row-pair Q6_K vs packed", &sidecar, &packed, 1.0e-5);
}
...
fn cuda_qwen35_sparse_experts_by_token_groups_repeated_expert() {
    ...
    for row_idx in 0..token_count * n_embd {
        ...
        assert!(
            diff < 0.2,
            "CUDA sparse by-token grouped row {row_idx} mismatch: expected {}, actual {}, diff {}",
            expected[row_idx],
            actual[row_idx],
            diff
        );
...
#[test]
fn cuda_qwen35_cached_q4_shared_sparse_device_topk_selected_base_matches_cpu_reference() {
    let _guard = runtime_test_lock();
    let _allow = EnvVarGuard::set("RNB_CUDA_ALLOW_EXPANDED_WEIGHT_CACHE", "1");
    reset_delta_state_cache().expect("reset CUDA runtime cache");
    let prev_enabled = std::env::var("RNB_CUDA_QWEN35_SHARED_Q4_F32_CACHE").ok();
...
#[test]
fn cuda_qwen35_sparse_experts_by_token_q6_pack4_gate_up_silu_group8_len8_matches_pack4_path() {
    let _guard = runtime_test_lock();
    reset_delta_state_cache().expect("reset CUDA runtime cache");
    ...
    let down = make_test_q6k_weights(1, n_embd, n_ff / 256, 521)
        .pop()
        .unwrap();
    let gate_refs = vec![gate.as_slice(); 8];
    let up_refs = vec![up.as_slice(); 8];
    let down_refs = vec![down.as_slice(); 8];
    let route = vec![0.125f32, 0.25, 0.375, 0.50, 0.625, 0.75, 0.875, 1.0];
    let token_ids = vec![0u32, 1, 2, 3, 0, 1, 2, 3];
    let input = (0..token_count * n_embd)
    ...
    assert!(
        max_abs <= 1.0e-4,
        "Q6 pack4 group8 len8 gate/up fused path changed baseline output: idx={max_idx} baseline={:.9} fused={:.9} abs={max_abs:.9} rel={max_rel:.9}",
        baseline[max_idx],
        fused[max_idx]
    );
}
    ...
    for t in 0..seq_len {
        ...
        for h in 0..heads {
            ...
            for dim in 0..head_dim {
                ...
                let freq = 1.0 / theta.powf((2 * pair_idx) as f32 / rope_dim as f32);
                let angle = pos as f32 * freq;
                let (sin_a, cos_a) = angle.sin_cos();
