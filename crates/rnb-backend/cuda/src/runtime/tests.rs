...
#[test]
fn cuda_moe_slice_cache_resident_path_is_cache_state_and_batch_invariant() {
    let _guard = runtime_test_lock();
    if let Err(error) = CudaState::open() {
        eprintln!("skipping resident MoE slice cache CUDA test: {error}");
        return;
    }
    // Fixed generous budget so the first pass admits everything.
    ...
    let route = (0..expert_count)
        ...
        .collect::<Vec<_>>();
    let input = (0..token_count * n_embd)
        .map(|index| 100.0 + (index % 29) as f32 * 0.25)
        .collect::<Vec<_>>();
    let gate = (0..expert_count)
        .map(|expert| synthetic_iq_weights(n_ff, n_embd, 66, expert * 3))
        .collect::<Vec<_>>();
    ...
    let run_batch = || {
        ...
        .expect("resident sparse dispatch")
        .expect("resident cache enabled")
    };

    // Cold (all misses) vs warm (all hits) must be bitwise identical.
    let cold = run_batch();
    let warm = run_batch();
    assert_eq!(cold, warm, "cache hit state changed resident output");

    // Contract 2: verify-style batch equals per-token decode-style calls.
    for token in 0..token_count {
        ...
        .expect("resident single-token dispatch")
        .expect("resident cache enabled");
        assert_eq!(
            single,
            cold[token * n_embd..(token + 1) * n_embd].to_vec(),
            "single-token decode diverged from batch verify for token {token}"
        );
    }

    // Contract 1 under pressure: a budget too small for the working set
    // forces steal/temp-overflow admission and must not change values.
    clear_moe_expert_slice_cache().expect("reset moe slice cache");
    let _tiny = EnvVarGuard::set("RNB_CUDA_MOE_EXPERT_CACHE_MB", "1");
    let pressured = run_batch();
    assert_eq!(cold, pressured, "cache pressure changed resident output");
    clear_moe_expert_slice_cache().expect("reset moe slice cache");
}
...
// Short Q5_K prefill keeps the F32 gate/up runner because deterministic Q5_K
// down MMQ starts at 32 tokens. Wider q8dot metadata must not be paired with
// that group8-only runner or groups longer than eight are silently skipped.
#[test]
fn cuda_qwen35_sparse_experts_q5_short_prefill_keeps_group8_gate_up() {
    let _guard = runtime_test_lock();
    reset_delta_state_cache().expect("reset CUDA runtime cache");
    ...
    let route = (1..=slots)
        .map(|value| value as f32 / slots as f32)
        .collect::<Vec<_>>();
    ...
    let run = || {
        ...
        .expect("CUDA qwen35 short Q5_K sparse experts")
    };
    ...
    let candidate = run();
    assert_close_rows_abs_rel(
        "short Q5_K prefill q8dot policy vs group8 gate/up",
        &candidate,
        &group8,
        1.0e-4,
        1.0e-6,
...
#[test]
fn cuda_qwen35_sparse_experts_q5_down_gate_up_mmq_groups_match_group8() {
    let _guard = runtime_test_lock();
    reset_delta_state_cache().expect("reset CUDA runtime cache");
    ...
    let _handoff = EnvVarGuard::set("RNB_CUDA_QWEN35_Q4_GATE_UP_Q8_HANDOFF", "1");

    let n_ff = 256usize;
    let n_embd = 512usize;
    let token_count = 32usize;
    let gates = make_test_q4k_weights(4, n_ff, n_embd / 256, 641);
    let ups = make_test_q4k_weights(4, n_ff, n_embd / 256, 643);
    let downs = (0..4)
        .map(|expert| make_test_q5k_weights(n_embd, n_ff / 256, 647 + expert))
        .collect::<Vec<_>>();
    let expert_ids = [(0usize, 3usize), (1, 9), (2, 15), (3, 7), (0, 31)]
        .into_iter()
        .flat_map(|(expert, len)| std::iter::repeat(expert).take(len))
        .collect::<Vec<_>>();
    let slots = expert_ids.len();
    let gate_refs = expert_ids
        .iter()
        .map(|&expert| gates[expert].as_slice())
        .collect::<Vec<_>>();
    let up_refs = expert_ids
        .iter()
        .map(|&expert| ups[expert].as_slice())
        .collect::<Vec<_>>();
    let down_refs = expert_ids
        .iter()
        .map(|&expert| downs[expert].as_slice())
        .collect::<Vec<_>>();
    let route = (1..=slots)
        .map(|value| value as f32 / slots as f32)
        .collect::<Vec<_>>();
    ...
    let group8 = {
        let _group16 = EnvVarGuard::set("RNB_CUDA_QWEN35_Q4_GATE_UP_Q8DOT_MMQ_GROUP16", "0");
        run()
    };
    let group16 = {
        let _group16 = EnvVarGuard::set("RNB_CUDA_QWEN35_Q4_GATE_UP_Q8DOT_MMQ_GROUP16", "1");
    ...
    assert_close_rows_abs_rel(
        "Q5_K down group16 Q8-dot gate/up vs group8",
        &group16,
        &group8,
        1.0e-4,
        1.0e-6,
    ...
    assert_close_rows_abs_rel(
        "Q5_K down group32 Q8-dot gate/up vs group8",
        &group32,
        &group8,
        1.0e-4,
        1.0e-6,
