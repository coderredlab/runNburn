...
fn ensure_mtp_verify_prior_cache_buffer(
    ...
    let allocated = unsafe { api.mem_alloc(bytes) }?;
    *ptr = Some(allocated);
    *capacity = bytes;
    Ok(true)
}
...
impl super::CudaState {
    ...
    pub(in crate::runtime) fn stage_mtp_verify_attention_prior_kv_bits_for_layer(
        ...
        if prior_k_bits.len() != prior_values || prior_v_bits.len() != prior_values {
            return Err(format!(
                "MTP verify attention prior K/V len mismatch: k={} v={} expected={prior_values}",
                prior_k_bits.len(),
                prior_v_bits.len()
            ));
        }
    ...
    pub(in crate::runtime) fn stage_mtp_verify_gdn_input_projections_q4k(
        ...
        match alpha_blocks {
            Some(blocks) => self.q4k_batch_dev_input_to_dev(
                request.alpha_q4k,
                request.alpha_rows,
                blocks,
                plan.window_tokens,
    ...
    fn stage_mtp_verify_k_quant_projection_to_dev(
        &mut self,
        label: &str,
        quant: u32,
        weights: &[u8],
    ...
    pub(in crate::runtime) fn stage_mtp_verify_attention_qk_norm_rope(
        ...
        if request.head_dim == 512 && (!request.rope_neox || rope_dim != request.head_dim) {
            return Err(format!(
                "MTP verify attention head_dim=512 only supports full NeoX RoPE: rope_dim={rope_dim}, rope_neox={}",
                request.rope_neox
            ));
        }
        ...
        let rope_table = self.rope_table_ptrs(
            rope_table_dim,
            projection_buffers.window_tokens,
            request.pos_start,
            request.rope_theta,
        ...
        match request.head_dim {
            ...
            512 => self.launch_qk_norm_rope_neox_hd512_f16kv(
                q_input_dev,
                projection_buffers.k_dev,
                projection_buffers.v_dev,
                q_norm_dev,
    ...
    #[allow(dead_code)]
    pub(in crate::runtime) fn stage_mtp_verify_attention_output_hd256_window(
        &mut self,
        post_buffers: &MtpVerifyAttentionQkNormRopeBuffers,
        ...
        if post_buffers.head_dim != 256 {
            return Err(format!(
                "MTP verify attention output hd256 requires head_dim=256, got {}",
    ...
    #[allow(dead_code)]
    pub(in crate::runtime) fn stage_mtp_verify_attention_output_hd256_prior_window(
        &mut self,
        post_buffers: &MtpVerifyAttentionQkNormRopeBuffers,
        ...
        if post_buffers.head_dim != 256 {
            return Err(format!(
                "MTP verify attention output hd256 with prior requires head_dim=256, got {}",
                post_buffers.head_dim
            ));
        }
        if post_buffers.q_rows != request.num_heads.saturating_mul(post_buffers.head_dim) {
    ...
    pub(in crate::runtime) fn stage_mtp_verify_attention_o_projection_residual_ffn_norm_q4k(
        ...
        self.stage_mtp_verify_hidden_rows_rms_norm(buffers, ffn_norm, norm_eps, false)
    }
    ...
    pub(in crate::runtime) fn stage_mtp_verify_gdn_conv1d_silu(
        ...
        let conv_buffers = self.ensure_mtp_verify_gdn_conv_buffers(
            projection_buffers.window_tokens,
            channels,
            kernel_size,
        )?;
    ...
    pub(in crate::runtime) fn stage_mtp_verify_gdn_delta_scan_snapshots(
        ...
        if self.mtp_verify_gdn_delta_decode_tokenwise_enabled(delta_buffers) {
            ...
            let result = (|| {
                for _ in snapshot_after_tokens {
                    snapshots.push(super::DeltaStateSnapshot {
                        ptr: unsafe { self.api.mem_alloc(state_bytes) }?,
                        bytes: state_bytes,
                    });
                }
        ...
        let result = (|| {
            for _ in snapshot_after_tokens {
                let ptr = unsafe { self.api.mem_alloc(state_bytes) }?;
                snapshots.push(super::DeltaStateSnapshot {
                    ptr,
                    bytes: state_bytes,
                });
    ...
    pub(in crate::runtime) fn stage_mtp_verify_gdn_sparse_moe_full_layer_from_router_q4k(
        ...
        let (gate_base, up_base, down_base) = self
            .resident_moe_layers
            .get(&key)
            .map(|entry| (entry.gate_base, entry.up_base, entry.down_base))
            .ok_or_else(|| {
                format!(
                    "MTP verify Qwen35 full-layer MoE requires resident weights: experts={n_expert} n_ff={n_ff} n_embd={n_embd}"
                )
            })?;
        self.touch_resident_moe_layer(key);
    ...
    pub(in crate::runtime) fn stage_mtp_verify_gdn_shared_expert_q4k(
        ...
        let expected_gate = n_ff.checked_mul(shared_row_bytes_embd).ok_or_else(|| {
            format!("MTP verify Qwen35 shared gate byte overflow: n_ff={n_ff} n_embd={n_embd}")
        })?;
        ...
        let expected_down = n_embd.checked_mul(shared_down_row_bytes).ok_or_else(|| {
            format!("MTP verify Qwen35 shared down byte overflow: n_ff={n_ff} n_embd={n_embd}")
        })?;
        if shared_gate.len() != expected_gate || shared_up.len() != expected_gate {
            return Err(format!(
                "MTP verify Qwen35 shared gate/up byte mismatch: gate={} up={} expected={expected_gate}",
                shared_gate.len(),
                shared_up.len()
            ));
        }
        if shared_down.len() != expected_down {
            return Err(format!(
                "MTP verify Qwen35 shared down byte mismatch: got {}, expected {expected_down}",
                shared_down.len()
            ));
        }

        let shared_gate_f32_dev = self.resident_q4k_f32_ptr(shared_gate, n_ff, n_embd / 256)?;
        let shared_up_f32_dev = self.resident_q4k_f32_ptr(shared_up, n_ff, n_embd / 256)?;
        let shared_down_f32_dev = match shared_down_quant {
    ...
    pub(in crate::runtime) fn stage_mtp_verify_qwen35_attention_moe_layer_q4k(
        ...
        let (router_buffers, _) = self.stage_mtp_verify_qwen35_attention_moe_layer_q4k_inner(
            buffers, layer, rope_dim, rope_neox, rope_theta, pos_start, norm_eps, false,
        )?;
        router_buffers
    ...
    pub(in crate::runtime) fn stage_mtp_verify_qwen35_attention_moe_layer_q4k_with_kv_state(
        ...
        let (_, attention_kv) = self.stage_mtp_verify_qwen35_attention_moe_layer_q4k_inner(
            buffers, layer, rope_dim, rope_neox, rope_theta, pos_start, norm_eps, true,
        )?;
    ...
    > {
        ...
        let projection_buffers = self.stage_mtp_verify_attention_qkv_projections_q4k(
            buffers,
        ...
        let stage_start = std::time::Instant::now();
        let post_buffers = self.stage_mtp_verify_attention_qk_norm_rope(
            &projection_buffers,
        ...
        let stage_start = std::time::Instant::now();
        let attention_buffers = if let Some((prior_k_bits_dev, prior_v_bits_dev)) = self
            ...
            self.stage_mtp_verify_attention_output_hd256_prior_window(
                &post_buffers,
            ...
            self.stage_mtp_verify_attention_output_hd256_window(
                &post_buffers,
        ...
        match result {
            Ok(router_buffers) => {
                ...
                for &prefix in prefix_tokens {
                    match self
                        .stage_mtp_verify_gdn_conv_prefix_state_deferred(&conv_buffers, prefix)
                    {
                        Ok(conv_state) => prefix_conv_states.push((prefix, conv_state)),
                        Err(err) => {
                            let _ = self.stream_synchronize();
                            for snapshot in prefix_snapshots {
                                self.free_delta_state_snapshot(snapshot)?;
                ...
                let prefix_states = prefix_conv_states
                    ...
                    .map(|((prefix_tokens, conv_state), resident_delta_snapshot)| {
