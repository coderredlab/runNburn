...
struct DirectFileSlabPlan {
    offsets_by_key: HashMap<(usize, usize), usize>,
    runs: Vec<DirectFileRun>,
    logical_bytes: usize,
    staging_bytes: usize,
}
...
impl CudaState {
    ...
    pub(in crate::runtime) fn temp_q4k_slot_ptrs_3_direct_file(
        ...
        for run in &plan.runs {
            let region = &file_regions[run.region_index];
            let destination =
                &mut host_slab[run.staging_offset..run.staging_offset + run.staging_len];
            self.direct_file_reader
                .read_aligned(
                    region.path(),
                    run.file_offset,
                    destination,
    ...
    fn temp_q4k_slot_ptrs_3_direct_file_pipelined(
        ...
        if std::env::var("RNB_CUDA_TEMP_SLAB_TRACE").ok().as_deref() == Some("1") {
            eprintln!(
                "[cuda-direct-file-pipeline] gate_up_unique={} down_unique={} logical_bytes={} staging_bytes={} gate_up_runs={} down_runs={} gate_up_read_ms={gate_up_read_ms:.3} down_read_ms={down_read_ms:.3} h2d_runs=2 h2d_bytes={staging_bytes}",
                gate_up_ptrs.len(),
                down_ptrs.len(),
