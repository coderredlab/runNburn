...
fn qwen35_gate_ptr_scratch_bytes(token_count: usize, slots: usize) -> Result<usize, String> {
    let gate_ptr_bytes = slots
        .checked_mul(std::mem::size_of::<u64>())
        .ok_or_else(|| "Qwen35 gate pointer scratch byte size overflow".to_string())?;
    let token_major_meta_bytes = token_count
        .checked_add(1)
        .and_then(|value| value.checked_add(slots))
        .and_then(|value| value.checked_mul(std::mem::size_of::<u32>()))
        .ok_or_else(|| "Qwen35 token-major metadata byte size overflow".to_string())?;
    Ok(gate_ptr_bytes.max(token_major_meta_bytes))
}
...
fn qwen35_selected_base_temp_slab_cache_device_slots(
    ...
    PreparedQwen35DeviceSlotPtrs {
        ...
        group_meta32: cache.group_meta32.clone(),
        group_meta64: cache.group_meta64.clone(),
    }
}
...
fn qwen35_pack4_group_offsets_from_group_meta(group_meta: &[u32]) -> Result<Vec<u32>, String> {
    ...
    for chunk in group_meta.chunks_exact(2) {
        ...
        if group_len == 0 || group_len > 32 {
            return Err(format!(
                "Qwen35 pack4 group offset requires group length 1..=32, got {group_len}"
            ));
        }
        next = next
...
fn qwen35_selected_sparse_gate_up_activation_runner_from_descriptor(
    ...
    match descriptor.activation_layout {
        Qwen35SelectedSparseActivationLayout::Q8DotFusedSilu => {
            if !gate_up_grouped {
                return Err(
                    "Qwen35 selected sparse Q8-dot runner requires gate/up group metadata"
                        .to_string(),
                );
            }
            Ok(Qwen35SelectedSparseGateUpActivationRunner::Q8DotFusedSilu)
        }
...
fn qwen35_selected_sparse_gate_up_launch_payload(
    ...
    gate_up_q8: Option<(u64, u64, usize)>,
    ...
    match runner {
        Qwen35SelectedSparseGateUpActivationRunner::Q8DotFusedSilu => {
            let (qs_dev, ds_dev, input_len) = gate_up_q8.ok_or_else(|| {
                "Qwen35 Q4 gate/up Q8-dot path missing quantized input buffers".to_string()
            })?;
            let packed_dev = down_pack4_f32.ok_or_else(|| {
                "Qwen35 Q4 gate/up Q8-dot path missing activation output buffer".to_string()
            })?;
            let (pack_group_offsets_dev, pack_group_count) =
                if let Some(offsets) = pack4_group_offsets {
                    (
                        group_meta_dev + gate_up_group_meta_bytes as u64,
                        offsets.len() - 1,
                    )
                } else {
                    (0, gate_up_group_meta.len() / 2)
                };
            Ok(Qwen35SelectedSparseGateUpLaunchPayload::Q8DotFusedSilu {
                qs_dev,
                ds_dev,
                packed_dev,
...
fn qwen35_selected_sparse_silu_launch_payload(
    ...
    if let Some((qs_dev, ds_dev)) = down_q8 {
        return if runner.needs_separate_silu() {
            Qwen35SelectedSparseSiluLaunchPayload::Q8 { qs_dev, ds_dev }
        } else {
            Qwen35SelectedSparseSiluLaunchPayload::QuantizeQ8 { qs_dev, ds_dev }
        };
    }
...
fn qwen35_selected_sparse_down_launch_runner(
    ...
    match down_quant {
        ...
        12 if group4_down && q4_down_group4 && q4_down_q8dot => {
            Ok(Qwen35SelectedSparseDownLaunchRunner::Q4Q8DotGroup4)
        }
        13 if group4_down && q5_down_q8dot_mmq => {
            Ok(Qwen35SelectedSparseDownLaunchRunner::Q5Q8DotMmqGroup16)
        }
...
fn qwen35_selected_sparse_down_launch_payload<'a>(
    ...
    match runner {
        ...
        Qwen35SelectedSparseDownLaunchRunner::Q4Q8DotGroup4 => {
            let (qs_dev, ds_dev) = down_q8
                .ok_or_else(|| "Qwen35 Q4 q8dot down launch missing q8 buffers".to_string())?;
            Ok(Qwen35SelectedSparseDownLaunchPayload::Q4Q8DotGroup4 {
                qs_dev,
                ds_dev,
                group_count: down_group_meta.len() / 2,
            })
        }
        Qwen35SelectedSparseDownLaunchRunner::Q5Q8DotMmqGroup16 => {
            let (qs_dev, ds_dev) = down_q8
                .ok_or_else(|| "Qwen35 Q5 q8dot MMQ down launch missing q8 buffers".to_string())?;
            Ok(Qwen35SelectedSparseDownLaunchPayload::Q5Q8DotMmqGroup16 {
                qs_dev,
                ds_dev,
                group_count: down_group_meta.len() / 2,
            })
...
fn qwen35_selected_sparse_exact_runner_plan(
    ...
    let q8_handoff = input.q4_gate_up_q8_handoff
        && gate_up == Qwen35SelectedSparseGateUpActivationRunner::Q8DotFusedSilu
        && down_launch == Qwen35SelectedSparseDownLaunchRunner::Q5Q8DotMmqGroup16;
...
fn qwen35_selected_sparse_exact_runner_payload<'a>(
    ...
    down_run_tile_meta: Option<&'a [u32]>,
    down_group_len_split: Option<&'a Qwen35GroupMetaLenSplit>,
    q8_handoff_output: Option<(u64, u64)>,
) -> Result<Qwen35SelectedSparseExactRunnerPayload<'a>, String> {
    let q8_output = if plan.q8_handoff {
        Some(q8_handoff_output.ok_or_else(|| {
            "Qwen35 selected sparse Q8 handoff missing output buffers".to_string()
        })?)
    } else {
        None
    };
    let selected_down_q8 = q8_output.or(down_q8);
...
pub(super) fn qwen35_selected_base_upload_source<'a>(
...
impl CudaState {
    ...
    pub(in crate::runtime) fn qwen35_prepare_selected_base_temp_slab_device_slot_ptrs_by_token(
        ...
        let group_meta32 = qwen35_selected_base_group_meta_from_offsets(&offsets, 32);
        let group_meta64 = qwen35_selected_base_group_meta_from_offsets(&offsets, 64);
    ...
    fn qwen35_selected_base_resident_role_ptrs(
        ...
        let resident_layer = self
            .resident_moe_layers
            .get(&layer_key)
            .map(|entry| (entry.gate_base, entry.up_base, entry.down_base));
        if let Some((gate_base, up_base, down_base)) = resident_layer {
    ...
    fn qwen35_launch_selected_sparse_gate_up(
        ...
        match payload {
            Qwen35SelectedSparseGateUpLaunchPayload::Q8DotFusedSilu {
                qs_dev,
                ds_dev,
                packed_dev,
                pack_group_offsets_dev,
                ...
                self.launch_quantize_q8_1_by_32(input_dev, qs_dev, ds_dev, input_len)?;
    ...
    fn qwen35_launch_selected_sparse_down(
        ...
        route_dev: u64,
        output_dev: u64,
        q5_down_partial_dev: Option<u64>,
        q5_down_token_meta_dev: Option<(u64, u64)>,
    ) -> Result<(), String> {
        match payload {
            ...
            Qwen35SelectedSparseDownLaunchPayload::Q4Q8DotGroup4 {
                qs_dev,
                ds_dev,
                group_count,
            } => self.launch_selected_q4k_down_accum_by_token_group4_q8dot(
                down_ptrs_dev,
                token_ids_dev,
                group_meta_dev,
                n_embd,
            ...
            Qwen35SelectedSparseDownLaunchPayload::Q5Q8DotMmqGroup16 {
                qs_dev,
                ds_dev,
                group_count,
            } => {
                let slot_outputs_dev = q5_down_partial_dev.ok_or_else(|| {
                    "Qwen35 selected Q5_K down MMQ missing deterministic slot output scratch"
                        .to_string()
                })?;
                let (token_offsets_dev, slot_indices_dev) =
                    q5_down_token_meta_dev.ok_or_else(|| {
                        "Qwen35 selected Q5_K down MMQ missing deterministic token metadata"
                            .to_string()
                    })?;
                self.launch_selected_q5k_down_accum_q8dot_mmq_group16(
                    down_ptrs_dev,
                    token_ids_dev,
                    group_meta_dev,
                    n_embd,
    ...
    pub(in crate::runtime) fn qwen35_sparse_experts_by_token_to_dev_prepared(
        ...
        let output_len = token_count * n_embd;
        let gate_ptrs_scratch_bytes = qwen35_gate_ptr_scratch_bytes(token_count, slots)?;
        let gate_ptrs_dev = self.compute_gate_ptrs_ptr(gate_ptrs_scratch_bytes)?;
        ...
        let group8_gate_up = group_gate_up
            && std::env::var("RNB_CUDA_GROUP8_GATE_UP_WARP4")
                ...
                .as_deref()
                != Some("0");
        let legacy_group16_gate_up = group8_gate_up
            && std::env::var("RNB_CUDA_GROUP16_GATE_UP_WARP4")
                .ok()
                .as_deref()
                == Some("1")
            && (!tuning::qwen35_q4_gate_up_q8dot_enabled()
                || tuning::qwen35_q4_gate_up_q8dot_mmq_enabled());
        let grouped_down_group4 = std::env::var("RNB_CUDA_GROUP4_DOWN").ok().as_deref()
            != Some("0")
            && !prefer_group2_down
            && !tuning::group2_down_warp4_enabled()
            && std::env::var("RNB_CUDA_GROUP8_DOWN_WARP4").ok().as_deref() != Some("1");
        let q8dot_mmq_group16_gate_up = group8_gate_up
            && grouped_down_group4
            && tuning::qwen35_q4_gate_up_q8dot_enabled()
            && tuning::qwen35_q4_gate_up_q8dot_mmq_enabled()
            && tuning::qwen35_q4_gate_up_q8dot_mmq_group16_enabled()
            && n_embd % 256 == 0
            && n_ff % 256 == 0
            && match down_quant {
                // The wider q8dot gate/up metadata is valid only when Q5_K down
                // also takes its q8dot MMQ path. Short prefill keeps the grouped
                // F32 gate/up runner, whose launcher supports at most eight slots.
                13 => tuning::qwen35_q5_down_q8dot_mmq_enabled(token_count),
                12 => {
                    tuning::qwen35_q4_down_group4_enabled()
                        && tuning::qwen35_q4_gate_up_q8dot_q4_down_enabled()
                }
                14 => {
                    !tuning::qwen35_down_token_major_enabled()
        ...
        let group32_gate_up =
            q8dot_mmq_group16_gate_up && tuning::qwen35_q4_gate_up_q8dot_mmq_group32_enabled();
        let group16_gate_up = legacy_group16_gate_up || q8dot_mmq_group16_gate_up;
        let gate_up_max_group = if group_gate_up {
            Some(if group32_gate_up {
                32
            } else if group16_gate_up {
                16
        ...
        let q5_down_q8dot_mmq = down_quant == 13
            && !group2_down
            && !group8_down
            && n_ff % 256 == 0
            && tuning::qwen35_q5_down_q8dot_mmq_enabled(token_count);
        let q5_down_token_major_plan = if q5_down_q8dot_mmq {
            Some(qwen35_down_token_major_plan(token_ids, token_count)?)
        } else {
            None
        };
        let down_max_group = if group_gate_up {
            Some(if group2_down {
                ...
                8
            } else if q5_down_q8dot_mmq && tuning::qwen35_q5_down_q8dot_mmq_group64_enabled() {
                64
            } else if q5_down_q8dot_mmq && tuning::qwen35_q5_down_q8dot_mmq_group32_enabled() {
                32
            } else if q5_down_q8dot_mmq {
                16
            } else {
                4
            })
        ...
        let q4_down_group4 = down_quant == 12
            ...
            && tuning::qwen35_q4_down_group4_enabled();
        let q4_down_q8dot =
            q4_down_group4 && tuning::qwen35_q4_down_q8dot_enabled() && n_ff % 32 == 0;
        ...
        let q6_down_pack4_f32 = down_quant == 14
            ...
            && !q6_down_q8dot
            && tuning::qwen35_q6_down_pack4_f32_enabled();
        let q4_gate_up_q8dot = (q6_down_pack4_f32
            || q5_down_q8dot_mmq
            || (q4_down_group4 && tuning::qwen35_q4_gate_up_q8dot_q4_down_enabled()))
            && tuning::qwen35_q4_gate_up_q8dot_enabled()
            && group8_gate_up
            && !gate_up_group_meta.is_empty()
            && n_embd % 256 == 0
            && n_ff % 256 == 0;
        let q4_gate_up_q8_handoff = q4_gate_up_q8dot
            && q5_down_q8dot_mmq
            && group32_gate_up
            && tuning::qwen35_q4_gate_up_q8_handoff_enabled();
        let q4_gate_up_silu_pack4_f32 = q6_down_pack4_f32
            && !q4_gate_up_q8dot
            && tuning::qwen35_q4_gate_up_silu_pack4_f32_enabled()
            && !down_group_meta.is_empty()
            && n_ff % 256 == 0;
        let q4_gate_up_silu_pack4_group8 = (q4_gate_up_q8dot || q4_gate_up_silu_pack4_f32)
            && group8_gate_up
            && !gate_up_group_meta.is_empty();
        let q4_gate_up_silu_pack4_group_offsets =
            if q4_gate_up_silu_pack4_group8 && q6_down_pack4_f32 {
        ...
        let gate_up_launches = 1usize + usize::from(q4_gate_up_q8dot);
        let silu_launches = if q4_gate_up_q8dot {
            usize::from(
                (q6_down_q8dot || q4_down_q8dot || q5_down_q8dot_mmq) && !q4_gate_up_q8_handoff,
            )
        } else if q4_gate_up_silu_fused || q4_gate_up_silu_pack4_f32 {
            0
        } else {
            1
        };
            ...
            || boundary_stats.is_some()
        {
            ...
            Some(qwen35_selected_sparse_runtime_descriptor(
                Qwen35SelectedSparseRuntimeDescriptorInput {
                    ...
                    q4_gate_up_silu_pack4_group8: q4_gate_up_silu_pack4_group8 && !q4_gate_up_q8dot,
                    q6_down_q8dot,
                    q6_down_pack4_f32,
        ...
        if let Some(stats) = boundary_stats.as_mut() {
            ...
            stats.gate_up_launches = gate_up_launches;
            stats.silu_launches = silu_launches;
            stats.down_launches = down_launches;
        ...
        let gate_up_q8_len = if q4_gate_up_q8dot {
            Some(token_count.checked_mul(n_embd).ok_or_else(|| {
                format!(
                    "Qwen35 Q4 gate/up Q8 input length overflow: tokens={token_count} n_embd={n_embd}"
                )
            })?)
        } else {
            None
        };
        let down_q8_len = if q6_down_q8dot || q4_down_q8dot || q5_down_q8dot_mmq {
            if n_ff % 32 != 0 {
                return Err(format!(
                    "Qwen35 q8dot down n_ff must be divisible by 32, got {n_ff}"
                ));
            }
            Some(slots.checked_mul(n_ff).ok_or_else(|| {
                format!("Qwen35 q8dot down input length overflow: slots={slots} n_ff={n_ff}")
            })?)
        } else {
            None
        };
        let reusable_down_q8_len = if q4_gate_up_q8_handoff {
            None
        } else {
            down_q8_len
        };
        let q8_capacity_len = gate_up_q8_len
            .into_iter()
            .chain(reusable_down_q8_len)
            .max()
            .unwrap_or(0);
        let q8_buffers = if q8_capacity_len > 0 {
            Some((
                self.compute_full_gate_ptr(q8_capacity_len)?,
                self.compute_full_up_ptr((q8_capacity_len / 32) * std::mem::size_of::<f32>())?,
            ))
        } else {
            None
        };
        let gate_up_q8 = gate_up_q8_len.map(|input_len| {
            let (qs_dev, ds_dev) = q8_buffers.expect("Q8 gate/up buffers exist when enabled");
            (qs_dev, ds_dev, input_len)
        });
        let down_q8 =
            reusable_down_q8_len.map(|_| q8_buffers.expect("Q8 down buffers exist when enabled"));
        let q8_handoff_output = q4_gate_up_q8_handoff.then_some((gate_dev, up_dev));
        ...
        } else if q4_gate_up_q8dot {
            Some(gate_dev)
        } else {
            None
        };
        let q5_down_partial_dev = if q5_down_q8dot_mmq {
            let bytes = slots
                .checked_mul(n_embd)
                .and_then(|value| value.checked_mul(std::mem::size_of::<f32>()))
                .ok_or_else(|| {
                    format!(
                        "Qwen35 Q5_K deterministic down partial byte size overflow: slots={slots} n_embd={n_embd}"
                    )
                })?;
            Some(self.qwen35_packed_act_ptr(bytes)?)
        } else {
        ...
        if let Some(compound_runner_payload) = compound_runner_payload {
            ...
            let q5_down_token_meta_dev = if let Some(plan) = q5_down_token_major_plan.as_ref() {
                let token_offsets_bytes = std::mem::size_of_val(plan.token_offsets.as_slice());
                let slot_indices_bytes = std::mem::size_of_val(plan.slot_indices.as_slice());
                let required_bytes = token_offsets_bytes
                    .checked_add(slot_indices_bytes)
                    .ok_or_else(|| {
                        "Qwen35 Q5_K deterministic token metadata byte size overflow".to_string()
                    })?;
                if required_bytes > gate_ptrs_scratch_bytes {
                    return Err(format!(
                        "Qwen35 Q5_K deterministic token metadata exceeds released gate pointer scratch: required={required_bytes} capacity={gate_ptrs_scratch_bytes}"
                    ));
                }
                unsafe {
                    self.api.memcpy_htod_async(
                        gate_ptrs_dev,
                        plan.token_offsets.as_ptr().cast::<libc::c_void>(),
                        token_offsets_bytes,
                        self.stream,
                    )?;
                    self.api.memcpy_htod_async(
                        gate_ptrs_dev + token_offsets_bytes as u64,
                        plan.slot_indices.as_ptr().cast::<libc::c_void>(),
                        slot_indices_bytes,
                        self.stream,
                    )?;
                }
                Some((gate_ptrs_dev, gate_ptrs_dev + token_offsets_bytes as u64))
            } else {
                None
            };
            let kernel_t0 = trace_kernel.then(std::time::Instant::now);
...
mod tests {
    use super::{
        ...
        qwen35_gate_ptr_scratch_bytes, qwen35_group_meta_from_ids_enabled,
        qwen35_pack4_group_offsets_for_down_group_meta,
        qwen35_selected_sparse_compound_graph_captures_zero_output,
    ...
    fn gate_up_activation_runner_preserves_existing_dispatch_priority() {
        assert_eq!(
            qwen35_selected_sparse_gate_up_activation_runner(true, false, true, true, true),
            Qwen35SelectedSparseGateUpActivationRunner::Pack4F32Group8
        );
        assert_eq!(
            qwen35_selected_sparse_gate_up_activation_runner(false, false, true, true, true),
            Qwen35SelectedSparseGateUpActivationRunner::Pack4F32Group4
        );
        assert_eq!(
            qwen35_selected_sparse_gate_up_activation_runner(false, false, false, true, true),
            Qwen35SelectedSparseGateUpActivationRunner::FusedSiluGroup8
        );
        assert_eq!(
            qwen35_selected_sparse_gate_up_activation_runner(false, false, false, false, false),
            Qwen35SelectedSparseGateUpActivationRunner::SeparateUngrouped
        );
        assert_eq!(
            qwen35_selected_sparse_gate_up_activation_runner(false, false, false, false, true),
            Qwen35SelectedSparseGateUpActivationRunner::SeparateGrouped
        );
    }
    ...
    fn silu_launch_payload_materializes_runner_requirements() {
        ...
        assert_eq!(
            ...
            Qwen35SelectedSparseSiluLaunchPayload::QuantizeQ8 {
                qs_dev: 11,
                ds_dev: 12,
            }
        );
    ...
    #[test]
    fn gate_pointer_scratch_covers_sparse_token_major_metadata() {
        assert_eq!(
            qwen35_gate_ptr_scratch_bytes(1139, 1130).expect("scratch bytes"),
            9080
        );
        assert_eq!(
            qwen35_gate_ptr_scratch_bytes(32, 64).expect("pointer bytes"),
            512
        );
    }
    ...
    fn down_launch_runner_preserves_existing_priority() {
        assert_eq!(
            qwen35_selected_sparse_down_launch_runner(
                14, true, true, false, false, false, true, true, true, true, true, true, true,
            )
        ...
        assert_eq!(
            qwen35_selected_sparse_down_launch_runner(
                14, true, true, false, false, false, false, true, true, true, true, true, true,
            )
        ...
        assert_eq!(
            qwen35_selected_sparse_down_launch_runner(
                14, true, true, false, false, false, false, false, true, true, true, true, true,
            )
        ...
        assert_eq!(
            qwen35_selected_sparse_down_launch_runner(
                14, true, true, false, false, false, false, false, false, true, true, true, true,
            )
        ...
        assert_eq!(
            qwen35_selected_sparse_down_launch_runner(
                14, true, true, false, false, false, false, false, false, false, true, true, true,
            )
        ...
        assert_eq!(
            qwen35_selected_sparse_down_launch_runner(
                14, true, true, false, false, false, false, false, false, false, false, true, true,
            )
        ...
        assert_eq!(
            qwen35_selected_sparse_down_launch_runner(
                13, true, true, false, false, true, false, false, false, false, false, false,
                false,
            )
            .expect("q5 q8dot MMQ priority"),
            Qwen35SelectedSparseDownLaunchRunner::Q5Q8DotMmqGroup16
        );
        assert_eq!(
            qwen35_selected_sparse_down_launch_runner(
    ...
    fn down_launch_runner_maps_descriptor_modes() {
        ...
        assert_eq!(
            qwen35_selected_sparse_down_launch_runner_from_descriptor(
                &q8dot, true, true, false, false, false, false, false, false, false, false,
            )
    ...
    fn exact_runner_plan_groups_descriptor_backed_launch_decisions() {
        ...
        let plan =
            qwen35_selected_sparse_exact_runner_plan(Qwen35SelectedSparseExactRunnerPlanInput {
                ...
                q4_down_q8dot: false,
                q5_down_q8dot_mmq: false,
                q4_gate_up_q8_handoff: false,
        ...
        assert!(!plan.q8_handoff);
    }

    #[test]
    fn exact_runner_plan_materializes_q8_handoff() {
        let plan =
            qwen35_selected_sparse_exact_runner_plan(Qwen35SelectedSparseExactRunnerPlanInput {
                descriptor: None,
                has_pack4_group_offsets: false,
                q4_gate_up_q8dot: true,
                q4_gate_up_silu_pack4_f32: false,
                q4_gate_up_silu_fused: false,
                has_gate_up_group_meta: true,
                has_down_token_major_plan: false,
                ...
                q6_down_pack4_f32: false,
                group4_down: true,
                gate_up_meta_matches_down_meta: true,
                down_quant: 13,
                warp_down: true,
                q4_down_group4: false,
                q4_down_q8dot: false,
                q5_down_q8dot_mmq: true,
                q4_gate_up_q8_handoff: true,
                down_token_major: false,
                q6_down_q8dot: false,
                q6_down_run_tiled4: false,
                q6_down_run_batched_ref: false,
            ...
            .expect("Q8 handoff exact runner plan");
        assert!(plan.q8_handoff);
        ...
        .expect("Q8 handoff exact runner payload");
        assert_eq!(
            payload.gate_up,
            Qwen35SelectedSparseGateUpLaunchPayload::Q8DotFusedSilu {
                qs_dev: 11,
                ds_dev: 12,
                packed_dev: 21,
                pack_group_offsets_dev: 0,
                pack_group_count: 1,
                input_len: 8192,
                q8_output: Some((21, 22)),
            }
        );
        assert_eq!(payload.silu, Qwen35SelectedSparseSiluLaunchPayload::None);
        assert_eq!(
            payload.down_launch,
            Qwen35SelectedSparseDownLaunchPayload::Q5Q8DotMmqGroup16 {
                qs_dev: 21,
    ...
    fn exact_runner_payload_groups_stage_payloads() {
        ...
        assert_eq!(
            ...
            Qwen35SelectedSparseExactRunnerPayload {
                ...
                silu: Qwen35SelectedSparseSiluLaunchPayload::QuantizeQ8 {
                    qs_dev: 11,
                    ds_dev: 12,
                },
