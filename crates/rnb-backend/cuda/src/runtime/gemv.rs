...
impl CudaState {
    ...
    pub(super) fn q6k_gemv_device_to_device(
        ...
        let mut blocks_per_row_arg = blocks_per_row as u32;
        let warp8_output = tuning::q6k_output_warp8_enabled();
        let kernel = if warp8_output && tuning::q6k_warp8_unrolled_enabled() {
            "rnb_q6k_gemv_warp8_unrolled"
        } else if warp8_output {
            "rnb_q6k_gemv_warp8"
        } else {
            "rnb_q6k_gemv"
        };
        let grid_x = if warp8_output {
    ...
    pub(super) fn q5_basic_gemv(
        ...
        let (kernel, grid, block) =
            ...
            } else if kernel.ends_with("_warp8") {
                (kernel, (rows.div_ceil(8) as u32, 1, 1), (256, 1, 1))
            } else {
    ...
    pub(super) fn q5_basic_gemv_batch(
        ...
        if kernel == "rnb_q8_0_gemv_batch"
            && tuning::q8_0_mmq_tile32_enabled(seq_len, rows, blocks_per_row)
        {
    ...
    fn q8_0_mmq_batch(
        ...
        let output_len = seq_len
            .checked_mul(rows)
            .ok_or_else(|| "Q8_0 MMQ output length overflow".to_string())?;
        let output_bytes = output_len
            .checked_mul(std::mem::size_of::<f32>())
            .ok_or_else(|| "Q8_0 MMQ output byte size overflow".to_string())?;
        ...
        let ds_bytes = (input.len() / 32)
            .checked_mul(std::mem::size_of::<f32>())
            .ok_or_else(|| "Q8_0 MMQ scale byte size overflow".to_string())?;
        ...
        self.launch_q8_0_q8_1_matmul_mmq_tile32(
            weights,
            rows,
            blocks_per_row,
            seq_len,
    ...
    pub(super) fn q8_0_head_gemv_batch(
        &mut self,
        weights: &[u8],
        head_count: usize,
        rows_per_head: usize,
        ...
        let rows = head_count
            ...
            .ok_or_else(|| "Q8_0 head batch row count overflow".to_string())?;
        let cols = blocks_per_row
            .checked_mul(32)
            .ok_or_else(|| "Q8_0 head batch column count overflow".to_string())?;
        ...
        let output_len = token_count
            .checked_mul(rows)
            .ok_or_else(|| "Q8_0 head batch output length overflow".to_string())?;
        let output_bytes = output_len
            .checked_mul(std::mem::size_of::<f32>())
            .ok_or_else(|| "Q8_0 head batch output byte size overflow".to_string())?;
        let weights_dev = self.resident_q4k_weights_ptr(weights)?;
        let input_dev = self.compute_input_ptr(std::mem::size_of_val(input))?;
        let output_dev = self.compute_output_ptr(output_bytes)?;
        unsafe {
        ...
        let mut rows_arg = u32::try_from(rows)
            .map_err(|_| format!("Q8_0 head batch rows exceed CUDA u32 limit: {rows}"))?;
        let mut rows_per_head_arg = u32::try_from(rows_per_head).map_err(|_| {
            format!("Q8_0 head batch rows_per_head exceeds CUDA u32 limit: {rows_per_head}")
        })?;
        let mut blocks_per_row_arg = u32::try_from(blocks_per_row).map_err(|_| {
            format!("Q8_0 head batch blocks_per_row exceeds CUDA u32 limit: {blocks_per_row}")
        })?;
        let mut token_count_arg = u32::try_from(token_count).map_err(|_| {
            format!("Q8_0 head batch token_count exceeds CUDA u32 limit: {token_count}")
        })?;
        self.launch_cached_gemv(
            "rnb_q8_0_head_gemv_batch",
            &[
                (&mut output_arg as *mut u64).cast::<libc::c_void>(),
                (&mut weights_arg as *mut u64).cast::<libc::c_void>(),
                (&mut input_arg as *mut u64).cast::<libc::c_void>(),
                (&mut rows_arg as *mut u32).cast::<libc::c_void>(),
                (&mut rows_per_head_arg as *mut u32).cast::<libc::c_void>(),
                (&mut blocks_per_row_arg as *mut u32).cast::<libc::c_void>(),
                (&mut token_count_arg as *mut u32).cast::<libc::c_void>(),
            ],
            (rows_arg.div_ceil(4), token_count_arg, 1),
            (32, 4, 1),
        )?;
        let mut output = vec![0.0f32; output_len];
        unsafe {
    ...
    pub(super) fn q6k_gemv_argmax_fused_warp8(
        ...
        let mut rows_arg = rows as u32;
        let mut blocks_per_row_arg = blocks_per_row as u32;
        if tuning::q6k_argmax_batched_single_enabled() {
            let mut input_stride_arg = (blocks_per_row * 256) as u32;
            let mut token_count_arg = 1u32;
            let mut block_count_arg = blocks as u32;
            self.launch_cached_gemv(
    ...
    pub(super) fn gemv_batch(
        ...
        } else if kernel_name.ends_with("_batch_warp8") {
            (
                kernel_name,
                (rows.div_ceil(8) as u32, seq_len as u32, 1),
                (256, 1, 1),
