...
use super::driver::PERSISTENT_DECODE_PTX;
...
impl CudaState {
    /// Load (or fetch cached) the persistent_decode PTX module and return its
    /// raw module handle.
    pub(in crate::runtime) fn ensure_persistent_decode_module(&mut self) -> Result<usize, String> {
        ...
        if self.persistent_decode_module.is_none() {
            let module = unsafe { self.api.module_load_data(PERSISTENT_DECODE_PTX)? };
