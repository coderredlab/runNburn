...
impl CudaState {
    ...
    fn cached_hd512_split_preferred(head_dim: usize, window_len: usize) -> bool {
        head_dim == 512
            && window_len >= 256
            && crate::tuning::decode_attention_hd512_split_enabled()
    }
    ...
    fn attention_decode_cached_range(
        ...
        let mut kv_heads_arg = num_kv_heads as u32;
        let mut scale_arg = scale;
        let prefer_hd512_split = Self::cached_hd512_split_preferred(head_dim, window_len);
        if use_device_len && !prefer_hd512_split {
            let kv_len_dev = self.cu68_graph_kv_len_ptr()?;
            unsafe {
                self.api.memcpy_htod_async(
                    kv_len_dev,
        ...
        } else if prefer_hd512_split {
            let chunk_size = crate::tuning::decode_attention_hd512_split_chunk_size();
            ...
            self.launch_cached_gemv(
                "rnb_attention_decode_hd512_split_partials",
                &[
            ...
            self.launch_cached_gemv(
                "rnb_attention_decode_hd512_split_reduce",
                &[
                    (&mut output_arg as *mut u64).cast::<libc::c_void>(),
                    (&mut partial_arg as *mut u64).cast::<libc::c_void>(),
                    (&mut meta_arg as *mut u64).cast::<libc::c_void>(),
