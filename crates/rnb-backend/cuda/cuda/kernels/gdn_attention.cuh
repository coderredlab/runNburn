...
extern "C" __global__ void rnb_qwen35_router_topk_softmax_f32(
    ...
    for (unsigned slot = 0; slot < selected_count; ++slot) {
        ...
        route_weights[out_base + slot] = expf(selected_logits[slot] - max_selected) / selected_sum;
...
extern "C" __global__ void rnb_gdn_prepare_delta_qkv_f32(
    ...
    float q_sum = 0.0f;
    float k_sum = 0.0f;
    for (unsigned d = tid; d < head_k_dim; d += 256u) {
    ...
    const float q_inv = rsqrtf(q_partial[0] + eps) * q_scale;
    const float k_inv = rsqrtf(k_partial[0] + eps);
    for (unsigned d = tid; d < head_k_dim; d += 256u) {
        q_out[row * head_k_dim + d] = conv_out[q_base + d] * q_inv;
        k_out[row * head_k_dim + d] = conv_out[k_base + d] * k_inv;
    }
    for (unsigned d = tid; d < head_v_dim; d += 256u) {
