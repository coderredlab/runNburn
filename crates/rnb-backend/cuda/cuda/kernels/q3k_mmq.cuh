// Q3_K x Q8_1 tiled matrix multiply for Ampere-class integer tensor cores.
//
// One 8-warp CTA computes a 32-row x 32-sequence output tile directly from
// the canonical 110-byte Q3_K block (hmask[32] | qs[64] | scales[12] | d).
// Q3_K carries a signed 6-bit scale per 16-value group and no separate min,
// so the structure matches Q6_K: the two 16-value groups inside each Q8_1
// chunk are evaluated by separate low/high MMA instructions.

extern "C" __global__ void rnb_q3k_q8_1_matmul_mmq_tile32(
        ...
        for (unsigned sub = 0; sub < 8u; ++sub) {
            ...
            if (global_row < rows) {
                const unsigned char* packed =
                    weights + global_row * row_bytes + block_index * 110u;
                if (sub == 0u && load_off == 0u) {
                    const unsigned raw_d = static_cast<unsigned>(packed[108])
                        | (static_cast<unsigned>(packed[109]) << 8);
                    weight_d[load_row] =
...
#pragma unroll
                    for (unsigned half = 0; half < 2u; ++half) {
                        const unsigned scale_idx = sub * 2u + half;
                        const unsigned scale_lane = scale_idx & 3u;
                        const unsigned packed_high = packed[104u + scale_lane];
                        unsigned scale_code;
                        if (scale_idx < 4u) {
                            scale_code = (packed[96u + scale_lane] & 0x0fu)
                                | ((packed_high & 0x03u) << 4);
                        } else if (scale_idx < 8u) {
                            scale_code = (packed[100u + scale_lane] & 0x0fu)
                                | (((packed_high >> 2) & 0x03u) << 4);
                        } else if (scale_idx < 12u) {
                            scale_code = (packed[96u + scale_lane] >> 4)
                                | (((packed_high >> 4) & 0x03u) << 4);
                        } else {
                            scale_code = (packed[100u + scale_lane] >> 4)
                                | (((packed_high >> 6) & 0x03u) << 4);
                        }
                        const signed char scale =
                            static_cast<signed char>(static_cast<int>(scale_code) - 32);
                        if (half == 0u) {
                            weight_scale_lo[load_row] = scale;
                        } else {
                            weight_scale_hi[load_row] = scale;
                        }
                    }
                }
#pragma unroll
                for (unsigned i = 0; i < 4u; ++i) {
                    ...
                    const unsigned q = (packed[32u + q_index] >> shift) & 3u;
                    const unsigned high_mask = 1u << (elem >> 5);
                    const int high = (packed[elem & 31u] & high_mask) != 0u ? 0 : 4;
                    a_dst[i] = static_cast<signed char>(static_cast<int>(q) - high);
                }
            } else {
