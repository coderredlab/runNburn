...
__device__ __forceinline__ float rnb_qk_norm_rope_apply_hd256_dim(
...
extern "C" __global__ void rnb_qk_norm_rope_select_hd256_f16kv(
    ...
    if (head_slot < num_heads) {
        ...
        q_out[base + tid] = rnb_qk_norm_rope_apply_hd256_dim(
            head, q_norm, inv_rms, tid, rope_dim, rope_neox, rope_sin, rope_cos, token,
            q_unit_offset);
        q_out[base + half + tid] = rnb_qk_norm_rope_apply_hd256_dim(
            head, q_norm, inv_rms, half + tid, rope_dim, rope_neox, rope_sin, rope_cos, token,
            q_unit_offset);
        return;
    }
    ...
    const float ky0 = rnb_qk_norm_rope_apply_hd256_dim(
        k_head, k_norm, k_inv_rms, tid, rope_dim, rope_neox, rope_sin, rope_cos, token,
        k_unit_offset);
    const float ky1 = rnb_qk_norm_rope_apply_hd256_dim(
        k_head, k_norm, k_inv_rms, half + tid, rope_dim, rope_neox, rope_sin, rope_cos, token,
        k_unit_offset);
