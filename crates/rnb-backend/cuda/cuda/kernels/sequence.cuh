...
extern "C" __global__ void rnb_delta_net_decode_predecay_hd128(
    float* __restrict__ output,
    float* __restrict__ state,
    const float* __restrict__ q,
    const float* __restrict__ k,
    ...
    const unsigned h = blockIdx.y;
    const unsigned tid = threadIdx.x;
    if (h >= num_heads || vi >= head_v_dim || tid >= 128u || head_k_dim != 128u) {
        return;
    }
    ...
    const unsigned state_off = (h * head_v_dim + vi) * 128u;
    const unsigned qk_off = h * 128u;
    const unsigned v_off = h * head_v_dim + vi;
    const float decay = expf(decay_values[h]);
    const unsigned idx = state_off + tid;
    const float old_state = state[idx];
    float sum = decay * old_state * k[qk_off + tid];
    ...
    __syncthreads();
    if (warp == 0u) {
        sum = lane < 4u ? warp_sums[lane] : 0.0f;
        for (unsigned offset = 16u; offset > 0u; offset >>= 1u) {
            sum += __shfl_down_sync(0xffffffffu, sum, offset);
        }
        if (lane == 0u) {
            correction = (v[v_off] - sum) * beta[h];
        }
    }
    __syncthreads();

    const float updated = decay * old_state + k[qk_off + tid] * correction;
    state[idx] = updated;
    sum = updated * q[qk_off + tid];
    for (unsigned offset = 16u; offset > 0u; offset >>= 1u) {
        sum += __shfl_down_sync(0xffffffffu, sum, offset);
    }
    if (lane == 0u) {
...
extern "C" __global__ void rnb_delta_net_decode_predecay(
    float* __restrict__ output,
    float* __restrict__ state,
    const float* __restrict__ q,
    const float* __restrict__ k,
    ...
    const unsigned h = blockIdx.y;
    const unsigned tid = threadIdx.x;
    if (h >= num_heads || vi >= head_v_dim || tid >= 256u) {
        return;
    }
