...
pub(super) fn run(args: &[String]) -> Result<(), String> {
    ...
    eprintln!(
        "OpenAI-compatible API listening on http://{}:{}/v1",
    ...
    for connection in listener.incoming() {
        ...
        thread::spawn(move || handle_connection(stream, &worker, api_key.as_deref()));
    }
    Ok(())
}
...
impl ServeOptions {
    fn parse(args: &[String]) -> Result<Self, String> {
        ...
        let api_key = std::env::var("RNB_API_KEY")
            .ok()
...
fn print_usage() {
    ...
    eprintln!("  RNB_API_KEY=KEY      Require Authorization: Bearer KEY");
}

fn handle_connection(mut stream: TcpStream, worker: &WorkerSender, api_key: Option<&str>) {
    ...
    let cancellation = RequestCancellation::monitor(&stream);
